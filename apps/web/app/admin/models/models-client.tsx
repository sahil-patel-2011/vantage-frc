...
export default function ModelsClient() {
  ...
  return <main className="intel-app"><header className="intel-header"><div><span className="eyebrow">VANTAGE / MODEL CONTROL</span><h1>AI models</h1><p className="app-muted">Manage encrypted provider keys and the models available to Vantage. Keys are never shown again after saving.</p></div></header>
    <section className="admin-grid">
      ...
      <section className="model-list" aria-label="Configured models">{models.map((model, index) => <details className="intel-panel model-config" key={model.id}>
        <summary><strong>{model.displayName}</strong><span>{model.enabled ? "Enabled" : "Disabled"} · {model.providerModelId || "Model ID needed"}</span></summary>
        <form onSubmit={(e) => { e.preventDefault(); void post({ action: "model", ...model }); }}>
        <div><span className="eyebrow">{model.paygOnly ? "PAYG ONLY" : "PLAN ROUTABLE"}</span><p className="app-muted">Routing, pricing, and plan eligibility.</p></div>
