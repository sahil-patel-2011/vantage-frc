...
export default function ModelsClient() {
  ...
  return <main className="intel-app"><header className="intel-header"><div><span className="eyebrow">VANTAGE / ROUTER</span><h1>Provider vault + router</h1><p className="app-muted">Encrypt platform keys, route models, then test the result in the AI hub. Keys never appear in Partners CRM.</p></div><nav className="admin-related" aria-label="AI entry points" style={{display:"flex",flexWrap:"wrap",gap:8}}><Button as="a" variant="secondary" href="/admin">Teams</Button><Button as="a" variant="secondary" href="/admin/partners">Partners</Button><Button as="a" variant="secondary" href="/admin/sponsored">Sponsored AI</Button><Button as="a" variant="primary" href="/ai">AI hub</Button><Button as="a" variant="secondary" href="/ai?tab=chat">Test chat</Button></nav></header>
    <section className="admin-grid">
      ...
      <section className="model-list">{models.map((model, index) => <form className="intel-panel model-config" key={model.id} onSubmit={(e) => { e.preventDefault(); void post({ action: "model", ...model }); }}>
        <div><span className="eyebrow">{model.paygOnly ? "PAY-AS-YOU-GO ONLY" : "INCLUDED ON A PLAN"}</span><h2>{model.displayName}</h2></div>
