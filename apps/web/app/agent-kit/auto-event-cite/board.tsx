...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/auto-event-cite/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-auto-event-cite-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-auto-event-cite-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-auto-event-cite-vs ak-auto-event-cite-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-auto-event-cite-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-auto-event-cite-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-auto-event-cite-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-auto-event-cite-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-auto-event-cite-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-auto-event-cite-split">
        <Panel className="ak-auto-event-cite-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-auto-event-cite-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-auto-event-cite-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-auto-event-cite-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-auto-event-cite-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-auto-event-cite-vs ak-auto-event-cite-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-auto-event-cite-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-auto-event-cite-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-auto-event-cite-setup">
  ...
  return (
    <div className="ak-auto-event-cite-wide">
      ...
      <div className="ak-auto-event-cite-split">
        <Panel className="ak-auto-event-cite-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-auto-event-cite-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-auto-event-cite-vs ak-auto-event-cite-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-auto-event-cite-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-auto-event-cite-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-auto-event-cite-lovat">
      ...
      <div className="ak-auto-event-cite-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-auto-event-cite-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-auto-event-cite-page">
      <PageHeader
