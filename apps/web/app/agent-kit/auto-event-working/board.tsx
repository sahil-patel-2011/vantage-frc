...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/auto-event-working/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-auto-event-working-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-auto-event-working-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-auto-event-working-vs ak-auto-event-working-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-auto-event-working-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-auto-event-working-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-auto-event-working-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-auto-event-working-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-auto-event-working-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-auto-event-working-split">
        <Panel className="ak-auto-event-working-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-auto-event-working-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-auto-event-working-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-auto-event-working-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-auto-event-working-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-auto-event-working-vs ak-auto-event-working-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-auto-event-working-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-auto-event-working-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-auto-event-working-setup">
  ...
  return (
    <div className="ak-auto-event-working-wide">
      ...
      <div className="ak-auto-event-working-split">
        <Panel className="ak-auto-event-working-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-auto-event-working-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-auto-event-working-vs ak-auto-event-working-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-auto-event-working-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-auto-event-working-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-auto-event-working-lovat">
      ...
      <div className="ak-auto-event-working-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-auto-event-working-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-auto-event-working-page">
      <PageHeader
