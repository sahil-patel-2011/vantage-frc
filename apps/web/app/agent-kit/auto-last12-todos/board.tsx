...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/auto-last12-todos/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-auto-last12-todos-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-auto-last12-todos-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-auto-last12-todos-vs ak-auto-last12-todos-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-auto-last12-todos-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-auto-last12-todos-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-auto-last12-todos-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-auto-last12-todos-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-auto-last12-todos-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-auto-last12-todos-split">
        <Panel className="ak-auto-last12-todos-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-auto-last12-todos-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-auto-last12-todos-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-auto-last12-todos-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-auto-last12-todos-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-auto-last12-todos-vs ak-auto-last12-todos-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-auto-last12-todos-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-auto-last12-todos-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-auto-last12-todos-setup">
  ...
  return (
    <div className="ak-auto-last12-todos-wide">
      ...
      <div className="ak-auto-last12-todos-split">
        <Panel className="ak-auto-last12-todos-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-auto-last12-todos-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-auto-last12-todos-vs ak-auto-last12-todos-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-auto-last12-todos-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-auto-last12-todos-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-auto-last12-todos-lovat">
      ...
      <div className="ak-auto-last12-todos-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-auto-last12-todos-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-auto-last12-todos-page">
      <PageHeader
