...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/auto-quals-autonomous/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-auto-quals-autonomous-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-auto-quals-autonomous-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-auto-quals-autonomous-vs ak-auto-quals-autonomous-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-auto-quals-autonomous-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-auto-quals-autonomous-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-auto-quals-autonomous-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-auto-quals-autonomous-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-auto-quals-autonomous-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-auto-quals-autonomous-split">
        <Panel className="ak-auto-quals-autonomous-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-auto-quals-autonomous-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-auto-quals-autonomous-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-auto-quals-autonomous-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-auto-quals-autonomous-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-auto-quals-autonomous-vs ak-auto-quals-autonomous-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-auto-quals-autonomous-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-auto-quals-autonomous-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-auto-quals-autonomous-setup">
  ...
  return (
    <div className="ak-auto-quals-autonomous-wide">
      ...
      <div className="ak-auto-quals-autonomous-split">
        <Panel className="ak-auto-quals-autonomous-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-auto-quals-autonomous-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-auto-quals-autonomous-vs ak-auto-quals-autonomous-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-auto-quals-autonomous-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-auto-quals-autonomous-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-auto-quals-autonomous-lovat">
      ...
      <div className="ak-auto-quals-autonomous-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-auto-quals-autonomous-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-auto-quals-autonomous-page">
      <PageHeader
