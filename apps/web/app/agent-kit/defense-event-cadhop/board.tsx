...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/defense-event-cadhop/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-defense-event-cadhop-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-defense-event-cadhop-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-defense-event-cadhop-vs ak-defense-event-cadhop-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-defense-event-cadhop-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-defense-event-cadhop-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-defense-event-cadhop-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-defense-event-cadhop-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-defense-event-cadhop-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-defense-event-cadhop-split">
        <Panel className="ak-defense-event-cadhop-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-defense-event-cadhop-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-defense-event-cadhop-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-defense-event-cadhop-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-defense-event-cadhop-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-defense-event-cadhop-vs ak-defense-event-cadhop-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-defense-event-cadhop-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-defense-event-cadhop-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-defense-event-cadhop-setup">
  ...
  return (
    <div className="ak-defense-event-cadhop-wide">
      ...
      <div className="ak-defense-event-cadhop-split">
        <Panel className="ak-defense-event-cadhop-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-defense-event-cadhop-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-defense-event-cadhop-vs ak-defense-event-cadhop-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-defense-event-cadhop-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-defense-event-cadhop-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-defense-event-cadhop-lovat">
      ...
      <div className="ak-defense-event-cadhop-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-defense-event-cadhop-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-defense-event-cadhop-page">
      <PageHeader
