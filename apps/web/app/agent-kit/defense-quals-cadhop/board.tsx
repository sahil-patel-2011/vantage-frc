...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/defense-quals-cadhop/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-defense-quals-cadhop-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-defense-quals-cadhop-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-defense-quals-cadhop-vs ak-defense-quals-cadhop-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-defense-quals-cadhop-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-defense-quals-cadhop-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-defense-quals-cadhop-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-defense-quals-cadhop-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-defense-quals-cadhop-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-defense-quals-cadhop-split">
        <Panel className="ak-defense-quals-cadhop-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-defense-quals-cadhop-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-defense-quals-cadhop-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-defense-quals-cadhop-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-defense-quals-cadhop-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-defense-quals-cadhop-vs ak-defense-quals-cadhop-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-defense-quals-cadhop-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-defense-quals-cadhop-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-defense-quals-cadhop-setup">
  ...
  return (
    <div className="ak-defense-quals-cadhop-wide">
      ...
      <div className="ak-defense-quals-cadhop-split">
        <Panel className="ak-defense-quals-cadhop-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-defense-quals-cadhop-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-defense-quals-cadhop-vs ak-defense-quals-cadhop-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-defense-quals-cadhop-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-defense-quals-cadhop-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-defense-quals-cadhop-lovat">
      ...
      <div className="ak-defense-quals-cadhop-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-defense-quals-cadhop-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-defense-quals-cadhop-page">
      <PageHeader
