...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/driver-last12-autonomous/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-driver-last12-autonomous-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-driver-last12-autonomous-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-driver-last12-autonomous-vs ak-driver-last12-autonomous-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-driver-last12-autonomous-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-driver-last12-autonomous-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-driver-last12-autonomous-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-driver-last12-autonomous-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-driver-last12-autonomous-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-driver-last12-autonomous-split">
        <Panel className="ak-driver-last12-autonomous-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-driver-last12-autonomous-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-driver-last12-autonomous-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-driver-last12-autonomous-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-driver-last12-autonomous-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-driver-last12-autonomous-vs ak-driver-last12-autonomous-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-driver-last12-autonomous-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-driver-last12-autonomous-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-driver-last12-autonomous-setup">
  ...
  return (
    <div className="ak-driver-last12-autonomous-wide">
      ...
      <div className="ak-driver-last12-autonomous-split">
        <Panel className="ak-driver-last12-autonomous-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-driver-last12-autonomous-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-driver-last12-autonomous-vs ak-driver-last12-autonomous-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-driver-last12-autonomous-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-driver-last12-autonomous-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-driver-last12-autonomous-lovat">
      ...
      <div className="ak-driver-last12-autonomous-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-driver-last12-autonomous-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-driver-last12-autonomous-page">
      <PageHeader
