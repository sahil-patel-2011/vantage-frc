...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/driver-last6-working/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-driver-last6-working-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-driver-last6-working-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-driver-last6-working-vs ak-driver-last6-working-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-driver-last6-working-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-driver-last6-working-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-driver-last6-working-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-driver-last6-working-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-driver-last6-working-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-driver-last6-working-split">
        <Panel className="ak-driver-last6-working-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-driver-last6-working-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-driver-last6-working-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-driver-last6-working-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-driver-last6-working-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-driver-last6-working-vs ak-driver-last6-working-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-driver-last6-working-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-driver-last6-working-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-driver-last6-working-setup">
  ...
  return (
    <div className="ak-driver-last6-working-wide">
      ...
      <div className="ak-driver-last6-working-split">
        <Panel className="ak-driver-last6-working-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-driver-last6-working-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-driver-last6-working-vs ak-driver-last6-working-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-driver-last6-working-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-driver-last6-working-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-driver-last6-working-lovat">
      ...
      <div className="ak-driver-last6-working-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-driver-last6-working-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-driver-last6-working-page">
      <PageHeader
