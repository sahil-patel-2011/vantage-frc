...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/driver-quals-autonomous/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-driver-quals-autonomous-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-driver-quals-autonomous-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-driver-quals-autonomous-vs ak-driver-quals-autonomous-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-driver-quals-autonomous-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-driver-quals-autonomous-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-driver-quals-autonomous-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-driver-quals-autonomous-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-driver-quals-autonomous-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-driver-quals-autonomous-split">
        <Panel className="ak-driver-quals-autonomous-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-driver-quals-autonomous-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-driver-quals-autonomous-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-driver-quals-autonomous-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-driver-quals-autonomous-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-driver-quals-autonomous-vs ak-driver-quals-autonomous-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-driver-quals-autonomous-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-driver-quals-autonomous-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-driver-quals-autonomous-setup">
  ...
  return (
    <div className="ak-driver-quals-autonomous-wide">
      ...
      <div className="ak-driver-quals-autonomous-split">
        <Panel className="ak-driver-quals-autonomous-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-driver-quals-autonomous-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-driver-quals-autonomous-vs ak-driver-quals-autonomous-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-driver-quals-autonomous-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-driver-quals-autonomous-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-driver-quals-autonomous-lovat">
      ...
      <div className="ak-driver-quals-autonomous-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-driver-quals-autonomous-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-driver-quals-autonomous-page">
      <PageHeader
