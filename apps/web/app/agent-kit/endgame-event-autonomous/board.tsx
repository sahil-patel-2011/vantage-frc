...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/endgame-event-autonomous/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-endgame-event-autonomous-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-endgame-event-autonomous-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-endgame-event-autonomous-vs ak-endgame-event-autonomous-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-endgame-event-autonomous-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-endgame-event-autonomous-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-endgame-event-autonomous-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-endgame-event-autonomous-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-endgame-event-autonomous-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-endgame-event-autonomous-split">
        <Panel className="ak-endgame-event-autonomous-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-endgame-event-autonomous-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-endgame-event-autonomous-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-endgame-event-autonomous-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-endgame-event-autonomous-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-endgame-event-autonomous-vs ak-endgame-event-autonomous-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-endgame-event-autonomous-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-endgame-event-autonomous-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-endgame-event-autonomous-setup">
  ...
  return (
    <div className="ak-endgame-event-autonomous-wide">
      ...
      <div className="ak-endgame-event-autonomous-split">
        <Panel className="ak-endgame-event-autonomous-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-endgame-event-autonomous-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-endgame-event-autonomous-vs ak-endgame-event-autonomous-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-endgame-event-autonomous-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-endgame-event-autonomous-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-endgame-event-autonomous-lovat">
      ...
      <div className="ak-endgame-event-autonomous-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-endgame-event-autonomous-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-endgame-event-autonomous-page">
      <PageHeader
