...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/path-last6-todos/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-path-last6-todos-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-path-last6-todos-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-path-last6-todos-vs ak-path-last6-todos-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-path-last6-todos-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-path-last6-todos-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-path-last6-todos-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-path-last6-todos-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-path-last6-todos-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-path-last6-todos-split">
        <Panel className="ak-path-last6-todos-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-path-last6-todos-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-path-last6-todos-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-path-last6-todos-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-path-last6-todos-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-path-last6-todos-vs ak-path-last6-todos-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-path-last6-todos-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-path-last6-todos-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-path-last6-todos-setup">
  ...
  return (
    <div className="ak-path-last6-todos-wide">
      ...
      <div className="ak-path-last6-todos-split">
        <Panel className="ak-path-last6-todos-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-path-last6-todos-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-path-last6-todos-vs ak-path-last6-todos-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-path-last6-todos-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-path-last6-todos-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-path-last6-todos-lovat">
      ...
      <div className="ak-path-last6-todos-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-path-last6-todos-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-path-last6-todos-page">
      <PageHeader
