...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/reliability-last12-excerpts/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-reliability-last12-excerpts-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-reliability-last12-excerpts-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-reliability-last12-excerpts-vs ak-reliability-last12-excerpts-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-reliability-last12-excerpts-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-reliability-last12-excerpts-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-reliability-last12-excerpts-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-reliability-last12-excerpts-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-reliability-last12-excerpts-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-reliability-last12-excerpts-split">
        <Panel className="ak-reliability-last12-excerpts-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-reliability-last12-excerpts-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-reliability-last12-excerpts-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-reliability-last12-excerpts-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-reliability-last12-excerpts-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-reliability-last12-excerpts-vs ak-reliability-last12-excerpts-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-reliability-last12-excerpts-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-reliability-last12-excerpts-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-reliability-last12-excerpts-setup">
  ...
  return (
    <div className="ak-reliability-last12-excerpts-wide">
      ...
      <div className="ak-reliability-last12-excerpts-split">
        <Panel className="ak-reliability-last12-excerpts-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-reliability-last12-excerpts-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-reliability-last12-excerpts-vs ak-reliability-last12-excerpts-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-reliability-last12-excerpts-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-reliability-last12-excerpts-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-reliability-last12-excerpts-lovat">
      ...
      <div className="ak-reliability-last12-excerpts-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-reliability-last12-excerpts-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-reliability-last12-excerpts-page">
      <PageHeader
