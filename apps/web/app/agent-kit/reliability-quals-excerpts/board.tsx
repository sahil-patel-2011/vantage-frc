...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/reliability-quals-excerpts/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-reliability-quals-excerpts-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-reliability-quals-excerpts-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-reliability-quals-excerpts-vs ak-reliability-quals-excerpts-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-reliability-quals-excerpts-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-reliability-quals-excerpts-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-reliability-quals-excerpts-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-reliability-quals-excerpts-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-reliability-quals-excerpts-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-reliability-quals-excerpts-split">
        <Panel className="ak-reliability-quals-excerpts-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-reliability-quals-excerpts-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-reliability-quals-excerpts-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-reliability-quals-excerpts-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-reliability-quals-excerpts-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-reliability-quals-excerpts-vs ak-reliability-quals-excerpts-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-reliability-quals-excerpts-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-reliability-quals-excerpts-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-reliability-quals-excerpts-setup">
  ...
  return (
    <div className="ak-reliability-quals-excerpts-wide">
      ...
      <div className="ak-reliability-quals-excerpts-split">
        <Panel className="ak-reliability-quals-excerpts-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-reliability-quals-excerpts-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-reliability-quals-excerpts-vs ak-reliability-quals-excerpts-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-reliability-quals-excerpts-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-reliability-quals-excerpts-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-reliability-quals-excerpts-lovat">
      ...
      <div className="ak-reliability-quals-excerpts-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-reliability-quals-excerpts-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-reliability-quals-excerpts-page">
      <PageHeader
