...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/schedule-event-todos/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-schedule-event-todos-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-schedule-event-todos-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-schedule-event-todos-vs ak-schedule-event-todos-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-schedule-event-todos-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-schedule-event-todos-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-schedule-event-todos-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-schedule-event-todos-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-schedule-event-todos-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-schedule-event-todos-split">
        <Panel className="ak-schedule-event-todos-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-schedule-event-todos-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-schedule-event-todos-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-schedule-event-todos-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-schedule-event-todos-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-schedule-event-todos-vs ak-schedule-event-todos-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-schedule-event-todos-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-schedule-event-todos-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-schedule-event-todos-setup">
  ...
  return (
    <div className="ak-schedule-event-todos-wide">
      ...
      <div className="ak-schedule-event-todos-split">
        <Panel className="ak-schedule-event-todos-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-schedule-event-todos-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-schedule-event-todos-vs ak-schedule-event-todos-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-schedule-event-todos-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-schedule-event-todos-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-schedule-event-todos-lovat">
      ...
      <div className="ak-schedule-event-todos-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-schedule-event-todos-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-schedule-event-todos-page">
      <PageHeader
