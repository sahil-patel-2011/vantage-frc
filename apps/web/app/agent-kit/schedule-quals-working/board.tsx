...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/agent-kit/schedule-quals-working/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="ak-schedule-quals-working-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="ak-schedule-quals-working-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`ak-schedule-quals-working-vs ak-schedule-quals-working-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="ak-schedule-quals-working-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="ak-schedule-quals-working-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="ak-schedule-quals-working-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="ak-schedule-quals-working-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="ak-schedule-quals-working-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="ak-schedule-quals-working-split">
        <Panel className="ak-schedule-quals-working-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="ak-schedule-quals-working-zones">
            {zones.map((zone) => (
        ...
        <Panel className="ak-schedule-quals-working-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="ak-schedule-quals-working-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="ak-schedule-quals-working-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`ak-schedule-quals-working-vs ak-schedule-quals-working-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="ak-schedule-quals-working-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="ak-schedule-quals-working-pareto">
            {front.map((point) => (
      ...
      <ul className="ak-schedule-quals-working-setup">
  ...
  return (
    <div className="ak-schedule-quals-working-wide">
      ...
      <div className="ak-schedule-quals-working-split">
        <Panel className="ak-schedule-quals-working-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="ak-schedule-quals-working-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`ak-schedule-quals-working-vs ak-schedule-quals-working-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="ak-schedule-quals-working-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="ak-schedule-quals-working-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="ak-schedule-quals-working-lovat">
      ...
      <div className="ak-schedule-quals-working-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="ak-schedule-quals-working-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page ak-schedule-quals-working-page">
      <PageHeader
