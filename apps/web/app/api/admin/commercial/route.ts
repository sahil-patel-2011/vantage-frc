    ...
    return Response.json(
      await runAdmin(async (client) => {
        const config =
          (
            await client.query(
              `SELECT stripe_fee_percent AS "stripeFeePercent",stripe_fixed_fee_usd AS "stripeFixedFeeUsd",infrastructure_allocation_usd AS "infrastructureAllocationUsd",gross_margin_warning_percent AS "grossMarginWarningPercent" FROM platform_margin_config WHERE id='default'`,
            )
          ).rows[0] ?? {
            stripeFeePercent: "2.9",
            stripeFixedFeeUsd: ".30",
            infrastructureAllocationUsd: "2",
            grossMarginWarningPercent: "10",
          };
        const margin = (
          await client.query(
            `SELECT COALESCE(sum(p.monthly_price_usd),0)::text AS revenue,COALESCE(sum(period.provider_cost_used_usd),0)::text AS "providerCost",count(*)::int AS subscriptions FROM org_entitlements e JOIN pricing_plans p ON p.code=e.plan_code LEFT JOIN org_plan_periods period ON period.org_id=e.org_id AND period.status='active' WHERE e.status IN ('active','trialing')`,
          )
        ).rows[0];
    ...
    await runAdmin(async (client) => {
      ...
      if (body.action === "gift") {
        ...
        await client.query(
          ...
          WHERE org_id=$1 AND source='admin_trial'`,
          [body.orgId, actor],
        );
        await client.query(
          `INSERT INTO entitlement_events(org_id,plan_code,action,source,actor_user_id)
          SELECT org_id,plan_code,'trial_revoked','admin',$2 FROM org_entitlements WHERE org_id=$1`,
          [body.orgId, actor],
        );
      } else if (body.action === "credit-pack") {
        await client.query(
          `INSERT INTO credit_packs(code,name,credit_amount_usd,purchase_price_usd,stripe_price_id,active)
