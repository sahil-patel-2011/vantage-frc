...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json()) as {
      action?: "save" | "toggle-private" | "toggle-team" | "promote";
      orgId?: string; id?: string; kind?: string; content?: string; enabled?: boolean;
      messageId?: string; retentionDays?: number; tokenBudget?: number;
    };
    const result = await withRls({ userId: current.user.id, orgId: body.orgId }, async (client) => {
      ...
      if (body.action === "toggle-private") {
        await repository.setUserMemoryEnabled(current.user.id, Boolean(body.enabled));
        return { success: true };
      }
      if (body.action === "toggle-team") {
        if (!body.orgId) throw new Error("orgId is required");
        await repository.setTeamMemory(body.orgId, current.user.id, {
          enabled: Boolean(body.enabled), retentionDays: body.retentionDays, tokenBudget: body.tokenBudget,
        });
        return { success: true };
      }
