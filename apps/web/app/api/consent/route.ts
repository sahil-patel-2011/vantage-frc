...
function fail(error: unknown) {
  ...
  return Response.json({ error: publicErrorMessage(error, "Consent request failed") }, { status });
}
      ...
      if (!row) {
        return {
          ...
          message: requestedOrg
            ? "Choose your team to track forms."
            : "Choose your team to track forms, or join the waitlist.",
        };
      }
