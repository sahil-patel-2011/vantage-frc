...
const fail = (error: unknown) =>
  Response.json({ error: error instanceof Error ? error.message : "Grant item request failed" }, { status: 400 });
  ...
  try {
    ...
    const items = await withRls({ userId: current.user.id, orgId }, async (client) => {
      const result = await client.query(
        `SELECT ${RETURNING} FROM grant_application_items WHERE org_id=$1 AND application_id=$2 ORDER BY sort_order`,
        [orgId, applicationId],
      );
      return result.rows;
    });
  ...
  try {
    ...
    const item = await withRls({ userId: current.user.id, orgId }, async (client) => {
      ...
      const order = await client.query(
        `SELECT COALESCE(max(sort_order),-1)+1 AS next FROM grant_application_items WHERE application_id=$1`,
        [applicationId],
      );
      const result = await client.query(
