...
import { parseMatchDebriefAction, summarizeDebriefs, type Alliance, type MatchResult } from "../../../lib/match-debrief";
...
export async function POST(request: Request) {
  try {
    const session = await requireSession();
    const action = parseMatchDebriefAction(await request.json());
