...
import {
  computeSponsorRenewalRoiView,
  currentSeasonYear,
  deleteSponsorRoiReport,
  generateSponsorRoiReport,
  type SponsorRenewalRoiView,
} from "../../../lib/sponsor-renewal-roi/compute-sponsor-renewal-roi";
...
export async function GET(request: Request) {
  ...
  try {
    const view = await withRls({ userId: session.user.id }, (client) =>
      computeSponsorRenewalRoiView(client, { userId: session.user.id, requestedOrg, seasonYear }),
    );
    ...
    return Response.json(
      {
        ...
        message: "Could not load sponsor renewal-risk scores. Select a workspace and confirm database access.",
        steps: [
          { id: "workspace", label: "Select workspace", detail: "Choose your team organization", href: "/workspace" },
        ],
        orgId: null,
        seasonYear: seasonYear ?? currentSeasonYear(),
      } satisfies SponsorRenewalRoiView,
      { status: 200 },
    );
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    const view = await withRls({ userId, orgId }, async (client) => {
      ...
      switch (action) {
        ...
        case "delete-report": {
          ...
          await deleteSponsorRoiReport(client, { orgId, reportId });
          break;
        }
        default:
    ...
    const message = error instanceof Error ? error.message : "Sponsor renewal ROI request failed";
