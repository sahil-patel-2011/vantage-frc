...
const fail = (error: unknown) =>
  Response.json({ error: error instanceof Error ? error.message : "Sponsor prospect request failed" }, { status: 400 });
    ...
    const prospects = await withRls({ userId: current.user.id, orgId }, async (client) => {
      const result = await client.query(
        `SELECT id, company_name AS "companyName", website, rationale, source_urls AS "sourceUrls", status,
         ...
         FROM sponsor_prospects WHERE org_id=$1 ${status ? "AND status=$2" : ""} ORDER BY created_at DESC`,
  ...
  try {
    ...
    const prospects = await withRls({ userId: current.user.id, orgId }, async (client) => {
      ...
      const existing = await client.query(
        `SELECT industry, tier, state_prov AS "stateProv" FROM sponsors WHERE org_id=$1`,
        [orgId],
      );
      ...
      for (const suggestion of suggestions) {
        const result = await client.query(
          `INSERT INTO sponsor_prospects(org_id, company_name, rationale, status, suggested_by)
           VALUES($1,$2,$3,'suggested','heuristic_agent')
...
export async function PATCH(request: Request) {
  try {
    ...
    const prospect = await withRls({ userId: current.user.id, orgId }, async (client) => {
      ...
      if (!admin.rowCount) throw new Error("Organization administrator access required");
      const result = await client.query(
        `UPDATE sponsor_prospects SET status=$1 WHERE id=$2 AND org_id=$3
