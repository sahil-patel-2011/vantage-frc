...
function isAwardTrackerView(value: unknown): value is AwardTrackerView {
  if (!value || typeof value !== "object") return false;
  const status = (value as { status?: unknown }).status;
  return status === "setup_required" || status === "live";
}

function awardTrackerCacheOrg(data: AwardTrackerView, orgHint: string): string {
  if (typeof data.orgId === "string" && data.orgId.trim()) return data.orgId;
  return orgHint;
}

async function persistAwardTrackerSnapshot(
  orgHint: string,
  seasonHint: string,
  data: AwardTrackerView,
): Promise<void> {
  const cacheOrg = awardTrackerCacheOrg(data, orgHint);
  if (!cacheOrg) return;
...
export default function AwardTrackerClient() {
  ...
  const viewRef = useRef<AwardTrackerView | null>(null);
  ...
  const load = useCallback((seasonOverride?: number) => {
    void (async () => {
      ...
      try {
        const cached = await getFeatureSnapshot<AwardTrackerView>(
          "award-tracker",
          urlOrg || "_",
          seasonHint,
        );
        if (!viewRef.current && cached?.data && isAwardTrackerView(cached.data)) {
          setView(cached.data);
      ...
      try {
        const response = await fetch(
          `/api/award-tracker${query.toString() ? `?${query.toString()}` : ""}`,
          {
            cache: "no-store",
        ...
        setFromCache(false);
        setCachedAt(null);
        await persistAwardTrackerSnapshot(urlOrg, seasonHint, data);
      } catch {
        if (hadCache || viewRef.current) {
          setFromCache(true);
          setError("Could not refresh Award Tracker. Showing the last copy on this device.");
  ...
  const mutate = useCallback(
    async (payload: Record<string, unknown>) => {
      ...
      try {
        ...
        setView(data);
        setSeason(data.seasonYear);
        void persistAwardTrackerSnapshot(orgId, season != null ? String(season) : "", data);
      } catch {
