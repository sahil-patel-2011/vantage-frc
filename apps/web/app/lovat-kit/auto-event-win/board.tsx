...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/auto-event-win/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-auto-event-win-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-auto-event-win-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-auto-event-win-vs lk-auto-event-win-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-auto-event-win-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-auto-event-win-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-auto-event-win-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-auto-event-win-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-auto-event-win-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-auto-event-win-split">
        <Panel className="lk-auto-event-win-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-auto-event-win-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-auto-event-win-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-auto-event-win-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-auto-event-win-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-auto-event-win-vs lk-auto-event-win-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-auto-event-win-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-auto-event-win-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-auto-event-win-setup">
  ...
  return (
    <div className="lk-auto-event-win-wide">
      ...
      <div className="lk-auto-event-win-split">
        <Panel className="lk-auto-event-win-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-auto-event-win-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-auto-event-win-vs lk-auto-event-win-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-auto-event-win-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-auto-event-win-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-auto-event-win-lovat">
      ...
      <div className="lk-auto-event-win-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-auto-event-win-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-auto-event-win-page">
      <PageHeader
