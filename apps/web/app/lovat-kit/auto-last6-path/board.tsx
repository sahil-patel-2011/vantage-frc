...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/auto-last6-path/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-auto-last6-path-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-auto-last6-path-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-auto-last6-path-vs lk-auto-last6-path-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-auto-last6-path-board" aria-label={chrome.event}>
      <header>
      ...
      <div className="lk-auto-last6-path-grid">
        {report.cards.map((card) => (
          <Tile key={card.id} card={card} />
        ))}
      </div>
      {setupReasons(report.cards).length ? (
        <ul className="lk-auto-last6-path-setup">
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-auto-last6-path-page">
      <PageHeader
