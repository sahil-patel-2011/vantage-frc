...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/auto-quals-trimmed/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-auto-quals-trimmed-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-auto-quals-trimmed-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-auto-quals-trimmed-vs lk-auto-quals-trimmed-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-auto-quals-trimmed-board" aria-label={chrome.event}>
      <header>
      ...
      <div className="lk-auto-quals-trimmed-grid">
        {report.cards.map((card) => (
          <Tile key={card.id} card={card} />
        ))}
      </div>
      {setupReasons(report.cards).length ? (
        <ul className="lk-auto-quals-trimmed-setup">
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-auto-quals-trimmed-page">
      <PageHeader
