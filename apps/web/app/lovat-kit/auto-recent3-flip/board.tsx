...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/auto-recent3-flip/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-auto-recent3-flip-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-auto-recent3-flip-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-auto-recent3-flip-vs lk-auto-recent3-flip-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-auto-recent3-flip-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-auto-recent3-flip-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-auto-recent3-flip-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-auto-recent3-flip-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-auto-recent3-flip-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-auto-recent3-flip-split">
        <Panel className="lk-auto-recent3-flip-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-auto-recent3-flip-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-auto-recent3-flip-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-auto-recent3-flip-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-auto-recent3-flip-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-auto-recent3-flip-vs lk-auto-recent3-flip-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-auto-recent3-flip-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-auto-recent3-flip-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-auto-recent3-flip-setup">
  ...
  return (
    <div className="lk-auto-recent3-flip-wide">
      ...
      <div className="lk-auto-recent3-flip-split">
        <Panel className="lk-auto-recent3-flip-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-auto-recent3-flip-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-auto-recent3-flip-vs lk-auto-recent3-flip-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-auto-recent3-flip-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-auto-recent3-flip-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-auto-recent3-flip-lovat">
      ...
      <div className="lk-auto-recent3-flip-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-auto-recent3-flip-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-auto-recent3-flip-page">
      <PageHeader
