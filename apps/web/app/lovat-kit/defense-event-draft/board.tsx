...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/defense-event-draft/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-defense-event-draft-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-defense-event-draft-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-defense-event-draft-vs lk-defense-event-draft-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-defense-event-draft-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-defense-event-draft-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-defense-event-draft-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-defense-event-draft-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-defense-event-draft-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-defense-event-draft-split">
        <Panel className="lk-defense-event-draft-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-defense-event-draft-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-defense-event-draft-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-defense-event-draft-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-defense-event-draft-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-defense-event-draft-vs lk-defense-event-draft-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-defense-event-draft-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-defense-event-draft-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-defense-event-draft-setup">
  ...
  return (
    <div className="lk-defense-event-draft-wide">
      ...
      <div className="lk-defense-event-draft-split">
        <Panel className="lk-defense-event-draft-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-defense-event-draft-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-defense-event-draft-vs lk-defense-event-draft-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-defense-event-draft-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-defense-event-draft-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-defense-event-draft-lovat">
      ...
      <div className="lk-defense-event-draft-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-defense-event-draft-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-defense-event-draft-page">
      <PageHeader
