...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/defense-last4-residual/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-defense-last4-residual-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-defense-last4-residual-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-defense-last4-residual-vs lk-defense-last4-residual-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-defense-last4-residual-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-defense-last4-residual-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-defense-last4-residual-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-defense-last4-residual-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-defense-last4-residual-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-defense-last4-residual-split">
        <Panel className="lk-defense-last4-residual-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-defense-last4-residual-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-defense-last4-residual-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-defense-last4-residual-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-defense-last4-residual-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-defense-last4-residual-vs lk-defense-last4-residual-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-defense-last4-residual-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-defense-last4-residual-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-defense-last4-residual-setup">
  ...
  return (
    <div className="lk-defense-last4-residual-wide">
      ...
      <div className="lk-defense-last4-residual-split">
        <Panel className="lk-defense-last4-residual-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-defense-last4-residual-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-defense-last4-residual-vs lk-defense-last4-residual-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-defense-last4-residual-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-defense-last4-residual-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-defense-last4-residual-lovat">
      ...
      <div className="lk-defense-last4-residual-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-defense-last4-residual-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-defense-last4-residual-page">
      <PageHeader
