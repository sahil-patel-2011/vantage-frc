...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/defense-playoffs-assign/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-defense-playoffs-assign-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-defense-playoffs-assign-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-defense-playoffs-assign-vs lk-defense-playoffs-assign-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-defense-playoffs-assign-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-defense-playoffs-assign-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-defense-playoffs-assign-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-defense-playoffs-assign-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-defense-playoffs-assign-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-defense-playoffs-assign-split">
        <Panel className="lk-defense-playoffs-assign-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-defense-playoffs-assign-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-defense-playoffs-assign-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-defense-playoffs-assign-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-defense-playoffs-assign-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-defense-playoffs-assign-vs lk-defense-playoffs-assign-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-defense-playoffs-assign-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-defense-playoffs-assign-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-defense-playoffs-assign-setup">
  ...
  return (
    <div className="lk-defense-playoffs-assign-wide">
      ...
      <div className="lk-defense-playoffs-assign-split">
        <Panel className="lk-defense-playoffs-assign-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-defense-playoffs-assign-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-defense-playoffs-assign-vs lk-defense-playoffs-assign-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-defense-playoffs-assign-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-defense-playoffs-assign-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-defense-playoffs-assign-lovat">
      ...
      <div className="lk-defense-playoffs-assign-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-defense-playoffs-assign-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-defense-playoffs-assign-page">
      <PageHeader
