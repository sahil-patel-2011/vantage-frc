...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/defense-quals-zone/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-defense-quals-zone-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-defense-quals-zone-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-defense-quals-zone-vs lk-defense-quals-zone-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-defense-quals-zone-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-defense-quals-zone-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-defense-quals-zone-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-defense-quals-zone-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-defense-quals-zone-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-defense-quals-zone-split">
        <Panel className="lk-defense-quals-zone-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-defense-quals-zone-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-defense-quals-zone-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-defense-quals-zone-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-defense-quals-zone-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-defense-quals-zone-vs lk-defense-quals-zone-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-defense-quals-zone-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-defense-quals-zone-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-defense-quals-zone-setup">
  ...
  return (
    <div className="lk-defense-quals-zone-wide">
      ...
      <div className="lk-defense-quals-zone-split">
        <Panel className="lk-defense-quals-zone-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-defense-quals-zone-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-defense-quals-zone-vs lk-defense-quals-zone-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-defense-quals-zone-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-defense-quals-zone-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-defense-quals-zone-lovat">
      ...
      <div className="lk-defense-quals-zone-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-defense-quals-zone-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-defense-quals-zone-page">
      <PageHeader
