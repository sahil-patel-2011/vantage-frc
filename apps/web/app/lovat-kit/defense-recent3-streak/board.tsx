...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/defense-recent3-streak/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-defense-recent3-streak-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-defense-recent3-streak-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-defense-recent3-streak-vs lk-defense-recent3-streak-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-defense-recent3-streak-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-defense-recent3-streak-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-defense-recent3-streak-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-defense-recent3-streak-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-defense-recent3-streak-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-defense-recent3-streak-split">
        <Panel className="lk-defense-recent3-streak-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-defense-recent3-streak-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-defense-recent3-streak-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-defense-recent3-streak-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-defense-recent3-streak-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-defense-recent3-streak-vs lk-defense-recent3-streak-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-defense-recent3-streak-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-defense-recent3-streak-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-defense-recent3-streak-setup">
  ...
  return (
    <div className="lk-defense-recent3-streak-wide">
      ...
      <div className="lk-defense-recent3-streak-split">
        <Panel className="lk-defense-recent3-streak-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-defense-recent3-streak-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-defense-recent3-streak-vs lk-defense-recent3-streak-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-defense-recent3-streak-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-defense-recent3-streak-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-defense-recent3-streak-lovat">
      ...
      <div className="lk-defense-recent3-streak-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-defense-recent3-streak-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-defense-recent3-streak-page">
      <PageHeader
