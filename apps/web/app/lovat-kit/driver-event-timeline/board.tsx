...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/driver-event-timeline/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-driver-event-timeline-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-driver-event-timeline-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-driver-event-timeline-vs lk-driver-event-timeline-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-driver-event-timeline-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-driver-event-timeline-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-driver-event-timeline-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-driver-event-timeline-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-driver-event-timeline-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-driver-event-timeline-split">
        <Panel className="lk-driver-event-timeline-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-driver-event-timeline-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-driver-event-timeline-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-driver-event-timeline-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-driver-event-timeline-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-driver-event-timeline-vs lk-driver-event-timeline-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-driver-event-timeline-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-driver-event-timeline-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-driver-event-timeline-setup">
  ...
  return (
    <div className="lk-driver-event-timeline-wide">
      ...
      <div className="lk-driver-event-timeline-split">
        <Panel className="lk-driver-event-timeline-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-driver-event-timeline-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-driver-event-timeline-vs lk-driver-event-timeline-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-driver-event-timeline-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-driver-event-timeline-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-driver-event-timeline-lovat">
      ...
      <div className="lk-driver-event-timeline-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-driver-event-timeline-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-driver-event-timeline-page">
      <PageHeader
