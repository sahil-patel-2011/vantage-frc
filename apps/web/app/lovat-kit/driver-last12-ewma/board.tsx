...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/driver-last12-ewma/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-driver-last12-ewma-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-driver-last12-ewma-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-driver-last12-ewma-vs lk-driver-last12-ewma-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-driver-last12-ewma-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-driver-last12-ewma-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-driver-last12-ewma-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-driver-last12-ewma-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-driver-last12-ewma-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-driver-last12-ewma-split">
        <Panel className="lk-driver-last12-ewma-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-driver-last12-ewma-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-driver-last12-ewma-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-driver-last12-ewma-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-driver-last12-ewma-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-driver-last12-ewma-vs lk-driver-last12-ewma-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-driver-last12-ewma-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-driver-last12-ewma-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-driver-last12-ewma-setup">
  ...
  return (
    <div className="lk-driver-last12-ewma-wide">
      ...
      <div className="lk-driver-last12-ewma-split">
        <Panel className="lk-driver-last12-ewma-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-driver-last12-ewma-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-driver-last12-ewma-vs lk-driver-last12-ewma-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-driver-last12-ewma-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-driver-last12-ewma-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-driver-last12-ewma-lovat">
      ...
      <div className="lk-driver-last12-ewma-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-driver-last12-ewma-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-driver-last12-ewma-page">
      <PageHeader
