...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/driver-last24-consistency/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-driver-last24-consistency-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-driver-last24-consistency-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-driver-last24-consistency-vs lk-driver-last24-consistency-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-driver-last24-consistency-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-driver-last24-consistency-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-driver-last24-consistency-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-driver-last24-consistency-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-driver-last24-consistency-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-driver-last24-consistency-split">
        <Panel className="lk-driver-last24-consistency-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-driver-last24-consistency-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-driver-last24-consistency-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-driver-last24-consistency-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-driver-last24-consistency-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-driver-last24-consistency-vs lk-driver-last24-consistency-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-driver-last24-consistency-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-driver-last24-consistency-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-driver-last24-consistency-setup">
  ...
  return (
    <div className="lk-driver-last24-consistency-wide">
      ...
      <div className="lk-driver-last24-consistency-split">
        <Panel className="lk-driver-last24-consistency-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-driver-last24-consistency-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-driver-last24-consistency-vs lk-driver-last24-consistency-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-driver-last24-consistency-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-driver-last24-consistency-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-driver-last24-consistency-lovat">
      ...
      <div className="lk-driver-last24-consistency-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-driver-last24-consistency-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-driver-last24-consistency-page">
      <PageHeader
