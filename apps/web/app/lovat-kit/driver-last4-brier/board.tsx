...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/driver-last4-brier/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-driver-last4-brier-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-driver-last4-brier-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-driver-last4-brier-vs lk-driver-last4-brier-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-driver-last4-brier-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-driver-last4-brier-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-driver-last4-brier-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-driver-last4-brier-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-driver-last4-brier-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-driver-last4-brier-split">
        <Panel className="lk-driver-last4-brier-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-driver-last4-brier-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-driver-last4-brier-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-driver-last4-brier-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-driver-last4-brier-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-driver-last4-brier-vs lk-driver-last4-brier-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-driver-last4-brier-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-driver-last4-brier-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-driver-last4-brier-setup">
  ...
  return (
    <div className="lk-driver-last4-brier-wide">
      ...
      <div className="lk-driver-last4-brier-split">
        <Panel className="lk-driver-last4-brier-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-driver-last4-brier-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-driver-last4-brier-vs lk-driver-last4-brier-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-driver-last4-brier-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-driver-last4-brier-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-driver-last4-brier-lovat">
      ...
      <div className="lk-driver-last4-brier-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-driver-last4-brier-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-driver-last4-brier-page">
      <PageHeader
