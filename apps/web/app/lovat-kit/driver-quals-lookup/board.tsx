...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/driver-quals-lookup/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-driver-quals-lookup-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-driver-quals-lookup-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-driver-quals-lookup-vs lk-driver-quals-lookup-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-driver-quals-lookup-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-driver-quals-lookup-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-driver-quals-lookup-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-driver-quals-lookup-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-driver-quals-lookup-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-driver-quals-lookup-split">
        <Panel className="lk-driver-quals-lookup-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-driver-quals-lookup-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-driver-quals-lookup-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-driver-quals-lookup-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-driver-quals-lookup-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-driver-quals-lookup-vs lk-driver-quals-lookup-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-driver-quals-lookup-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-driver-quals-lookup-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-driver-quals-lookup-setup">
  ...
  return (
    <div className="lk-driver-quals-lookup-wide">
      ...
      <div className="lk-driver-quals-lookup-split">
        <Panel className="lk-driver-quals-lookup-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-driver-quals-lookup-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-driver-quals-lookup-vs lk-driver-quals-lookup-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-driver-quals-lookup-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-driver-quals-lookup-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-driver-quals-lookup-lovat">
      ...
      <div className="lk-driver-quals-lookup-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-driver-quals-lookup-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-driver-quals-lookup-page">
      <PageHeader
