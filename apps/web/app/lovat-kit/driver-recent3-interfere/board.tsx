...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/driver-recent3-interfere/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-driver-recent3-interfere-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-driver-recent3-interfere-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-driver-recent3-interfere-vs lk-driver-recent3-interfere-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-driver-recent3-interfere-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-driver-recent3-interfere-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-driver-recent3-interfere-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-driver-recent3-interfere-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-driver-recent3-interfere-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-driver-recent3-interfere-split">
        <Panel className="lk-driver-recent3-interfere-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-driver-recent3-interfere-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-driver-recent3-interfere-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-driver-recent3-interfere-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-driver-recent3-interfere-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-driver-recent3-interfere-vs lk-driver-recent3-interfere-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-driver-recent3-interfere-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-driver-recent3-interfere-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-driver-recent3-interfere-setup">
  ...
  return (
    <div className="lk-driver-recent3-interfere-wide">
      ...
      <div className="lk-driver-recent3-interfere-split">
        <Panel className="lk-driver-recent3-interfere-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-driver-recent3-interfere-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-driver-recent3-interfere-vs lk-driver-recent3-interfere-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-driver-recent3-interfere-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-driver-recent3-interfere-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-driver-recent3-interfere-lovat">
      ...
      <div className="lk-driver-recent3-interfere-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-driver-recent3-interfere-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-driver-recent3-interfere-page">
      <PageHeader
