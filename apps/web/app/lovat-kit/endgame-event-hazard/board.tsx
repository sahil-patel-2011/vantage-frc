...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/endgame-event-hazard/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-endgame-event-hazard-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-endgame-event-hazard-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-endgame-event-hazard-vs lk-endgame-event-hazard-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-endgame-event-hazard-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-endgame-event-hazard-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-endgame-event-hazard-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-endgame-event-hazard-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-endgame-event-hazard-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-endgame-event-hazard-split">
        <Panel className="lk-endgame-event-hazard-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-endgame-event-hazard-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-endgame-event-hazard-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-endgame-event-hazard-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-endgame-event-hazard-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-endgame-event-hazard-vs lk-endgame-event-hazard-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-endgame-event-hazard-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-endgame-event-hazard-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-endgame-event-hazard-setup">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-endgame-event-hazard-board" aria-label={chrome.event}>
      <header>
      ...
      <div className="lk-endgame-event-hazard-grid">
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-endgame-event-hazard-page">
      <PageHeader
