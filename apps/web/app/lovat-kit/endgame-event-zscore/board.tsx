...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/endgame-event-zscore/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-endgame-event-zscore-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-endgame-event-zscore-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-endgame-event-zscore-vs lk-endgame-event-zscore-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-endgame-event-zscore-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-endgame-event-zscore-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-endgame-event-zscore-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-endgame-event-zscore-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-endgame-event-zscore-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-endgame-event-zscore-split">
        <Panel className="lk-endgame-event-zscore-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-endgame-event-zscore-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-endgame-event-zscore-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-endgame-event-zscore-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-endgame-event-zscore-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-endgame-event-zscore-vs lk-endgame-event-zscore-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-endgame-event-zscore-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-endgame-event-zscore-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-endgame-event-zscore-setup">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-endgame-event-zscore-board" aria-label={chrome.event}>
      <header>
      ...
      <div className="lk-endgame-event-zscore-grid">
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-endgame-event-zscore-page">
      <PageHeader
