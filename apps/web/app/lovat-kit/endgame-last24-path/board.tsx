...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/endgame-last24-path/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-endgame-last24-path-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-endgame-last24-path-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-endgame-last24-path-vs lk-endgame-last24-path-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-endgame-last24-path-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-endgame-last24-path-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-endgame-last24-path-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-endgame-last24-path-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-endgame-last24-path-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-endgame-last24-path-split">
        <Panel className="lk-endgame-last24-path-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-endgame-last24-path-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-endgame-last24-path-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-endgame-last24-path-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-endgame-last24-path-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-endgame-last24-path-vs lk-endgame-last24-path-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-endgame-last24-path-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-endgame-last24-path-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-endgame-last24-path-setup">
  ...
  return (
    <div className="lk-endgame-last24-path-wide">
      ...
      <div className="lk-endgame-last24-path-split">
        <Panel className="lk-endgame-last24-path-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-endgame-last24-path-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-endgame-last24-path-vs lk-endgame-last24-path-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-endgame-last24-path-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-endgame-last24-path-scouts">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-endgame-last24-path-board" aria-label={chrome.event}>
      <header>
      ...
      <div className="lk-endgame-last24-path-grid">
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-endgame-last24-path-page">
      <PageHeader
