...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/endgame-last6-hazard/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-endgame-last6-hazard-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-endgame-last6-hazard-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-endgame-last6-hazard-vs lk-endgame-last6-hazard-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-endgame-last6-hazard-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-endgame-last6-hazard-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-endgame-last6-hazard-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-endgame-last6-hazard-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-endgame-last6-hazard-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-endgame-last6-hazard-split">
        <Panel className="lk-endgame-last6-hazard-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-endgame-last6-hazard-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-endgame-last6-hazard-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-endgame-last6-hazard-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-endgame-last6-hazard-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-endgame-last6-hazard-vs lk-endgame-last6-hazard-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-endgame-last6-hazard-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-endgame-last6-hazard-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-endgame-last6-hazard-setup">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-endgame-last6-hazard-board" aria-label={chrome.event}>
      <header>
      ...
      <div className="lk-endgame-last6-hazard-grid">
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-endgame-last6-hazard-page">
      <PageHeader
