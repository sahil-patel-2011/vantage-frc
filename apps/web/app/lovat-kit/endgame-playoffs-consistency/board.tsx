...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/endgame-playoffs-consistency/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-endgame-playoffs-consistency-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-endgame-playoffs-consistency-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-endgame-playoffs-consistency-vs lk-endgame-playoffs-consistency-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-endgame-playoffs-consistency-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-endgame-playoffs-consistency-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-endgame-playoffs-consistency-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-endgame-playoffs-consistency-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-endgame-playoffs-consistency-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-endgame-playoffs-consistency-split">
        <Panel className="lk-endgame-playoffs-consistency-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-endgame-playoffs-consistency-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-endgame-playoffs-consistency-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-endgame-playoffs-consistency-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-endgame-playoffs-consistency-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-endgame-playoffs-consistency-vs lk-endgame-playoffs-consistency-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-endgame-playoffs-consistency-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-endgame-playoffs-consistency-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-endgame-playoffs-consistency-setup">
...
function WideBoard({ report }: { report: ReturnType<typeof buildWideReport> }) {
  ...
  return (
    <div className="lk-endgame-playoffs-consistency-wide">
      <p className="app-muted">{report.headline}. Hodges–Lehmann and Theil–Sen stay blank until a real series exists.</p>
      <ul className="lk-endgame-playoffs-consistency-wide-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-endgame-playoffs-consistency-split">
        <Panel className="lk-endgame-playoffs-consistency-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-endgame-playoffs-consistency-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-endgame-playoffs-consistency-vs lk-endgame-playoffs-consistency-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-endgame-playoffs-consistency-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-endgame-playoffs-consistency-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-endgame-playoffs-consistency-lovat">
      ...
      <div className="lk-endgame-playoffs-consistency-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-endgame-playoffs-consistency-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-endgame-playoffs-consistency-page">
      <PageHeader
