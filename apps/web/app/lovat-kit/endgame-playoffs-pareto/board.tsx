...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/endgame-playoffs-pareto/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-endgame-playoffs-pareto-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-endgame-playoffs-pareto-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-endgame-playoffs-pareto-vs lk-endgame-playoffs-pareto-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-endgame-playoffs-pareto-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-endgame-playoffs-pareto-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-endgame-playoffs-pareto-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-endgame-playoffs-pareto-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-endgame-playoffs-pareto-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-endgame-playoffs-pareto-split">
        <Panel className="lk-endgame-playoffs-pareto-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-endgame-playoffs-pareto-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-endgame-playoffs-pareto-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-endgame-playoffs-pareto-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-endgame-playoffs-pareto-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-endgame-playoffs-pareto-vs lk-endgame-playoffs-pareto-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-endgame-playoffs-pareto-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-endgame-playoffs-pareto-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-endgame-playoffs-pareto-setup">
  ...
  return (
    <div className="lk-endgame-playoffs-pareto-wide">
      ...
      <div className="lk-endgame-playoffs-pareto-split">
        <Panel className="lk-endgame-playoffs-pareto-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-endgame-playoffs-pareto-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-endgame-playoffs-pareto-vs lk-endgame-playoffs-pareto-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-endgame-playoffs-pareto-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-endgame-playoffs-pareto-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-endgame-playoffs-pareto-lovat">
      ...
      <div className="lk-endgame-playoffs-pareto-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-endgame-playoffs-pareto-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-endgame-playoffs-pareto-page">
      <PageHeader
