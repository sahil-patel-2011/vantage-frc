...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/endgame-recent3-combo/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-endgame-recent3-combo-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-endgame-recent3-combo-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-endgame-recent3-combo-vs lk-endgame-recent3-combo-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-endgame-recent3-combo-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-endgame-recent3-combo-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-endgame-recent3-combo-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-endgame-recent3-combo-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-endgame-recent3-combo-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-endgame-recent3-combo-split">
        <Panel className="lk-endgame-recent3-combo-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-endgame-recent3-combo-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-endgame-recent3-combo-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-endgame-recent3-combo-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-endgame-recent3-combo-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-endgame-recent3-combo-vs lk-endgame-recent3-combo-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-endgame-recent3-combo-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-endgame-recent3-combo-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-endgame-recent3-combo-setup">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-endgame-recent3-combo-board" aria-label={chrome.event}>
      <header>
      ...
      <div className="lk-endgame-recent3-combo-grid">
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-endgame-recent3-combo-page">
      <PageHeader
