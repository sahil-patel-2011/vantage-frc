...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/endgame-recent3-residual/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-endgame-recent3-residual-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-endgame-recent3-residual-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-endgame-recent3-residual-vs lk-endgame-recent3-residual-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-endgame-recent3-residual-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-endgame-recent3-residual-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-endgame-recent3-residual-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-endgame-recent3-residual-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-endgame-recent3-residual-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-endgame-recent3-residual-split">
        <Panel className="lk-endgame-recent3-residual-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-endgame-recent3-residual-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-endgame-recent3-residual-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-endgame-recent3-residual-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-endgame-recent3-residual-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-endgame-recent3-residual-vs lk-endgame-recent3-residual-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-endgame-recent3-residual-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-endgame-recent3-residual-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-endgame-recent3-residual-setup">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-endgame-recent3-residual-board" aria-label={chrome.event}>
      <header>
      ...
      <div className="lk-endgame-recent3-residual-grid">
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-endgame-recent3-residual-page">
      <PageHeader
