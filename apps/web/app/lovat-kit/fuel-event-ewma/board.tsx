...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/fuel-event-ewma/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-fuel-event-ewma-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-fuel-event-ewma-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-fuel-event-ewma-vs lk-fuel-event-ewma-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-fuel-event-ewma-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-fuel-event-ewma-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-fuel-event-ewma-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-fuel-event-ewma-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-fuel-event-ewma-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-fuel-event-ewma-split">
        <Panel className="lk-fuel-event-ewma-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-fuel-event-ewma-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-fuel-event-ewma-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-fuel-event-ewma-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-fuel-event-ewma-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-fuel-event-ewma-vs lk-fuel-event-ewma-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-fuel-event-ewma-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-fuel-event-ewma-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-fuel-event-ewma-setup">
  ...
  return (
    <div className="lk-fuel-event-ewma-wide">
      ...
      <div className="lk-fuel-event-ewma-split">
        <Panel className="lk-fuel-event-ewma-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-fuel-event-ewma-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-fuel-event-ewma-vs lk-fuel-event-ewma-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-fuel-event-ewma-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-fuel-event-ewma-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-fuel-event-ewma-lovat">
      ...
      <div className="lk-fuel-event-ewma-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-fuel-event-ewma-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-fuel-event-ewma-page">
      <PageHeader
