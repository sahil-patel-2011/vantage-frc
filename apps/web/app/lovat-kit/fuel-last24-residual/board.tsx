...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/fuel-last24-residual/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-fuel-last24-residual-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-fuel-last24-residual-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-fuel-last24-residual-vs lk-fuel-last24-residual-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-fuel-last24-residual-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-fuel-last24-residual-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-fuel-last24-residual-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-fuel-last24-residual-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-fuel-last24-residual-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-fuel-last24-residual-split">
        <Panel className="lk-fuel-last24-residual-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-fuel-last24-residual-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-fuel-last24-residual-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-fuel-last24-residual-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-fuel-last24-residual-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-fuel-last24-residual-vs lk-fuel-last24-residual-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-fuel-last24-residual-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-fuel-last24-residual-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-fuel-last24-residual-setup">
  ...
  return (
    <div className="lk-fuel-last24-residual-wide">
      ...
      <div className="lk-fuel-last24-residual-split">
        <Panel className="lk-fuel-last24-residual-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-fuel-last24-residual-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-fuel-last24-residual-vs lk-fuel-last24-residual-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-fuel-last24-residual-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-fuel-last24-residual-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-fuel-last24-residual-lovat">
      ...
      <div className="lk-fuel-last24-residual-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-fuel-last24-residual-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-fuel-last24-residual-page">
      <PageHeader
