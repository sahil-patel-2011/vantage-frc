...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/fuel-last4-timeline22/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-fuel-last4-timeline22-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-fuel-last4-timeline22-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-fuel-last4-timeline22-vs lk-fuel-last4-timeline22-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-fuel-last4-timeline22-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-fuel-last4-timeline22-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-fuel-last4-timeline22-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-fuel-last4-timeline22-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-fuel-last4-timeline22-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-fuel-last4-timeline22-split">
        <Panel className="lk-fuel-last4-timeline22-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-fuel-last4-timeline22-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-fuel-last4-timeline22-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-fuel-last4-timeline22-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-fuel-last4-timeline22-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-fuel-last4-timeline22-vs lk-fuel-last4-timeline22-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-fuel-last4-timeline22-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-fuel-last4-timeline22-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-fuel-last4-timeline22-setup">
  ...
  return (
    <div className="lk-fuel-last4-timeline22-wide">
      ...
      <div className="lk-fuel-last4-timeline22-split">
        <Panel className="lk-fuel-last4-timeline22-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-fuel-last4-timeline22-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-fuel-last4-timeline22-vs lk-fuel-last4-timeline22-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-fuel-last4-timeline22-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-fuel-last4-timeline22-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-fuel-last4-timeline22-lovat">
      ...
      <div className="lk-fuel-last4-timeline22-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-fuel-last4-timeline22-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-fuel-last4-timeline22-page">
      <PageHeader
