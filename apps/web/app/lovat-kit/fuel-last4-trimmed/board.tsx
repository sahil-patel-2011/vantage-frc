...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/fuel-last4-trimmed/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-fuel-last4-trimmed-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-fuel-last4-trimmed-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-fuel-last4-trimmed-vs lk-fuel-last4-trimmed-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-fuel-last4-trimmed-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-fuel-last4-trimmed-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-fuel-last4-trimmed-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-fuel-last4-trimmed-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-fuel-last4-trimmed-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-fuel-last4-trimmed-split">
        <Panel className="lk-fuel-last4-trimmed-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-fuel-last4-trimmed-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-fuel-last4-trimmed-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-fuel-last4-trimmed-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-fuel-last4-trimmed-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-fuel-last4-trimmed-vs lk-fuel-last4-trimmed-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-fuel-last4-trimmed-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-fuel-last4-trimmed-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-fuel-last4-trimmed-setup">
  ...
  return (
    <div className="lk-fuel-last4-trimmed-wide">
      ...
      <div className="lk-fuel-last4-trimmed-split">
        <Panel className="lk-fuel-last4-trimmed-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-fuel-last4-trimmed-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-fuel-last4-trimmed-vs lk-fuel-last4-trimmed-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-fuel-last4-trimmed-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-fuel-last4-trimmed-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-fuel-last4-trimmed-lovat">
      ...
      <div className="lk-fuel-last4-trimmed-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-fuel-last4-trimmed-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-fuel-last4-trimmed-page">
      <PageHeader
