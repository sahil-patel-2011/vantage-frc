...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/reliability-event-context/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-reliability-event-context-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-reliability-event-context-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-reliability-event-context-vs lk-reliability-event-context-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-reliability-event-context-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-reliability-event-context-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-reliability-event-context-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-reliability-event-context-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-reliability-event-context-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-reliability-event-context-split">
        <Panel className="lk-reliability-event-context-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-reliability-event-context-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-reliability-event-context-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-reliability-event-context-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-reliability-event-context-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-reliability-event-context-vs lk-reliability-event-context-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-reliability-event-context-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-reliability-event-context-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-reliability-event-context-setup">
  ...
  return (
    <div className="lk-reliability-event-context-wide">
      ...
      <div className="lk-reliability-event-context-split">
        <Panel className="lk-reliability-event-context-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-reliability-event-context-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-reliability-event-context-vs lk-reliability-event-context-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-reliability-event-context-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-reliability-event-context-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-reliability-event-context-lovat">
      ...
      <div className="lk-reliability-event-context-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-reliability-event-context-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-reliability-event-context-page">
      <PageHeader
