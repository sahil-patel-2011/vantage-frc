...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/reliability-event-pareto/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-reliability-event-pareto-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-reliability-event-pareto-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-reliability-event-pareto-vs lk-reliability-event-pareto-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-reliability-event-pareto-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-reliability-event-pareto-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-reliability-event-pareto-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-reliability-event-pareto-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-reliability-event-pareto-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-reliability-event-pareto-split">
        <Panel className="lk-reliability-event-pareto-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-reliability-event-pareto-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-reliability-event-pareto-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-reliability-event-pareto-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-reliability-event-pareto-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-reliability-event-pareto-vs lk-reliability-event-pareto-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-reliability-event-pareto-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-reliability-event-pareto-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-reliability-event-pareto-setup">
  ...
  return (
    <div className="lk-reliability-event-pareto-wide">
      ...
      <div className="lk-reliability-event-pareto-split">
        <Panel className="lk-reliability-event-pareto-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-reliability-event-pareto-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-reliability-event-pareto-vs lk-reliability-event-pareto-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-reliability-event-pareto-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-reliability-event-pareto-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-reliability-event-pareto-lovat">
      ...
      <div className="lk-reliability-event-pareto-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-reliability-event-pareto-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-reliability-event-pareto-page">
      <PageHeader
