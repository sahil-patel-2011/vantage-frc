...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/teleop-event-playhead/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-teleop-event-playhead-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-teleop-event-playhead-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-teleop-event-playhead-vs lk-teleop-event-playhead-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-teleop-event-playhead-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-teleop-event-playhead-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-teleop-event-playhead-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-teleop-event-playhead-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-teleop-event-playhead-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-teleop-event-playhead-split">
        <Panel className="lk-teleop-event-playhead-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-teleop-event-playhead-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-teleop-event-playhead-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-teleop-event-playhead-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-teleop-event-playhead-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-teleop-event-playhead-vs lk-teleop-event-playhead-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-teleop-event-playhead-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-teleop-event-playhead-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-teleop-event-playhead-setup">
  ...
  return (
    <div className="lk-teleop-event-playhead-wide">
      ...
      <div className="lk-teleop-event-playhead-split">
        <Panel className="lk-teleop-event-playhead-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-teleop-event-playhead-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-teleop-event-playhead-vs lk-teleop-event-playhead-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-teleop-event-playhead-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-teleop-event-playhead-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-teleop-event-playhead-lovat">
      ...
      <div className="lk-teleop-event-playhead-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-teleop-event-playhead-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-teleop-event-playhead-page">
      <PageHeader
