...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/teleop-event-rank/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-teleop-event-rank-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-teleop-event-rank-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-teleop-event-rank-vs lk-teleop-event-rank-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-teleop-event-rank-board" aria-label={chrome.event}>
      <header>
      ...
      <div className="lk-teleop-event-rank-grid">
        {report.cards.map((card) => (
          <Tile key={card.id} card={card} />
        ))}
      </div>
      {setupReasons(report.cards).length ? (
        <ul className="lk-teleop-event-rank-setup">
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-teleop-event-rank-page">
      <PageHeader
