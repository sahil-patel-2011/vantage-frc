...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/teleop-last24-trimmed/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-teleop-last24-trimmed-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-teleop-last24-trimmed-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-teleop-last24-trimmed-vs lk-teleop-last24-trimmed-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-teleop-last24-trimmed-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-teleop-last24-trimmed-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-teleop-last24-trimmed-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-teleop-last24-trimmed-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-teleop-last24-trimmed-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-teleop-last24-trimmed-split">
        <Panel className="lk-teleop-last24-trimmed-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-teleop-last24-trimmed-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-teleop-last24-trimmed-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-teleop-last24-trimmed-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-teleop-last24-trimmed-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-teleop-last24-trimmed-vs lk-teleop-last24-trimmed-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-teleop-last24-trimmed-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-teleop-last24-trimmed-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-teleop-last24-trimmed-setup">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-teleop-last24-trimmed-board" aria-label={chrome.event}>
      <header>
      ...
      <div className="lk-teleop-last24-trimmed-grid">
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-teleop-last24-trimmed-page">
      <PageHeader
