...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/teleop-last4-consistency/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-teleop-last4-consistency-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-teleop-last4-consistency-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-teleop-last4-consistency-vs lk-teleop-last4-consistency-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-teleop-last4-consistency-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-teleop-last4-consistency-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-teleop-last4-consistency-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-teleop-last4-consistency-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-teleop-last4-consistency-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-teleop-last4-consistency-split">
        <Panel className="lk-teleop-last4-consistency-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-teleop-last4-consistency-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-teleop-last4-consistency-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-teleop-last4-consistency-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-teleop-last4-consistency-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-teleop-last4-consistency-vs lk-teleop-last4-consistency-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-teleop-last4-consistency-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-teleop-last4-consistency-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-teleop-last4-consistency-setup">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-teleop-last4-consistency-board" aria-label={chrome.event}>
      <header>
      ...
      <div className="lk-teleop-last4-consistency-grid">
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-teleop-last4-consistency-page">
      <PageHeader
