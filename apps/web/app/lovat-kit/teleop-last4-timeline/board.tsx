...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/teleop-last4-timeline/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-teleop-last4-timeline-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-teleop-last4-timeline-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-teleop-last4-timeline-vs lk-teleop-last4-timeline-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-teleop-last4-timeline-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-teleop-last4-timeline-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-teleop-last4-timeline-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-teleop-last4-timeline-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-teleop-last4-timeline-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-teleop-last4-timeline-split">
        <Panel className="lk-teleop-last4-timeline-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-teleop-last4-timeline-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-teleop-last4-timeline-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-teleop-last4-timeline-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-teleop-last4-timeline-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-teleop-last4-timeline-vs lk-teleop-last4-timeline-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-teleop-last4-timeline-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-teleop-last4-timeline-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-teleop-last4-timeline-setup">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-teleop-last4-timeline-board" aria-label={chrome.event}>
      <header>
      ...
      <div className="lk-teleop-last4-timeline-grid">
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-teleop-last4-timeline-page">
      <PageHeader
