...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/teleop-playoffs-win/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-teleop-playoffs-win-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-teleop-playoffs-win-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-teleop-playoffs-win-vs lk-teleop-playoffs-win-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-teleop-playoffs-win-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-teleop-playoffs-win-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-teleop-playoffs-win-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-teleop-playoffs-win-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-teleop-playoffs-win-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-teleop-playoffs-win-split">
        <Panel className="lk-teleop-playoffs-win-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-teleop-playoffs-win-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-teleop-playoffs-win-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-teleop-playoffs-win-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-teleop-playoffs-win-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-teleop-playoffs-win-vs lk-teleop-playoffs-win-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-teleop-playoffs-win-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-teleop-playoffs-win-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-teleop-playoffs-win-setup">
  ...
  return (
    <div className="lk-teleop-playoffs-win-wide">
      ...
      <div className="lk-teleop-playoffs-win-split">
        <Panel className="lk-teleop-playoffs-win-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-teleop-playoffs-win-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-teleop-playoffs-win-vs lk-teleop-playoffs-win-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-teleop-playoffs-win-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-teleop-playoffs-win-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-teleop-playoffs-win-lovat">
      ...
      <div className="lk-teleop-playoffs-win-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-teleop-playoffs-win-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-teleop-playoffs-win-page">
      <PageHeader
