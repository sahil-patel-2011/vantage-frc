...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/teleop-quals-hazard/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-teleop-quals-hazard-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-teleop-quals-hazard-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-teleop-quals-hazard-vs lk-teleop-quals-hazard-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-teleop-quals-hazard-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-teleop-quals-hazard-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-teleop-quals-hazard-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-teleop-quals-hazard-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-teleop-quals-hazard-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-teleop-quals-hazard-split">
        <Panel className="lk-teleop-quals-hazard-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-teleop-quals-hazard-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-teleop-quals-hazard-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-teleop-quals-hazard-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-teleop-quals-hazard-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-teleop-quals-hazard-vs lk-teleop-quals-hazard-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-teleop-quals-hazard-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-teleop-quals-hazard-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-teleop-quals-hazard-setup">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-teleop-quals-hazard-board" aria-label={chrome.event}>
      <header>
      ...
      <div className="lk-teleop-quals-hazard-grid">
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-teleop-quals-hazard-page">
      <PageHeader
