...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/lovat-kit/teleop-quals-picklist/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="lk-teleop-quals-picklist-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="lk-teleop-quals-picklist-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`lk-teleop-quals-picklist-vs lk-teleop-quals-picklist-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="lk-teleop-quals-picklist-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="lk-teleop-quals-picklist-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="lk-teleop-quals-picklist-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="lk-teleop-quals-picklist-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="lk-teleop-quals-picklist-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="lk-teleop-quals-picklist-split">
        <Panel className="lk-teleop-quals-picklist-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="lk-teleop-quals-picklist-zones">
            {zones.map((zone) => (
        ...
        <Panel className="lk-teleop-quals-picklist-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="lk-teleop-quals-picklist-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="lk-teleop-quals-picklist-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`lk-teleop-quals-picklist-vs lk-teleop-quals-picklist-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="lk-teleop-quals-picklist-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="lk-teleop-quals-picklist-pareto">
            {front.map((point) => (
      ...
      <ul className="lk-teleop-quals-picklist-setup">
  ...
  return (
    <div className="lk-teleop-quals-picklist-wide">
      ...
      <div className="lk-teleop-quals-picklist-split">
        <Panel className="lk-teleop-quals-picklist-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="lk-teleop-quals-picklist-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`lk-teleop-quals-picklist-vs lk-teleop-quals-picklist-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="lk-teleop-quals-picklist-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="lk-teleop-quals-picklist-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="lk-teleop-quals-picklist-lovat">
      ...
      <div className="lk-teleop-quals-picklist-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="lk-teleop-quals-picklist-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page lk-teleop-quals-picklist-page">
      <PageHeader
