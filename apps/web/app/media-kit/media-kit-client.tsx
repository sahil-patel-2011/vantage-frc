...
function isMediaKitView(value: unknown): value is MediaKitView {
  if (!value || typeof value !== "object") return false;
  const status = (value as { status?: unknown }).status;
  return status === "setup_required" || status === "live";
}

function mediaKitCacheOrg(data: MediaKitView, orgHint: string): string {
  if (typeof data.orgId === "string" && data.orgId.trim()) return data.orgId;
  return orgHint;
}

async function persistMediaKitSnapshot(
  orgHint: string,
  seasonHint: string,
  data: MediaKitView,
): Promise<void> {
  const cacheOrg = mediaKitCacheOrg(data, orgHint);
  if (!cacheOrg) return;
...
export default function MediaKitClient() {
  ...
  const viewRef = useRef<MediaKitView | null>(null);
  ...
  const load = useCallback((seasonOverride?: number) => {
    void (async () => {
      ...
      try {
        const cached = await getFeatureSnapshot<MediaKitView>(
          "media-kit",
          urlOrg || "_",
          seasonHint,
        );
        if (!viewRef.current && cached?.data && isMediaKitView(cached.data)) {
          setView(cached.data);
      ...
      try {
        const response = await fetch(
          `/api/media-kit${query.toString() ? `?${query.toString()}` : ""}`,
          {
            cache: "no-store",
        ...
        setFromCache(false);
        setCachedAt(null);
        await persistMediaKitSnapshot(urlOrg, seasonHint, data);
      } catch {
        if (hadCache || viewRef.current) {
          setFromCache(true);
          setError("Could not refresh Media Kit. Showing the last copy on this device.");
  ...
  const mutate = useCallback(
    async (payload: Record<string, unknown>) => {
      ...
      try {
        ...
        setView(data);
        setSeason(data.seasonYear);
        void persistMediaKitSnapshot(orgId, season != null ? String(season) : "", data);
      } catch {
