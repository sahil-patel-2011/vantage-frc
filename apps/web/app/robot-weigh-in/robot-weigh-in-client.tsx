...
function isRobotWeighInView(value: unknown): value is RobotWeighInView {
  if (!value || typeof value !== "object") return false;
  const status = (value as { status?: unknown }).status;
  return status === "setup_required" || status === "live";
}

async function persistWeighInSnapshot(
  orgHint: string,
  seasonHint: string,
  data: RobotWeighInView,
): Promise<void> {
  const cacheOrg =
  ...
  try {
    await putFeatureSnapshot("weigh-in", cacheOrg, data, seasonHint || seasonKey);
    if (!orgHint) await putFeatureSnapshot("weigh-in", "_", data, seasonHint || seasonKey);
  } catch {
    // Live weigh-in already painted; IndexedDB is best-effort.
  }
}
...
export default function RobotWeighInClient() {
  ...
  const viewRef = useRef<RobotWeighInView | null>(null);
  ...
  const load = useCallback((seasonOverride?: number) => {
    void (async () => {
      ...
      try {
        const cached = await getFeatureSnapshot<RobotWeighInView>(
          "weigh-in",
          urlOrg || "_",
          seasonHint,
        );
        if (!viewRef.current && cached?.data && isRobotWeighInView(cached.data)) {
          setView(cached.data);
      ...
      try {
        const response = await fetch(
          `/api/robot-weigh-in${query.toString() ? `?${query.toString()}` : ""}`,
          {
            cache: "no-store",
        ...
        if (!response.ok || !isRobotWeighInView(data)) {
          if (hadCache || viewRef.current) {
            setFromCache(true);
            setError("Could not refresh Robot weigh-in. Showing the last copy on this device.");
            setFetchFailed(false);
          } else {
            setFetchFailed(true);
          }
        ...
        setFromCache(false);
        setCachedAt(null);
        await persistWeighInSnapshot(urlOrg, seasonHint, data);
      } catch {
        if (hadCache || viewRef.current) {
          setFromCache(true);
          setError("Could not refresh Robot weigh-in. Showing the last copy on this device.");
  ...
  const mutate = useCallback(
    async (payload: Record<string, unknown>) => {
      ...
      try {
        ...
        setView(data);
        setSeason(data.seasonYear);
        void persistWeighInSnapshot(orgId, season != null ? String(season) : "", data);
      } catch {
  ...
  if (shell === "loading") {
    return (
      <WeighShell description={shellCopy.description} orgId={null} shell="loading">
        <OfflineBanner feature="Robot weigh-in" fromCache={fromCache} cachedAt={cachedAt} />
      </WeighShell>
    );
  }
  if (shell === "error") {
...
function LogWeighInForm({
  ...
  return (
    ...
    >
      ...
      <div>
        <Button variant="primary" type="submit" disabled={busy || !form.weighedOn || !form.weightLbs}>
          Log weigh-in
        </Button>
      </div>
    </Panel>
