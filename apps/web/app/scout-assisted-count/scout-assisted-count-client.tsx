...
function isScoutAssistedCountView(value: unknown): value is ScoutAssistedCountView {
  if (!value || typeof value !== "object") return false;
  const status = (value as { status?: unknown }).status;
  return status === "setup_required" || status === "live";
}

async function persistScoutAssistedCountSnapshot(
  orgHint: string,
  data: ScoutAssistedCountView,
): Promise<void> {
  const cacheOrg =
...
export default function ScoutAssistedCountClient() {
  ...
  const viewRef = useRef<ScoutAssistedCountView | null>(null);
  ...
  const load = useCallback(() => {
    void (async () => {
      ...
      try {
        const cached = await getFeatureSnapshot<ScoutAssistedCountView>(
          "scout-assisted-count",
          urlOrg || "_",
        );
        if (!viewRef.current && cached?.data && isScoutAssistedCountView(cached.data)) {
          setView(cached.data);
          setFromCache(true);
      ...
      try {
        const response = await fetch(
          `/api/scout-assisted-count${query.toString() ? `?${query.toString()}` : ""}`,
          {
            cache: "no-store",
        ...
        setFromCache(false);
        setCachedAt(null);
        await persistScoutAssistedCountSnapshot(urlOrg, data);
      } catch {
        if (hadCache || viewRef.current) {
          setFromCache(true);
          setError("Could not refresh Scout-Assisted Count. Showing the last copy on this device.");
  ...
  const mutate = useCallback(
    async (payload: Record<string, unknown>) => {
      ...
      try {
        ...
        setView(data);
        void persistScoutAssistedCountSnapshot(orgId, data);
      } catch {
