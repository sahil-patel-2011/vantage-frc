...
export default function SubsystemsClient({ orgId }: { orgId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    if (!response.ok) { setMessage(data.error ?? "Failed to load subsystems"); return; }
    setView(data);
  }, [orgId, seasonYear]);
  ...
  if (!view) return <main className="intel-app"><p className="telemetry-status">{message || "Loading subsystems…"}</p></main>;
  ...
  return (
    <main className="intel-app">
      <header className="intel-header">
        ...
        <nav className="intel-actions"><a href={`/wiring${orgId ? `?orgId=${orgId}` : ""}`}>Wiring</a><a href={`/cad${orgId ? `?orgId=${orgId}` : ""}`}>CAD</a><a href="/workspace">Workspace →</a></nav>
      </header>
