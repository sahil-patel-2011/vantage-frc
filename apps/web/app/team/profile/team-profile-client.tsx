...
export default function TeamProfileClient() {
  ...
  const header = (
    <PageHeader
      ...
      title={title}
      description="What the public record has on this team. Ask AI uses these facts as context."
    >
    ...
    <main className="module-page tp-page">
      ...
      <div className="tp-grid">
        <Panel className="tp-card">
          ...
          <dl className="tp-facts">
            ...
            <Fact label="Seasons competed" value={seasons > 0 ? `${seasons} (on record)` : null} />
            <Fact
              label="Website"
          ...
          {p?.name ? <small className="app-muted">Official name: {p.name}</small> : null}
        </Panel>
          ...
          {view.events.length === 0 ? (
            <p className="app-muted">
              {view.sources.tba?.ok === false
                ? `The public record did not answer${view.sources.tba.error ? ` (${view.sources.tba.error})` : ""}.`
        ...
        <Panel className="tp-card tp-wide">
          ...
          {view.awards.length === 0 ? (
            <p className="app-muted">
              {view.sources.tba?.ok === false ? "The public record did not answer." : "No awards on the public record."}
            </p>
          ) : (
      ...
      <footer className="tp-foot">
        <span className="app-muted">
          Sources: public record {view.sources.tba?.ok ? "✓" : "✗"} · season ratings {view.sources.statbotics?.ok ? "✓" : "✗"}
