...
export default function TeamAdminClient({ orgId }: { orgId: string }) {
  ...
  return (
    <main className="intel-app">
      <header className="intel-header"><div><span className="eyebrow">VANTAGE / TEAM ADMIN</span><h1>Membership control</h1></div><nav className="intel-actions"><a href={`/business?orgId=${orgId}`}>Business Portal</a><a href={`/team/security?orgId=${orgId}`}>Sign-in + 2FA</a><a href={`/team/data?orgId=${orgId}`}>Live data</a><a href={`/team/finance?orgId=${orgId}`}>Finance</a><a href={`/team/sponsors?orgId=${orgId}`}>Sponsors</a><a href={`/team/grants?orgId=${orgId}`}>Grants</a><a href={`/team/awards?orgId=${orgId}`}>Awards</a><a href={`/team/budgets?orgId=${orgId}`}>API budgets</a><a href={`/exports?orgId=${orgId}`}>Export</a><a href={`/showcase?orgId=${orgId}`}>Showcase</a><a href={`/intel?orgId=${orgId}`}>Intel →</a></nav></header>
