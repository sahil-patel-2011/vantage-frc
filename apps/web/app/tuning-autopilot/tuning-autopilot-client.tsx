...
function isTuningAutopilotView(value: unknown): value is TuningAutopilotView {
  if (!value || typeof value !== "object") return false;
  const status = (value as { status?: unknown }).status;
  return status === "setup_required" || status === "live";
}

function tuningAutopilotCacheOrg(data: TuningAutopilotView, orgHint: string): string {
  if ("orgId" in data && typeof data.orgId === "string" && data.orgId.trim()) return data.orgId;
  return orgHint;
}

async function persistTuningAutopilotSnapshot(
  orgHint: string,
  seasonHint: string,
  data: TuningAutopilotView,
): Promise<void> {
  const cacheOrg = tuningAutopilotCacheOrg(data, orgHint);
  if (!cacheOrg) return;
...
export default function TuningAutopilotClient() {
  ...
  const viewRef = useRef<TuningAutopilotView | null>(null);
  ...
  const load = useCallback((overrides?: { seasonYear?: number; sessionId?: string | null }) => {
    void (async () => {
      ...
      const seasonQuery =
        overrides?.seasonYear ?? (params.get("season") ? Number(params.get("season")) : null);
      const seasonHint =
        seasonQuery != null && Number.isFinite(seasonQuery) ? String(seasonQuery) : "";
      let hadCache = Boolean(viewRef.current);
      try {
        const cached = await getFeatureSnapshot<TuningAutopilotView>(
          "tuning-autopilot",
          urlOrg || "_",
          seasonHint,
        );
        if (!viewRef.current && cached?.data && isTuningAutopilotView(cached.data)) {
          setView(cached.data);
      ...
      try {
        const response = await fetch(
          `/api/tuning-autopilot${query.toString() ? `?${query.toString()}` : ""}`,
          {
            cache: "no-store",
        ...
        setFromCache(false);
        setCachedAt(null);
        await persistTuningAutopilotSnapshot(urlOrg, seasonHint, data);
      } catch {
        if (hadCache || viewRef.current) {
          setFromCache(true);
          setError("Could not refresh Tuning Autopilot. Showing the last copy on this device.");
  ...
  const mutate = useCallback(
    async (payload: Record<string, unknown>) => {
      ...
      try {
        ...
        setView(data);
        setSeason(data.seasonYear);
        void persistTuningAutopilotSnapshot(orgId, season != null ? String(season) : "", data);
      } catch {
    ...
    <main className="module-page tuning-autopilot-page">
      ...
      {shell === "empty" ? (
        <EmptyState
          ...
          description={shellCopy.description}
        >
          <Button as="a" variant="primary" href="#tuning-autopilot-new-session">
            Start a session
          </Button>
        </EmptyState>
      ) : null}
        ...
        <div className="tuning-autopilot-session-meta">
          ...
          <div className="tuning-autopilot-session-actions">
            {selected.session.status !== "converged" ? (
              <Button variant="secondary" type="button" disabled={busy} onClick={() => mutate({ action: "update-session-status", sessionId: selected.session.id, status: "converged" }) }>
                Mark converged
              </Button>
            ) : null}
            {selected.session.status !== "abandoned" ? (
              <Button variant="secondary" type="button" disabled={busy} onClick={() => mutate({ action: "update-session-status", sessionId: selected.session.id, status: "abandoned" }) }>
                Abandon
              </Button>
            ) : null}
            <button
...
function NewSessionForm({
  ...
  return (
    ...
    >
      ...
      <div>
        <Button variant="primary" type="submit" disabled={busy || !form.subsystem.trim()}>
          Start session
        </Button>
      </div>
    </Panel>
...
function IterationHistory({
  ...
  if (view.iterations.length === 0) {
    return (
      ...
      >
        <Button as="a" variant="primary" href="#tuning-autopilot-log-iteration">
          Log iteration
        </Button>
      </EmptyState>
    );
