...
function isVendorLeadTimesView(value: unknown): value is VendorLeadTimesView {
  if (!value || typeof value !== "object") return false;
  const status = (value as { status?: unknown }).status;
  return status === "setup_required" || status === "live";
}

async function persistVendorLeadTimesSnapshot(orgHint: string, data: VendorLeadTimesView): Promise<void> {
  const cacheOrg =
...
export default function VendorLeadTimesClient() {
  ...
  const viewRef = useRef<VendorLeadTimesView | null>(null);
  ...
  const load = useCallback(() => {
    void (async () => {
      ...
      try {
        const cached = await getFeatureSnapshot<VendorLeadTimesView>("vendor-lead-times", urlOrg || "_");
        if (!viewRef.current && cached?.data && isVendorLeadTimesView(cached.data)) {
          setView(cached.data);
          setFromCache(true);
      ...
      try {
        const response = await fetch(
          `/api/vendor-lead-times${query.toString() ? `?${query.toString()}` : ""}`,
          {
            cache: "no-store",
        ...
        setFromCache(false);
        setCachedAt(null);
        await persistVendorLeadTimesSnapshot(urlOrg, data);
      } catch {
        if (hadCache || viewRef.current) {
          setFromCache(true);
          setError("Could not refresh Lead times. Showing the last copy on this device.");
  ...
  const mutate = useCallback(
    async (payload: Record<string, unknown>) => {
      ...
      try {
        ...
        setView(data);
        void persistVendorLeadTimesSnapshot(orgId, data);
      } catch {
