...
async function persistVendorsSnapshot(orgHint: string, data: VendorsView): Promise<void> {
  const cacheOrg =
    "orgId" in data && typeof data.orgId === "string" && data.orgId.trim() ? data.orgId : orgHint;
  if (!cacheOrg) return;
  try {
    await putFeatureSnapshot("vendors", cacheOrg, data);
    if (!orgHint) await putFeatureSnapshot("vendors", "_", data);
  } catch {
    // Live Vendors already painted; IndexedDB is best-effort.
  }
}
...
export default function VendorsClient() {
  ...
  const viewRef = useRef<VendorsView | null>(null);
  ...
  const load = useCallback(() => {
    void (async () => {
      ...
      try {
        const cached = await getFeatureSnapshot<VendorsView>("vendors", urlOrg || "_");
        if (!viewRef.current && cached?.data && isVendorsView(cached.data)) {
          setView(cached.data);
          setFromCache(true);
      ...
      try {
        const response = await fetch(
          `/api/vendors${urlOrg ? `?orgId=${encodeURIComponent(urlOrg)}` : ""}`,
          {
            cache: "no-store",
        ...
        if (!response.ok || !isVendorsView(data)) {
          if (hadCache || viewRef.current) {
            setFromCache(true);
            setError("Could not refresh Vendors. Showing the last copy on this device.");
            setFetchFailed(false);
          } else {
            setFetchFailed(true);
          }
    ...
    (payload) => {
      ...
      void fetch("/api/vendors", {
        ...
        .then(async (response) => {
          ...
          setView(data);
          void persistVendorsSnapshot(orgId, data);
        })
  ...
  if (shell === "loading") {
    return (
      <VendorsShell description={shellCopy.description} orgId={null} shell="loading">
        <OfflineBanner feature="Vendors" fromCache={fromCache} cachedAt={cachedAt} />
      </VendorsShell>
    );
  }
  ...
  if (view?.status !== "live") {
    return (
      <VendorsShell description={shellCopy.description} orgId={orgId} shell="setup">
        <OfflineBanner feature="Vendors" fromCache={fromCache} cachedAt={cachedAt} />
      </VendorsShell>
    );
  }
