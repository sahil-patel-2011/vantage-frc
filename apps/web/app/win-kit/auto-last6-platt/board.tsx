...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/win-kit/auto-last6-platt/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="wk-auto-last6-platt-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="wk-auto-last6-platt-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`wk-auto-last6-platt-vs wk-auto-last6-platt-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="wk-auto-last6-platt-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="wk-auto-last6-platt-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="wk-auto-last6-platt-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="wk-auto-last6-platt-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="wk-auto-last6-platt-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="wk-auto-last6-platt-split">
        <Panel className="wk-auto-last6-platt-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="wk-auto-last6-platt-zones">
            {zones.map((zone) => (
        ...
        <Panel className="wk-auto-last6-platt-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="wk-auto-last6-platt-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="wk-auto-last6-platt-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`wk-auto-last6-platt-vs wk-auto-last6-platt-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="wk-auto-last6-platt-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="wk-auto-last6-platt-pareto">
            {front.map((point) => (
      ...
      <ul className="wk-auto-last6-platt-setup">
  ...
  return (
    <div className="wk-auto-last6-platt-wide">
      ...
      <div className="wk-auto-last6-platt-split">
        <Panel className="wk-auto-last6-platt-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="wk-auto-last6-platt-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`wk-auto-last6-platt-vs wk-auto-last6-platt-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="wk-auto-last6-platt-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="wk-auto-last6-platt-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="wk-auto-last6-platt-lovat">
      ...
      <div className="wk-auto-last6-platt-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="wk-auto-last6-platt-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page wk-auto-last6-platt-page">
      <PageHeader
