...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/win-kit/auto-quals-corr/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="wk-auto-quals-corr-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="wk-auto-quals-corr-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`wk-auto-quals-corr-vs wk-auto-quals-corr-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="wk-auto-quals-corr-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="wk-auto-quals-corr-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="wk-auto-quals-corr-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="wk-auto-quals-corr-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="wk-auto-quals-corr-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="wk-auto-quals-corr-split">
        <Panel className="wk-auto-quals-corr-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="wk-auto-quals-corr-zones">
            {zones.map((zone) => (
        ...
        <Panel className="wk-auto-quals-corr-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="wk-auto-quals-corr-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="wk-auto-quals-corr-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`wk-auto-quals-corr-vs wk-auto-quals-corr-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="wk-auto-quals-corr-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="wk-auto-quals-corr-pareto">
            {front.map((point) => (
      ...
      <ul className="wk-auto-quals-corr-setup">
  ...
  return (
    <div className="wk-auto-quals-corr-wide">
      ...
      <div className="wk-auto-quals-corr-split">
        <Panel className="wk-auto-quals-corr-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="wk-auto-quals-corr-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`wk-auto-quals-corr-vs wk-auto-quals-corr-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="wk-auto-quals-corr-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="wk-auto-quals-corr-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="wk-auto-quals-corr-lovat">
      ...
      <div className="wk-auto-quals-corr-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="wk-auto-quals-corr-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page wk-auto-quals-corr-page">
      <PageHeader
