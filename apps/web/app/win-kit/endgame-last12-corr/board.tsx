...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/win-kit/endgame-last12-corr/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="wk-endgame-last12-corr-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="wk-endgame-last12-corr-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`wk-endgame-last12-corr-vs wk-endgame-last12-corr-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="wk-endgame-last12-corr-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="wk-endgame-last12-corr-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="wk-endgame-last12-corr-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="wk-endgame-last12-corr-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="wk-endgame-last12-corr-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="wk-endgame-last12-corr-split">
        <Panel className="wk-endgame-last12-corr-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="wk-endgame-last12-corr-zones">
            {zones.map((zone) => (
        ...
        <Panel className="wk-endgame-last12-corr-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="wk-endgame-last12-corr-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="wk-endgame-last12-corr-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`wk-endgame-last12-corr-vs wk-endgame-last12-corr-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="wk-endgame-last12-corr-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="wk-endgame-last12-corr-pareto">
            {front.map((point) => (
      ...
      <ul className="wk-endgame-last12-corr-setup">
  ...
  return (
    <div className="wk-endgame-last12-corr-wide">
      ...
      <div className="wk-endgame-last12-corr-split">
        <Panel className="wk-endgame-last12-corr-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="wk-endgame-last12-corr-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`wk-endgame-last12-corr-vs wk-endgame-last12-corr-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="wk-endgame-last12-corr-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="wk-endgame-last12-corr-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="wk-endgame-last12-corr-lovat">
      ...
      <div className="wk-endgame-last12-corr-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="wk-endgame-last12-corr-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page wk-endgame-last12-corr-page">
      <PageHeader
