...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/win-kit/endgame-quals-temperature/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="wk-endgame-quals-temperature-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="wk-endgame-quals-temperature-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`wk-endgame-quals-temperature-vs wk-endgame-quals-temperature-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="wk-endgame-quals-temperature-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="wk-endgame-quals-temperature-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="wk-endgame-quals-temperature-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="wk-endgame-quals-temperature-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="wk-endgame-quals-temperature-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="wk-endgame-quals-temperature-split">
        <Panel className="wk-endgame-quals-temperature-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="wk-endgame-quals-temperature-zones">
            {zones.map((zone) => (
        ...
        <Panel className="wk-endgame-quals-temperature-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="wk-endgame-quals-temperature-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="wk-endgame-quals-temperature-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`wk-endgame-quals-temperature-vs wk-endgame-quals-temperature-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="wk-endgame-quals-temperature-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="wk-endgame-quals-temperature-pareto">
            {front.map((point) => (
      ...
      <ul className="wk-endgame-quals-temperature-setup">
  ...
  return (
    <div className="wk-endgame-quals-temperature-wide">
      ...
      <div className="wk-endgame-quals-temperature-split">
        <Panel className="wk-endgame-quals-temperature-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="wk-endgame-quals-temperature-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`wk-endgame-quals-temperature-vs wk-endgame-quals-temperature-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="wk-endgame-quals-temperature-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="wk-endgame-quals-temperature-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="wk-endgame-quals-temperature-lovat">
      ...
      <div className="wk-endgame-quals-temperature-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="wk-endgame-quals-temperature-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page wk-endgame-quals-temperature-page">
      <PageHeader
