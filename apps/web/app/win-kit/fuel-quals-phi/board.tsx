...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/win-kit/fuel-quals-phi/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="wk-fuel-quals-phi-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="wk-fuel-quals-phi-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`wk-fuel-quals-phi-vs wk-fuel-quals-phi-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="wk-fuel-quals-phi-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="wk-fuel-quals-phi-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="wk-fuel-quals-phi-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="wk-fuel-quals-phi-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="wk-fuel-quals-phi-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="wk-fuel-quals-phi-split">
        <Panel className="wk-fuel-quals-phi-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="wk-fuel-quals-phi-zones">
            {zones.map((zone) => (
        ...
        <Panel className="wk-fuel-quals-phi-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="wk-fuel-quals-phi-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="wk-fuel-quals-phi-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`wk-fuel-quals-phi-vs wk-fuel-quals-phi-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="wk-fuel-quals-phi-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="wk-fuel-quals-phi-pareto">
            {front.map((point) => (
      ...
      <ul className="wk-fuel-quals-phi-setup">
  ...
  return (
    <div className="wk-fuel-quals-phi-wide">
      ...
      <div className="wk-fuel-quals-phi-split">
        <Panel className="wk-fuel-quals-phi-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="wk-fuel-quals-phi-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`wk-fuel-quals-phi-vs wk-fuel-quals-phi-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="wk-fuel-quals-phi-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="wk-fuel-quals-phi-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="wk-fuel-quals-phi-lovat">
      ...
      <div className="wk-fuel-quals-phi-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="wk-fuel-quals-phi-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page wk-fuel-quals-phi-page">
      <PageHeader
