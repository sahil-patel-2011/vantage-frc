...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/win-kit/teleop-event-platt/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="wk-teleop-event-platt-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="wk-teleop-event-platt-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`wk-teleop-event-platt-vs wk-teleop-event-platt-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="wk-teleop-event-platt-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="wk-teleop-event-platt-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="wk-teleop-event-platt-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="wk-teleop-event-platt-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="wk-teleop-event-platt-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="wk-teleop-event-platt-split">
        <Panel className="wk-teleop-event-platt-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="wk-teleop-event-platt-zones">
            {zones.map((zone) => (
        ...
        <Panel className="wk-teleop-event-platt-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="wk-teleop-event-platt-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="wk-teleop-event-platt-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`wk-teleop-event-platt-vs wk-teleop-event-platt-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="wk-teleop-event-platt-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="wk-teleop-event-platt-pareto">
            {front.map((point) => (
      ...
      <ul className="wk-teleop-event-platt-setup">
  ...
  return (
    <div className="wk-teleop-event-platt-wide">
      ...
      <div className="wk-teleop-event-platt-split">
        <Panel className="wk-teleop-event-platt-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="wk-teleop-event-platt-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`wk-teleop-event-platt-vs wk-teleop-event-platt-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="wk-teleop-event-platt-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="wk-teleop-event-platt-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="wk-teleop-event-platt-lovat">
      ...
      <div className="wk-teleop-event-platt-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="wk-teleop-event-platt-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page wk-teleop-event-platt-page">
      <PageHeader
