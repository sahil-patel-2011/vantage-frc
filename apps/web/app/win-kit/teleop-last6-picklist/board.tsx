...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/win-kit/teleop-last6-picklist/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="wk-teleop-last6-picklist-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="wk-teleop-last6-picklist-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`wk-teleop-last6-picklist-vs wk-teleop-last6-picklist-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="wk-teleop-last6-picklist-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="wk-teleop-last6-picklist-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="wk-teleop-last6-picklist-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="wk-teleop-last6-picklist-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="wk-teleop-last6-picklist-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="wk-teleop-last6-picklist-split">
        <Panel className="wk-teleop-last6-picklist-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="wk-teleop-last6-picklist-zones">
            {zones.map((zone) => (
        ...
        <Panel className="wk-teleop-last6-picklist-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="wk-teleop-last6-picklist-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="wk-teleop-last6-picklist-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`wk-teleop-last6-picklist-vs wk-teleop-last6-picklist-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="wk-teleop-last6-picklist-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="wk-teleop-last6-picklist-pareto">
            {front.map((point) => (
      ...
      <ul className="wk-teleop-last6-picklist-setup">
  ...
  return (
    <div className="wk-teleop-last6-picklist-wide">
      ...
      <div className="wk-teleop-last6-picklist-split">
        <Panel className="wk-teleop-last6-picklist-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="wk-teleop-last6-picklist-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`wk-teleop-last6-picklist-vs wk-teleop-last6-picklist-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="wk-teleop-last6-picklist-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="wk-teleop-last6-picklist-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="wk-teleop-last6-picklist-lovat">
      ...
      <div className="wk-teleop-last6-picklist-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="wk-teleop-last6-picklist-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page wk-teleop-last6-picklist-page">
      <PageHeader
