...
import {
  ...
  type MetricCard,
  type SampleRow,
} from "../../../lib/win-kit/teleop-quals-picklist/compute";
import "./board.css";

function Spark({ path }: { path: string }) {
  return (
    <svg className="wk-teleop-quals-picklist-spark" viewBox="0 0 72 28" aria-hidden="true">
...
function Tile({ card }: { card: MetricCard }) {
  return (
    <Panel className="wk-teleop-quals-picklist-tile motion-tile" style={{ minHeight: "auto" }}>
      <span className="app-muted">{card.label}</span>
      <strong>{card.display}</strong>
      <small className={`wk-teleop-quals-picklist-vs wk-teleop-quals-picklist-vs-${card.compare.tone}`}>{card.compare.label}</small>
...
function DeepMeter({ label, value }: { label: string; value: string }) {
  return (
    <li className="wk-teleop-quals-picklist-meter">
...
function ZoneRow({ label, width, empty }: { label: string; width: number; empty: boolean }) {
  return (
    <li className="wk-teleop-quals-picklist-zone">
      <span>{label}</span>
      {empty ? (
        <em className="app-muted">Needs setup</em>
      ) : (
        <i className="wk-teleop-quals-picklist-bar" style={{ width: `${width}%` }} />
      )}
    </li>
  );
}
...
function DeepBoard({
  ...
  return (
    <div className="wk-teleop-quals-picklist-deep">
      <p className="app-muted">{deepHeadline(report)}. {noticeableFocus()}</p>
      <ul className="wk-teleop-quals-picklist-meters">
        {sections.map((section) => (
          <DeepMeter key={section.id} label={section.title} value={section.body} />
        ))}
      </ul>
      <div className="wk-teleop-quals-picklist-split">
        <Panel className="wk-teleop-quals-picklist-panel" style={{ minHeight: "auto" }}>
          <h4>Field zones</h4>
          <ul className="wk-teleop-quals-picklist-zones">
            {zones.map((zone) => (
        ...
        <Panel className="wk-teleop-quals-picklist-panel" style={{ minHeight: "auto" }}>
          ...
          {colley.length === 0 ? (
            ...
            <ol className="wk-teleop-quals-picklist-colley">
              {colley.map((row) => (
                <li key={row.teamKey}>
          ...
          <ul className="wk-teleop-quals-picklist-residual">
            {report.residuals.map((row) => (
              <li key={row.metric} className={`wk-teleop-quals-picklist-vs wk-teleop-quals-picklist-vs-${residualTone(row.residual)}`}>
      ...
      <Panel className="wk-teleop-quals-picklist-panel" style={{ minHeight: "auto" }}>
        ...
        {front.length === 0 ? (
          ...
          <ul className="wk-teleop-quals-picklist-pareto">
            {front.map((point) => (
      ...
      <ul className="wk-teleop-quals-picklist-setup">
  ...
  return (
    <div className="wk-teleop-quals-picklist-wide">
      ...
      <div className="wk-teleop-quals-picklist-split">
        <Panel className="wk-teleop-quals-picklist-panel" style={{ minHeight: "auto" }}>
          <h4>Match slots</h4>
          <ol className="wk-teleop-quals-picklist-slots">
            {report.slots.map((slot) => (
              <li key={slot.index} className={`wk-teleop-quals-picklist-vs wk-teleop-quals-picklist-vs-${slot.tone}`}>{formatSlot(slot)}</li>
            ))}
          </ol>
        </Panel>
        <Panel className="wk-teleop-quals-picklist-panel" style={{ minHeight: "auto" }}>
          <h4>Scout agreement</h4>
          <ul className="wk-teleop-quals-picklist-scouts">
...
function LovatExactBoard({ report }: { report: ReturnType<typeof buildLovatExactReport> }) {
  return (
    <div className="wk-teleop-quals-picklist-lovat">
      ...
      <div className="wk-teleop-quals-picklist-grid">
...
export function LovatKitBoard({
  ...
  return (
    <section className="wk-teleop-quals-picklist-board" aria-label={chrome.event}>
      <header>
...
export default function LovatKitPageClient() {
  return (
    <main className="module-page wk-teleop-quals-picklist-page">
      <PageHeader
