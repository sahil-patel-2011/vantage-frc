...
describe("CAN-bus map last snapshot stays on the phone", () => {
  it("reads and writes the wiring IndexedDB feature cache and does not blank a painted board", () => {
    const src = [
      readFileSync(join(DIR, "wiring-client.tsx"), "utf8"),
      readFileSync(join(DIR, "../api/wiring/route.ts"), "utf8"),
    ].join("\n");
    expect(src).toMatch(/getFeatureSnapshot/);
    expect(src).toMatch(/putFeatureSnapshot/);
    expect(src).toMatch(/"wiring"/);
    expect(src).toMatch(/FEATURE_API_TIMEOUT_MS/);
    expect(src).toMatch(/feature="CAN-bus map"/);
    ...
    expect(src).not.toMatch(/VANTAGE \/ WIRING/);
