/**
 * Leftover agent kit — alliance-event-autonomous.
...
/**
 * Lovat-style Alliance board — this event, autonomous loop.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "alliance-event-autonomous";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 925179704): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
