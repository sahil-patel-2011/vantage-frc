/**
 * Leftover agent kit — alliance-event-cadhop.
...
/**
 * Lovat-style Alliance board — this event, CAD hop persist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "alliance-event-cadhop";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4031813947): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
