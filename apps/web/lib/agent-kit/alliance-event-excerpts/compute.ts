/**
 * Leftover agent kit — alliance-event-excerpts.
...
/**
 * Lovat-style Alliance board — this event, tool excerpts.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "alliance-event-excerpts";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4268278542): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
