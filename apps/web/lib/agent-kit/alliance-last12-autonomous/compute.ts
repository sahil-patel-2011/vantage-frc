/**
 * Leftover agent kit — alliance-last12-autonomous.
...
/**
 * Lovat-style Alliance board — last 12 matches, autonomous loop.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "alliance-last12-autonomous";
export const PHASE_LABEL = "Alliance";
...
export type LovatMetricId = "totalPoints" | "autoPoints" | "teleopPoints" | "endgameClimb";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3341766647): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
...
export function seriesTotalPoints(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["totalPoints" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statTotalPoints(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesTotalPoints(rows, teamKey));
}

export function shrinkTotalPoints(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesTotalPoints(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["totalPoints" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "totalPoints" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastTotalPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesTotalPoints(rows, teamKey));
}

export function fitTotalPoints(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesTotalPoints(rows, teamKey));
}

export function controlTotalPoints(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesTotalPoints(rows, teamKey));
}

export function slotsTotalPoints(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesTotalPoints(rows, teamKey));
}

export function residualTotalPoints(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "totalPoints" as LovatMetricId, oppKeys);
}

export function histTotalPoints(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesTotalPoints(rows, teamKey));
}

export function cvTotalPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesTotalPoints(rows, teamKey));
}

export function trimTotalPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesTotalPoints(rows, teamKey));
}

export function ewmaTotalPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesTotalPoints(rows, teamKey));
}

export function jackTotalPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesTotalPoints(rows, teamKey));
}

export function bootTotalPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesTotalPoints(rows, teamKey));
}

export function hazardTotalPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesTotalPoints(rows, teamKey));
}

export function streakTotalPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesTotalPoints(rows, teamKey));
}

export function giniTotalPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesTotalPoints(rows, teamKey));
}

export function theilTotalPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesTotalPoints(rows, teamKey));
}

export function skewTotalPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesTotalPoints(rows, teamKey));
}

export function rangeTotalPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesTotalPoints(rows, teamKey));
}

export function madMeanTotalPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesTotalPoints(rows, teamKey));
}

export function packTotalPoints(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesTotalPoints(rows, teamKey);
  return {
    id: "totalPoints" as LovatMetricId,
    label: "Total points",
    series,
    stat: wideStat(series),
    shrink: shrinkTotalPoints(rows, teamKey, fieldRows),
    forecast: forecastTotalPoints(rows, teamKey),
    fit: fitTotalPoints(rows, teamKey),
    control: controlTotalPoints(rows, teamKey),
    slots: slotsTotalPoints(rows, teamKey),
  };
}

export function describeTotalPoints(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Total points · Needs setup`;
  return `Total points · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}
...
export function rollTotalPoints2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 2);
}

export function rollTotalPoints2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints2(rows, teamKey);
  const last = series[series.length - 1];
...
export function rollTotalPoints2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints2Last(rows, teamKey) != null;
}

export function describeRollTotalPoints2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints2Last(rows, teamKey);
  if (last == null) return "Total points" + " last 2: Needs setup";
  return "Total points" + " last 2: " + formatValue(last, 1);
}

export function rollTotalPoints3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 3);
}

export function rollTotalPoints3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints3Last(rows, teamKey) != null;
}

export function describeRollTotalPoints3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints3Last(rows, teamKey);
  if (last == null) return "Total points" + " last 3: Needs setup";
  return "Total points" + " last 3: " + formatValue(last, 1);
}

export function rollTotalPoints4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 4);
}

export function rollTotalPoints4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints4Last(rows, teamKey) != null;
}

export function describeRollTotalPoints4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints4Last(rows, teamKey);
  if (last == null) return "Total points" + " last 4: Needs setup";
  return "Total points" + " last 4: " + formatValue(last, 1);
}

export function rollTotalPoints5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 5);
}

export function rollTotalPoints5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints5Last(rows, teamKey) != null;
}

export function describeRollTotalPoints5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints5Last(rows, teamKey);
  if (last == null) return "Total points" + " last 5: Needs setup";
  return "Total points" + " last 5: " + formatValue(last, 1);
}

export function rollTotalPoints6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 6);
}

export function rollTotalPoints6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints6Last(rows, teamKey) != null;
}

export function describeRollTotalPoints6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints6Last(rows, teamKey);
  if (last == null) return "Total points" + " last 6: Needs setup";
  return "Total points" + " last 6: " + formatValue(last, 1);
}

export function rollTotalPoints7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 7);
}

export function rollTotalPoints7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints7Last(rows, teamKey) != null;
}

export function describeRollTotalPoints7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints7Last(rows, teamKey);
  if (last == null) return "Total points" + " last 7: Needs setup";
  return "Total points" + " last 7: " + formatValue(last, 1);
}

export function rollTotalPoints8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 8);
}

export function rollTotalPoints8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints8Last(rows, teamKey) != null;
}

export function describeRollTotalPoints8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints8Last(rows, teamKey);
  if (last == null) return "Total points" + " last 8: Needs setup";
  return "Total points" + " last 8: " + formatValue(last, 1);
}

export function rollTotalPoints9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 9);
}

export function rollTotalPoints9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints9Last(rows, teamKey) != null;
}

export function describeRollTotalPoints9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints9Last(rows, teamKey);
  if (last == null) return "Total points" + " last 9: Needs setup";
  return "Total points" + " last 9: " + formatValue(last, 1);
}

export function rollTotalPoints10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 10);
}

export function rollTotalPoints10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints10Last(rows, teamKey) != null;
}

export function describeRollTotalPoints10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints10Last(rows, teamKey);
  if (last == null) return "Total points" + " last 10: Needs setup";
  return "Total points" + " last 10: " + formatValue(last, 1);
}

export function rollTotalPoints12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 12);
}

export function rollTotalPoints12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints12Last(rows, teamKey) != null;
}

export function describeRollTotalPoints12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints12Last(rows, teamKey);
  if (last == null) return "Total points" + " last 12: Needs setup";
  return "Total points" + " last 12: " + formatValue(last, 1);
}

export function rollTotalPoints14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 14);
}

export function rollTotalPoints14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints14Last(rows, teamKey) != null;
}

export function describeRollTotalPoints14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints14Last(rows, teamKey);
  if (last == null) return "Total points" + " last 14: Needs setup";
  return "Total points" + " last 14: " + formatValue(last, 1);
}

export function rollTotalPoints16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 16);
}

export function rollTotalPoints16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints16Last(rows, teamKey) != null;
}

export function describeRollTotalPoints16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints16Last(rows, teamKey);
  if (last == null) return "Total points" + " last 16: Needs setup";
  return "Total points" + " last 16: " + formatValue(last, 1);
}

export function rollTotalPoints18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 18);
}

export function rollTotalPoints18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints18Last(rows, teamKey) != null;
}

export function describeRollTotalPoints18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints18Last(rows, teamKey);
  if (last == null) return "Total points" + " last 18: Needs setup";
  return "Total points" + " last 18: " + formatValue(last, 1);
}

export function rollTotalPoints20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 20);
}

export function rollTotalPoints20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints20Last(rows, teamKey) != null;
}

export function describeRollTotalPoints20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints20Last(rows, teamKey);
  if (last == null) return "Total points" + " last 20: Needs setup";
  return "Total points" + " last 20: " + formatValue(last, 1);
}

export function rollTotalPoints22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 22);
}

export function rollTotalPoints22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints22Last(rows, teamKey) != null;
}

export function describeRollTotalPoints22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints22Last(rows, teamKey);
  if (last == null) return "Total points" + " last 22: Needs setup";
  return "Total points" + " last 22: " + formatValue(last, 1);
}

export function rollTotalPoints24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 24);
}

export function rollTotalPoints24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints24Last(rows, teamKey) != null;
}

export function describeRollTotalPoints24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints24Last(rows, teamKey);
  if (last == null) return "Total points" + " last 24: Needs setup";
  return "Total points" + " last 24: " + formatValue(last, 1);
}

export function rollTotalPoints28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 28);
}

export function rollTotalPoints28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints28Last(rows, teamKey) != null;
}

export function describeRollTotalPoints28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints28Last(rows, teamKey);
  if (last == null) return "Total points" + " last 28: Needs setup";
  return "Total points" + " last 28: " + formatValue(last, 1);
}

export function rollTotalPoints32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTotalPoints(rows, teamKey), 32);
}

export function rollTotalPoints32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTotalPoints32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTotalPoints32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTotalPoints32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTotalPoints32Last(rows, teamKey) != null;
}

export function describeRollTotalPoints32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTotalPoints32Last(rows, teamKey);
  if (last == null) return "Total points" + " last 32: Needs setup";
  return "Total points" + " last 32: " + formatValue(last, 1);
}
...
export function buildWideReport(input: { teamKey: string; rows: readonly SampleRow[]; fieldRows?: readonly SampleRow[] }): WideReport {
  const packs: WideMetricPack[] = [
    packTotalPoints(input.rows, input.teamKey, input.fieldRows),
