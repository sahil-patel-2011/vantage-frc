/**
 * Leftover agent kit — alliance-last12-excerpts.
...
/**
 * Lovat-style Alliance board — last 12 matches, tool excerpts.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "alliance-last12-excerpts";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2497387317): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
