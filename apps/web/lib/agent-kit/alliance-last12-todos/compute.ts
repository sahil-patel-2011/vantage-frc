/**
 * Leftover agent kit — alliance-last12-todos.
...
/**
 * Lovat-style Alliance board — last 12 matches, working todos.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "alliance-last12-todos";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1306504364): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
