/**
 * Leftover agent kit — alliance-last6-autonomous.
...
/**
 * Lovat-style Alliance board — last 6 matches, autonomous loop.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "alliance-last6-autonomous";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3682432950): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
