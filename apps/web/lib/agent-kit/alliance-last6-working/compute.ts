/**
 * Leftover agent kit — alliance-last6-working.
...
/**
 * Lovat-style Alliance board — last 6 matches, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "alliance-last6-working";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1479234665): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
