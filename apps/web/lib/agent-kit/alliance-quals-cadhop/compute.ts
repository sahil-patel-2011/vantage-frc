/**
 * Leftover agent kit — alliance-quals-cadhop.
...
/**
 * Lovat-style Alliance board — qualification matches, CAD hop persist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "alliance-quals-cadhop";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1542110319): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
