/**
 * Leftover agent kit — alliance-quals-excerpts.
...
/**
 * Lovat-style Alliance board — qualification matches, tool excerpts.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "alliance-quals-excerpts";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2067550858): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
