/**
 * Leftover agent kit — auto-event-cite.
...
/**
 * Lovat-style Auto board — this event, cite-before-dump.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-event-cite";
export const PHASE_LABEL = "Auto";
export const WINDOW_LABEL = "this event";
export const KERNEL_LABEL = "cite-before-dump";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3774875121): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
