/**
 * Leftover agent kit — auto-event-memsearch.
...
/**
 * Lovat-style Auto board — this event, memory-search.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-event-memsearch";
export const PHASE_LABEL = "Auto";
export const WINDOW_LABEL = "this event";
export const KERNEL_LABEL = "memory-search";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 311332335): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
