/**
 * Leftover agent kit — auto-event-recall.
...
/**
 * Lovat-style Auto board — this event, episodic recall.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-event-recall";
export const PHASE_LABEL = "Auto";
export const WINDOW_LABEL = "this event";
export const KERNEL_LABEL = "episodic recall";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2203974303): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
