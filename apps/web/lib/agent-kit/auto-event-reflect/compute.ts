...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3003131927): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
...
export function seriesAutoPoints(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["autoPoints" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statAutoPoints(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesAutoPoints(rows, teamKey));
}

export function shrinkAutoPoints(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesAutoPoints(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["autoPoints" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "autoPoints" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastAutoPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesAutoPoints(rows, teamKey));
}

export function fitAutoPoints(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesAutoPoints(rows, teamKey));
}

export function controlAutoPoints(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesAutoPoints(rows, teamKey));
}

export function slotsAutoPoints(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesAutoPoints(rows, teamKey));
}

export function residualAutoPoints(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "autoPoints" as LovatMetricId, oppKeys);
}

export function histAutoPoints(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesAutoPoints(rows, teamKey));
}

export function cvAutoPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesAutoPoints(rows, teamKey));
}

export function trimAutoPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesAutoPoints(rows, teamKey));
}

export function ewmaAutoPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesAutoPoints(rows, teamKey));
}

export function jackAutoPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesAutoPoints(rows, teamKey));
}

export function bootAutoPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesAutoPoints(rows, teamKey));
}

export function hazardAutoPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesAutoPoints(rows, teamKey));
}

export function streakAutoPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesAutoPoints(rows, teamKey));
}

export function giniAutoPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesAutoPoints(rows, teamKey));
}

export function theilAutoPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesAutoPoints(rows, teamKey));
}

export function skewAutoPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesAutoPoints(rows, teamKey));
}

export function rangeAutoPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesAutoPoints(rows, teamKey));
}

export function madMeanAutoPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesAutoPoints(rows, teamKey));
}

export function packAutoPoints(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesAutoPoints(rows, teamKey);
  return {
    id: "autoPoints" as LovatMetricId,
    label: "Auto points",
    series,
    stat: wideStat(series),
    shrink: shrinkAutoPoints(rows, teamKey, fieldRows),
    forecast: forecastAutoPoints(rows, teamKey),
    fit: fitAutoPoints(rows, teamKey),
    control: controlAutoPoints(rows, teamKey),
    slots: slotsAutoPoints(rows, teamKey),
  };
}

export function describeAutoPoints(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Auto points · Needs setup`;
  return `Auto points · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesAutoFuel(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["autoFuel" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statAutoFuel(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesAutoFuel(rows, teamKey));
}

export function shrinkAutoFuel(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesAutoFuel(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["autoFuel" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "autoFuel" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastAutoFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesAutoFuel(rows, teamKey));
}

export function fitAutoFuel(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesAutoFuel(rows, teamKey));
}

export function controlAutoFuel(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesAutoFuel(rows, teamKey));
}

export function slotsAutoFuel(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesAutoFuel(rows, teamKey));
}

export function residualAutoFuel(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "autoFuel" as LovatMetricId, oppKeys);
}

export function histAutoFuel(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesAutoFuel(rows, teamKey));
}

export function cvAutoFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesAutoFuel(rows, teamKey));
}

export function trimAutoFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesAutoFuel(rows, teamKey));
}

export function ewmaAutoFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesAutoFuel(rows, teamKey));
}

export function jackAutoFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesAutoFuel(rows, teamKey));
}

export function bootAutoFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesAutoFuel(rows, teamKey));
}

export function hazardAutoFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesAutoFuel(rows, teamKey));
}

export function streakAutoFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesAutoFuel(rows, teamKey));
}

export function giniAutoFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesAutoFuel(rows, teamKey));
}

export function theilAutoFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesAutoFuel(rows, teamKey));
}

export function skewAutoFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesAutoFuel(rows, teamKey));
}

export function rangeAutoFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesAutoFuel(rows, teamKey));
}

export function madMeanAutoFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesAutoFuel(rows, teamKey));
}

export function packAutoFuel(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesAutoFuel(rows, teamKey);
  return {
    id: "autoFuel" as LovatMetricId,
    label: "Auto fuel",
    series,
    stat: wideStat(series),
    shrink: shrinkAutoFuel(rows, teamKey, fieldRows),
    forecast: forecastAutoFuel(rows, teamKey),
    fit: fitAutoFuel(rows, teamKey),
    control: controlAutoFuel(rows, teamKey),
    slots: slotsAutoFuel(rows, teamKey),
  };
}

export function describeAutoFuel(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Auto fuel · Needs setup`;
  return `Auto fuel · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesAutoLeave(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["autoLeave" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statAutoLeave(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesAutoLeave(rows, teamKey));
}

export function shrinkAutoLeave(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesAutoLeave(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["autoLeave" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "autoLeave" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastAutoLeave(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesAutoLeave(rows, teamKey));
}

export function fitAutoLeave(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesAutoLeave(rows, teamKey));
}

export function controlAutoLeave(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesAutoLeave(rows, teamKey));
}

export function slotsAutoLeave(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesAutoLeave(rows, teamKey));
}

export function residualAutoLeave(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "autoLeave" as LovatMetricId, oppKeys);
}

export function histAutoLeave(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesAutoLeave(rows, teamKey));
}

export function cvAutoLeave(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesAutoLeave(rows, teamKey));
}

export function trimAutoLeave(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesAutoLeave(rows, teamKey));
}

export function ewmaAutoLeave(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesAutoLeave(rows, teamKey));
}

export function jackAutoLeave(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesAutoLeave(rows, teamKey));
}

export function bootAutoLeave(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesAutoLeave(rows, teamKey));
}

export function hazardAutoLeave(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesAutoLeave(rows, teamKey));
}

export function streakAutoLeave(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesAutoLeave(rows, teamKey));
}

export function giniAutoLeave(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesAutoLeave(rows, teamKey));
}

export function theilAutoLeave(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesAutoLeave(rows, teamKey));
}

export function skewAutoLeave(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesAutoLeave(rows, teamKey));
}

export function rangeAutoLeave(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesAutoLeave(rows, teamKey));
}

export function madMeanAutoLeave(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesAutoLeave(rows, teamKey));
}

export function packAutoLeave(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesAutoLeave(rows, teamKey);
  return {
    id: "autoLeave" as LovatMetricId,
    label: "Auto leave",
    series,
    stat: wideStat(series),
    shrink: shrinkAutoLeave(rows, teamKey, fieldRows),
    forecast: forecastAutoLeave(rows, teamKey),
    fit: fitAutoLeave(rows, teamKey),
    control: controlAutoLeave(rows, teamKey),
    slots: slotsAutoLeave(rows, teamKey),
  };
}

export function describeAutoLeave(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Auto leave · Needs setup`;
  return `Auto leave · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesAutoClimb(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["autoClimb" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statAutoClimb(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesAutoClimb(rows, teamKey));
}

export function shrinkAutoClimb(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesAutoClimb(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["autoClimb" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "autoClimb" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastAutoClimb(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesAutoClimb(rows, teamKey));
}

export function fitAutoClimb(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesAutoClimb(rows, teamKey));
}

export function controlAutoClimb(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesAutoClimb(rows, teamKey));
}

export function slotsAutoClimb(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesAutoClimb(rows, teamKey));
}

export function residualAutoClimb(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "autoClimb" as LovatMetricId, oppKeys);
}

export function histAutoClimb(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesAutoClimb(rows, teamKey));
}

export function cvAutoClimb(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesAutoClimb(rows, teamKey));
}

export function trimAutoClimb(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesAutoClimb(rows, teamKey));
}

export function ewmaAutoClimb(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesAutoClimb(rows, teamKey));
}

export function jackAutoClimb(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesAutoClimb(rows, teamKey));
}

export function bootAutoClimb(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesAutoClimb(rows, teamKey));
}

export function hazardAutoClimb(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesAutoClimb(rows, teamKey));
}

export function streakAutoClimb(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesAutoClimb(rows, teamKey));
}

export function giniAutoClimb(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesAutoClimb(rows, teamKey));
}

export function theilAutoClimb(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesAutoClimb(rows, teamKey));
}

export function skewAutoClimb(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesAutoClimb(rows, teamKey));
}

export function rangeAutoClimb(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesAutoClimb(rows, teamKey));
}

export function madMeanAutoClimb(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesAutoClimb(rows, teamKey));
}

export function packAutoClimb(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesAutoClimb(rows, teamKey);
  return {
    id: "autoClimb" as LovatMetricId,
    label: "Auto climb",
    series,
    stat: wideStat(series),
    shrink: shrinkAutoClimb(rows, teamKey, fieldRows),
    forecast: forecastAutoClimb(rows, teamKey),
    fit: fitAutoClimb(rows, teamKey),
    control: controlAutoClimb(rows, teamKey),
    slots: slotsAutoClimb(rows, teamKey),
  };
}

export function describeAutoClimb(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Auto climb · Needs setup`;
  return `Auto climb · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}
...
export function rollAutoPoints2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 2);
}

export function rollAutoPoints2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints2(rows, teamKey);
  const last = series[series.length - 1];
...
export function rollAutoPoints2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints2Last(rows, teamKey) != null;
}

export function describeRollAutoPoints2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints2Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 2: Needs setup";
  return "Auto points" + " last 2: " + formatValue(last, 1);
}

export function rollAutoPoints3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 3);
}

export function rollAutoPoints3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints3Last(rows, teamKey) != null;
}

export function describeRollAutoPoints3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints3Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 3: Needs setup";
  return "Auto points" + " last 3: " + formatValue(last, 1);
}

export function rollAutoPoints4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 4);
}

export function rollAutoPoints4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints4Last(rows, teamKey) != null;
}

export function describeRollAutoPoints4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints4Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 4: Needs setup";
  return "Auto points" + " last 4: " + formatValue(last, 1);
}

export function rollAutoPoints5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 5);
}

export function rollAutoPoints5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints5Last(rows, teamKey) != null;
}

export function describeRollAutoPoints5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints5Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 5: Needs setup";
  return "Auto points" + " last 5: " + formatValue(last, 1);
}

export function rollAutoPoints6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 6);
}

export function rollAutoPoints6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints6Last(rows, teamKey) != null;
}

export function describeRollAutoPoints6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints6Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 6: Needs setup";
  return "Auto points" + " last 6: " + formatValue(last, 1);
}

export function rollAutoPoints7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 7);
}

export function rollAutoPoints7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints7Last(rows, teamKey) != null;
}

export function describeRollAutoPoints7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints7Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 7: Needs setup";
  return "Auto points" + " last 7: " + formatValue(last, 1);
}

export function rollAutoPoints8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 8);
}

export function rollAutoPoints8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints8Last(rows, teamKey) != null;
}

export function describeRollAutoPoints8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints8Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 8: Needs setup";
  return "Auto points" + " last 8: " + formatValue(last, 1);
}

export function rollAutoPoints9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 9);
}

export function rollAutoPoints9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints9Last(rows, teamKey) != null;
}

export function describeRollAutoPoints9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints9Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 9: Needs setup";
  return "Auto points" + " last 9: " + formatValue(last, 1);
}

export function rollAutoPoints10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 10);
}

export function rollAutoPoints10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints10Last(rows, teamKey) != null;
}

export function describeRollAutoPoints10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints10Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 10: Needs setup";
  return "Auto points" + " last 10: " + formatValue(last, 1);
}

export function rollAutoPoints12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 12);
}

export function rollAutoPoints12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints12Last(rows, teamKey) != null;
}

export function describeRollAutoPoints12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints12Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 12: Needs setup";
  return "Auto points" + " last 12: " + formatValue(last, 1);
}

export function rollAutoPoints14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 14);
}

export function rollAutoPoints14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints14Last(rows, teamKey) != null;
}

export function describeRollAutoPoints14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints14Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 14: Needs setup";
  return "Auto points" + " last 14: " + formatValue(last, 1);
}

export function rollAutoPoints16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 16);
}

export function rollAutoPoints16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints16Last(rows, teamKey) != null;
}

export function describeRollAutoPoints16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints16Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 16: Needs setup";
  return "Auto points" + " last 16: " + formatValue(last, 1);
}

export function rollAutoPoints18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 18);
}

export function rollAutoPoints18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints18Last(rows, teamKey) != null;
}

export function describeRollAutoPoints18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints18Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 18: Needs setup";
  return "Auto points" + " last 18: " + formatValue(last, 1);
}

export function rollAutoPoints20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 20);
}

export function rollAutoPoints20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints20Last(rows, teamKey) != null;
}

export function describeRollAutoPoints20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints20Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 20: Needs setup";
  return "Auto points" + " last 20: " + formatValue(last, 1);
}

export function rollAutoPoints22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 22);
}

export function rollAutoPoints22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints22Last(rows, teamKey) != null;
}

export function describeRollAutoPoints22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints22Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 22: Needs setup";
  return "Auto points" + " last 22: " + formatValue(last, 1);
}

export function rollAutoPoints24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 24);
}

export function rollAutoPoints24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints24Last(rows, teamKey) != null;
}

export function describeRollAutoPoints24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints24Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 24: Needs setup";
  return "Auto points" + " last 24: " + formatValue(last, 1);
}

export function rollAutoPoints28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 28);
}

export function rollAutoPoints28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints28Last(rows, teamKey) != null;
}

export function describeRollAutoPoints28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints28Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 28: Needs setup";
  return "Auto points" + " last 28: " + formatValue(last, 1);
}

export function rollAutoPoints32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoPoints(rows, teamKey), 32);
}

export function rollAutoPoints32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoPoints32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoPoints32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoPoints32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoPoints32Last(rows, teamKey) != null;
}

export function describeRollAutoPoints32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoPoints32Last(rows, teamKey);
  if (last == null) return "Auto points" + " last 32: Needs setup";
  return "Auto points" + " last 32: " + formatValue(last, 1);
}

export function rollAutoFuel2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 2);
}

export function rollAutoFuel2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel2Last(rows, teamKey) != null;
}

export function describeRollAutoFuel2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel2Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 2: Needs setup";
  return "Auto fuel" + " last 2: " + formatValue(last, 1);
}

export function rollAutoFuel3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 3);
}

export function rollAutoFuel3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel3Last(rows, teamKey) != null;
}

export function describeRollAutoFuel3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel3Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 3: Needs setup";
  return "Auto fuel" + " last 3: " + formatValue(last, 1);
}

export function rollAutoFuel4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 4);
}

export function rollAutoFuel4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel4Last(rows, teamKey) != null;
}

export function describeRollAutoFuel4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel4Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 4: Needs setup";
  return "Auto fuel" + " last 4: " + formatValue(last, 1);
}

export function rollAutoFuel5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 5);
}

export function rollAutoFuel5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel5Last(rows, teamKey) != null;
}

export function describeRollAutoFuel5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel5Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 5: Needs setup";
  return "Auto fuel" + " last 5: " + formatValue(last, 1);
}

export function rollAutoFuel6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 6);
}

export function rollAutoFuel6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel6Last(rows, teamKey) != null;
}

export function describeRollAutoFuel6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel6Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 6: Needs setup";
  return "Auto fuel" + " last 6: " + formatValue(last, 1);
}

export function rollAutoFuel7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 7);
}

export function rollAutoFuel7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel7Last(rows, teamKey) != null;
}

export function describeRollAutoFuel7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel7Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 7: Needs setup";
  return "Auto fuel" + " last 7: " + formatValue(last, 1);
}

export function rollAutoFuel8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 8);
}

export function rollAutoFuel8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel8Last(rows, teamKey) != null;
}

export function describeRollAutoFuel8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel8Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 8: Needs setup";
  return "Auto fuel" + " last 8: " + formatValue(last, 1);
}

export function rollAutoFuel9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 9);
}

export function rollAutoFuel9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel9Last(rows, teamKey) != null;
}

export function describeRollAutoFuel9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel9Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 9: Needs setup";
  return "Auto fuel" + " last 9: " + formatValue(last, 1);
}

export function rollAutoFuel10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 10);
}

export function rollAutoFuel10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel10Last(rows, teamKey) != null;
}

export function describeRollAutoFuel10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel10Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 10: Needs setup";
  return "Auto fuel" + " last 10: " + formatValue(last, 1);
}

export function rollAutoFuel12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 12);
}

export function rollAutoFuel12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel12Last(rows, teamKey) != null;
}

export function describeRollAutoFuel12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel12Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 12: Needs setup";
  return "Auto fuel" + " last 12: " + formatValue(last, 1);
}

export function rollAutoFuel14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 14);
}

export function rollAutoFuel14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel14Last(rows, teamKey) != null;
}

export function describeRollAutoFuel14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel14Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 14: Needs setup";
  return "Auto fuel" + " last 14: " + formatValue(last, 1);
}

export function rollAutoFuel16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 16);
}

export function rollAutoFuel16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel16Last(rows, teamKey) != null;
}

export function describeRollAutoFuel16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel16Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 16: Needs setup";
  return "Auto fuel" + " last 16: " + formatValue(last, 1);
}

export function rollAutoFuel18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 18);
}

export function rollAutoFuel18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel18Last(rows, teamKey) != null;
}

export function describeRollAutoFuel18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel18Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 18: Needs setup";
  return "Auto fuel" + " last 18: " + formatValue(last, 1);
}

export function rollAutoFuel20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 20);
}

export function rollAutoFuel20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel20Last(rows, teamKey) != null;
}

export function describeRollAutoFuel20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel20Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 20: Needs setup";
  return "Auto fuel" + " last 20: " + formatValue(last, 1);
}

export function rollAutoFuel22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 22);
}

export function rollAutoFuel22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel22Last(rows, teamKey) != null;
}

export function describeRollAutoFuel22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel22Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 22: Needs setup";
  return "Auto fuel" + " last 22: " + formatValue(last, 1);
}

export function rollAutoFuel24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 24);
}

export function rollAutoFuel24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel24Last(rows, teamKey) != null;
}

export function describeRollAutoFuel24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel24Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 24: Needs setup";
  return "Auto fuel" + " last 24: " + formatValue(last, 1);
}

export function rollAutoFuel28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 28);
}

export function rollAutoFuel28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel28Last(rows, teamKey) != null;
}

export function describeRollAutoFuel28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel28Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 28: Needs setup";
  return "Auto fuel" + " last 28: " + formatValue(last, 1);
}

export function rollAutoFuel32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoFuel(rows, teamKey), 32);
}

export function rollAutoFuel32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoFuel32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoFuel32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoFuel32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoFuel32Last(rows, teamKey) != null;
}

export function describeRollAutoFuel32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoFuel32Last(rows, teamKey);
  if (last == null) return "Auto fuel" + " last 32: Needs setup";
  return "Auto fuel" + " last 32: " + formatValue(last, 1);
}

export function rollAutoLeave2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 2);
}

export function rollAutoLeave2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave2Last(rows, teamKey) != null;
}

export function describeRollAutoLeave2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave2Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 2: Needs setup";
  return "Auto leave" + " last 2: " + formatValue(last, 1);
}

export function rollAutoLeave3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 3);
}

export function rollAutoLeave3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave3Last(rows, teamKey) != null;
}

export function describeRollAutoLeave3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave3Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 3: Needs setup";
  return "Auto leave" + " last 3: " + formatValue(last, 1);
}

export function rollAutoLeave4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 4);
}

export function rollAutoLeave4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave4Last(rows, teamKey) != null;
}

export function describeRollAutoLeave4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave4Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 4: Needs setup";
  return "Auto leave" + " last 4: " + formatValue(last, 1);
}

export function rollAutoLeave5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 5);
}

export function rollAutoLeave5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave5Last(rows, teamKey) != null;
}

export function describeRollAutoLeave5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave5Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 5: Needs setup";
  return "Auto leave" + " last 5: " + formatValue(last, 1);
}

export function rollAutoLeave6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 6);
}

export function rollAutoLeave6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave6Last(rows, teamKey) != null;
}

export function describeRollAutoLeave6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave6Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 6: Needs setup";
  return "Auto leave" + " last 6: " + formatValue(last, 1);
}

export function rollAutoLeave7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 7);
}

export function rollAutoLeave7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave7Last(rows, teamKey) != null;
}

export function describeRollAutoLeave7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave7Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 7: Needs setup";
  return "Auto leave" + " last 7: " + formatValue(last, 1);
}

export function rollAutoLeave8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 8);
}

export function rollAutoLeave8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave8Last(rows, teamKey) != null;
}

export function describeRollAutoLeave8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave8Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 8: Needs setup";
  return "Auto leave" + " last 8: " + formatValue(last, 1);
}

export function rollAutoLeave9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 9);
}

export function rollAutoLeave9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave9Last(rows, teamKey) != null;
}

export function describeRollAutoLeave9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave9Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 9: Needs setup";
  return "Auto leave" + " last 9: " + formatValue(last, 1);
}

export function rollAutoLeave10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 10);
}

export function rollAutoLeave10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave10Last(rows, teamKey) != null;
}

export function describeRollAutoLeave10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave10Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 10: Needs setup";
  return "Auto leave" + " last 10: " + formatValue(last, 1);
}

export function rollAutoLeave12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 12);
}

export function rollAutoLeave12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave12Last(rows, teamKey) != null;
}

export function describeRollAutoLeave12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave12Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 12: Needs setup";
  return "Auto leave" + " last 12: " + formatValue(last, 1);
}

export function rollAutoLeave14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 14);
}

export function rollAutoLeave14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave14Last(rows, teamKey) != null;
}

export function describeRollAutoLeave14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave14Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 14: Needs setup";
  return "Auto leave" + " last 14: " + formatValue(last, 1);
}

export function rollAutoLeave16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 16);
}

export function rollAutoLeave16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave16Last(rows, teamKey) != null;
}

export function describeRollAutoLeave16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave16Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 16: Needs setup";
  return "Auto leave" + " last 16: " + formatValue(last, 1);
}

export function rollAutoLeave18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 18);
}

export function rollAutoLeave18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave18Last(rows, teamKey) != null;
}

export function describeRollAutoLeave18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave18Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 18: Needs setup";
  return "Auto leave" + " last 18: " + formatValue(last, 1);
}

export function rollAutoLeave20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 20);
}

export function rollAutoLeave20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave20Last(rows, teamKey) != null;
}

export function describeRollAutoLeave20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave20Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 20: Needs setup";
  return "Auto leave" + " last 20: " + formatValue(last, 1);
}

export function rollAutoLeave22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 22);
}

export function rollAutoLeave22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave22Last(rows, teamKey) != null;
}

export function describeRollAutoLeave22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave22Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 22: Needs setup";
  return "Auto leave" + " last 22: " + formatValue(last, 1);
}

export function rollAutoLeave24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 24);
}

export function rollAutoLeave24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave24Last(rows, teamKey) != null;
}

export function describeRollAutoLeave24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave24Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 24: Needs setup";
  return "Auto leave" + " last 24: " + formatValue(last, 1);
}

export function rollAutoLeave28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 28);
}

export function rollAutoLeave28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave28Last(rows, teamKey) != null;
}

export function describeRollAutoLeave28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave28Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 28: Needs setup";
  return "Auto leave" + " last 28: " + formatValue(last, 1);
}

export function rollAutoLeave32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoLeave(rows, teamKey), 32);
}

export function rollAutoLeave32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoLeave32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoLeave32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoLeave32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoLeave32Last(rows, teamKey) != null;
}

export function describeRollAutoLeave32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoLeave32Last(rows, teamKey);
  if (last == null) return "Auto leave" + " last 32: Needs setup";
  return "Auto leave" + " last 32: " + formatValue(last, 1);
}

export function rollAutoClimb2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 2);
}

export function rollAutoClimb2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb2Last(rows, teamKey) != null;
}

export function describeRollAutoClimb2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb2Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 2: Needs setup";
  return "Auto climb" + " last 2: " + formatValue(last, 1);
}

export function rollAutoClimb3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 3);
}

export function rollAutoClimb3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb3Last(rows, teamKey) != null;
}

export function describeRollAutoClimb3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb3Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 3: Needs setup";
  return "Auto climb" + " last 3: " + formatValue(last, 1);
}

export function rollAutoClimb4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 4);
}

export function rollAutoClimb4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb4Last(rows, teamKey) != null;
}

export function describeRollAutoClimb4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb4Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 4: Needs setup";
  return "Auto climb" + " last 4: " + formatValue(last, 1);
}

export function rollAutoClimb5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 5);
}

export function rollAutoClimb5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb5Last(rows, teamKey) != null;
}

export function describeRollAutoClimb5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb5Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 5: Needs setup";
  return "Auto climb" + " last 5: " + formatValue(last, 1);
}

export function rollAutoClimb6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 6);
}

export function rollAutoClimb6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb6Last(rows, teamKey) != null;
}

export function describeRollAutoClimb6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb6Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 6: Needs setup";
  return "Auto climb" + " last 6: " + formatValue(last, 1);
}

export function rollAutoClimb7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 7);
}

export function rollAutoClimb7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb7Last(rows, teamKey) != null;
}

export function describeRollAutoClimb7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb7Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 7: Needs setup";
  return "Auto climb" + " last 7: " + formatValue(last, 1);
}

export function rollAutoClimb8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 8);
}

export function rollAutoClimb8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb8Last(rows, teamKey) != null;
}

export function describeRollAutoClimb8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb8Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 8: Needs setup";
  return "Auto climb" + " last 8: " + formatValue(last, 1);
}

export function rollAutoClimb9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 9);
}

export function rollAutoClimb9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb9Last(rows, teamKey) != null;
}

export function describeRollAutoClimb9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb9Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 9: Needs setup";
  return "Auto climb" + " last 9: " + formatValue(last, 1);
}

export function rollAutoClimb10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 10);
}

export function rollAutoClimb10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb10Last(rows, teamKey) != null;
}

export function describeRollAutoClimb10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb10Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 10: Needs setup";
  return "Auto climb" + " last 10: " + formatValue(last, 1);
}

export function rollAutoClimb12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 12);
}

export function rollAutoClimb12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb12Last(rows, teamKey) != null;
}

export function describeRollAutoClimb12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb12Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 12: Needs setup";
  return "Auto climb" + " last 12: " + formatValue(last, 1);
}

export function rollAutoClimb14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 14);
}

export function rollAutoClimb14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb14Last(rows, teamKey) != null;
}

export function describeRollAutoClimb14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb14Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 14: Needs setup";
  return "Auto climb" + " last 14: " + formatValue(last, 1);
}

export function rollAutoClimb16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 16);
}

export function rollAutoClimb16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb16Last(rows, teamKey) != null;
}

export function describeRollAutoClimb16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb16Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 16: Needs setup";
  return "Auto climb" + " last 16: " + formatValue(last, 1);
}

export function rollAutoClimb18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 18);
}

export function rollAutoClimb18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb18Last(rows, teamKey) != null;
}

export function describeRollAutoClimb18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb18Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 18: Needs setup";
  return "Auto climb" + " last 18: " + formatValue(last, 1);
}

export function rollAutoClimb20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 20);
}

export function rollAutoClimb20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb20Last(rows, teamKey) != null;
}

export function describeRollAutoClimb20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb20Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 20: Needs setup";
  return "Auto climb" + " last 20: " + formatValue(last, 1);
}

export function rollAutoClimb22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 22);
}

export function rollAutoClimb22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb22Last(rows, teamKey) != null;
}

export function describeRollAutoClimb22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb22Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 22: Needs setup";
  return "Auto climb" + " last 22: " + formatValue(last, 1);
}

export function rollAutoClimb24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 24);
}

export function rollAutoClimb24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb24Last(rows, teamKey) != null;
}

export function describeRollAutoClimb24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb24Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 24: Needs setup";
  return "Auto climb" + " last 24: " + formatValue(last, 1);
}

export function rollAutoClimb28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 28);
}

export function rollAutoClimb28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb28Last(rows, teamKey) != null;
}

export function describeRollAutoClimb28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb28Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 28: Needs setup";
  return "Auto climb" + " last 28: " + formatValue(last, 1);
}

export function rollAutoClimb32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAutoClimb(rows, teamKey), 32);
}

export function rollAutoClimb32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAutoClimb32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAutoClimb32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAutoClimb32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAutoClimb32Last(rows, teamKey) != null;
}

export function describeRollAutoClimb32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAutoClimb32Last(rows, teamKey);
  if (last == null) return "Auto climb" + " last 32: Needs setup";
  return "Auto climb" + " last 32: " + formatValue(last, 1);
}
...
export function buildWideReport(input: { teamKey: string; rows: readonly SampleRow[]; fieldRows?: readonly SampleRow[] }): WideReport {
  const packs: WideMetricPack[] = [
    packAutoPoints(input.rows, input.teamKey, input.fieldRows),
    packAutoFuel(input.rows, input.teamKey, input.fieldRows),
    packAutoLeave(input.rows, input.teamKey, input.fieldRows),
    packAutoClimb(input.rows, input.teamKey, input.fieldRows)
  ];
  const lead = packs[0];
...
export function buildLovatExactReport(input: { teamKey: string; rows: readonly SampleRow[]; fieldRows?: readonly SampleRow[] }): LovatExactReport {
  ...
  return {
    ...
    visible: improveContextRules("auto", LOVAT_EXACT_METRICS.map((metric) => metric.id)),
