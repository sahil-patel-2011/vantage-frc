/**
 * Leftover agent kit — auto-event-todos.
...
/**
 * Lovat-style Auto board — this event, working todos.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-event-todos";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 195881811): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
