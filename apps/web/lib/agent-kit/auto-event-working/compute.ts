/**
 * Leftover agent kit — auto-event-working.
...
/**
 * Lovat-style Auto board — this event, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-event-working";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3225873713): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
