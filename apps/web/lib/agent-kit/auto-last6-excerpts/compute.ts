/**
 * Leftover agent kit — auto-last6-excerpts.
...
/**
 * Lovat-style Auto board — last 6 matches, tool excerpts.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-last6-excerpts";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2050713018): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
