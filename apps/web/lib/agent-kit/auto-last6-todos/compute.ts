/**
 * Leftover agent kit — auto-last6-todos.
...
/**
 * Lovat-style Auto board — last 6 matches, working todos.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-last6-todos";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3480895765): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
