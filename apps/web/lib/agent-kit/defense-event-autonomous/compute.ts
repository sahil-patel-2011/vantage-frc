/**
 * Leftover agent kit — defense-event-autonomous.
...
/**
 * Lovat-style Defense board — this event, autonomous loop.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-autonomous";
...
export const EWMA_ALPHA = 0.261;
export const TRIM_FRACTION = 0.103;
export const NEAR_Z = 0.282;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 447590277): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
