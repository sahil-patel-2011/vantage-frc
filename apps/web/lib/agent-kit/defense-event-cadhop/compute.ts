/**
 * Leftover agent kit — defense-event-cadhop.
...
/**
 * Lovat-style Defense board — this event, CAD hop persist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-cadhop";
...
export const EWMA_ALPHA = 0.275;
export const TRIM_FRACTION = 0.108;
export const NEAR_Z = 0.286;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 799390574): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
