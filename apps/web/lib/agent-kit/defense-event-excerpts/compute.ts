/**
 * Leftover agent kit — defense-event-excerpts.
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4211908203): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
