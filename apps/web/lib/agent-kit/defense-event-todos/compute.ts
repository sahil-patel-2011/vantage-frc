/**
 * Leftover agent kit — defense-event-todos.
...
/**
 * Lovat-style Defense board — this event, working todos.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-todos";
...
export const EWMA_ALPHA = 0.634;
export const TRIM_FRACTION = 0.228;
export const NEAR_Z = 0.382;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1246158558): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
