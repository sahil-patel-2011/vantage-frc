/**
 * Leftover agent kit — defense-event-working.
...
/**
 * Lovat-style Defense board — this event, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-working";
...
export const EWMA_ALPHA = 0.354;
export const TRIM_FRACTION = 0.134;
export const NEAR_Z = 0.307;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2343112348): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
