/**
 * Leftover agent kit — defense-last12-cadhop.
...
/**
 * Lovat-style Defense board — last 12 matches, CAD hop persist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last12-cadhop";
export const PHASE_LABEL = "Defense";
export const WINDOW_LABEL = "last 12 matches";
export const KERNEL_LABEL = "CAD hop persist";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3282235107): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
