/**
 * Leftover agent kit — defense-last12-excerpts.
...
/**
 * Lovat-style Defense board — last 12 matches, tool excerpts.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last12-excerpts";
...
export const EWMA_ALPHA = 0.454;
export const TRIM_FRACTION = 0.168;
export const NEAR_Z = 0.334;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3720114566): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
