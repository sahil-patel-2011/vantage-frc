/**
 * Leftover agent kit — defense-last6-autonomous.
...
/**
 * Lovat-style Defense board — last 6 matches, autonomous loop.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-autonomous";
...
export const EWMA_ALPHA = 0.265;
export const TRIM_FRACTION = 0.104;
export const NEAR_Z = 0.283;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2743830351): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
