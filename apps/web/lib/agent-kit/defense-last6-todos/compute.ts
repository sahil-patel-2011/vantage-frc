/**
 * Leftover agent kit — defense-last6-todos.
...
/**
 * Lovat-style Defense board — last 6 matches, working todos.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-todos";
...
export const EWMA_ALPHA = 0.591;
export const TRIM_FRACTION = 0.213;
export const NEAR_Z = 0.370;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4249217604): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
