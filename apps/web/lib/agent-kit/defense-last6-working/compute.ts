/**
 * Leftover agent kit — defense-last6-working.
...
/**
 * Lovat-style Defense board — last 6 matches, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-working";
...
export const EWMA_ALPHA = 0.469;
export const TRIM_FRACTION = 0.172;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2709654886): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
