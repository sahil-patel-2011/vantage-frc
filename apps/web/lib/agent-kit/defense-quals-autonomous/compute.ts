/**
 * Leftover agent kit — defense-quals-autonomous.
...
/**
 * Lovat-style Defense board — qualification matches, autonomous loop.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-autonomous";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 211095109): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
