/**
 * Leftover agent kit — defense-quals-cadhop.
...
/**
 * Lovat-style Defense board — qualification matches, CAD hop persist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-cadhop";
...
export const EWMA_ALPHA = 0.694;
export const TRIM_FRACTION = 0.248;
export const NEAR_Z = 0.398;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2936089902): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
