/**
 * Leftover agent kit — defense-quals-excerpts.
...
/**
 * Lovat-style Defense board — qualification matches, tool excerpts.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-excerpts";
...
export const EWMA_ALPHA = 0.326;
export const TRIM_FRACTION = 0.125;
export const NEAR_Z = 0.300;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3593201707): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
