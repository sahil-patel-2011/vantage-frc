/**
 * Leftover agent kit — defense-quals-todos.
...
/**
 * Lovat-style Defense board — qualification matches, working todos.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-todos";
...
export const EWMA_ALPHA = 0.684;
export const TRIM_FRACTION = 0.244;
export const NEAR_Z = 0.395;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2869419678): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
