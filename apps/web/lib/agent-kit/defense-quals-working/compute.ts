/**
 * Leftover agent kit — defense-quals-working.
...
/**
 * Lovat-style Defense board — qualification matches, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-working";
...
export const EWMA_ALPHA = 0.548;
export const TRIM_FRACTION = 0.199;
export const NEAR_Z = 0.359;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 472066652): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
