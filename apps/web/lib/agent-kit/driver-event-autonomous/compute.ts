/**
 * Leftover agent kit — driver-event-autonomous.
...
/**
 * Lovat-style Driver board — this event, autonomous loop.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-event-autonomous";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 348994385): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
