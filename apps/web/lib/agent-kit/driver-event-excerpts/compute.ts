/**
 * Leftover agent kit — driver-event-excerpts.
...
/**
 * Lovat-style Driver board — this event, tool excerpts.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-event-excerpts";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2130609647): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
