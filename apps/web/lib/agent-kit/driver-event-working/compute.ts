/**
 * Leftover agent kit — driver-event-working.
...
/**
 * Lovat-style Driver board — this event, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-event-working";
...
export const EWMA_ALPHA = 0.338;
export const TRIM_FRACTION = 0.129;
export const NEAR_Z = 0.303;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3598371984): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
