/**
 * Leftover agent kit — driver-last12-autonomous.
...
/**
 * Lovat-style Driver board — last 12 matches, autonomous loop.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last12-autonomous";
export const PHASE_LABEL = "Driver";
...
export type LovatMetricId = "driverAbility" | "cyclePace" | "avoidFouls" | "positioning";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1670804556): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
...
export function seriesDriverAbility(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["driverAbility" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statDriverAbility(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesDriverAbility(rows, teamKey));
}

export function shrinkDriverAbility(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesDriverAbility(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["driverAbility" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "driverAbility" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastDriverAbility(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesDriverAbility(rows, teamKey));
}

export function fitDriverAbility(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesDriverAbility(rows, teamKey));
}

export function controlDriverAbility(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesDriverAbility(rows, teamKey));
}

export function slotsDriverAbility(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesDriverAbility(rows, teamKey));
}

export function residualDriverAbility(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "driverAbility" as LovatMetricId, oppKeys);
}

export function histDriverAbility(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesDriverAbility(rows, teamKey));
}

export function cvDriverAbility(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesDriverAbility(rows, teamKey));
}

export function trimDriverAbility(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesDriverAbility(rows, teamKey));
}

export function ewmaDriverAbility(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesDriverAbility(rows, teamKey));
}

export function jackDriverAbility(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesDriverAbility(rows, teamKey));
}

export function bootDriverAbility(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesDriverAbility(rows, teamKey));
}

export function hazardDriverAbility(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesDriverAbility(rows, teamKey));
}

export function streakDriverAbility(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesDriverAbility(rows, teamKey));
}

export function giniDriverAbility(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesDriverAbility(rows, teamKey));
}

export function theilDriverAbility(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesDriverAbility(rows, teamKey));
}

export function skewDriverAbility(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesDriverAbility(rows, teamKey));
}

export function rangeDriverAbility(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesDriverAbility(rows, teamKey));
}

export function madMeanDriverAbility(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesDriverAbility(rows, teamKey));
}

export function packDriverAbility(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesDriverAbility(rows, teamKey);
  return {
    id: "driverAbility" as LovatMetricId,
    label: "Driver ability",
    series,
    stat: wideStat(series),
    shrink: shrinkDriverAbility(rows, teamKey, fieldRows),
    forecast: forecastDriverAbility(rows, teamKey),
    fit: fitDriverAbility(rows, teamKey),
    control: controlDriverAbility(rows, teamKey),
    slots: slotsDriverAbility(rows, teamKey),
  };
}

export function describeDriverAbility(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Driver ability · Needs setup`;
  return `Driver ability · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesCyclePace(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["cyclePace" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statCyclePace(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesCyclePace(rows, teamKey));
}

export function shrinkCyclePace(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesCyclePace(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["cyclePace" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "cyclePace" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastCyclePace(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesCyclePace(rows, teamKey));
}

export function fitCyclePace(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesCyclePace(rows, teamKey));
}

export function controlCyclePace(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesCyclePace(rows, teamKey));
}

export function slotsCyclePace(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesCyclePace(rows, teamKey));
}

export function residualCyclePace(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "cyclePace" as LovatMetricId, oppKeys);
}

export function histCyclePace(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesCyclePace(rows, teamKey));
}

export function cvCyclePace(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesCyclePace(rows, teamKey));
}

export function trimCyclePace(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesCyclePace(rows, teamKey));
}

export function ewmaCyclePace(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesCyclePace(rows, teamKey));
}

export function jackCyclePace(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesCyclePace(rows, teamKey));
}

export function bootCyclePace(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesCyclePace(rows, teamKey));
}

export function hazardCyclePace(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesCyclePace(rows, teamKey));
}

export function streakCyclePace(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesCyclePace(rows, teamKey));
}

export function giniCyclePace(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesCyclePace(rows, teamKey));
}

export function theilCyclePace(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesCyclePace(rows, teamKey));
}

export function skewCyclePace(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesCyclePace(rows, teamKey));
}

export function rangeCyclePace(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesCyclePace(rows, teamKey));
}

export function madMeanCyclePace(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesCyclePace(rows, teamKey));
}

export function packCyclePace(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesCyclePace(rows, teamKey);
  return {
    id: "cyclePace" as LovatMetricId,
    label: "Cycle pace",
    series,
    stat: wideStat(series),
    shrink: shrinkCyclePace(rows, teamKey, fieldRows),
    forecast: forecastCyclePace(rows, teamKey),
    fit: fitCyclePace(rows, teamKey),
    control: controlCyclePace(rows, teamKey),
    slots: slotsCyclePace(rows, teamKey),
  };
}

export function describeCyclePace(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Cycle pace · Needs setup`;
  return `Cycle pace · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesAvoidFouls(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["avoidFouls" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statAvoidFouls(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesAvoidFouls(rows, teamKey));
}

export function shrinkAvoidFouls(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesAvoidFouls(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["avoidFouls" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "avoidFouls" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastAvoidFouls(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesAvoidFouls(rows, teamKey));
}

export function fitAvoidFouls(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesAvoidFouls(rows, teamKey));
}

export function controlAvoidFouls(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesAvoidFouls(rows, teamKey));
}

export function slotsAvoidFouls(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesAvoidFouls(rows, teamKey));
}

export function residualAvoidFouls(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "avoidFouls" as LovatMetricId, oppKeys);
}

export function histAvoidFouls(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesAvoidFouls(rows, teamKey));
}

export function cvAvoidFouls(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesAvoidFouls(rows, teamKey));
}

export function trimAvoidFouls(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesAvoidFouls(rows, teamKey));
}

export function ewmaAvoidFouls(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesAvoidFouls(rows, teamKey));
}

export function jackAvoidFouls(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesAvoidFouls(rows, teamKey));
}

export function bootAvoidFouls(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesAvoidFouls(rows, teamKey));
}

export function hazardAvoidFouls(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesAvoidFouls(rows, teamKey));
}

export function streakAvoidFouls(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesAvoidFouls(rows, teamKey));
}

export function giniAvoidFouls(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesAvoidFouls(rows, teamKey));
}

export function theilAvoidFouls(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesAvoidFouls(rows, teamKey));
}

export function skewAvoidFouls(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesAvoidFouls(rows, teamKey));
}

export function rangeAvoidFouls(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesAvoidFouls(rows, teamKey));
}

export function madMeanAvoidFouls(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesAvoidFouls(rows, teamKey));
}

export function packAvoidFouls(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesAvoidFouls(rows, teamKey);
  return {
    id: "avoidFouls" as LovatMetricId,
    label: "Avoid fouls",
    series,
    stat: wideStat(series),
    shrink: shrinkAvoidFouls(rows, teamKey, fieldRows),
    forecast: forecastAvoidFouls(rows, teamKey),
    fit: fitAvoidFouls(rows, teamKey),
    control: controlAvoidFouls(rows, teamKey),
    slots: slotsAvoidFouls(rows, teamKey),
  };
}

export function describeAvoidFouls(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Avoid fouls · Needs setup`;
  return `Avoid fouls · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesPositioning(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["positioning" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statPositioning(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesPositioning(rows, teamKey));
}

export function shrinkPositioning(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesPositioning(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["positioning" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "positioning" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastPositioning(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesPositioning(rows, teamKey));
}

export function fitPositioning(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesPositioning(rows, teamKey));
}

export function controlPositioning(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesPositioning(rows, teamKey));
}

export function slotsPositioning(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesPositioning(rows, teamKey));
}

export function residualPositioning(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "positioning" as LovatMetricId, oppKeys);
}

export function histPositioning(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesPositioning(rows, teamKey));
}

export function cvPositioning(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesPositioning(rows, teamKey));
}

export function trimPositioning(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesPositioning(rows, teamKey));
}

export function ewmaPositioning(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesPositioning(rows, teamKey));
}

export function jackPositioning(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesPositioning(rows, teamKey));
}

export function bootPositioning(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesPositioning(rows, teamKey));
}

export function hazardPositioning(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesPositioning(rows, teamKey));
}

export function streakPositioning(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesPositioning(rows, teamKey));
}

export function giniPositioning(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesPositioning(rows, teamKey));
}

export function theilPositioning(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesPositioning(rows, teamKey));
}

export function skewPositioning(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesPositioning(rows, teamKey));
}

export function rangePositioning(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesPositioning(rows, teamKey));
}

export function madMeanPositioning(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesPositioning(rows, teamKey));
}

export function packPositioning(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesPositioning(rows, teamKey);
  return {
    id: "positioning" as LovatMetricId,
    label: "Positioning",
    series,
    stat: wideStat(series),
    shrink: shrinkPositioning(rows, teamKey, fieldRows),
    forecast: forecastPositioning(rows, teamKey),
    fit: fitPositioning(rows, teamKey),
    control: controlPositioning(rows, teamKey),
    slots: slotsPositioning(rows, teamKey),
  };
}

export function describePositioning(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Positioning · Needs setup`;
  return `Positioning · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}
...
export function rollDriverAbility2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 2);
}

export function rollDriverAbility2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility2(rows, teamKey);
  const last = series[series.length - 1];
...
export function rollDriverAbility2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility2Last(rows, teamKey) != null;
}

export function describeRollDriverAbility2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility2Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 2: Needs setup";
  return "Driver ability" + " last 2: " + formatValue(last, 1);
}

export function rollDriverAbility3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 3);
}

export function rollDriverAbility3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility3Last(rows, teamKey) != null;
}

export function describeRollDriverAbility3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility3Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 3: Needs setup";
  return "Driver ability" + " last 3: " + formatValue(last, 1);
}

export function rollDriverAbility4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 4);
}

export function rollDriverAbility4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility4Last(rows, teamKey) != null;
}

export function describeRollDriverAbility4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility4Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 4: Needs setup";
  return "Driver ability" + " last 4: " + formatValue(last, 1);
}

export function rollDriverAbility5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 5);
}

export function rollDriverAbility5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility5Last(rows, teamKey) != null;
}

export function describeRollDriverAbility5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility5Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 5: Needs setup";
  return "Driver ability" + " last 5: " + formatValue(last, 1);
}

export function rollDriverAbility6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 6);
}

export function rollDriverAbility6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility6Last(rows, teamKey) != null;
}

export function describeRollDriverAbility6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility6Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 6: Needs setup";
  return "Driver ability" + " last 6: " + formatValue(last, 1);
}

export function rollDriverAbility7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 7);
}

export function rollDriverAbility7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility7Last(rows, teamKey) != null;
}

export function describeRollDriverAbility7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility7Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 7: Needs setup";
  return "Driver ability" + " last 7: " + formatValue(last, 1);
}

export function rollDriverAbility8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 8);
}

export function rollDriverAbility8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility8Last(rows, teamKey) != null;
}

export function describeRollDriverAbility8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility8Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 8: Needs setup";
  return "Driver ability" + " last 8: " + formatValue(last, 1);
}

export function rollDriverAbility9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 9);
}

export function rollDriverAbility9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility9Last(rows, teamKey) != null;
}

export function describeRollDriverAbility9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility9Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 9: Needs setup";
  return "Driver ability" + " last 9: " + formatValue(last, 1);
}

export function rollDriverAbility10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 10);
}

export function rollDriverAbility10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility10Last(rows, teamKey) != null;
}

export function describeRollDriverAbility10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility10Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 10: Needs setup";
  return "Driver ability" + " last 10: " + formatValue(last, 1);
}

export function rollDriverAbility12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 12);
}

export function rollDriverAbility12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility12Last(rows, teamKey) != null;
}

export function describeRollDriverAbility12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility12Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 12: Needs setup";
  return "Driver ability" + " last 12: " + formatValue(last, 1);
}

export function rollDriverAbility14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 14);
}

export function rollDriverAbility14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility14Last(rows, teamKey) != null;
}

export function describeRollDriverAbility14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility14Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 14: Needs setup";
  return "Driver ability" + " last 14: " + formatValue(last, 1);
}

export function rollDriverAbility16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 16);
}

export function rollDriverAbility16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility16Last(rows, teamKey) != null;
}

export function describeRollDriverAbility16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility16Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 16: Needs setup";
  return "Driver ability" + " last 16: " + formatValue(last, 1);
}

export function rollDriverAbility18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 18);
}

export function rollDriverAbility18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility18Last(rows, teamKey) != null;
}

export function describeRollDriverAbility18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility18Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 18: Needs setup";
  return "Driver ability" + " last 18: " + formatValue(last, 1);
}

export function rollDriverAbility20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 20);
}

export function rollDriverAbility20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility20Last(rows, teamKey) != null;
}

export function describeRollDriverAbility20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility20Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 20: Needs setup";
  return "Driver ability" + " last 20: " + formatValue(last, 1);
}

export function rollDriverAbility22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 22);
}

export function rollDriverAbility22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility22Last(rows, teamKey) != null;
}

export function describeRollDriverAbility22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility22Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 22: Needs setup";
  return "Driver ability" + " last 22: " + formatValue(last, 1);
}

export function rollDriverAbility24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 24);
}

export function rollDriverAbility24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility24Last(rows, teamKey) != null;
}

export function describeRollDriverAbility24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility24Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 24: Needs setup";
  return "Driver ability" + " last 24: " + formatValue(last, 1);
}

export function rollDriverAbility28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 28);
}

export function rollDriverAbility28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility28Last(rows, teamKey) != null;
}

export function describeRollDriverAbility28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility28Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 28: Needs setup";
  return "Driver ability" + " last 28: " + formatValue(last, 1);
}

export function rollDriverAbility32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDriverAbility(rows, teamKey), 32);
}

export function rollDriverAbility32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDriverAbility32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDriverAbility32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDriverAbility32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDriverAbility32Last(rows, teamKey) != null;
}

export function describeRollDriverAbility32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDriverAbility32Last(rows, teamKey);
  if (last == null) return "Driver ability" + " last 32: Needs setup";
  return "Driver ability" + " last 32: " + formatValue(last, 1);
}

export function rollCyclePace2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 2);
}

export function rollCyclePace2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace2Last(rows, teamKey) != null;
}

export function describeRollCyclePace2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace2Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 2: Needs setup";
  return "Cycle pace" + " last 2: " + formatValue(last, 1);
}

export function rollCyclePace3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 3);
}

export function rollCyclePace3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace3Last(rows, teamKey) != null;
}

export function describeRollCyclePace3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace3Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 3: Needs setup";
  return "Cycle pace" + " last 3: " + formatValue(last, 1);
}

export function rollCyclePace4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 4);
}

export function rollCyclePace4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace4Last(rows, teamKey) != null;
}

export function describeRollCyclePace4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace4Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 4: Needs setup";
  return "Cycle pace" + " last 4: " + formatValue(last, 1);
}

export function rollCyclePace5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 5);
}

export function rollCyclePace5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace5Last(rows, teamKey) != null;
}

export function describeRollCyclePace5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace5Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 5: Needs setup";
  return "Cycle pace" + " last 5: " + formatValue(last, 1);
}

export function rollCyclePace6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 6);
}

export function rollCyclePace6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace6Last(rows, teamKey) != null;
}

export function describeRollCyclePace6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace6Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 6: Needs setup";
  return "Cycle pace" + " last 6: " + formatValue(last, 1);
}

export function rollCyclePace7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 7);
}

export function rollCyclePace7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace7Last(rows, teamKey) != null;
}

export function describeRollCyclePace7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace7Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 7: Needs setup";
  return "Cycle pace" + " last 7: " + formatValue(last, 1);
}

export function rollCyclePace8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 8);
}

export function rollCyclePace8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace8Last(rows, teamKey) != null;
}

export function describeRollCyclePace8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace8Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 8: Needs setup";
  return "Cycle pace" + " last 8: " + formatValue(last, 1);
}

export function rollCyclePace9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 9);
}

export function rollCyclePace9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace9Last(rows, teamKey) != null;
}

export function describeRollCyclePace9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace9Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 9: Needs setup";
  return "Cycle pace" + " last 9: " + formatValue(last, 1);
}

export function rollCyclePace10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 10);
}

export function rollCyclePace10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace10Last(rows, teamKey) != null;
}

export function describeRollCyclePace10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace10Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 10: Needs setup";
  return "Cycle pace" + " last 10: " + formatValue(last, 1);
}

export function rollCyclePace12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 12);
}

export function rollCyclePace12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace12Last(rows, teamKey) != null;
}

export function describeRollCyclePace12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace12Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 12: Needs setup";
  return "Cycle pace" + " last 12: " + formatValue(last, 1);
}

export function rollCyclePace14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 14);
}

export function rollCyclePace14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace14Last(rows, teamKey) != null;
}

export function describeRollCyclePace14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace14Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 14: Needs setup";
  return "Cycle pace" + " last 14: " + formatValue(last, 1);
}

export function rollCyclePace16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 16);
}

export function rollCyclePace16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace16Last(rows, teamKey) != null;
}

export function describeRollCyclePace16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace16Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 16: Needs setup";
  return "Cycle pace" + " last 16: " + formatValue(last, 1);
}

export function rollCyclePace18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 18);
}

export function rollCyclePace18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace18Last(rows, teamKey) != null;
}

export function describeRollCyclePace18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace18Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 18: Needs setup";
  return "Cycle pace" + " last 18: " + formatValue(last, 1);
}

export function rollCyclePace20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 20);
}

export function rollCyclePace20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace20Last(rows, teamKey) != null;
}

export function describeRollCyclePace20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace20Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 20: Needs setup";
  return "Cycle pace" + " last 20: " + formatValue(last, 1);
}

export function rollCyclePace22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 22);
}

export function rollCyclePace22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace22Last(rows, teamKey) != null;
}

export function describeRollCyclePace22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace22Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 22: Needs setup";
  return "Cycle pace" + " last 22: " + formatValue(last, 1);
}

export function rollCyclePace24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 24);
}

export function rollCyclePace24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace24Last(rows, teamKey) != null;
}

export function describeRollCyclePace24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace24Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 24: Needs setup";
  return "Cycle pace" + " last 24: " + formatValue(last, 1);
}

export function rollCyclePace28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 28);
}

export function rollCyclePace28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace28Last(rows, teamKey) != null;
}

export function describeRollCyclePace28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace28Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 28: Needs setup";
  return "Cycle pace" + " last 28: " + formatValue(last, 1);
}

export function rollCyclePace32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCyclePace(rows, teamKey), 32);
}

export function rollCyclePace32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCyclePace32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCyclePace32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCyclePace32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCyclePace32Last(rows, teamKey) != null;
}

export function describeRollCyclePace32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCyclePace32Last(rows, teamKey);
  if (last == null) return "Cycle pace" + " last 32: Needs setup";
  return "Cycle pace" + " last 32: " + formatValue(last, 1);
}

export function rollAvoidFouls2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 2);
}

export function rollAvoidFouls2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls2Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls2Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 2: Needs setup";
  return "Avoid fouls" + " last 2: " + formatValue(last, 1);
}

export function rollAvoidFouls3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 3);
}

export function rollAvoidFouls3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls3Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls3Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 3: Needs setup";
  return "Avoid fouls" + " last 3: " + formatValue(last, 1);
}

export function rollAvoidFouls4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 4);
}

export function rollAvoidFouls4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls4Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls4Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 4: Needs setup";
  return "Avoid fouls" + " last 4: " + formatValue(last, 1);
}

export function rollAvoidFouls5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 5);
}

export function rollAvoidFouls5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls5Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls5Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 5: Needs setup";
  return "Avoid fouls" + " last 5: " + formatValue(last, 1);
}

export function rollAvoidFouls6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 6);
}

export function rollAvoidFouls6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls6Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls6Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 6: Needs setup";
  return "Avoid fouls" + " last 6: " + formatValue(last, 1);
}

export function rollAvoidFouls7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 7);
}

export function rollAvoidFouls7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls7Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls7Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 7: Needs setup";
  return "Avoid fouls" + " last 7: " + formatValue(last, 1);
}

export function rollAvoidFouls8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 8);
}

export function rollAvoidFouls8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls8Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls8Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 8: Needs setup";
  return "Avoid fouls" + " last 8: " + formatValue(last, 1);
}

export function rollAvoidFouls9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 9);
}

export function rollAvoidFouls9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls9Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls9Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 9: Needs setup";
  return "Avoid fouls" + " last 9: " + formatValue(last, 1);
}

export function rollAvoidFouls10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 10);
}

export function rollAvoidFouls10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls10Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls10Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 10: Needs setup";
  return "Avoid fouls" + " last 10: " + formatValue(last, 1);
}

export function rollAvoidFouls12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 12);
}

export function rollAvoidFouls12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls12Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls12Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 12: Needs setup";
  return "Avoid fouls" + " last 12: " + formatValue(last, 1);
}

export function rollAvoidFouls14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 14);
}

export function rollAvoidFouls14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls14Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls14Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 14: Needs setup";
  return "Avoid fouls" + " last 14: " + formatValue(last, 1);
}

export function rollAvoidFouls16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 16);
}

export function rollAvoidFouls16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls16Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls16Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 16: Needs setup";
  return "Avoid fouls" + " last 16: " + formatValue(last, 1);
}

export function rollAvoidFouls18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 18);
}

export function rollAvoidFouls18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls18Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls18Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 18: Needs setup";
  return "Avoid fouls" + " last 18: " + formatValue(last, 1);
}

export function rollAvoidFouls20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 20);
}

export function rollAvoidFouls20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls20Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls20Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 20: Needs setup";
  return "Avoid fouls" + " last 20: " + formatValue(last, 1);
}

export function rollAvoidFouls22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 22);
}

export function rollAvoidFouls22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls22Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls22Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 22: Needs setup";
  return "Avoid fouls" + " last 22: " + formatValue(last, 1);
}

export function rollAvoidFouls24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 24);
}

export function rollAvoidFouls24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls24Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls24Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 24: Needs setup";
  return "Avoid fouls" + " last 24: " + formatValue(last, 1);
}

export function rollAvoidFouls28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 28);
}

export function rollAvoidFouls28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls28Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls28Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 28: Needs setup";
  return "Avoid fouls" + " last 28: " + formatValue(last, 1);
}

export function rollAvoidFouls32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesAvoidFouls(rows, teamKey), 32);
}

export function rollAvoidFouls32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollAvoidFouls32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollAvoidFouls32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollAvoidFouls32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollAvoidFouls32Last(rows, teamKey) != null;
}

export function describeRollAvoidFouls32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollAvoidFouls32Last(rows, teamKey);
  if (last == null) return "Avoid fouls" + " last 32: Needs setup";
  return "Avoid fouls" + " last 32: " + formatValue(last, 1);
}

export function rollPositioning2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 2);
}

export function rollPositioning2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning2Last(rows, teamKey) != null;
}

export function describeRollPositioning2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning2Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 2: Needs setup";
  return "Positioning" + " last 2: " + formatValue(last, 1);
}

export function rollPositioning3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 3);
}

export function rollPositioning3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning3Last(rows, teamKey) != null;
}

export function describeRollPositioning3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning3Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 3: Needs setup";
  return "Positioning" + " last 3: " + formatValue(last, 1);
}

export function rollPositioning4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 4);
}

export function rollPositioning4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning4Last(rows, teamKey) != null;
}

export function describeRollPositioning4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning4Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 4: Needs setup";
  return "Positioning" + " last 4: " + formatValue(last, 1);
}

export function rollPositioning5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 5);
}

export function rollPositioning5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning5Last(rows, teamKey) != null;
}

export function describeRollPositioning5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning5Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 5: Needs setup";
  return "Positioning" + " last 5: " + formatValue(last, 1);
}

export function rollPositioning6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 6);
}

export function rollPositioning6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning6Last(rows, teamKey) != null;
}

export function describeRollPositioning6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning6Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 6: Needs setup";
  return "Positioning" + " last 6: " + formatValue(last, 1);
}

export function rollPositioning7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 7);
}

export function rollPositioning7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning7Last(rows, teamKey) != null;
}

export function describeRollPositioning7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning7Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 7: Needs setup";
  return "Positioning" + " last 7: " + formatValue(last, 1);
}

export function rollPositioning8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 8);
}

export function rollPositioning8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning8Last(rows, teamKey) != null;
}

export function describeRollPositioning8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning8Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 8: Needs setup";
  return "Positioning" + " last 8: " + formatValue(last, 1);
}

export function rollPositioning9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 9);
}

export function rollPositioning9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning9Last(rows, teamKey) != null;
}

export function describeRollPositioning9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning9Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 9: Needs setup";
  return "Positioning" + " last 9: " + formatValue(last, 1);
}

export function rollPositioning10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 10);
}

export function rollPositioning10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning10Last(rows, teamKey) != null;
}

export function describeRollPositioning10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning10Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 10: Needs setup";
  return "Positioning" + " last 10: " + formatValue(last, 1);
}

export function rollPositioning12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 12);
}

export function rollPositioning12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning12Last(rows, teamKey) != null;
}

export function describeRollPositioning12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning12Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 12: Needs setup";
  return "Positioning" + " last 12: " + formatValue(last, 1);
}

export function rollPositioning14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 14);
}

export function rollPositioning14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning14Last(rows, teamKey) != null;
}

export function describeRollPositioning14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning14Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 14: Needs setup";
  return "Positioning" + " last 14: " + formatValue(last, 1);
}

export function rollPositioning16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 16);
}

export function rollPositioning16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning16Last(rows, teamKey) != null;
}

export function describeRollPositioning16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning16Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 16: Needs setup";
  return "Positioning" + " last 16: " + formatValue(last, 1);
}

export function rollPositioning18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 18);
}

export function rollPositioning18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning18Last(rows, teamKey) != null;
}

export function describeRollPositioning18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning18Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 18: Needs setup";
  return "Positioning" + " last 18: " + formatValue(last, 1);
}

export function rollPositioning20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 20);
}

export function rollPositioning20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning20Last(rows, teamKey) != null;
}

export function describeRollPositioning20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning20Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 20: Needs setup";
  return "Positioning" + " last 20: " + formatValue(last, 1);
}

export function rollPositioning22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 22);
}

export function rollPositioning22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning22Last(rows, teamKey) != null;
}

export function describeRollPositioning22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning22Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 22: Needs setup";
  return "Positioning" + " last 22: " + formatValue(last, 1);
}

export function rollPositioning24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 24);
}

export function rollPositioning24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning24Last(rows, teamKey) != null;
}

export function describeRollPositioning24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning24Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 24: Needs setup";
  return "Positioning" + " last 24: " + formatValue(last, 1);
}

export function rollPositioning28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 28);
}

export function rollPositioning28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning28Last(rows, teamKey) != null;
}

export function describeRollPositioning28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning28Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 28: Needs setup";
  return "Positioning" + " last 28: " + formatValue(last, 1);
}

export function rollPositioning32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPositioning(rows, teamKey), 32);
}

export function rollPositioning32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPositioning32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPositioning32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPositioning32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPositioning32Last(rows, teamKey) != null;
}

export function describeRollPositioning32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPositioning32Last(rows, teamKey);
  if (last == null) return "Positioning" + " last 32: Needs setup";
  return "Positioning" + " last 32: " + formatValue(last, 1);
}
...
export function buildWideReport(input: { teamKey: string; rows: readonly SampleRow[]; fieldRows?: readonly SampleRow[] }): WideReport {
  const packs: WideMetricPack[] = [
    packDriverAbility(input.rows, input.teamKey, input.fieldRows),
    packCyclePace(input.rows, input.teamKey, input.fieldRows),
    packAvoidFouls(input.rows, input.teamKey, input.fieldRows),
    packPositioning(input.rows, input.teamKey, input.fieldRows)
  ];
  const lead = packs[0];
