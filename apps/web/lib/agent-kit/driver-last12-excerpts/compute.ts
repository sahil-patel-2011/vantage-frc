/**
 * Leftover agent kit — driver-last12-excerpts.
...
/**
 * Lovat-style Driver board — last 12 matches, tool excerpts.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last12-excerpts";
...
export const EWMA_ALPHA = 0.614;
export const TRIM_FRACTION = 0.221;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 903338106): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
