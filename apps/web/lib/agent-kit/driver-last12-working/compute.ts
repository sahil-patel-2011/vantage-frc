/**
 * Leftover agent kit — driver-last12-working.
...
/**
 * Lovat-style Driver board — last 12 matches, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last12-working";
...
export const EWMA_ALPHA = 0.523;
export const TRIM_FRACTION = 0.191;
export const NEAR_Z = 0.352;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2319286103): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
