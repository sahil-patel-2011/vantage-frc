/**
 * Leftover agent kit — driver-quals-excerpts.
...
/**
 * Lovat-style Driver board — qualification matches, tool excerpts.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-quals-excerpts";
...
export const EWMA_ALPHA = 0.453;
export const TRIM_FRACTION = 0.167;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2136294535): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
