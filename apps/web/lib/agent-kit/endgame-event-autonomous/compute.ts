/**
 * Leftover agent kit — endgame-event-autonomous.
...
/**
 * Lovat-style Endgame board — this event, autonomous loop.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-event-autonomous";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 393518000): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
