/**
 * Leftover agent kit — endgame-event-working.
...
/**
 * Lovat-style Endgame board — this event, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-event-working";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1868779683): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
