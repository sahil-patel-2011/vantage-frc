/**
 * Leftover agent kit — endgame-quals-excerpts.
...
/**
 * Lovat-style Endgame board — qualification matches, tool excerpts.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-quals-excerpts";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 527241538): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
