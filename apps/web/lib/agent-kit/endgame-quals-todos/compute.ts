/**
 * Leftover agent kit — endgame-quals-todos.
...
/**
 * Lovat-style Endgame board — qualification matches, working todos.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-quals-todos";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 698148797): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
