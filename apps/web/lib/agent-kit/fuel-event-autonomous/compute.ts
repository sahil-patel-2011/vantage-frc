/**
 * Leftover agent kit — fuel-event-autonomous.
...
/**
 * Lovat-style Fuel board — this event, autonomous loop.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-event-autonomous";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3344966815): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
