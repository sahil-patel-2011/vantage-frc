/**
 * Leftover agent kit — fuel-event-excerpts.
...
/**
 * Lovat-style Fuel board — this event, tool excerpts.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-event-excerpts";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 939293885): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
