/**
 * Leftover agent kit — fuel-event-working.
...
/**
 * Lovat-style Fuel board — this event, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-event-working";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3469156246): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
