/**
 * Leftover agent kit — fuel-last12-todos.
...
/**
 * Lovat-style Fuel board — last 12 matches, working todos.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-last12-todos";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3439380475): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
