/**
 * Leftover agent kit — fuel-quals-todos.
...
/**
 * Lovat-style Fuel board — qualification matches, working todos.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-quals-todos";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1376227096): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
