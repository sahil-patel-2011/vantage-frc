/**
 * Leftover agent kit — fuel-quals-working.
...
/**
 * Lovat-style Fuel board — qualification matches, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-quals-working";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3520975386): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
