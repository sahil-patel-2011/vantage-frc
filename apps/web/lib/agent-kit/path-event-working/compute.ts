/**
 * Leftover agent kit — path-event-working.
...
/**
 * Lovat-style Auto path board — this event, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "path-event-working";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1908515921): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
