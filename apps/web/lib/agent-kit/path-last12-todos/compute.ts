/**
 * Leftover agent kit — path-last12-todos.
...
/**
 * Lovat-style Auto path board — last 12 matches, working todos.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "path-last12-todos";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3149429306): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
