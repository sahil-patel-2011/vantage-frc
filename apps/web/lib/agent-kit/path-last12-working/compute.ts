/**
 * Leftover agent kit — path-last12-working.
...
/**
 * Lovat-style Auto path board — last 12 matches, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "path-last12-working";
export const PHASE_LABEL = "Auto path";
...
export type LovatMetricId = "pathLength" | "pathTime" | "pathPoints" | "pathClear";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2783824088): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
...
export function seriesPathLength(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["pathLength" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statPathLength(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesPathLength(rows, teamKey));
}

export function shrinkPathLength(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesPathLength(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["pathLength" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "pathLength" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastPathLength(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesPathLength(rows, teamKey));
}

export function fitPathLength(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesPathLength(rows, teamKey));
}

export function controlPathLength(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesPathLength(rows, teamKey));
}

export function slotsPathLength(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesPathLength(rows, teamKey));
}

export function residualPathLength(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "pathLength" as LovatMetricId, oppKeys);
}

export function histPathLength(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesPathLength(rows, teamKey));
}

export function cvPathLength(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesPathLength(rows, teamKey));
}

export function trimPathLength(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesPathLength(rows, teamKey));
}

export function ewmaPathLength(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesPathLength(rows, teamKey));
}

export function jackPathLength(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesPathLength(rows, teamKey));
}

export function bootPathLength(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesPathLength(rows, teamKey));
}

export function hazardPathLength(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesPathLength(rows, teamKey));
}

export function streakPathLength(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesPathLength(rows, teamKey));
}

export function giniPathLength(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesPathLength(rows, teamKey));
}

export function theilPathLength(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesPathLength(rows, teamKey));
}

export function skewPathLength(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesPathLength(rows, teamKey));
}

export function rangePathLength(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesPathLength(rows, teamKey));
}

export function madMeanPathLength(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesPathLength(rows, teamKey));
}

export function packPathLength(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesPathLength(rows, teamKey);
  return {
    id: "pathLength" as LovatMetricId,
    label: "Path length",
    series,
    stat: wideStat(series),
    shrink: shrinkPathLength(rows, teamKey, fieldRows),
    forecast: forecastPathLength(rows, teamKey),
    fit: fitPathLength(rows, teamKey),
    control: controlPathLength(rows, teamKey),
    slots: slotsPathLength(rows, teamKey),
  };
}

export function describePathLength(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Path length · Needs setup`;
  return `Path length · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesPathTime(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["pathTime" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statPathTime(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesPathTime(rows, teamKey));
}

export function shrinkPathTime(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesPathTime(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["pathTime" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "pathTime" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastPathTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesPathTime(rows, teamKey));
}

export function fitPathTime(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesPathTime(rows, teamKey));
}

export function controlPathTime(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesPathTime(rows, teamKey));
}

export function slotsPathTime(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesPathTime(rows, teamKey));
}

export function residualPathTime(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "pathTime" as LovatMetricId, oppKeys);
}

export function histPathTime(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesPathTime(rows, teamKey));
}

export function cvPathTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesPathTime(rows, teamKey));
}

export function trimPathTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesPathTime(rows, teamKey));
}

export function ewmaPathTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesPathTime(rows, teamKey));
}

export function jackPathTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesPathTime(rows, teamKey));
}

export function bootPathTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesPathTime(rows, teamKey));
}

export function hazardPathTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesPathTime(rows, teamKey));
}

export function streakPathTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesPathTime(rows, teamKey));
}

export function giniPathTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesPathTime(rows, teamKey));
}

export function theilPathTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesPathTime(rows, teamKey));
}

export function skewPathTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesPathTime(rows, teamKey));
}

export function rangePathTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesPathTime(rows, teamKey));
}

export function madMeanPathTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesPathTime(rows, teamKey));
}

export function packPathTime(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesPathTime(rows, teamKey);
  return {
    id: "pathTime" as LovatMetricId,
    label: "Path time",
    series,
    stat: wideStat(series),
    shrink: shrinkPathTime(rows, teamKey, fieldRows),
    forecast: forecastPathTime(rows, teamKey),
    fit: fitPathTime(rows, teamKey),
    control: controlPathTime(rows, teamKey),
    slots: slotsPathTime(rows, teamKey),
  };
}

export function describePathTime(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Path time · Needs setup`;
  return `Path time · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesPathPoints(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["pathPoints" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statPathPoints(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesPathPoints(rows, teamKey));
}

export function shrinkPathPoints(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesPathPoints(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["pathPoints" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "pathPoints" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastPathPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesPathPoints(rows, teamKey));
}

export function fitPathPoints(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesPathPoints(rows, teamKey));
}

export function controlPathPoints(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesPathPoints(rows, teamKey));
}

export function slotsPathPoints(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesPathPoints(rows, teamKey));
}

export function residualPathPoints(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "pathPoints" as LovatMetricId, oppKeys);
}

export function histPathPoints(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesPathPoints(rows, teamKey));
}

export function cvPathPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesPathPoints(rows, teamKey));
}

export function trimPathPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesPathPoints(rows, teamKey));
}

export function ewmaPathPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesPathPoints(rows, teamKey));
}

export function jackPathPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesPathPoints(rows, teamKey));
}

export function bootPathPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesPathPoints(rows, teamKey));
}

export function hazardPathPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesPathPoints(rows, teamKey));
}

export function streakPathPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesPathPoints(rows, teamKey));
}

export function giniPathPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesPathPoints(rows, teamKey));
}

export function theilPathPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesPathPoints(rows, teamKey));
}

export function skewPathPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesPathPoints(rows, teamKey));
}

export function rangePathPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesPathPoints(rows, teamKey));
}

export function madMeanPathPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesPathPoints(rows, teamKey));
}

export function packPathPoints(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesPathPoints(rows, teamKey);
  return {
    id: "pathPoints" as LovatMetricId,
    label: "Path points",
    series,
    stat: wideStat(series),
    shrink: shrinkPathPoints(rows, teamKey, fieldRows),
    forecast: forecastPathPoints(rows, teamKey),
    fit: fitPathPoints(rows, teamKey),
    control: controlPathPoints(rows, teamKey),
    slots: slotsPathPoints(rows, teamKey),
  };
}

export function describePathPoints(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Path points · Needs setup`;
  return `Path points · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesPathClear(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["pathClear" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statPathClear(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesPathClear(rows, teamKey));
}

export function shrinkPathClear(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesPathClear(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["pathClear" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "pathClear" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastPathClear(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesPathClear(rows, teamKey));
}

export function fitPathClear(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesPathClear(rows, teamKey));
}

export function controlPathClear(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesPathClear(rows, teamKey));
}

export function slotsPathClear(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesPathClear(rows, teamKey));
}

export function residualPathClear(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "pathClear" as LovatMetricId, oppKeys);
}

export function histPathClear(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesPathClear(rows, teamKey));
}

export function cvPathClear(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesPathClear(rows, teamKey));
}

export function trimPathClear(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesPathClear(rows, teamKey));
}

export function ewmaPathClear(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesPathClear(rows, teamKey));
}

export function jackPathClear(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesPathClear(rows, teamKey));
}

export function bootPathClear(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesPathClear(rows, teamKey));
}

export function hazardPathClear(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesPathClear(rows, teamKey));
}

export function streakPathClear(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesPathClear(rows, teamKey));
}

export function giniPathClear(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesPathClear(rows, teamKey));
}

export function theilPathClear(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesPathClear(rows, teamKey));
}

export function skewPathClear(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesPathClear(rows, teamKey));
}

export function rangePathClear(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesPathClear(rows, teamKey));
}

export function madMeanPathClear(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesPathClear(rows, teamKey));
}

export function packPathClear(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesPathClear(rows, teamKey);
  return {
    id: "pathClear" as LovatMetricId,
    label: "Path clear",
    series,
    stat: wideStat(series),
    shrink: shrinkPathClear(rows, teamKey, fieldRows),
    forecast: forecastPathClear(rows, teamKey),
    fit: fitPathClear(rows, teamKey),
    control: controlPathClear(rows, teamKey),
    slots: slotsPathClear(rows, teamKey),
  };
}

export function describePathClear(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Path clear · Needs setup`;
  return `Path clear · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}
...
export function rollPathLength2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 2);
}

export function rollPathLength2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength2(rows, teamKey);
  const last = series[series.length - 1];
...
export function rollPathLength2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength2Last(rows, teamKey) != null;
}

export function describeRollPathLength2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength2Last(rows, teamKey);
  if (last == null) return "Path length" + " last 2: Needs setup";
  return "Path length" + " last 2: " + formatValue(last, 1);
}

export function rollPathLength3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 3);
}

export function rollPathLength3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength3Last(rows, teamKey) != null;
}

export function describeRollPathLength3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength3Last(rows, teamKey);
  if (last == null) return "Path length" + " last 3: Needs setup";
  return "Path length" + " last 3: " + formatValue(last, 1);
}

export function rollPathLength4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 4);
}

export function rollPathLength4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength4Last(rows, teamKey) != null;
}

export function describeRollPathLength4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength4Last(rows, teamKey);
  if (last == null) return "Path length" + " last 4: Needs setup";
  return "Path length" + " last 4: " + formatValue(last, 1);
}

export function rollPathLength5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 5);
}

export function rollPathLength5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength5Last(rows, teamKey) != null;
}

export function describeRollPathLength5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength5Last(rows, teamKey);
  if (last == null) return "Path length" + " last 5: Needs setup";
  return "Path length" + " last 5: " + formatValue(last, 1);
}

export function rollPathLength6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 6);
}

export function rollPathLength6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength6Last(rows, teamKey) != null;
}

export function describeRollPathLength6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength6Last(rows, teamKey);
  if (last == null) return "Path length" + " last 6: Needs setup";
  return "Path length" + " last 6: " + formatValue(last, 1);
}

export function rollPathLength7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 7);
}

export function rollPathLength7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength7Last(rows, teamKey) != null;
}

export function describeRollPathLength7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength7Last(rows, teamKey);
  if (last == null) return "Path length" + " last 7: Needs setup";
  return "Path length" + " last 7: " + formatValue(last, 1);
}

export function rollPathLength8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 8);
}

export function rollPathLength8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength8Last(rows, teamKey) != null;
}

export function describeRollPathLength8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength8Last(rows, teamKey);
  if (last == null) return "Path length" + " last 8: Needs setup";
  return "Path length" + " last 8: " + formatValue(last, 1);
}

export function rollPathLength9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 9);
}

export function rollPathLength9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength9Last(rows, teamKey) != null;
}

export function describeRollPathLength9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength9Last(rows, teamKey);
  if (last == null) return "Path length" + " last 9: Needs setup";
  return "Path length" + " last 9: " + formatValue(last, 1);
}

export function rollPathLength10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 10);
}

export function rollPathLength10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength10Last(rows, teamKey) != null;
}

export function describeRollPathLength10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength10Last(rows, teamKey);
  if (last == null) return "Path length" + " last 10: Needs setup";
  return "Path length" + " last 10: " + formatValue(last, 1);
}

export function rollPathLength12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 12);
}

export function rollPathLength12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength12Last(rows, teamKey) != null;
}

export function describeRollPathLength12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength12Last(rows, teamKey);
  if (last == null) return "Path length" + " last 12: Needs setup";
  return "Path length" + " last 12: " + formatValue(last, 1);
}

export function rollPathLength14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 14);
}

export function rollPathLength14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength14Last(rows, teamKey) != null;
}

export function describeRollPathLength14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength14Last(rows, teamKey);
  if (last == null) return "Path length" + " last 14: Needs setup";
  return "Path length" + " last 14: " + formatValue(last, 1);
}

export function rollPathLength16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 16);
}

export function rollPathLength16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength16Last(rows, teamKey) != null;
}

export function describeRollPathLength16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength16Last(rows, teamKey);
  if (last == null) return "Path length" + " last 16: Needs setup";
  return "Path length" + " last 16: " + formatValue(last, 1);
}

export function rollPathLength18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 18);
}

export function rollPathLength18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength18Last(rows, teamKey) != null;
}

export function describeRollPathLength18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength18Last(rows, teamKey);
  if (last == null) return "Path length" + " last 18: Needs setup";
  return "Path length" + " last 18: " + formatValue(last, 1);
}

export function rollPathLength20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 20);
}

export function rollPathLength20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength20Last(rows, teamKey) != null;
}

export function describeRollPathLength20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength20Last(rows, teamKey);
  if (last == null) return "Path length" + " last 20: Needs setup";
  return "Path length" + " last 20: " + formatValue(last, 1);
}

export function rollPathLength22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 22);
}

export function rollPathLength22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength22Last(rows, teamKey) != null;
}

export function describeRollPathLength22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength22Last(rows, teamKey);
  if (last == null) return "Path length" + " last 22: Needs setup";
  return "Path length" + " last 22: " + formatValue(last, 1);
}

export function rollPathLength24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 24);
}

export function rollPathLength24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength24Last(rows, teamKey) != null;
}

export function describeRollPathLength24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength24Last(rows, teamKey);
  if (last == null) return "Path length" + " last 24: Needs setup";
  return "Path length" + " last 24: " + formatValue(last, 1);
}

export function rollPathLength28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 28);
}

export function rollPathLength28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength28Last(rows, teamKey) != null;
}

export function describeRollPathLength28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength28Last(rows, teamKey);
  if (last == null) return "Path length" + " last 28: Needs setup";
  return "Path length" + " last 28: " + formatValue(last, 1);
}

export function rollPathLength32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathLength(rows, teamKey), 32);
}

export function rollPathLength32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathLength32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathLength32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathLength32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathLength32Last(rows, teamKey) != null;
}

export function describeRollPathLength32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathLength32Last(rows, teamKey);
  if (last == null) return "Path length" + " last 32: Needs setup";
  return "Path length" + " last 32: " + formatValue(last, 1);
}

export function rollPathTime2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 2);
}

export function rollPathTime2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime2Last(rows, teamKey) != null;
}

export function describeRollPathTime2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime2Last(rows, teamKey);
  if (last == null) return "Path time" + " last 2: Needs setup";
  return "Path time" + " last 2: " + formatValue(last, 1);
}

export function rollPathTime3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 3);
}

export function rollPathTime3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime3Last(rows, teamKey) != null;
}

export function describeRollPathTime3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime3Last(rows, teamKey);
  if (last == null) return "Path time" + " last 3: Needs setup";
  return "Path time" + " last 3: " + formatValue(last, 1);
}

export function rollPathTime4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 4);
}

export function rollPathTime4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime4Last(rows, teamKey) != null;
}

export function describeRollPathTime4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime4Last(rows, teamKey);
  if (last == null) return "Path time" + " last 4: Needs setup";
  return "Path time" + " last 4: " + formatValue(last, 1);
}

export function rollPathTime5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 5);
}

export function rollPathTime5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime5Last(rows, teamKey) != null;
}

export function describeRollPathTime5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime5Last(rows, teamKey);
  if (last == null) return "Path time" + " last 5: Needs setup";
  return "Path time" + " last 5: " + formatValue(last, 1);
}

export function rollPathTime6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 6);
}

export function rollPathTime6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime6Last(rows, teamKey) != null;
}

export function describeRollPathTime6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime6Last(rows, teamKey);
  if (last == null) return "Path time" + " last 6: Needs setup";
  return "Path time" + " last 6: " + formatValue(last, 1);
}

export function rollPathTime7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 7);
}

export function rollPathTime7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime7Last(rows, teamKey) != null;
}

export function describeRollPathTime7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime7Last(rows, teamKey);
  if (last == null) return "Path time" + " last 7: Needs setup";
  return "Path time" + " last 7: " + formatValue(last, 1);
}

export function rollPathTime8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 8);
}

export function rollPathTime8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime8Last(rows, teamKey) != null;
}

export function describeRollPathTime8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime8Last(rows, teamKey);
  if (last == null) return "Path time" + " last 8: Needs setup";
  return "Path time" + " last 8: " + formatValue(last, 1);
}

export function rollPathTime9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 9);
}

export function rollPathTime9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime9Last(rows, teamKey) != null;
}

export function describeRollPathTime9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime9Last(rows, teamKey);
  if (last == null) return "Path time" + " last 9: Needs setup";
  return "Path time" + " last 9: " + formatValue(last, 1);
}

export function rollPathTime10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 10);
}

export function rollPathTime10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime10Last(rows, teamKey) != null;
}

export function describeRollPathTime10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime10Last(rows, teamKey);
  if (last == null) return "Path time" + " last 10: Needs setup";
  return "Path time" + " last 10: " + formatValue(last, 1);
}

export function rollPathTime12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 12);
}

export function rollPathTime12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime12Last(rows, teamKey) != null;
}

export function describeRollPathTime12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime12Last(rows, teamKey);
  if (last == null) return "Path time" + " last 12: Needs setup";
  return "Path time" + " last 12: " + formatValue(last, 1);
}

export function rollPathTime14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 14);
}

export function rollPathTime14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime14Last(rows, teamKey) != null;
}

export function describeRollPathTime14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime14Last(rows, teamKey);
  if (last == null) return "Path time" + " last 14: Needs setup";
  return "Path time" + " last 14: " + formatValue(last, 1);
}

export function rollPathTime16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 16);
}

export function rollPathTime16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime16Last(rows, teamKey) != null;
}

export function describeRollPathTime16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime16Last(rows, teamKey);
  if (last == null) return "Path time" + " last 16: Needs setup";
  return "Path time" + " last 16: " + formatValue(last, 1);
}

export function rollPathTime18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 18);
}

export function rollPathTime18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime18Last(rows, teamKey) != null;
}

export function describeRollPathTime18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime18Last(rows, teamKey);
  if (last == null) return "Path time" + " last 18: Needs setup";
  return "Path time" + " last 18: " + formatValue(last, 1);
}

export function rollPathTime20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 20);
}

export function rollPathTime20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime20Last(rows, teamKey) != null;
}

export function describeRollPathTime20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime20Last(rows, teamKey);
  if (last == null) return "Path time" + " last 20: Needs setup";
  return "Path time" + " last 20: " + formatValue(last, 1);
}

export function rollPathTime22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 22);
}

export function rollPathTime22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime22Last(rows, teamKey) != null;
}

export function describeRollPathTime22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime22Last(rows, teamKey);
  if (last == null) return "Path time" + " last 22: Needs setup";
  return "Path time" + " last 22: " + formatValue(last, 1);
}

export function rollPathTime24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 24);
}

export function rollPathTime24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime24Last(rows, teamKey) != null;
}

export function describeRollPathTime24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime24Last(rows, teamKey);
  if (last == null) return "Path time" + " last 24: Needs setup";
  return "Path time" + " last 24: " + formatValue(last, 1);
}

export function rollPathTime28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 28);
}

export function rollPathTime28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime28Last(rows, teamKey) != null;
}

export function describeRollPathTime28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime28Last(rows, teamKey);
  if (last == null) return "Path time" + " last 28: Needs setup";
  return "Path time" + " last 28: " + formatValue(last, 1);
}

export function rollPathTime32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathTime(rows, teamKey), 32);
}

export function rollPathTime32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathTime32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathTime32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathTime32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathTime32Last(rows, teamKey) != null;
}

export function describeRollPathTime32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathTime32Last(rows, teamKey);
  if (last == null) return "Path time" + " last 32: Needs setup";
  return "Path time" + " last 32: " + formatValue(last, 1);
}

export function rollPathPoints2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 2);
}

export function rollPathPoints2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints2Last(rows, teamKey) != null;
}

export function describeRollPathPoints2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints2Last(rows, teamKey);
  if (last == null) return "Path points" + " last 2: Needs setup";
  return "Path points" + " last 2: " + formatValue(last, 1);
}

export function rollPathPoints3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 3);
}

export function rollPathPoints3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints3Last(rows, teamKey) != null;
}

export function describeRollPathPoints3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints3Last(rows, teamKey);
  if (last == null) return "Path points" + " last 3: Needs setup";
  return "Path points" + " last 3: " + formatValue(last, 1);
}

export function rollPathPoints4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 4);
}

export function rollPathPoints4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints4Last(rows, teamKey) != null;
}

export function describeRollPathPoints4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints4Last(rows, teamKey);
  if (last == null) return "Path points" + " last 4: Needs setup";
  return "Path points" + " last 4: " + formatValue(last, 1);
}

export function rollPathPoints5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 5);
}

export function rollPathPoints5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints5Last(rows, teamKey) != null;
}

export function describeRollPathPoints5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints5Last(rows, teamKey);
  if (last == null) return "Path points" + " last 5: Needs setup";
  return "Path points" + " last 5: " + formatValue(last, 1);
}

export function rollPathPoints6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 6);
}

export function rollPathPoints6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints6Last(rows, teamKey) != null;
}

export function describeRollPathPoints6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints6Last(rows, teamKey);
  if (last == null) return "Path points" + " last 6: Needs setup";
  return "Path points" + " last 6: " + formatValue(last, 1);
}

export function rollPathPoints7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 7);
}

export function rollPathPoints7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints7Last(rows, teamKey) != null;
}

export function describeRollPathPoints7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints7Last(rows, teamKey);
  if (last == null) return "Path points" + " last 7: Needs setup";
  return "Path points" + " last 7: " + formatValue(last, 1);
}

export function rollPathPoints8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 8);
}

export function rollPathPoints8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints8Last(rows, teamKey) != null;
}

export function describeRollPathPoints8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints8Last(rows, teamKey);
  if (last == null) return "Path points" + " last 8: Needs setup";
  return "Path points" + " last 8: " + formatValue(last, 1);
}

export function rollPathPoints9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 9);
}

export function rollPathPoints9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints9Last(rows, teamKey) != null;
}

export function describeRollPathPoints9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints9Last(rows, teamKey);
  if (last == null) return "Path points" + " last 9: Needs setup";
  return "Path points" + " last 9: " + formatValue(last, 1);
}

export function rollPathPoints10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 10);
}

export function rollPathPoints10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints10Last(rows, teamKey) != null;
}

export function describeRollPathPoints10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints10Last(rows, teamKey);
  if (last == null) return "Path points" + " last 10: Needs setup";
  return "Path points" + " last 10: " + formatValue(last, 1);
}

export function rollPathPoints12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 12);
}

export function rollPathPoints12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints12Last(rows, teamKey) != null;
}

export function describeRollPathPoints12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints12Last(rows, teamKey);
  if (last == null) return "Path points" + " last 12: Needs setup";
  return "Path points" + " last 12: " + formatValue(last, 1);
}

export function rollPathPoints14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 14);
}

export function rollPathPoints14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints14Last(rows, teamKey) != null;
}

export function describeRollPathPoints14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints14Last(rows, teamKey);
  if (last == null) return "Path points" + " last 14: Needs setup";
  return "Path points" + " last 14: " + formatValue(last, 1);
}

export function rollPathPoints16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 16);
}

export function rollPathPoints16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints16Last(rows, teamKey) != null;
}

export function describeRollPathPoints16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints16Last(rows, teamKey);
  if (last == null) return "Path points" + " last 16: Needs setup";
  return "Path points" + " last 16: " + formatValue(last, 1);
}

export function rollPathPoints18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 18);
}

export function rollPathPoints18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints18Last(rows, teamKey) != null;
}

export function describeRollPathPoints18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints18Last(rows, teamKey);
  if (last == null) return "Path points" + " last 18: Needs setup";
  return "Path points" + " last 18: " + formatValue(last, 1);
}

export function rollPathPoints20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 20);
}

export function rollPathPoints20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints20Last(rows, teamKey) != null;
}

export function describeRollPathPoints20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints20Last(rows, teamKey);
  if (last == null) return "Path points" + " last 20: Needs setup";
  return "Path points" + " last 20: " + formatValue(last, 1);
}

export function rollPathPoints22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 22);
}

export function rollPathPoints22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints22Last(rows, teamKey) != null;
}

export function describeRollPathPoints22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints22Last(rows, teamKey);
  if (last == null) return "Path points" + " last 22: Needs setup";
  return "Path points" + " last 22: " + formatValue(last, 1);
}

export function rollPathPoints24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 24);
}

export function rollPathPoints24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints24Last(rows, teamKey) != null;
}

export function describeRollPathPoints24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints24Last(rows, teamKey);
  if (last == null) return "Path points" + " last 24: Needs setup";
  return "Path points" + " last 24: " + formatValue(last, 1);
}

export function rollPathPoints28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 28);
}

export function rollPathPoints28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints28Last(rows, teamKey) != null;
}

export function describeRollPathPoints28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints28Last(rows, teamKey);
  if (last == null) return "Path points" + " last 28: Needs setup";
  return "Path points" + " last 28: " + formatValue(last, 1);
}

export function rollPathPoints32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathPoints(rows, teamKey), 32);
}

export function rollPathPoints32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathPoints32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathPoints32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathPoints32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathPoints32Last(rows, teamKey) != null;
}

export function describeRollPathPoints32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathPoints32Last(rows, teamKey);
  if (last == null) return "Path points" + " last 32: Needs setup";
  return "Path points" + " last 32: " + formatValue(last, 1);
}

export function rollPathClear2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 2);
}

export function rollPathClear2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear2Last(rows, teamKey) != null;
}

export function describeRollPathClear2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear2Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 2: Needs setup";
  return "Path clear" + " last 2: " + formatValue(last, 1);
}

export function rollPathClear3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 3);
}

export function rollPathClear3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear3Last(rows, teamKey) != null;
}

export function describeRollPathClear3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear3Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 3: Needs setup";
  return "Path clear" + " last 3: " + formatValue(last, 1);
}

export function rollPathClear4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 4);
}

export function rollPathClear4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear4Last(rows, teamKey) != null;
}

export function describeRollPathClear4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear4Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 4: Needs setup";
  return "Path clear" + " last 4: " + formatValue(last, 1);
}

export function rollPathClear5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 5);
}

export function rollPathClear5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear5Last(rows, teamKey) != null;
}

export function describeRollPathClear5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear5Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 5: Needs setup";
  return "Path clear" + " last 5: " + formatValue(last, 1);
}

export function rollPathClear6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 6);
}

export function rollPathClear6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear6Last(rows, teamKey) != null;
}

export function describeRollPathClear6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear6Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 6: Needs setup";
  return "Path clear" + " last 6: " + formatValue(last, 1);
}

export function rollPathClear7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 7);
}

export function rollPathClear7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear7Last(rows, teamKey) != null;
}

export function describeRollPathClear7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear7Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 7: Needs setup";
  return "Path clear" + " last 7: " + formatValue(last, 1);
}

export function rollPathClear8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 8);
}

export function rollPathClear8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear8Last(rows, teamKey) != null;
}

export function describeRollPathClear8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear8Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 8: Needs setup";
  return "Path clear" + " last 8: " + formatValue(last, 1);
}

export function rollPathClear9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 9);
}

export function rollPathClear9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear9Last(rows, teamKey) != null;
}

export function describeRollPathClear9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear9Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 9: Needs setup";
  return "Path clear" + " last 9: " + formatValue(last, 1);
}

export function rollPathClear10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 10);
}

export function rollPathClear10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear10Last(rows, teamKey) != null;
}

export function describeRollPathClear10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear10Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 10: Needs setup";
  return "Path clear" + " last 10: " + formatValue(last, 1);
}

export function rollPathClear12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 12);
}

export function rollPathClear12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear12Last(rows, teamKey) != null;
}

export function describeRollPathClear12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear12Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 12: Needs setup";
  return "Path clear" + " last 12: " + formatValue(last, 1);
}

export function rollPathClear14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 14);
}

export function rollPathClear14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear14Last(rows, teamKey) != null;
}

export function describeRollPathClear14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear14Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 14: Needs setup";
  return "Path clear" + " last 14: " + formatValue(last, 1);
}

export function rollPathClear16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 16);
}

export function rollPathClear16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear16Last(rows, teamKey) != null;
}

export function describeRollPathClear16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear16Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 16: Needs setup";
  return "Path clear" + " last 16: " + formatValue(last, 1);
}

export function rollPathClear18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 18);
}

export function rollPathClear18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear18Last(rows, teamKey) != null;
}

export function describeRollPathClear18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear18Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 18: Needs setup";
  return "Path clear" + " last 18: " + formatValue(last, 1);
}

export function rollPathClear20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 20);
}

export function rollPathClear20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear20Last(rows, teamKey) != null;
}

export function describeRollPathClear20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear20Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 20: Needs setup";
  return "Path clear" + " last 20: " + formatValue(last, 1);
}

export function rollPathClear22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 22);
}

export function rollPathClear22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear22Last(rows, teamKey) != null;
}

export function describeRollPathClear22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear22Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 22: Needs setup";
  return "Path clear" + " last 22: " + formatValue(last, 1);
}

export function rollPathClear24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 24);
}

export function rollPathClear24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear24Last(rows, teamKey) != null;
}

export function describeRollPathClear24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear24Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 24: Needs setup";
  return "Path clear" + " last 24: " + formatValue(last, 1);
}

export function rollPathClear28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 28);
}

export function rollPathClear28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear28Last(rows, teamKey) != null;
}

export function describeRollPathClear28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear28Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 28: Needs setup";
  return "Path clear" + " last 28: " + formatValue(last, 1);
}

export function rollPathClear32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPathClear(rows, teamKey), 32);
}

export function rollPathClear32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPathClear32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPathClear32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPathClear32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPathClear32Last(rows, teamKey) != null;
}

export function describeRollPathClear32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPathClear32Last(rows, teamKey);
  if (last == null) return "Path clear" + " last 32: Needs setup";
  return "Path clear" + " last 32: " + formatValue(last, 1);
}
...
export function buildWideReport(input: { teamKey: string; rows: readonly SampleRow[]; fieldRows?: readonly SampleRow[] }): WideReport {
  const packs: WideMetricPack[] = [
    packPathLength(input.rows, input.teamKey, input.fieldRows),
    packPathTime(input.rows, input.teamKey, input.fieldRows),
    packPathPoints(input.rows, input.teamKey, input.fieldRows),
    packPathClear(input.rows, input.teamKey, input.fieldRows)
  ];
  const lead = packs[0];
