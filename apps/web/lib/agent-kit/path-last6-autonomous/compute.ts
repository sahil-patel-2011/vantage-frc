/**
 * Leftover agent kit — path-last6-autonomous.
...
/**
 * Lovat-style Auto path board — last 6 matches, autonomous loop.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "path-last6-autonomous";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 239815852): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
