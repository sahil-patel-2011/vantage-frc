/**
 * Leftover agent kit — path-quals-excerpts.
...
/**
 * Lovat-style Auto path board — qualification matches, tool excerpts.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "path-quals-excerpts";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4284868508): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
