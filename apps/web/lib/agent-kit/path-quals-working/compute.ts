/**
 * Leftover agent kit — path-quals-working.
...
/**
 * Lovat-style Auto path board — qualification matches, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "path-quals-working";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2728558409): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
