/**
 * Leftover agent kit — reliability-event-cadhop.
...
/**
 * Lovat-style Reliability board — this event, CAD hop persist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "reliability-event-cadhop";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1555380182): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
