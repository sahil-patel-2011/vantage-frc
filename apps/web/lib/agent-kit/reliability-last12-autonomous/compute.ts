/**
 * Leftover agent kit — reliability-last12-autonomous.
...
/**
 * Lovat-style Reliability board — last 12 matches, autonomous loop.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "reliability-last12-autonomous";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 917951704): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
