/**
 * Leftover agent kit — reliability-last12-working.
...
/**
 * Lovat-style Reliability board — last 12 matches, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "reliability-last12-working";
export const PHASE_LABEL = "Reliability";
...
export type LovatMetricId = "uptime" | "disabled" | "consistency" | "tipRisk";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3147157131): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
...
export function seriesUptime(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["uptime" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statUptime(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesUptime(rows, teamKey));
}

export function shrinkUptime(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesUptime(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["uptime" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "uptime" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastUptime(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesUptime(rows, teamKey));
}

export function fitUptime(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesUptime(rows, teamKey));
}

export function controlUptime(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesUptime(rows, teamKey));
}

export function slotsUptime(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesUptime(rows, teamKey));
}

export function residualUptime(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "uptime" as LovatMetricId, oppKeys);
}

export function histUptime(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesUptime(rows, teamKey));
}

export function cvUptime(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesUptime(rows, teamKey));
}

export function trimUptime(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesUptime(rows, teamKey));
}

export function ewmaUptime(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesUptime(rows, teamKey));
}

export function jackUptime(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesUptime(rows, teamKey));
}

export function bootUptime(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesUptime(rows, teamKey));
}

export function hazardUptime(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesUptime(rows, teamKey));
}

export function streakUptime(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesUptime(rows, teamKey));
}

export function giniUptime(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesUptime(rows, teamKey));
}

export function theilUptime(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesUptime(rows, teamKey));
}

export function skewUptime(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesUptime(rows, teamKey));
}

export function rangeUptime(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesUptime(rows, teamKey));
}

export function madMeanUptime(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesUptime(rows, teamKey));
}

export function packUptime(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesUptime(rows, teamKey);
  return {
    id: "uptime" as LovatMetricId,
    label: "Uptime",
    series,
    stat: wideStat(series),
    shrink: shrinkUptime(rows, teamKey, fieldRows),
    forecast: forecastUptime(rows, teamKey),
    fit: fitUptime(rows, teamKey),
    control: controlUptime(rows, teamKey),
    slots: slotsUptime(rows, teamKey),
  };
}

export function describeUptime(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Uptime · Needs setup`;
  return `Uptime · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesDisabled(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["disabled" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statDisabled(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesDisabled(rows, teamKey));
}

export function shrinkDisabled(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesDisabled(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["disabled" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "disabled" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastDisabled(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesDisabled(rows, teamKey));
}

export function fitDisabled(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesDisabled(rows, teamKey));
}

export function controlDisabled(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesDisabled(rows, teamKey));
}

export function slotsDisabled(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesDisabled(rows, teamKey));
}

export function residualDisabled(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "disabled" as LovatMetricId, oppKeys);
}

export function histDisabled(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesDisabled(rows, teamKey));
}

export function cvDisabled(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesDisabled(rows, teamKey));
}

export function trimDisabled(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesDisabled(rows, teamKey));
}

export function ewmaDisabled(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesDisabled(rows, teamKey));
}

export function jackDisabled(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesDisabled(rows, teamKey));
}

export function bootDisabled(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesDisabled(rows, teamKey));
}

export function hazardDisabled(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesDisabled(rows, teamKey));
}

export function streakDisabled(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesDisabled(rows, teamKey));
}

export function giniDisabled(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesDisabled(rows, teamKey));
}

export function theilDisabled(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesDisabled(rows, teamKey));
}

export function skewDisabled(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesDisabled(rows, teamKey));
}

export function rangeDisabled(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesDisabled(rows, teamKey));
}

export function madMeanDisabled(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesDisabled(rows, teamKey));
}

export function packDisabled(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesDisabled(rows, teamKey);
  return {
    id: "disabled" as LovatMetricId,
    label: "Disabled time",
    series,
    stat: wideStat(series),
    shrink: shrinkDisabled(rows, teamKey, fieldRows),
    forecast: forecastDisabled(rows, teamKey),
    fit: fitDisabled(rows, teamKey),
    control: controlDisabled(rows, teamKey),
    slots: slotsDisabled(rows, teamKey),
  };
}

export function describeDisabled(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Disabled time · Needs setup`;
  return `Disabled time · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesConsistency(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["consistency" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statConsistency(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesConsistency(rows, teamKey));
}

export function shrinkConsistency(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesConsistency(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["consistency" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "consistency" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastConsistency(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesConsistency(rows, teamKey));
}

export function fitConsistency(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesConsistency(rows, teamKey));
}

export function controlConsistency(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesConsistency(rows, teamKey));
}

export function slotsConsistency(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesConsistency(rows, teamKey));
}

export function residualConsistency(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "consistency" as LovatMetricId, oppKeys);
}

export function histConsistency(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesConsistency(rows, teamKey));
}

export function cvConsistency(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesConsistency(rows, teamKey));
}

export function trimConsistency(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesConsistency(rows, teamKey));
}

export function ewmaConsistency(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesConsistency(rows, teamKey));
}

export function jackConsistency(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesConsistency(rows, teamKey));
}

export function bootConsistency(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesConsistency(rows, teamKey));
}

export function hazardConsistency(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesConsistency(rows, teamKey));
}

export function streakConsistency(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesConsistency(rows, teamKey));
}

export function giniConsistency(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesConsistency(rows, teamKey));
}

export function theilConsistency(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesConsistency(rows, teamKey));
}

export function skewConsistency(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesConsistency(rows, teamKey));
}

export function rangeConsistency(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesConsistency(rows, teamKey));
}

export function madMeanConsistency(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesConsistency(rows, teamKey));
}

export function packConsistency(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesConsistency(rows, teamKey);
  return {
    id: "consistency" as LovatMetricId,
    label: "Consistency",
    series,
    stat: wideStat(series),
    shrink: shrinkConsistency(rows, teamKey, fieldRows),
    forecast: forecastConsistency(rows, teamKey),
    fit: fitConsistency(rows, teamKey),
    control: controlConsistency(rows, teamKey),
    slots: slotsConsistency(rows, teamKey),
  };
}

export function describeConsistency(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Consistency · Needs setup`;
  return `Consistency · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesTipRisk(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["tipRisk" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statTipRisk(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesTipRisk(rows, teamKey));
}

export function shrinkTipRisk(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesTipRisk(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["tipRisk" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "tipRisk" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastTipRisk(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesTipRisk(rows, teamKey));
}

export function fitTipRisk(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesTipRisk(rows, teamKey));
}

export function controlTipRisk(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesTipRisk(rows, teamKey));
}

export function slotsTipRisk(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesTipRisk(rows, teamKey));
}

export function residualTipRisk(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "tipRisk" as LovatMetricId, oppKeys);
}

export function histTipRisk(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesTipRisk(rows, teamKey));
}

export function cvTipRisk(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesTipRisk(rows, teamKey));
}

export function trimTipRisk(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesTipRisk(rows, teamKey));
}

export function ewmaTipRisk(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesTipRisk(rows, teamKey));
}

export function jackTipRisk(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesTipRisk(rows, teamKey));
}

export function bootTipRisk(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesTipRisk(rows, teamKey));
}

export function hazardTipRisk(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesTipRisk(rows, teamKey));
}

export function streakTipRisk(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesTipRisk(rows, teamKey));
}

export function giniTipRisk(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesTipRisk(rows, teamKey));
}

export function theilTipRisk(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesTipRisk(rows, teamKey));
}

export function skewTipRisk(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesTipRisk(rows, teamKey));
}

export function rangeTipRisk(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesTipRisk(rows, teamKey));
}

export function madMeanTipRisk(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesTipRisk(rows, teamKey));
}

export function packTipRisk(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesTipRisk(rows, teamKey);
  return {
    id: "tipRisk" as LovatMetricId,
    label: "Tip risk",
    series,
    stat: wideStat(series),
    shrink: shrinkTipRisk(rows, teamKey, fieldRows),
    forecast: forecastTipRisk(rows, teamKey),
    fit: fitTipRisk(rows, teamKey),
    control: controlTipRisk(rows, teamKey),
    slots: slotsTipRisk(rows, teamKey),
  };
}

export function describeTipRisk(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Tip risk · Needs setup`;
  return `Tip risk · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}
...
export function rollUptime2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 2);
}

export function rollUptime2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime2(rows, teamKey);
  const last = series[series.length - 1];
...
export function rollUptime2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime2Last(rows, teamKey) != null;
}

export function describeRollUptime2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime2Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 2: Needs setup";
  return "Uptime" + " last 2: " + formatValue(last, 1);
}

export function rollUptime3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 3);
}

export function rollUptime3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime3Last(rows, teamKey) != null;
}

export function describeRollUptime3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime3Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 3: Needs setup";
  return "Uptime" + " last 3: " + formatValue(last, 1);
}

export function rollUptime4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 4);
}

export function rollUptime4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime4Last(rows, teamKey) != null;
}

export function describeRollUptime4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime4Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 4: Needs setup";
  return "Uptime" + " last 4: " + formatValue(last, 1);
}

export function rollUptime5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 5);
}

export function rollUptime5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime5Last(rows, teamKey) != null;
}

export function describeRollUptime5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime5Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 5: Needs setup";
  return "Uptime" + " last 5: " + formatValue(last, 1);
}

export function rollUptime6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 6);
}

export function rollUptime6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime6Last(rows, teamKey) != null;
}

export function describeRollUptime6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime6Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 6: Needs setup";
  return "Uptime" + " last 6: " + formatValue(last, 1);
}

export function rollUptime7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 7);
}

export function rollUptime7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime7Last(rows, teamKey) != null;
}

export function describeRollUptime7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime7Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 7: Needs setup";
  return "Uptime" + " last 7: " + formatValue(last, 1);
}

export function rollUptime8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 8);
}

export function rollUptime8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime8Last(rows, teamKey) != null;
}

export function describeRollUptime8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime8Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 8: Needs setup";
  return "Uptime" + " last 8: " + formatValue(last, 1);
}

export function rollUptime9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 9);
}

export function rollUptime9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime9Last(rows, teamKey) != null;
}

export function describeRollUptime9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime9Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 9: Needs setup";
  return "Uptime" + " last 9: " + formatValue(last, 1);
}

export function rollUptime10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 10);
}

export function rollUptime10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime10Last(rows, teamKey) != null;
}

export function describeRollUptime10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime10Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 10: Needs setup";
  return "Uptime" + " last 10: " + formatValue(last, 1);
}

export function rollUptime12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 12);
}

export function rollUptime12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime12Last(rows, teamKey) != null;
}

export function describeRollUptime12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime12Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 12: Needs setup";
  return "Uptime" + " last 12: " + formatValue(last, 1);
}

export function rollUptime14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 14);
}

export function rollUptime14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime14Last(rows, teamKey) != null;
}

export function describeRollUptime14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime14Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 14: Needs setup";
  return "Uptime" + " last 14: " + formatValue(last, 1);
}

export function rollUptime16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 16);
}

export function rollUptime16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime16Last(rows, teamKey) != null;
}

export function describeRollUptime16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime16Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 16: Needs setup";
  return "Uptime" + " last 16: " + formatValue(last, 1);
}

export function rollUptime18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 18);
}

export function rollUptime18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime18Last(rows, teamKey) != null;
}

export function describeRollUptime18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime18Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 18: Needs setup";
  return "Uptime" + " last 18: " + formatValue(last, 1);
}

export function rollUptime20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 20);
}

export function rollUptime20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime20Last(rows, teamKey) != null;
}

export function describeRollUptime20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime20Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 20: Needs setup";
  return "Uptime" + " last 20: " + formatValue(last, 1);
}

export function rollUptime22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 22);
}

export function rollUptime22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime22Last(rows, teamKey) != null;
}

export function describeRollUptime22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime22Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 22: Needs setup";
  return "Uptime" + " last 22: " + formatValue(last, 1);
}

export function rollUptime24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 24);
}

export function rollUptime24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime24Last(rows, teamKey) != null;
}

export function describeRollUptime24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime24Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 24: Needs setup";
  return "Uptime" + " last 24: " + formatValue(last, 1);
}

export function rollUptime28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 28);
}

export function rollUptime28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime28Last(rows, teamKey) != null;
}

export function describeRollUptime28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime28Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 28: Needs setup";
  return "Uptime" + " last 28: " + formatValue(last, 1);
}

export function rollUptime32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesUptime(rows, teamKey), 32);
}

export function rollUptime32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollUptime32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollUptime32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollUptime32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollUptime32Last(rows, teamKey) != null;
}

export function describeRollUptime32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollUptime32Last(rows, teamKey);
  if (last == null) return "Uptime" + " last 32: Needs setup";
  return "Uptime" + " last 32: " + formatValue(last, 1);
}

export function rollDisabled2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 2);
}

export function rollDisabled2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled2Last(rows, teamKey) != null;
}

export function describeRollDisabled2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled2Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 2: Needs setup";
  return "Disabled time" + " last 2: " + formatValue(last, 1);
}

export function rollDisabled3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 3);
}

export function rollDisabled3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled3Last(rows, teamKey) != null;
}

export function describeRollDisabled3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled3Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 3: Needs setup";
  return "Disabled time" + " last 3: " + formatValue(last, 1);
}

export function rollDisabled4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 4);
}

export function rollDisabled4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled4Last(rows, teamKey) != null;
}

export function describeRollDisabled4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled4Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 4: Needs setup";
  return "Disabled time" + " last 4: " + formatValue(last, 1);
}

export function rollDisabled5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 5);
}

export function rollDisabled5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled5Last(rows, teamKey) != null;
}

export function describeRollDisabled5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled5Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 5: Needs setup";
  return "Disabled time" + " last 5: " + formatValue(last, 1);
}

export function rollDisabled6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 6);
}

export function rollDisabled6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled6Last(rows, teamKey) != null;
}

export function describeRollDisabled6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled6Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 6: Needs setup";
  return "Disabled time" + " last 6: " + formatValue(last, 1);
}

export function rollDisabled7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 7);
}

export function rollDisabled7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled7Last(rows, teamKey) != null;
}

export function describeRollDisabled7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled7Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 7: Needs setup";
  return "Disabled time" + " last 7: " + formatValue(last, 1);
}

export function rollDisabled8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 8);
}

export function rollDisabled8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled8Last(rows, teamKey) != null;
}

export function describeRollDisabled8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled8Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 8: Needs setup";
  return "Disabled time" + " last 8: " + formatValue(last, 1);
}

export function rollDisabled9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 9);
}

export function rollDisabled9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled9Last(rows, teamKey) != null;
}

export function describeRollDisabled9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled9Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 9: Needs setup";
  return "Disabled time" + " last 9: " + formatValue(last, 1);
}

export function rollDisabled10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 10);
}

export function rollDisabled10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled10Last(rows, teamKey) != null;
}

export function describeRollDisabled10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled10Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 10: Needs setup";
  return "Disabled time" + " last 10: " + formatValue(last, 1);
}

export function rollDisabled12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 12);
}

export function rollDisabled12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled12Last(rows, teamKey) != null;
}

export function describeRollDisabled12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled12Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 12: Needs setup";
  return "Disabled time" + " last 12: " + formatValue(last, 1);
}

export function rollDisabled14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 14);
}

export function rollDisabled14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled14Last(rows, teamKey) != null;
}

export function describeRollDisabled14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled14Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 14: Needs setup";
  return "Disabled time" + " last 14: " + formatValue(last, 1);
}

export function rollDisabled16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 16);
}

export function rollDisabled16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled16Last(rows, teamKey) != null;
}

export function describeRollDisabled16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled16Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 16: Needs setup";
  return "Disabled time" + " last 16: " + formatValue(last, 1);
}

export function rollDisabled18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 18);
}

export function rollDisabled18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled18Last(rows, teamKey) != null;
}

export function describeRollDisabled18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled18Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 18: Needs setup";
  return "Disabled time" + " last 18: " + formatValue(last, 1);
}

export function rollDisabled20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 20);
}

export function rollDisabled20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled20Last(rows, teamKey) != null;
}

export function describeRollDisabled20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled20Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 20: Needs setup";
  return "Disabled time" + " last 20: " + formatValue(last, 1);
}

export function rollDisabled22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 22);
}

export function rollDisabled22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled22Last(rows, teamKey) != null;
}

export function describeRollDisabled22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled22Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 22: Needs setup";
  return "Disabled time" + " last 22: " + formatValue(last, 1);
}

export function rollDisabled24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 24);
}

export function rollDisabled24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled24Last(rows, teamKey) != null;
}

export function describeRollDisabled24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled24Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 24: Needs setup";
  return "Disabled time" + " last 24: " + formatValue(last, 1);
}

export function rollDisabled28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 28);
}

export function rollDisabled28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled28Last(rows, teamKey) != null;
}

export function describeRollDisabled28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled28Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 28: Needs setup";
  return "Disabled time" + " last 28: " + formatValue(last, 1);
}

export function rollDisabled32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDisabled(rows, teamKey), 32);
}

export function rollDisabled32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDisabled32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDisabled32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDisabled32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDisabled32Last(rows, teamKey) != null;
}

export function describeRollDisabled32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDisabled32Last(rows, teamKey);
  if (last == null) return "Disabled time" + " last 32: Needs setup";
  return "Disabled time" + " last 32: " + formatValue(last, 1);
}

export function rollConsistency2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 2);
}

export function rollConsistency2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency2Last(rows, teamKey) != null;
}

export function describeRollConsistency2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency2Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 2: Needs setup";
  return "Consistency" + " last 2: " + formatValue(last, 1);
}

export function rollConsistency3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 3);
}

export function rollConsistency3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency3Last(rows, teamKey) != null;
}

export function describeRollConsistency3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency3Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 3: Needs setup";
  return "Consistency" + " last 3: " + formatValue(last, 1);
}

export function rollConsistency4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 4);
}

export function rollConsistency4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency4Last(rows, teamKey) != null;
}

export function describeRollConsistency4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency4Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 4: Needs setup";
  return "Consistency" + " last 4: " + formatValue(last, 1);
}

export function rollConsistency5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 5);
}

export function rollConsistency5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency5Last(rows, teamKey) != null;
}

export function describeRollConsistency5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency5Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 5: Needs setup";
  return "Consistency" + " last 5: " + formatValue(last, 1);
}

export function rollConsistency6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 6);
}

export function rollConsistency6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency6Last(rows, teamKey) != null;
}

export function describeRollConsistency6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency6Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 6: Needs setup";
  return "Consistency" + " last 6: " + formatValue(last, 1);
}

export function rollConsistency7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 7);
}

export function rollConsistency7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency7Last(rows, teamKey) != null;
}

export function describeRollConsistency7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency7Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 7: Needs setup";
  return "Consistency" + " last 7: " + formatValue(last, 1);
}

export function rollConsistency8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 8);
}

export function rollConsistency8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency8Last(rows, teamKey) != null;
}

export function describeRollConsistency8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency8Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 8: Needs setup";
  return "Consistency" + " last 8: " + formatValue(last, 1);
}

export function rollConsistency9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 9);
}

export function rollConsistency9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency9Last(rows, teamKey) != null;
}

export function describeRollConsistency9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency9Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 9: Needs setup";
  return "Consistency" + " last 9: " + formatValue(last, 1);
}

export function rollConsistency10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 10);
}

export function rollConsistency10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency10Last(rows, teamKey) != null;
}

export function describeRollConsistency10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency10Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 10: Needs setup";
  return "Consistency" + " last 10: " + formatValue(last, 1);
}

export function rollConsistency12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 12);
}

export function rollConsistency12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency12Last(rows, teamKey) != null;
}

export function describeRollConsistency12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency12Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 12: Needs setup";
  return "Consistency" + " last 12: " + formatValue(last, 1);
}

export function rollConsistency14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 14);
}

export function rollConsistency14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency14Last(rows, teamKey) != null;
}

export function describeRollConsistency14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency14Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 14: Needs setup";
  return "Consistency" + " last 14: " + formatValue(last, 1);
}

export function rollConsistency16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 16);
}

export function rollConsistency16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency16Last(rows, teamKey) != null;
}

export function describeRollConsistency16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency16Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 16: Needs setup";
  return "Consistency" + " last 16: " + formatValue(last, 1);
}

export function rollConsistency18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 18);
}

export function rollConsistency18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency18Last(rows, teamKey) != null;
}

export function describeRollConsistency18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency18Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 18: Needs setup";
  return "Consistency" + " last 18: " + formatValue(last, 1);
}

export function rollConsistency20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 20);
}

export function rollConsistency20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency20Last(rows, teamKey) != null;
}

export function describeRollConsistency20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency20Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 20: Needs setup";
  return "Consistency" + " last 20: " + formatValue(last, 1);
}

export function rollConsistency22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 22);
}

export function rollConsistency22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency22Last(rows, teamKey) != null;
}

export function describeRollConsistency22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency22Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 22: Needs setup";
  return "Consistency" + " last 22: " + formatValue(last, 1);
}

export function rollConsistency24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 24);
}

export function rollConsistency24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency24Last(rows, teamKey) != null;
}

export function describeRollConsistency24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency24Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 24: Needs setup";
  return "Consistency" + " last 24: " + formatValue(last, 1);
}

export function rollConsistency28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 28);
}

export function rollConsistency28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency28Last(rows, teamKey) != null;
}

export function describeRollConsistency28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency28Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 28: Needs setup";
  return "Consistency" + " last 28: " + formatValue(last, 1);
}

export function rollConsistency32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesConsistency(rows, teamKey), 32);
}

export function rollConsistency32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollConsistency32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollConsistency32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollConsistency32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollConsistency32Last(rows, teamKey) != null;
}

export function describeRollConsistency32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollConsistency32Last(rows, teamKey);
  if (last == null) return "Consistency" + " last 32: Needs setup";
  return "Consistency" + " last 32: " + formatValue(last, 1);
}

export function rollTipRisk2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 2);
}

export function rollTipRisk2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk2Last(rows, teamKey) != null;
}

export function describeRollTipRisk2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk2Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 2: Needs setup";
  return "Tip risk" + " last 2: " + formatValue(last, 1);
}

export function rollTipRisk3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 3);
}

export function rollTipRisk3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk3Last(rows, teamKey) != null;
}

export function describeRollTipRisk3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk3Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 3: Needs setup";
  return "Tip risk" + " last 3: " + formatValue(last, 1);
}

export function rollTipRisk4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 4);
}

export function rollTipRisk4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk4Last(rows, teamKey) != null;
}

export function describeRollTipRisk4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk4Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 4: Needs setup";
  return "Tip risk" + " last 4: " + formatValue(last, 1);
}

export function rollTipRisk5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 5);
}

export function rollTipRisk5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk5Last(rows, teamKey) != null;
}

export function describeRollTipRisk5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk5Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 5: Needs setup";
  return "Tip risk" + " last 5: " + formatValue(last, 1);
}

export function rollTipRisk6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 6);
}

export function rollTipRisk6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk6Last(rows, teamKey) != null;
}

export function describeRollTipRisk6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk6Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 6: Needs setup";
  return "Tip risk" + " last 6: " + formatValue(last, 1);
}

export function rollTipRisk7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 7);
}

export function rollTipRisk7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk7Last(rows, teamKey) != null;
}

export function describeRollTipRisk7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk7Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 7: Needs setup";
  return "Tip risk" + " last 7: " + formatValue(last, 1);
}

export function rollTipRisk8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 8);
}

export function rollTipRisk8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk8Last(rows, teamKey) != null;
}

export function describeRollTipRisk8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk8Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 8: Needs setup";
  return "Tip risk" + " last 8: " + formatValue(last, 1);
}

export function rollTipRisk9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 9);
}

export function rollTipRisk9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk9Last(rows, teamKey) != null;
}

export function describeRollTipRisk9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk9Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 9: Needs setup";
  return "Tip risk" + " last 9: " + formatValue(last, 1);
}

export function rollTipRisk10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 10);
}

export function rollTipRisk10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk10Last(rows, teamKey) != null;
}

export function describeRollTipRisk10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk10Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 10: Needs setup";
  return "Tip risk" + " last 10: " + formatValue(last, 1);
}

export function rollTipRisk12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 12);
}

export function rollTipRisk12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk12Last(rows, teamKey) != null;
}

export function describeRollTipRisk12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk12Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 12: Needs setup";
  return "Tip risk" + " last 12: " + formatValue(last, 1);
}

export function rollTipRisk14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 14);
}

export function rollTipRisk14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk14Last(rows, teamKey) != null;
}

export function describeRollTipRisk14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk14Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 14: Needs setup";
  return "Tip risk" + " last 14: " + formatValue(last, 1);
}

export function rollTipRisk16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 16);
}

export function rollTipRisk16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk16Last(rows, teamKey) != null;
}

export function describeRollTipRisk16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk16Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 16: Needs setup";
  return "Tip risk" + " last 16: " + formatValue(last, 1);
}

export function rollTipRisk18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 18);
}

export function rollTipRisk18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk18Last(rows, teamKey) != null;
}

export function describeRollTipRisk18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk18Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 18: Needs setup";
  return "Tip risk" + " last 18: " + formatValue(last, 1);
}

export function rollTipRisk20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 20);
}

export function rollTipRisk20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk20Last(rows, teamKey) != null;
}

export function describeRollTipRisk20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk20Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 20: Needs setup";
  return "Tip risk" + " last 20: " + formatValue(last, 1);
}

export function rollTipRisk22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 22);
}

export function rollTipRisk22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk22Last(rows, teamKey) != null;
}

export function describeRollTipRisk22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk22Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 22: Needs setup";
  return "Tip risk" + " last 22: " + formatValue(last, 1);
}

export function rollTipRisk24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 24);
}

export function rollTipRisk24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk24Last(rows, teamKey) != null;
}

export function describeRollTipRisk24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk24Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 24: Needs setup";
  return "Tip risk" + " last 24: " + formatValue(last, 1);
}

export function rollTipRisk28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 28);
}

export function rollTipRisk28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk28Last(rows, teamKey) != null;
}

export function describeRollTipRisk28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk28Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 28: Needs setup";
  return "Tip risk" + " last 28: " + formatValue(last, 1);
}

export function rollTipRisk32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTipRisk(rows, teamKey), 32);
}

export function rollTipRisk32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTipRisk32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTipRisk32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTipRisk32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTipRisk32Last(rows, teamKey) != null;
}

export function describeRollTipRisk32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTipRisk32Last(rows, teamKey);
  if (last == null) return "Tip risk" + " last 32: Needs setup";
  return "Tip risk" + " last 32: " + formatValue(last, 1);
}
...
export function buildWideReport(input: { teamKey: string; rows: readonly SampleRow[]; fieldRows?: readonly SampleRow[] }): WideReport {
  const packs: WideMetricPack[] = [
    packUptime(input.rows, input.teamKey, input.fieldRows),
    packDisabled(input.rows, input.teamKey, input.fieldRows),
    packConsistency(input.rows, input.teamKey, input.fieldRows),
    packTipRisk(input.rows, input.teamKey, input.fieldRows)
  ];
  const lead = packs[0];
