/**
 * Leftover agent kit — reliability-quals-cadhop.
...
/**
 * Lovat-style Reliability board — qualification matches, CAD hop persist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "reliability-quals-cadhop";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 568295414): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
