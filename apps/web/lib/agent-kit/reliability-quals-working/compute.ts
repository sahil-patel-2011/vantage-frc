/**
 * Leftover agent kit — reliability-quals-working.
...
/**
 * Lovat-style Reliability board — qualification matches, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "reliability-quals-working";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3145019860): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
