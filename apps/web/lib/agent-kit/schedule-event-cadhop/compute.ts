/**
 * Leftover agent kit — schedule-event-cadhop.
...
/**
 * Lovat-style Schedule board — this event, CAD hop persist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "schedule-event-cadhop";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2082823751): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
