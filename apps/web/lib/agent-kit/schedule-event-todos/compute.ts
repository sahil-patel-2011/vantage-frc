/**
 * Leftover agent kit — schedule-event-todos.
...
/**
 * Lovat-style Schedule board — this event, working todos.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "schedule-event-todos";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2404842349): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
