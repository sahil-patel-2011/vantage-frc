/**
 * Leftover agent kit — schedule-last12-autonomous.
...
/**
 * Lovat-style Schedule board — last 12 matches, autonomous loop.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "schedule-last12-autonomous";
export const PHASE_LABEL = "Schedule";
...
export type LovatMetricId = "oppRating" | "partnerRating" | "winChance" | "margin";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1293636339): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
...
export function seriesOppRating(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["oppRating" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statOppRating(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesOppRating(rows, teamKey));
}

export function shrinkOppRating(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesOppRating(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["oppRating" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "oppRating" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastOppRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesOppRating(rows, teamKey));
}

export function fitOppRating(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesOppRating(rows, teamKey));
}

export function controlOppRating(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesOppRating(rows, teamKey));
}

export function slotsOppRating(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesOppRating(rows, teamKey));
}

export function residualOppRating(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "oppRating" as LovatMetricId, oppKeys);
}

export function histOppRating(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesOppRating(rows, teamKey));
}

export function cvOppRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesOppRating(rows, teamKey));
}

export function trimOppRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesOppRating(rows, teamKey));
}

export function ewmaOppRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesOppRating(rows, teamKey));
}

export function jackOppRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesOppRating(rows, teamKey));
}

export function bootOppRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesOppRating(rows, teamKey));
}

export function hazardOppRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesOppRating(rows, teamKey));
}

export function streakOppRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesOppRating(rows, teamKey));
}

export function giniOppRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesOppRating(rows, teamKey));
}

export function theilOppRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesOppRating(rows, teamKey));
}

export function skewOppRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesOppRating(rows, teamKey));
}

export function rangeOppRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesOppRating(rows, teamKey));
}

export function madMeanOppRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesOppRating(rows, teamKey));
}

export function packOppRating(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesOppRating(rows, teamKey);
  return {
    id: "oppRating" as LovatMetricId,
    label: "Opponent rating",
    series,
    stat: wideStat(series),
    shrink: shrinkOppRating(rows, teamKey, fieldRows),
    forecast: forecastOppRating(rows, teamKey),
    fit: fitOppRating(rows, teamKey),
    control: controlOppRating(rows, teamKey),
    slots: slotsOppRating(rows, teamKey),
  };
}

export function describeOppRating(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Opponent rating · Needs setup`;
  return `Opponent rating · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesPartnerRating(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["partnerRating" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statPartnerRating(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesPartnerRating(rows, teamKey));
}

export function shrinkPartnerRating(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesPartnerRating(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["partnerRating" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "partnerRating" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastPartnerRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesPartnerRating(rows, teamKey));
}

export function fitPartnerRating(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesPartnerRating(rows, teamKey));
}

export function controlPartnerRating(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesPartnerRating(rows, teamKey));
}

export function slotsPartnerRating(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesPartnerRating(rows, teamKey));
}

export function residualPartnerRating(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "partnerRating" as LovatMetricId, oppKeys);
}

export function histPartnerRating(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesPartnerRating(rows, teamKey));
}

export function cvPartnerRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesPartnerRating(rows, teamKey));
}

export function trimPartnerRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesPartnerRating(rows, teamKey));
}

export function ewmaPartnerRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesPartnerRating(rows, teamKey));
}

export function jackPartnerRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesPartnerRating(rows, teamKey));
}

export function bootPartnerRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesPartnerRating(rows, teamKey));
}

export function hazardPartnerRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesPartnerRating(rows, teamKey));
}

export function streakPartnerRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesPartnerRating(rows, teamKey));
}

export function giniPartnerRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesPartnerRating(rows, teamKey));
}

export function theilPartnerRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesPartnerRating(rows, teamKey));
}

export function skewPartnerRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesPartnerRating(rows, teamKey));
}

export function rangePartnerRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesPartnerRating(rows, teamKey));
}

export function madMeanPartnerRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesPartnerRating(rows, teamKey));
}

export function packPartnerRating(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesPartnerRating(rows, teamKey);
  return {
    id: "partnerRating" as LovatMetricId,
    label: "Partner rating",
    series,
    stat: wideStat(series),
    shrink: shrinkPartnerRating(rows, teamKey, fieldRows),
    forecast: forecastPartnerRating(rows, teamKey),
    fit: fitPartnerRating(rows, teamKey),
    control: controlPartnerRating(rows, teamKey),
    slots: slotsPartnerRating(rows, teamKey),
  };
}

export function describePartnerRating(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Partner rating · Needs setup`;
  return `Partner rating · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesWinChance(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["winChance" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statWinChance(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesWinChance(rows, teamKey));
}

export function shrinkWinChance(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesWinChance(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["winChance" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "winChance" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastWinChance(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesWinChance(rows, teamKey));
}

export function fitWinChance(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesWinChance(rows, teamKey));
}

export function controlWinChance(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesWinChance(rows, teamKey));
}

export function slotsWinChance(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesWinChance(rows, teamKey));
}

export function residualWinChance(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "winChance" as LovatMetricId, oppKeys);
}

export function histWinChance(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesWinChance(rows, teamKey));
}

export function cvWinChance(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesWinChance(rows, teamKey));
}

export function trimWinChance(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesWinChance(rows, teamKey));
}

export function ewmaWinChance(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesWinChance(rows, teamKey));
}

export function jackWinChance(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesWinChance(rows, teamKey));
}

export function bootWinChance(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesWinChance(rows, teamKey));
}

export function hazardWinChance(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesWinChance(rows, teamKey));
}

export function streakWinChance(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesWinChance(rows, teamKey));
}

export function giniWinChance(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesWinChance(rows, teamKey));
}

export function theilWinChance(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesWinChance(rows, teamKey));
}

export function skewWinChance(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesWinChance(rows, teamKey));
}

export function rangeWinChance(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesWinChance(rows, teamKey));
}

export function madMeanWinChance(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesWinChance(rows, teamKey));
}

export function packWinChance(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesWinChance(rows, teamKey);
  return {
    id: "winChance" as LovatMetricId,
    label: "Win chance",
    series,
    stat: wideStat(series),
    shrink: shrinkWinChance(rows, teamKey, fieldRows),
    forecast: forecastWinChance(rows, teamKey),
    fit: fitWinChance(rows, teamKey),
    control: controlWinChance(rows, teamKey),
    slots: slotsWinChance(rows, teamKey),
  };
}

export function describeWinChance(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Win chance · Needs setup`;
  return `Win chance · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesMargin(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["margin" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statMargin(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesMargin(rows, teamKey));
}

export function shrinkMargin(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesMargin(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["margin" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "margin" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastMargin(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesMargin(rows, teamKey));
}

export function fitMargin(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesMargin(rows, teamKey));
}

export function controlMargin(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesMargin(rows, teamKey));
}

export function slotsMargin(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesMargin(rows, teamKey));
}

export function residualMargin(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "margin" as LovatMetricId, oppKeys);
}

export function histMargin(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesMargin(rows, teamKey));
}

export function cvMargin(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesMargin(rows, teamKey));
}

export function trimMargin(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesMargin(rows, teamKey));
}

export function ewmaMargin(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesMargin(rows, teamKey));
}

export function jackMargin(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesMargin(rows, teamKey));
}

export function bootMargin(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesMargin(rows, teamKey));
}

export function hazardMargin(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesMargin(rows, teamKey));
}

export function streakMargin(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesMargin(rows, teamKey));
}

export function giniMargin(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesMargin(rows, teamKey));
}

export function theilMargin(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesMargin(rows, teamKey));
}

export function skewMargin(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesMargin(rows, teamKey));
}

export function rangeMargin(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesMargin(rows, teamKey));
}

export function madMeanMargin(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesMargin(rows, teamKey));
}

export function packMargin(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesMargin(rows, teamKey);
  return {
    id: "margin" as LovatMetricId,
    label: "Predicted margin",
    series,
    stat: wideStat(series),
    shrink: shrinkMargin(rows, teamKey, fieldRows),
    forecast: forecastMargin(rows, teamKey),
    fit: fitMargin(rows, teamKey),
    control: controlMargin(rows, teamKey),
    slots: slotsMargin(rows, teamKey),
  };
}

export function describeMargin(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Predicted margin · Needs setup`;
  return `Predicted margin · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}
...
export function rollOppRating2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 2);
}

export function rollOppRating2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating2(rows, teamKey);
  const last = series[series.length - 1];
...
export function rollOppRating2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating2Last(rows, teamKey) != null;
}

export function describeRollOppRating2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating2Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 2: Needs setup";
  return "Opponent rating" + " last 2: " + formatValue(last, 1);
}

export function rollOppRating3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 3);
}

export function rollOppRating3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating3Last(rows, teamKey) != null;
}

export function describeRollOppRating3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating3Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 3: Needs setup";
  return "Opponent rating" + " last 3: " + formatValue(last, 1);
}

export function rollOppRating4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 4);
}

export function rollOppRating4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating4Last(rows, teamKey) != null;
}

export function describeRollOppRating4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating4Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 4: Needs setup";
  return "Opponent rating" + " last 4: " + formatValue(last, 1);
}

export function rollOppRating5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 5);
}

export function rollOppRating5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating5Last(rows, teamKey) != null;
}

export function describeRollOppRating5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating5Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 5: Needs setup";
  return "Opponent rating" + " last 5: " + formatValue(last, 1);
}

export function rollOppRating6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 6);
}

export function rollOppRating6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating6Last(rows, teamKey) != null;
}

export function describeRollOppRating6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating6Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 6: Needs setup";
  return "Opponent rating" + " last 6: " + formatValue(last, 1);
}

export function rollOppRating7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 7);
}

export function rollOppRating7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating7Last(rows, teamKey) != null;
}

export function describeRollOppRating7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating7Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 7: Needs setup";
  return "Opponent rating" + " last 7: " + formatValue(last, 1);
}

export function rollOppRating8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 8);
}

export function rollOppRating8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating8Last(rows, teamKey) != null;
}

export function describeRollOppRating8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating8Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 8: Needs setup";
  return "Opponent rating" + " last 8: " + formatValue(last, 1);
}

export function rollOppRating9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 9);
}

export function rollOppRating9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating9Last(rows, teamKey) != null;
}

export function describeRollOppRating9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating9Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 9: Needs setup";
  return "Opponent rating" + " last 9: " + formatValue(last, 1);
}

export function rollOppRating10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 10);
}

export function rollOppRating10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating10Last(rows, teamKey) != null;
}

export function describeRollOppRating10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating10Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 10: Needs setup";
  return "Opponent rating" + " last 10: " + formatValue(last, 1);
}

export function rollOppRating12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 12);
}

export function rollOppRating12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating12Last(rows, teamKey) != null;
}

export function describeRollOppRating12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating12Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 12: Needs setup";
  return "Opponent rating" + " last 12: " + formatValue(last, 1);
}

export function rollOppRating14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 14);
}

export function rollOppRating14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating14Last(rows, teamKey) != null;
}

export function describeRollOppRating14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating14Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 14: Needs setup";
  return "Opponent rating" + " last 14: " + formatValue(last, 1);
}

export function rollOppRating16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 16);
}

export function rollOppRating16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating16Last(rows, teamKey) != null;
}

export function describeRollOppRating16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating16Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 16: Needs setup";
  return "Opponent rating" + " last 16: " + formatValue(last, 1);
}

export function rollOppRating18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 18);
}

export function rollOppRating18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating18Last(rows, teamKey) != null;
}

export function describeRollOppRating18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating18Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 18: Needs setup";
  return "Opponent rating" + " last 18: " + formatValue(last, 1);
}

export function rollOppRating20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 20);
}

export function rollOppRating20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating20Last(rows, teamKey) != null;
}

export function describeRollOppRating20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating20Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 20: Needs setup";
  return "Opponent rating" + " last 20: " + formatValue(last, 1);
}

export function rollOppRating22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 22);
}

export function rollOppRating22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating22Last(rows, teamKey) != null;
}

export function describeRollOppRating22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating22Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 22: Needs setup";
  return "Opponent rating" + " last 22: " + formatValue(last, 1);
}

export function rollOppRating24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 24);
}

export function rollOppRating24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating24Last(rows, teamKey) != null;
}

export function describeRollOppRating24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating24Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 24: Needs setup";
  return "Opponent rating" + " last 24: " + formatValue(last, 1);
}

export function rollOppRating28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 28);
}

export function rollOppRating28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating28Last(rows, teamKey) != null;
}

export function describeRollOppRating28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating28Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 28: Needs setup";
  return "Opponent rating" + " last 28: " + formatValue(last, 1);
}

export function rollOppRating32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesOppRating(rows, teamKey), 32);
}

export function rollOppRating32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollOppRating32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollOppRating32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollOppRating32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollOppRating32Last(rows, teamKey) != null;
}

export function describeRollOppRating32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollOppRating32Last(rows, teamKey);
  if (last == null) return "Opponent rating" + " last 32: Needs setup";
  return "Opponent rating" + " last 32: " + formatValue(last, 1);
}

export function rollPartnerRating2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 2);
}

export function rollPartnerRating2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating2Last(rows, teamKey) != null;
}

export function describeRollPartnerRating2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating2Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 2: Needs setup";
  return "Partner rating" + " last 2: " + formatValue(last, 1);
}

export function rollPartnerRating3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 3);
}

export function rollPartnerRating3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating3Last(rows, teamKey) != null;
}

export function describeRollPartnerRating3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating3Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 3: Needs setup";
  return "Partner rating" + " last 3: " + formatValue(last, 1);
}

export function rollPartnerRating4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 4);
}

export function rollPartnerRating4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating4Last(rows, teamKey) != null;
}

export function describeRollPartnerRating4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating4Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 4: Needs setup";
  return "Partner rating" + " last 4: " + formatValue(last, 1);
}

export function rollPartnerRating5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 5);
}

export function rollPartnerRating5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating5Last(rows, teamKey) != null;
}

export function describeRollPartnerRating5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating5Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 5: Needs setup";
  return "Partner rating" + " last 5: " + formatValue(last, 1);
}

export function rollPartnerRating6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 6);
}

export function rollPartnerRating6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating6Last(rows, teamKey) != null;
}

export function describeRollPartnerRating6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating6Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 6: Needs setup";
  return "Partner rating" + " last 6: " + formatValue(last, 1);
}

export function rollPartnerRating7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 7);
}

export function rollPartnerRating7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating7Last(rows, teamKey) != null;
}

export function describeRollPartnerRating7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating7Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 7: Needs setup";
  return "Partner rating" + " last 7: " + formatValue(last, 1);
}

export function rollPartnerRating8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 8);
}

export function rollPartnerRating8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating8Last(rows, teamKey) != null;
}

export function describeRollPartnerRating8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating8Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 8: Needs setup";
  return "Partner rating" + " last 8: " + formatValue(last, 1);
}

export function rollPartnerRating9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 9);
}

export function rollPartnerRating9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating9Last(rows, teamKey) != null;
}

export function describeRollPartnerRating9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating9Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 9: Needs setup";
  return "Partner rating" + " last 9: " + formatValue(last, 1);
}

export function rollPartnerRating10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 10);
}

export function rollPartnerRating10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating10Last(rows, teamKey) != null;
}

export function describeRollPartnerRating10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating10Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 10: Needs setup";
  return "Partner rating" + " last 10: " + formatValue(last, 1);
}

export function rollPartnerRating12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 12);
}

export function rollPartnerRating12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating12Last(rows, teamKey) != null;
}

export function describeRollPartnerRating12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating12Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 12: Needs setup";
  return "Partner rating" + " last 12: " + formatValue(last, 1);
}

export function rollPartnerRating14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 14);
}

export function rollPartnerRating14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating14Last(rows, teamKey) != null;
}

export function describeRollPartnerRating14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating14Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 14: Needs setup";
  return "Partner rating" + " last 14: " + formatValue(last, 1);
}

export function rollPartnerRating16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 16);
}

export function rollPartnerRating16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating16Last(rows, teamKey) != null;
}

export function describeRollPartnerRating16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating16Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 16: Needs setup";
  return "Partner rating" + " last 16: " + formatValue(last, 1);
}

export function rollPartnerRating18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 18);
}

export function rollPartnerRating18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating18Last(rows, teamKey) != null;
}

export function describeRollPartnerRating18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating18Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 18: Needs setup";
  return "Partner rating" + " last 18: " + formatValue(last, 1);
}

export function rollPartnerRating20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 20);
}

export function rollPartnerRating20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating20Last(rows, teamKey) != null;
}

export function describeRollPartnerRating20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating20Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 20: Needs setup";
  return "Partner rating" + " last 20: " + formatValue(last, 1);
}

export function rollPartnerRating22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 22);
}

export function rollPartnerRating22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating22Last(rows, teamKey) != null;
}

export function describeRollPartnerRating22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating22Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 22: Needs setup";
  return "Partner rating" + " last 22: " + formatValue(last, 1);
}

export function rollPartnerRating24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 24);
}

export function rollPartnerRating24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating24Last(rows, teamKey) != null;
}

export function describeRollPartnerRating24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating24Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 24: Needs setup";
  return "Partner rating" + " last 24: " + formatValue(last, 1);
}

export function rollPartnerRating28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 28);
}

export function rollPartnerRating28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating28Last(rows, teamKey) != null;
}

export function describeRollPartnerRating28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating28Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 28: Needs setup";
  return "Partner rating" + " last 28: " + formatValue(last, 1);
}

export function rollPartnerRating32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesPartnerRating(rows, teamKey), 32);
}

export function rollPartnerRating32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollPartnerRating32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollPartnerRating32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollPartnerRating32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollPartnerRating32Last(rows, teamKey) != null;
}

export function describeRollPartnerRating32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollPartnerRating32Last(rows, teamKey);
  if (last == null) return "Partner rating" + " last 32: Needs setup";
  return "Partner rating" + " last 32: " + formatValue(last, 1);
}

export function rollWinChance2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 2);
}

export function rollWinChance2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance2Last(rows, teamKey) != null;
}

export function describeRollWinChance2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance2Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 2: Needs setup";
  return "Win chance" + " last 2: " + formatValue(last, 1);
}

export function rollWinChance3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 3);
}

export function rollWinChance3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance3Last(rows, teamKey) != null;
}

export function describeRollWinChance3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance3Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 3: Needs setup";
  return "Win chance" + " last 3: " + formatValue(last, 1);
}

export function rollWinChance4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 4);
}

export function rollWinChance4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance4Last(rows, teamKey) != null;
}

export function describeRollWinChance4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance4Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 4: Needs setup";
  return "Win chance" + " last 4: " + formatValue(last, 1);
}

export function rollWinChance5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 5);
}

export function rollWinChance5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance5Last(rows, teamKey) != null;
}

export function describeRollWinChance5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance5Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 5: Needs setup";
  return "Win chance" + " last 5: " + formatValue(last, 1);
}

export function rollWinChance6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 6);
}

export function rollWinChance6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance6Last(rows, teamKey) != null;
}

export function describeRollWinChance6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance6Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 6: Needs setup";
  return "Win chance" + " last 6: " + formatValue(last, 1);
}

export function rollWinChance7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 7);
}

export function rollWinChance7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance7Last(rows, teamKey) != null;
}

export function describeRollWinChance7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance7Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 7: Needs setup";
  return "Win chance" + " last 7: " + formatValue(last, 1);
}

export function rollWinChance8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 8);
}

export function rollWinChance8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance8Last(rows, teamKey) != null;
}

export function describeRollWinChance8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance8Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 8: Needs setup";
  return "Win chance" + " last 8: " + formatValue(last, 1);
}

export function rollWinChance9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 9);
}

export function rollWinChance9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance9Last(rows, teamKey) != null;
}

export function describeRollWinChance9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance9Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 9: Needs setup";
  return "Win chance" + " last 9: " + formatValue(last, 1);
}

export function rollWinChance10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 10);
}

export function rollWinChance10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance10Last(rows, teamKey) != null;
}

export function describeRollWinChance10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance10Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 10: Needs setup";
  return "Win chance" + " last 10: " + formatValue(last, 1);
}

export function rollWinChance12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 12);
}

export function rollWinChance12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance12Last(rows, teamKey) != null;
}

export function describeRollWinChance12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance12Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 12: Needs setup";
  return "Win chance" + " last 12: " + formatValue(last, 1);
}

export function rollWinChance14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 14);
}

export function rollWinChance14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance14Last(rows, teamKey) != null;
}

export function describeRollWinChance14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance14Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 14: Needs setup";
  return "Win chance" + " last 14: " + formatValue(last, 1);
}

export function rollWinChance16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 16);
}

export function rollWinChance16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance16Last(rows, teamKey) != null;
}

export function describeRollWinChance16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance16Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 16: Needs setup";
  return "Win chance" + " last 16: " + formatValue(last, 1);
}

export function rollWinChance18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 18);
}

export function rollWinChance18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance18Last(rows, teamKey) != null;
}

export function describeRollWinChance18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance18Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 18: Needs setup";
  return "Win chance" + " last 18: " + formatValue(last, 1);
}

export function rollWinChance20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 20);
}

export function rollWinChance20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance20Last(rows, teamKey) != null;
}

export function describeRollWinChance20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance20Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 20: Needs setup";
  return "Win chance" + " last 20: " + formatValue(last, 1);
}

export function rollWinChance22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 22);
}

export function rollWinChance22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance22Last(rows, teamKey) != null;
}

export function describeRollWinChance22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance22Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 22: Needs setup";
  return "Win chance" + " last 22: " + formatValue(last, 1);
}

export function rollWinChance24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 24);
}

export function rollWinChance24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance24Last(rows, teamKey) != null;
}

export function describeRollWinChance24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance24Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 24: Needs setup";
  return "Win chance" + " last 24: " + formatValue(last, 1);
}

export function rollWinChance28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 28);
}

export function rollWinChance28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance28Last(rows, teamKey) != null;
}

export function describeRollWinChance28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance28Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 28: Needs setup";
  return "Win chance" + " last 28: " + formatValue(last, 1);
}

export function rollWinChance32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesWinChance(rows, teamKey), 32);
}

export function rollWinChance32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollWinChance32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollWinChance32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollWinChance32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollWinChance32Last(rows, teamKey) != null;
}

export function describeRollWinChance32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollWinChance32Last(rows, teamKey);
  if (last == null) return "Win chance" + " last 32: Needs setup";
  return "Win chance" + " last 32: " + formatValue(last, 1);
}

export function rollMargin2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 2);
}

export function rollMargin2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin2Last(rows, teamKey) != null;
}

export function describeRollMargin2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin2Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 2: Needs setup";
  return "Predicted margin" + " last 2: " + formatValue(last, 1);
}

export function rollMargin3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 3);
}

export function rollMargin3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin3Last(rows, teamKey) != null;
}

export function describeRollMargin3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin3Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 3: Needs setup";
  return "Predicted margin" + " last 3: " + formatValue(last, 1);
}

export function rollMargin4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 4);
}

export function rollMargin4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin4Last(rows, teamKey) != null;
}

export function describeRollMargin4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin4Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 4: Needs setup";
  return "Predicted margin" + " last 4: " + formatValue(last, 1);
}

export function rollMargin5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 5);
}

export function rollMargin5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin5Last(rows, teamKey) != null;
}

export function describeRollMargin5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin5Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 5: Needs setup";
  return "Predicted margin" + " last 5: " + formatValue(last, 1);
}

export function rollMargin6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 6);
}

export function rollMargin6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin6Last(rows, teamKey) != null;
}

export function describeRollMargin6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin6Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 6: Needs setup";
  return "Predicted margin" + " last 6: " + formatValue(last, 1);
}

export function rollMargin7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 7);
}

export function rollMargin7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin7Last(rows, teamKey) != null;
}

export function describeRollMargin7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin7Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 7: Needs setup";
  return "Predicted margin" + " last 7: " + formatValue(last, 1);
}

export function rollMargin8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 8);
}

export function rollMargin8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin8Last(rows, teamKey) != null;
}

export function describeRollMargin8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin8Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 8: Needs setup";
  return "Predicted margin" + " last 8: " + formatValue(last, 1);
}

export function rollMargin9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 9);
}

export function rollMargin9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin9Last(rows, teamKey) != null;
}

export function describeRollMargin9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin9Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 9: Needs setup";
  return "Predicted margin" + " last 9: " + formatValue(last, 1);
}

export function rollMargin10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 10);
}

export function rollMargin10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin10Last(rows, teamKey) != null;
}

export function describeRollMargin10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin10Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 10: Needs setup";
  return "Predicted margin" + " last 10: " + formatValue(last, 1);
}

export function rollMargin12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 12);
}

export function rollMargin12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin12Last(rows, teamKey) != null;
}

export function describeRollMargin12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin12Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 12: Needs setup";
  return "Predicted margin" + " last 12: " + formatValue(last, 1);
}

export function rollMargin14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 14);
}

export function rollMargin14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin14Last(rows, teamKey) != null;
}

export function describeRollMargin14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin14Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 14: Needs setup";
  return "Predicted margin" + " last 14: " + formatValue(last, 1);
}

export function rollMargin16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 16);
}

export function rollMargin16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin16Last(rows, teamKey) != null;
}

export function describeRollMargin16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin16Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 16: Needs setup";
  return "Predicted margin" + " last 16: " + formatValue(last, 1);
}

export function rollMargin18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 18);
}

export function rollMargin18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin18Last(rows, teamKey) != null;
}

export function describeRollMargin18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin18Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 18: Needs setup";
  return "Predicted margin" + " last 18: " + formatValue(last, 1);
}

export function rollMargin20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 20);
}

export function rollMargin20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin20Last(rows, teamKey) != null;
}

export function describeRollMargin20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin20Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 20: Needs setup";
  return "Predicted margin" + " last 20: " + formatValue(last, 1);
}

export function rollMargin22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 22);
}

export function rollMargin22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin22Last(rows, teamKey) != null;
}

export function describeRollMargin22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin22Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 22: Needs setup";
  return "Predicted margin" + " last 22: " + formatValue(last, 1);
}

export function rollMargin24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 24);
}

export function rollMargin24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin24Last(rows, teamKey) != null;
}

export function describeRollMargin24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin24Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 24: Needs setup";
  return "Predicted margin" + " last 24: " + formatValue(last, 1);
}

export function rollMargin28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 28);
}

export function rollMargin28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin28Last(rows, teamKey) != null;
}

export function describeRollMargin28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin28Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 28: Needs setup";
  return "Predicted margin" + " last 28: " + formatValue(last, 1);
}

export function rollMargin32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesMargin(rows, teamKey), 32);
}

export function rollMargin32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollMargin32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollMargin32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollMargin32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollMargin32Last(rows, teamKey) != null;
}

export function describeRollMargin32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollMargin32Last(rows, teamKey);
  if (last == null) return "Predicted margin" + " last 32: Needs setup";
  return "Predicted margin" + " last 32: " + formatValue(last, 1);
}
...
export function buildWideReport(input: { teamKey: string; rows: readonly SampleRow[]; fieldRows?: readonly SampleRow[] }): WideReport {
  const packs: WideMetricPack[] = [
    packOppRating(input.rows, input.teamKey, input.fieldRows),
    packPartnerRating(input.rows, input.teamKey, input.fieldRows),
    packWinChance(input.rows, input.teamKey, input.fieldRows),
    packMargin(input.rows, input.teamKey, input.fieldRows)
  ];
  const lead = packs[0];
