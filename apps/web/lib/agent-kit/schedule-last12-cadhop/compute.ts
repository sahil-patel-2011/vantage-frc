/**
 * Leftover agent kit — schedule-last12-cadhop.
...
/**
 * Lovat-style Schedule board — last 12 matches, CAD hop persist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "schedule-last12-cadhop";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 661386328): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
