/**
 * Leftover agent kit — schedule-last12-working.
...
/**
 * Lovat-style Schedule board — last 12 matches, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "schedule-last12-working";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2104836370): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
