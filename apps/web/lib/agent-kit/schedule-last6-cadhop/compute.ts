/**
 * Leftover agent kit — schedule-last6-cadhop.
...
/**
 * Lovat-style Schedule board — last 6 matches, CAD hop persist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "schedule-last6-cadhop";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 506406305): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
