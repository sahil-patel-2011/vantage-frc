/**
 * Leftover agent kit — schedule-last6-excerpts.
...
/**
 * Lovat-style Schedule board — last 6 matches, tool excerpts.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "schedule-last6-excerpts";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 503217616): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
