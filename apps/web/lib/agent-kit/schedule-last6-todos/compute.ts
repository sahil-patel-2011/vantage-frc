/**
 * Leftover agent kit — schedule-last6-todos.
...
/**
 * Lovat-style Schedule board — last 6 matches, working todos.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "schedule-last6-todos";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 854767519): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
