/**
 * Leftover agent kit — schedule-last6-working.
...
/**
 * Lovat-style Schedule board — last 6 matches, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "schedule-last6-working";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1660743869): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
