/**
 * Leftover agent kit — teleop-event-excerpts.
...
/**
 * Lovat-style Teleop board — this event, tool excerpts.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-event-excerpts";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2403158328): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
