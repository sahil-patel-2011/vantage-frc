/**
 * Leftover agent kit — teleop-event-todos.
...
/**
 * Lovat-style Teleop board — this event, working todos.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-event-todos";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 769242311): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
