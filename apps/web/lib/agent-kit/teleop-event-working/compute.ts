/**
 * Leftover agent kit — teleop-event-working.
...
/**
 * Lovat-style Teleop board — this event, working-memory pin.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-event-working";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2744139909): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
