/**
 * Leftover agent kit — teleop-quals-todos.
...
/**
 * Lovat-style Teleop board — qualification matches, working todos.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-quals-todos";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3744189927): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
