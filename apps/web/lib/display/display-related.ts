...
export function displaySetupNextActions(input: {
  ...
  if (boardCount === 0) {
    actions.push({
      ...
      detail: "Pick a preset below and save. Countdowns, ranks, and odds stay blank until official scores and Strategy sync.",
      href: setupHref,
      primary: true,
    });
  ...
  if (input.hasActiveEvent === false) {
    actions.push({
      ...
      detail: "Event Day picks the official schedule pit boards read for next match and coverage.",
