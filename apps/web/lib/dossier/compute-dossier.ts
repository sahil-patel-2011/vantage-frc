...
export async function computeTeamDossier(
  ...
  const steps: DossierSetupStep[] = [
    ...
    {
      id: "tba",
      label: "Sync team identity + schedule",
      detail: access.tbaConfigured
        ? "Team identity or cache available"
        : "Save a team number in Team Data, then sync.",
    ...
    },
    {
      ...
      detail: access.statbotics.cacheHasMetrics
        ? `${access.statbotics.yearMetricRows} year + ${access.statbotics.eventMetricRows} event rating rows saved`
        : "No season ratings saved yet — tap Sync under Team Data.",
      href: withOrgHref("/team/data", orgId),
