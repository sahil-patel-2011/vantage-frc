...
describe("canDeleteDriverSession", () => {
  it("lets the author and an owner or admin delete, and keeps other members out", () => {
    expect(canDeleteDriverSession({ role: "scout", userId: "noah", authorId: "noah" })).toBe(true);
    expect(canDeleteDriverSession({ role: "scout", userId: "noah", authorId: "ada" })).toBe(false);
    expect(canDeleteDriverSession({ role: "owner", userId: "ada", authorId: "noah" })).toBe(true);
    expect(canDeleteDriverSession({ role: null, userId: null, authorId: "noah" })).toBe(false);
  });
});
