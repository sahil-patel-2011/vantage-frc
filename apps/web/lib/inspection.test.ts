...
describe("INSPECTION_TEMPLATE", () => {
  it("covers the core FRC inspection areas with unique requirements", () => {
    ...
    expect(all.some((item) => /R712/i.test(item) && /coprocessor/i.test(item) && /PWM/i.test(item))).toBe(true);
    expect(all.some((item) => /R716/i.test(item) && /heartbeat/i.test(item))).toBe(true);
    ...
    expect(all.some((item) => /4\.25 in/i.test(item) && /R403/i.test(item))).toBe(true);
    ...
    expect(all.some((item) => /No painting or large labels/i.test(item))).toBe(true);
    expect(all.some((item) => /1\/8 in NPT/i.test(item) && /outputs not combined/i.test(item))).toBe(true);
    expect(all.some((item) => /Gauges/i.test(item) && /stored and working/i.test(item))).toBe(true);
    expect(all.some((item) => /RIO-port/i.test(item) && /VRM\/RPM/i.test(item))).toBe(true);
    expect(all.some((item) => /dedicated 10A/i.test(item) && /no other load/i.test(item))).toBe(true);
    expect(all.some((item) => /TU07/i.test(item) && /10A PD branch/i.test(item))).toBe(true);
