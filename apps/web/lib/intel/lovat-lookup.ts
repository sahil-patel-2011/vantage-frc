...
export type ScoutContextPhase = "auto" | "teleop" | "endgame";
...
const SCOUT_PAYLOAD_KEYS: Partial<Record<LovatLookupMetricId, readonly string[]>> = {
  driverAbility: ["driverAbility", "driver", "driver_skill"],
  autoClimb: ["autoClimb", "auto_climb"],
  defenseEffectiveness: ["defenseEffectiveness", "defense", "defense_rating"],
  contactDefenseTime: ["contactDefenseTime", "contact_defense"],
  campingDefenseTime: ["campingDefenseTime", "camping"],
  totalDefenseTime: ["totalDefenseTime", "defenseTime", "defense_time"],
  totalFuelThroughput: ["totalFuelThroughput", "throughput"],
  totalFuelFed: ["totalFuelFed", "fed"],
  feedingRate: ["feedingRate", "feed_rate"],
  scoringRate: ["scoringRate", "cycle_rate"],
  estimatedSuccessfulFuelRate: ["estimatedSuccessfulFuelRate", "success_rate"],
  estimatedTotalFuelScored: ["estimatedTotalFuelScored", "fuel_scored"],
  reliability: ["reliability"],
};

/** Event standing / overall ratings stay on every Collection phase chip. */
const EVENT_STANDING_IDS = new Set<LovatLookupMetricId>([
  "totalPoints",
  "rank",
  "wins",
  "opr",
  ...
  return LOVAT_LOOKUP_METRICS.map((metric) => {
    ...
    const spark =
      metric.id === "totalPoints"
        ? sparklinePath(input.history ?? input.series?.[metric.id] ?? [])
        : sparklinePath(input.series?.[metric.id] ?? []);
    return {
      id: metric.id,
      label: metric.label,
...
export function lookupCardsWithValues(cards: LookupCard[]): LookupCard[] {
  return cards.filter((card) => card.value != null && Number.isFinite(card.value));
}

/**
 * Lovat Collection shows only the actions the robot can do in this phase.
 * Event standing cards stay visible; scout cards filter by auto / teleop / endgame.
 */
export function scoutMetricVisibleWhen(phase: ScoutContextPhase, id: LovatLookupMetricId): boolean {
  switch (phase) {
    case "auto":
      return id === "autoPoints" || id === "autoClimb";
    case "teleop":
      return id !== "autoPoints" && id !== "autoClimb" && id !== "endgameClimb";
    case "endgame":
      return id === "endgameClimb";
    default: {
...
/**
 * Dashboard tiles: skip blanks (never 0-fill), keep event standing cards on
 * every phase chip, and hide scout/phase metrics the robot cannot do now.
 */
export function lookupCardVisible(phase: ScoutContextPhase | "all", card: LookupCard): boolean {
  if (card.value == null || !Number.isFinite(card.value)) return false;
  if (phase === "all") return true;
  if (card.source === "event" && EVENT_STANDING_IDS.has(card.id)) return true;
  return scoutMetricVisibleWhen(phase, card.id);
}

export function scoutSeriesFromPayloads(
  payloads: Array<Record<string, unknown>>,
): Partial<Record<LovatLookupMetricId, number[]>> {
  const series: Partial<Record<LovatLookupMetricId, number[]>> = {};
  for (const [id, keys] of Object.entries(SCOUT_PAYLOAD_KEYS) as Array<
    [LovatLookupMetricId, readonly string[]]
  >) {
    const values: number[] = [];
    ...
    if (values.length >= 2) series[id] = values;
  }
  return series;
}
...
export function scoutAveragesFromPayloads(
  ...
  const values: ScoutAverageRow["values"] = {
    driverAbility: averageScoutMetric(payloads, [...(SCOUT_PAYLOAD_KEYS.driverAbility ?? [])]),
    autoClimb: averageScoutMetric(payloads, [...(SCOUT_PAYLOAD_KEYS.autoClimb ?? [])]),
    defenseEffectiveness: averageScoutMetric(payloads, [...(SCOUT_PAYLOAD_KEYS.defenseEffectiveness ?? [])]),
    contactDefenseTime: averageScoutMetric(payloads, [...(SCOUT_PAYLOAD_KEYS.contactDefenseTime ?? [])]),
    campingDefenseTime: averageScoutMetric(payloads, [...(SCOUT_PAYLOAD_KEYS.campingDefenseTime ?? [])]),
    totalDefenseTime: averageScoutMetric(payloads, [...(SCOUT_PAYLOAD_KEYS.totalDefenseTime ?? [])]),
    totalFuelThroughput: averageScoutMetric(payloads, [...(SCOUT_PAYLOAD_KEYS.totalFuelThroughput ?? [])]),
    totalFuelFed: averageScoutMetric(payloads, [...(SCOUT_PAYLOAD_KEYS.totalFuelFed ?? [])]),
    feedingRate: averageScoutMetric(payloads, [...(SCOUT_PAYLOAD_KEYS.feedingRate ?? [])]),
    scoringRate: averageScoutMetric(payloads, [...(SCOUT_PAYLOAD_KEYS.scoringRate ?? [])]),
    estimatedSuccessfulFuelRate: averageScoutMetric(
      payloads,
      [...(SCOUT_PAYLOAD_KEYS.estimatedSuccessfulFuelRate ?? [])],
    ),
    estimatedTotalFuelScored: averageScoutMetric(payloads, [...(SCOUT_PAYLOAD_KEYS.estimatedTotalFuelScored ?? [])]),
    reliability: averageScoutMetric(payloads, [...(SCOUT_PAYLOAD_KEYS.reliability ?? [])]),
  };
