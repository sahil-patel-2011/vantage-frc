/**
 * Lovat-style Auto board — this event, dense rank.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-event-rank";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 478005556): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
