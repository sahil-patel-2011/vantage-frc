/**
 * Lovat-style Auto board — last 12 matches, consistency.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-last12-consistency";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2190964855): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
