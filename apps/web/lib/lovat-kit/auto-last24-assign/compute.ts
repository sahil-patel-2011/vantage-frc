/**
 * Lovat-style Auto board — last 24 matches, assignment range.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-last24-assign";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1405158707): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
