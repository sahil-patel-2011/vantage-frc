/**
 * Lovat-style Auto board — last 24 matches, z-score picklist sliders.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-last24-picklist";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2688846467): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
