/**
 * Lovat-style Auto board — playoff matches, recency-weighted.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-playoffs-ewma";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1101651138): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
