/**
 * Lovat-style Auto board — playoff matches, per-match rate.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-playoffs-rate";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3610727658): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
