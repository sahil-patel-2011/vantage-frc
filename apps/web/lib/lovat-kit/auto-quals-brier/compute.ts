/**
 * Lovat-style Auto board — qualification matches, forecast calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-quals-brier";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1590769126): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
