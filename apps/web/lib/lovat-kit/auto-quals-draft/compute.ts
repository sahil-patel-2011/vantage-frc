/**
 * Lovat-style Auto board — qualification matches, draft board tier.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-quals-draft";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3473544409): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
