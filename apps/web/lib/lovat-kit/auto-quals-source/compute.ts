/**
 * Lovat-style Auto board — qualification matches, event-scout mix.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-quals-source";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1556964535): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
