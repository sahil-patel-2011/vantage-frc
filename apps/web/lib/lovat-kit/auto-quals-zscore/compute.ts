/**
 * Lovat-style Auto board — qualification matches, compared to this event.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-quals-zscore";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1386716482): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
