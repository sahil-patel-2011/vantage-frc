/**
 * Lovat-style Defense board — this event, assignment range.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-assign";
...
export const EWMA_ALPHA = 0.385;
export const TRIM_FRACTION = 0.145;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2115603032): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
