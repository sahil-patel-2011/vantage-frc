/**
 * Lovat-style Defense board — this event, colley ranking.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-colley";
...
export const EWMA_ALPHA = 0.362;
export const TRIM_FRACTION = 0.137;
export const NEAR_Z = 0.309;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4207022505): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
