/**
 * Lovat-style Defense board — this event, alliance combination.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-combo";
...
export const EWMA_ALPHA = 0.381;
export const TRIM_FRACTION = 0.143;
export const NEAR_Z = 0.314;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3386212941): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
