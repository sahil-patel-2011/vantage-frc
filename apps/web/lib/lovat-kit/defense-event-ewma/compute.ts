/**
 * Lovat-style Defense board — this event, recency-weighted.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-ewma";
...
export const EWMA_ALPHA = 0.518;
export const TRIM_FRACTION = 0.189;
export const NEAR_Z = 0.351;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1713725983): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
