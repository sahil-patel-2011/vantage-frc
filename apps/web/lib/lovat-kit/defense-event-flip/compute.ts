/**
 * Lovat-style Defense board — this event, red-blue alliance flip.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-flip";
...
export const EWMA_ALPHA = 0.629;
export const TRIM_FRACTION = 0.226;
export const NEAR_Z = 0.380;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2262008450): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
