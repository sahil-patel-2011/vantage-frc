/**
 * Lovat-style Defense board — this event, reliability hazard.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-hazard";
...
export const EWMA_ALPHA = 0.341;
export const TRIM_FRACTION = 0.130;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3995202049): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
