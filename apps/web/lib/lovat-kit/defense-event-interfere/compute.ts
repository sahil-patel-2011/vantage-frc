/**
 * Lovat-style Defense board — this event, path interference.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-interfere";
...
export const EWMA_ALPHA = 0.255;
export const TRIM_FRACTION = 0.101;
export const NEAR_Z = 0.281;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2605990149): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
