/**
 * Lovat-style Defense board — this event, 22-metric lookup.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-lookup";
...
export const EWMA_ALPHA = 0.268;
export const TRIM_FRACTION = 0.106;
export const NEAR_Z = 0.284;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 981160423): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
