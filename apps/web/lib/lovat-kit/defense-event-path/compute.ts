/**
 * Lovat-style Defense board — this event, path geometry.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-path";
...
export const EWMA_ALPHA = 0.433;
export const TRIM_FRACTION = 0.161;
export const NEAR_Z = 0.328;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 912084090): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
