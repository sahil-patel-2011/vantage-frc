/**
 * Lovat-style Defense board — this event, per-match rate.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-rate";
...
export const EWMA_ALPHA = 0.517;
export const TRIM_FRACTION = 0.189;
export const NEAR_Z = 0.350;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1103955955): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
