/**
 * Lovat-style Defense board — this event, skill posterior.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-trueskill";
...
export const EWMA_ALPHA = 0.457;
export const TRIM_FRACTION = 0.169;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 738504626): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
