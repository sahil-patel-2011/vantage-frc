/**
 * Lovat-style Defense board — this event, match win probability.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-win";
...
export const EWMA_ALPHA = 0.552;
export const TRIM_FRACTION = 0.200;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3320066733): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
