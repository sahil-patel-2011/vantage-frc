/**
 * Lovat-style Defense board — this event, compared to this event.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-zscore";
...
export const EWMA_ALPHA = 0.598;
export const TRIM_FRACTION = 0.215;
export const NEAR_Z = 0.372;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3620567753): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
