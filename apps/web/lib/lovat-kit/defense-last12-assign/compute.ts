/**
 * Lovat-style Defense board — last 12 matches, assignment range.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last12-assign";
export const PHASE_LABEL = "Defense";
export const WINDOW_LABEL = "last 12 matches";
export const KERNEL_LABEL = "assignment range";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2817795329): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
