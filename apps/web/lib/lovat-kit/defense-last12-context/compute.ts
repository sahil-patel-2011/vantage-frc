/**
 * Lovat-style Defense board — last 12 matches, context-visible scouting.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last12-context";
export const PHASE_LABEL = "Defense";
export const WINDOW_LABEL = "last 12 matches";
export const KERNEL_LABEL = "context-visible scouting";
export const WINDOW_N: number | null = 12;
export const QUALS_ONLY = false;
export const PLAYOFFS_ONLY = false;
export const EWMA_ALPHA = 0.332;
export const TRIM_FRACTION = 0.127;
export const NEAR_Z = 0.301;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 849751843): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
