/**
 * Lovat-style Defense board — last 12 matches, per-match rate.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last12-rate";
...
export const EWMA_ALPHA = 0.291;
export const TRIM_FRACTION = 0.113;
export const NEAR_Z = 0.290;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2040340946): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
