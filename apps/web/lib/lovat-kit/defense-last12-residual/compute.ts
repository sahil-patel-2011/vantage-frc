/**
 * Lovat-style Defense board — last 12 matches, opponent-adjusted.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last12-residual";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2771936655): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
