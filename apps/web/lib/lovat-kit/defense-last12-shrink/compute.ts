/**
 * Lovat-style Defense board — last 12 matches, field-shrunk.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last12-shrink";
...
export const EWMA_ALPHA = 0.417;
export const TRIM_FRACTION = 0.155;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1873053729): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
