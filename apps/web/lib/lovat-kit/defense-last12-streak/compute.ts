/**
 * Lovat-style Defense board — last 12 matches, hot-cold streak.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last12-streak";
...
export const EWMA_ALPHA = 0.445;
export const TRIM_FRACTION = 0.165;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3233114370): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
