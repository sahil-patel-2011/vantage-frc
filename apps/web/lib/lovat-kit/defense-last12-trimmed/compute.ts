/**
 * Lovat-style Defense board — last 12 matches, trimmed average.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last12-trimmed";
...
export const EWMA_ALPHA = 0.282;
export const TRIM_FRACTION = 0.110;
export const NEAR_Z = 0.288;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 722730738): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
