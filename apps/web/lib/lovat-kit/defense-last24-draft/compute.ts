/**
 * Lovat-style Defense board — last 24 matches, draft board tier.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last24-draft";
...
export const EWMA_ALPHA = 0.324;
export const TRIM_FRACTION = 0.124;
export const NEAR_Z = 0.299;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2851261666): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
