/**
 * Lovat-style Defense board — last 24 matches, red-blue alliance flip.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last24-flip";
...
export const EWMA_ALPHA = 0.515;
export const TRIM_FRACTION = 0.188;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3748885908): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
