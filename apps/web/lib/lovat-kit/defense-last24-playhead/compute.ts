/**
 * Lovat-style Defense board — last 24 matches, auto path playhead.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last24-playhead";
...
export const EWMA_ALPHA = 0.649;
export const TRIM_FRACTION = 0.233;
export const NEAR_Z = 0.386;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2329758899): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
