/**
 * Lovat-style Defense board — last 24 matches, skill posterior.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last24-trueskill";
...
export const EWMA_ALPHA = 0.606;
export const TRIM_FRACTION = 0.218;
export const NEAR_Z = 0.374;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2717207928): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
