/**
 * Lovat-style Defense board — last 24 matches, match win probability.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last24-win";
...
export const EWMA_ALPHA = 0.512;
export const TRIM_FRACTION = 0.187;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3224475855): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
