/**
 * Lovat-style Defense board — last 24 matches, field zone occupancy.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last24-zone";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 870979269): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
