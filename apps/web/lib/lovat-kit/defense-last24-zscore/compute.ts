/**
 * Lovat-style Defense board — last 24 matches, compared to this event.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last24-zscore";
...
export const EWMA_ALPHA = 0.336;
export const TRIM_FRACTION = 0.128;
export const NEAR_Z = 0.302;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3002371931): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
