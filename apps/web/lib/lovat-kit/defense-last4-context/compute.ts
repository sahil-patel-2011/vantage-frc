/**
 * Lovat-style Defense board — last 4 matches, context-visible scouting.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last4-context";
...
export const EWMA_ALPHA = 0.422;
export const TRIM_FRACTION = 0.157;
export const NEAR_Z = 0.325;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3122293840): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
