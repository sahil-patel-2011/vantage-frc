/**
 * Lovat-style Defense board — last 4 matches, recency-weighted.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last4-ewma";
...
export const EWMA_ALPHA = 0.258;
export const TRIM_FRACTION = 0.102;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1163230199): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
