/**
 * Lovat-style Defense board — last 4 matches, reliability hazard.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last4-hazard";
...
export const EWMA_ALPHA = 0.534;
export const TRIM_FRACTION = 0.194;
export const NEAR_Z = 0.355;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 774346345): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
