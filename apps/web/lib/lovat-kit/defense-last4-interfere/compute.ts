/**
 * Lovat-style Defense board — last 4 matches, path interference.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last4-interfere";
...
export const EWMA_ALPHA = 0.601;
export const TRIM_FRACTION = 0.217;
export const NEAR_Z = 0.373;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3386237821): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
