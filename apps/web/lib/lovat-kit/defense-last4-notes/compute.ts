/**
 * Lovat-style Defense board — last 4 matches, lead lookup notes.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last4-notes";
...
export const EWMA_ALPHA = 0.393;
export const TRIM_FRACTION = 0.147;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 204193206): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
