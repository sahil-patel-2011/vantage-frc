/**
 * Lovat-style Defense board — last 4 matches, auto path playhead.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last4-playhead";
...
export const EWMA_ALPHA = 0.446;
export const TRIM_FRACTION = 0.165;
export const NEAR_Z = 0.332;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3503674373): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
