/**
 * Lovat-style Defense board — last 4 matches, dense rank.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last4-rank";
...
export const EWMA_ALPHA = 0.626;
export const TRIM_FRACTION = 0.225;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 382138387): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
