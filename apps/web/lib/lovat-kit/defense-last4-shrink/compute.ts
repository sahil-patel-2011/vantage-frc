/**
 * Lovat-style Defense board — last 4 matches, field-shrunk.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last4-shrink";
...
export const TRIM_FRACTION = 0.163;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1259384284): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
