/**
 * Lovat-style Defense board — last 4 matches, event-scout mix.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last4-source";
...
export const EWMA_ALPHA = 0.595;
export const TRIM_FRACTION = 0.215;
export const NEAR_Z = 0.371;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 838507700): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
