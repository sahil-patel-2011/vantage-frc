/**
 * Lovat-style Defense board — last 4 matches, hot-cold streak.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last4-streak";
...
export const EWMA_ALPHA = 0.431;
export const TRIM_FRACTION = 0.160;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 885134039): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
