/**
 * Lovat-style Defense board — last 4 matches, timed occupancy.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last4-timeline";
...
export const EWMA_ALPHA = 0.349;
export const TRIM_FRACTION = 0.132;
export const NEAR_Z = 0.306;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3764352218): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
