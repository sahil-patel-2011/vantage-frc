/**
 * Lovat-style Defense board — last 4 matches, 22-metric timeline.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last4-timeline22";
...
export const EWMA_ALPHA = 0.374;
export const TRIM_FRACTION = 0.141;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2964142782): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
