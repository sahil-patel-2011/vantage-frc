/**
 * Lovat-style Defense board — last 4 matches, trimmed average.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last4-trimmed";
...
export const EWMA_ALPHA = 0.507;
export const TRIM_FRACTION = 0.185;
export const NEAR_Z = 0.348;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2391605729): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
