/**
 * Lovat-style Defense board — last 4 matches, compared to this event.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last4-zscore";
...
export const EWMA_ALPHA = 0.510;
export const TRIM_FRACTION = 0.186;
export const NEAR_Z = 0.349;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3098505809): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
