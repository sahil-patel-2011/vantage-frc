/**
 * Lovat-style Defense board — last 6 matches, assignment range.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-assign";
...
export const EWMA_ALPHA = 0.286;
export const TRIM_FRACTION = 0.112;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 488700838): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
