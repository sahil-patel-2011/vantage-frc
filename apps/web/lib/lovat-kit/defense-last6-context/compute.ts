/**
 * Lovat-style Defense board — last 6 matches, context-visible scouting.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-context";
...
export const EWMA_ALPHA = 0.361;
export const TRIM_FRACTION = 0.136;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 761342486): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
