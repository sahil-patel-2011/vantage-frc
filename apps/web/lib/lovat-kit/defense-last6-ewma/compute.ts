/**
 * Lovat-style Defense board — last 6 matches, recency-weighted.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-ewma";
...
export const EWMA_ALPHA = 0.428;
export const TRIM_FRACTION = 0.159;
export const NEAR_Z = 0.327;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3404083977): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
