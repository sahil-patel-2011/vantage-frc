/**
 * Lovat-style Defense board — last 6 matches, 22-metric lookup.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-lookup";
...
export const EWMA_ALPHA = 0.449;
export const TRIM_FRACTION = 0.166;
export const NEAR_Z = 0.333;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1991714457): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
