/**
 * Lovat-style Defense board — last 6 matches, hot-cold streak.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-streak";
...
export const EWMA_ALPHA = 0.410;
export const TRIM_FRACTION = 0.153;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 892533577): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
