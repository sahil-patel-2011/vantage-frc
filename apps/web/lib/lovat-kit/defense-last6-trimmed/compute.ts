/**
 * Lovat-style Defense board — last 6 matches, trimmed average.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-trimmed";
...
export const TRIM_FRACTION = 0.121;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 679721423): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
