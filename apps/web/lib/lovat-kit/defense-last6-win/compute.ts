/**
 * Lovat-style Defense board — last 6 matches, match win probability.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-win";
...
export const EWMA_ALPHA = 0.544;
export const TRIM_FRACTION = 0.197;
export const NEAR_Z = 0.358;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 143476551): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
