/**
 * Lovat-style Defense board — last 6 matches, compared to this event.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-zscore";
...
export const EWMA_ALPHA = 0.561;
export const TRIM_FRACTION = 0.203;
export const NEAR_Z = 0.362;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1702386947): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
