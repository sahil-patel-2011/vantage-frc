/**
 * Lovat-style Defense board — playoff matches, assignment range.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-assign";
...
export const EWMA_ALPHA = 0.670;
export const TRIM_FRACTION = 0.240;
export const NEAR_Z = 0.391;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3110649360): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
