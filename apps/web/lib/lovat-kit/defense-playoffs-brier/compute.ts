/**
 * Lovat-style Defense board — playoff matches, forecast calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-brier";
...
export const EWMA_ALPHA = 0.608;
export const TRIM_FRACTION = 0.219;
export const NEAR_Z = 0.375;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2697577975): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
