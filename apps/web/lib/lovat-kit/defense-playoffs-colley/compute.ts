/**
 * Lovat-style Defense board — playoff matches, colley ranking.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-colley";
...
export const EWMA_ALPHA = 0.414;
export const TRIM_FRACTION = 0.154;
export const NEAR_Z = 0.323;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2114383681): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
