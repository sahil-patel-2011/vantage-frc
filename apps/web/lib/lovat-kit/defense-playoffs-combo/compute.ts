/**
 * Lovat-style Defense board — playoff matches, alliance combination.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-combo";
export const PHASE_LABEL = "Defense";
export const WINDOW_LABEL = "playoff matches";
export const KERNEL_LABEL = "alliance combination";
export const WINDOW_N: number | null = null;
export const QUALS_ONLY = false;
export const PLAYOFFS_ONLY = true;
export const EWMA_ALPHA = 0.644;
export const TRIM_FRACTION = 0.231;
export const NEAR_Z = 0.384;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3761108773): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
