/**
 * Lovat-style Defense board — playoff matches, context-visible scouting.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-context";
...
export const EWMA_ALPHA = 0.419;
export const TRIM_FRACTION = 0.156;
export const NEAR_Z = 0.324;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3240663776): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
