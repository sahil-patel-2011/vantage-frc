/**
 * Lovat-style Defense board — playoff matches, draft board tier.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-draft";
...
export const EWMA_ALPHA = 0.464;
export const TRIM_FRACTION = 0.171;
export const NEAR_Z = 0.336;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1675744784): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
