/**
 * Lovat-style Defense board — playoff matches, recency-weighted.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-ewma";
...
export const EWMA_ALPHA = 0.577;
export const TRIM_FRACTION = 0.208;
export const NEAR_Z = 0.366;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3291277287): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
