/**
 * Lovat-style Defense board — playoff matches, reliability hazard.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-hazard";
...
export const EWMA_ALPHA = 0.659;
export const TRIM_FRACTION = 0.236;
export const NEAR_Z = 0.388;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1707949113): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
