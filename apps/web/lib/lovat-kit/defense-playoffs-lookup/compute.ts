/**
 * Lovat-style Defense board — playoff matches, 22-metric lookup.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-lookup";
export const PHASE_LABEL = "Defense";
export const WINDOW_LABEL = "playoff matches";
export const KERNEL_LABEL = "22-metric lookup";
export const WINDOW_N: number | null = null;
export const QUALS_ONLY = false;
export const PLAYOFFS_ONLY = true;
export const EWMA_ALPHA = 0.434;
export const TRIM_FRACTION = 0.161;
export const NEAR_Z = 0.329;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3796324127): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
