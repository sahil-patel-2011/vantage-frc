/**
 * Lovat-style Defense board — playoff matches, z-score picklist sliders.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-picklist";
...
export const EWMA_ALPHA = 0.442;
export const TRIM_FRACTION = 0.164;
export const NEAR_Z = 0.331;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2227784292): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
