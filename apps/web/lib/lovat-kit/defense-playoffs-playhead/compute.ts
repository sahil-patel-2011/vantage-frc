/**
 * Lovat-style Defense board — playoff matches, auto path playhead.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-playhead";
...
export const EWMA_ALPHA = 0.693;
export const TRIM_FRACTION = 0.247;
export const NEAR_Z = 0.397;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2687099861): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
