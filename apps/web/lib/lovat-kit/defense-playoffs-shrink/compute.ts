/**
 * Lovat-style Defense board — playoff matches, field-shrunk.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-shrink";
...
export const EWMA_ALPHA = 0.347;
export const TRIM_FRACTION = 0.132;
export const NEAR_Z = 0.305;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2066232172): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
