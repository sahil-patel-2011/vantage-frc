/**
 * Lovat-style Defense board — playoff matches, event-scout mix.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-source";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3298502372): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
