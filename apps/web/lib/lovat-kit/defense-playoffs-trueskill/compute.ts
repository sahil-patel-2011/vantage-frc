/**
 * Lovat-style Defense board — playoff matches, skill posterior.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-trueskill";
...
export const EWMA_ALPHA = 0.499;
export const TRIM_FRACTION = 0.183;
export const NEAR_Z = 0.346;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1937825562): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
