/**
 * Lovat-style Defense board — playoff matches, match win probability.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-win";
...
export const EWMA_ALPHA = 0.304;
export const TRIM_FRACTION = 0.118;
export const NEAR_Z = 0.294;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3876111221): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
