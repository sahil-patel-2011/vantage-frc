/**
 * Lovat-style Defense board — playoff matches, compared to this event.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-playoffs-zscore";
...
export const EWMA_ALPHA = 0.640;
export const TRIM_FRACTION = 0.230;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 362178689): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
