/**
 * Lovat-style Defense board — qualification matches, assignment range.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-assign";
...
export const EWMA_ALPHA = 0.476;
export const TRIM_FRACTION = 0.175;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 275105048): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
