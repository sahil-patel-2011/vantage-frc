/**
 * Lovat-style Defense board — qualification matches, colley ranking.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-colley";
...
export const EWMA_ALPHA = 0.572;
export const TRIM_FRACTION = 0.207;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1582617193): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
