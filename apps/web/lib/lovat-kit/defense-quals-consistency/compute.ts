/**
 * Lovat-style Defense board — qualification matches, consistency.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-consistency";
...
export const EWMA_ALPHA = 0.425;
export const TRIM_FRACTION = 0.158;
export const NEAR_Z = 0.326;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3374133923): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
