/**
 * Lovat-style Defense board — qualification matches, recency-weighted.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-ewma";
...
export const EWMA_ALPHA = 0.585;
export const TRIM_FRACTION = 0.211;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4005557471): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
