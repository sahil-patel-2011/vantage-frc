/**
 * Lovat-style Defense board — qualification matches, reliability hazard.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-hazard";
...
export const EWMA_ALPHA = 0.581;
export const TRIM_FRACTION = 0.210;
export const NEAR_Z = 0.368;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1795357377): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
