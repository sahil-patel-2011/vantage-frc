/**
 * Lovat-style Defense board — qualification matches, 22-metric lookup.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-lookup";
...
export const EWMA_ALPHA = 0.633;
export const TRIM_FRACTION = 0.227;
export const NEAR_Z = 0.381;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 311258535): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
