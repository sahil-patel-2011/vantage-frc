/**
 * Lovat-style Defense board — qualification matches, path geometry.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-path";
...
export const EWMA_ALPHA = 0.351;
export const TRIM_FRACTION = 0.133;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3245492282): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
