/**
 * Lovat-style Defense board — qualification matches, auto path playhead.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-playhead";
...
export const EWMA_ALPHA = 0.468;
export const TRIM_FRACTION = 0.172;
export const NEAR_Z = 0.337;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3210774861): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
