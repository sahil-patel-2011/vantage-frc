/**
 * Lovat-style Defense board — qualification matches, event-scout mix.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-source";
...
export const EWMA_ALPHA = 0.570;
export const TRIM_FRACTION = 0.206;
export const NEAR_Z = 0.365;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4206677148): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
