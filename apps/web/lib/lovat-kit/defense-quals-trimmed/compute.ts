/**
 * Lovat-style Defense board — qualification matches, trimmed average.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-trimmed";
...
export const EWMA_ALPHA = 0.344;
export const TRIM_FRACTION = 0.131;
export const NEAR_Z = 0.304;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2436242105): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
