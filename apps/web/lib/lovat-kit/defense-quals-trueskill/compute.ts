/**
 * Lovat-style Defense board — qualification matches, skill posterior.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-trueskill";
...
export const TRIM_FRACTION = 0.177;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1726735218): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
