/**
 * Lovat-style Defense board — qualification matches, match win probability.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-win";
...
export const EWMA_ALPHA = 0.568;
export const TRIM_FRACTION = 0.205;
export const NEAR_Z = 0.364;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3119647085): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
