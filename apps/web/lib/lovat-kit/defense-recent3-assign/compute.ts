/**
 * Lovat-style Defense board — last 3 matches, assignment range.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-assign";
...
export const EWMA_ALPHA = 0.365;
export const TRIM_FRACTION = 0.138;
export const NEAR_Z = 0.310;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 734852584): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
