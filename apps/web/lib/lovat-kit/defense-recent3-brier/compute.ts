/**
 * Lovat-style Defense board — last 3 matches, forecast calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-brier";
...
export const EWMA_ALPHA = 0.619;
export const TRIM_FRACTION = 0.223;
export const NEAR_Z = 0.378;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4269798223): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
