/**
 * Lovat-style Defense board — last 3 matches, colley ranking.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-colley";
...
export const EWMA_ALPHA = 0.313;
export const TRIM_FRACTION = 0.120;
export const NEAR_Z = 0.296;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3675931417): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
