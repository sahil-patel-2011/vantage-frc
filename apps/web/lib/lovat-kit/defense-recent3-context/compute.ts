/**
 * Lovat-style Defense board — last 3 matches, context-visible scouting.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-context";
...
export const EWMA_ALPHA = 0.460;
export const TRIM_FRACTION = 0.170;
export const NEAR_Z = 0.335;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 811104696): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
