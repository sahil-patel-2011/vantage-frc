/**
 * Lovat-style Defense board — last 3 matches, recency-weighted.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-ewma";
export const PHASE_LABEL = "Defense";
export const WINDOW_LABEL = "last 3 matches";
export const KERNEL_LABEL = "recency-weighted";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2133256655): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
