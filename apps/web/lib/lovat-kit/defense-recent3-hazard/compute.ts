/**
 * Lovat-style Defense board — last 3 matches, reliability hazard.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-hazard";
...
export const EWMA_ALPHA = 0.586;
export const TRIM_FRACTION = 0.212;
export const NEAR_Z = 0.369;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1519857489): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
