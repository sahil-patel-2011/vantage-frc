/**
 * Lovat-style Defense board — last 3 matches, path interference.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-interfere";
...
export const EWMA_ALPHA = 0.485;
export const TRIM_FRACTION = 0.178;
export const NEAR_Z = 0.342;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3052975253): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
