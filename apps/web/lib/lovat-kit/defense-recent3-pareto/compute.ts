/**
 * Lovat-style Defense board — last 3 matches, pareto front.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-pareto";
...
export const EWMA_ALPHA = 0.687;
export const TRIM_FRACTION = 0.245;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 824389734): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
