/**
 * Lovat-style Defense board — last 3 matches, path geometry.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-path";
...
export const EWMA_ALPHA = 0.295;
export const TRIM_FRACTION = 0.115;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 749761034): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
