/**
 * Lovat-style Defense board — last 3 matches, z-score picklist sliders.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-picklist";
export const PHASE_LABEL = "Defense";
export const WINDOW_LABEL = "last 3 matches";
export const KERNEL_LABEL = "z-score picklist sliders";
export const WINDOW_N: number | null = 3;
export const QUALS_ONLY = false;
export const PLAYOFFS_ONLY = false;
export const EWMA_ALPHA = 0.540;
export const TRIM_FRACTION = 0.196;
export const NEAR_Z = 0.357;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 918696476): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
