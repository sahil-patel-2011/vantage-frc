/**
 * Lovat-style Defense board — last 3 matches, auto path playhead.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-playhead";
...
export const EWMA_ALPHA = 0.368;
export const TRIM_FRACTION = 0.139;
export const NEAR_Z = 0.311;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1524082653): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
