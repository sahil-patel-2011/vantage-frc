/**
 * Lovat-style Defense board — last 3 matches, dense rank.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-rank";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2112520635): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
