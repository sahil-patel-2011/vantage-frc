/**
 * Lovat-style Defense board — last 3 matches, per-match rate.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-rate";
...
export const EWMA_ALPHA = 0.272;
export const TRIM_FRACTION = 0.107;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2015400515): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
