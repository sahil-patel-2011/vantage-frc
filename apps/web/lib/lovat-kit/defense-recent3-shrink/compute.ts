/**
 * Lovat-style Defense board — last 3 matches, field-shrunk.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-shrink";
...
export const EWMA_ALPHA = 0.317;
export const TRIM_FRACTION = 0.122;
export const NEAR_Z = 0.297;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1017331524): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
