/**
 * Lovat-style Defense board — last 3 matches, hot-cold streak.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-streak";
...
export const EWMA_ALPHA = 0.252;
export const TRIM_FRACTION = 0.100;
export const NEAR_Z = 0.400;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3096880079): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
