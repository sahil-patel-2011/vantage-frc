/**
 * Lovat-style Defense board — last 3 matches, timed occupancy.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-timeline";
...
export const EWMA_ALPHA = 0.298;
export const TRIM_FRACTION = 0.116;
export const NEAR_Z = 0.292;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2755311090): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
