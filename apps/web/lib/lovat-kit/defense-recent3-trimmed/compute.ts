/**
 * Lovat-style Defense board — last 3 matches, trimmed average.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-recent3-trimmed";
export const PHASE_LABEL = "Defense";
export const WINDOW_LABEL = "last 3 matches";
export const KERNEL_LABEL = "trimmed average";
export const WINDOW_N: number | null = 3;
export const QUALS_ONLY = false;
export const PLAYOFFS_ONLY = false;
export const EWMA_ALPHA = 0.489;
export const TRIM_FRACTION = 0.179;
export const NEAR_Z = 0.343;
...
export function fieldStats(rows: readonly SampleRow[]): Partial<Record<LovatMetricId, FieldStat>> {
  ...
  for (const metric of METRICS) {
    ...
    const std = populationStdDev(winsorize(values));
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3053135337): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
