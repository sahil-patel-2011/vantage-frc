/**
 * Lovat-style Driver board — this event, assignment range.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-event-assign";
...
export const EWMA_ALPHA = 0.376;
export const TRIM_FRACTION = 0.142;
export const NEAR_Z = 0.313;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1236212836): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
