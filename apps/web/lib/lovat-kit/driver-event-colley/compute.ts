/**
 * Lovat-style Driver board — this event, colley ranking.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-event-colley";
...
export const EWMA_ALPHA = 0.251;
export const TRIM_FRACTION = 0.250;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2367310053): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
