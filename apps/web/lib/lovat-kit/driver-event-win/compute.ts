/**
 * Lovat-style Driver board — this event, match win probability.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-event-win";
...
export const EWMA_ALPHA = 0.599;
export const TRIM_FRACTION = 0.216;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4176647777): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
