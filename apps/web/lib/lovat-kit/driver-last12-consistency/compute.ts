/**
 * Lovat-style Driver board — last 12 matches, consistency.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last12-consistency";
...
export const EWMA_ALPHA = 0.530;
export const TRIM_FRACTION = 0.193;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4230146256): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
