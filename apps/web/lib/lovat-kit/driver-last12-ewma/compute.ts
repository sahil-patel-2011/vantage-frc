/**
 * Lovat-style Driver board — last 12 matches, recency-weighted.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last12-ewma";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4282710206): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
