/**
 * Lovat-style Driver board — last 12 matches, lead lookup notes.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last12-notes";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3031168673): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
