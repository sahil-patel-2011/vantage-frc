/**
 * Lovat-style Driver board — last 12 matches, per-match rate.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last12-rate";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2108152366): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
