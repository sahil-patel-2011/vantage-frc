/**
 * Lovat-style Driver board — last 12 matches, compared to this event.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last12-zscore";
...
export const EWMA_ALPHA = 0.266;
export const TRIM_FRACTION = 0.105;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 405530392): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
