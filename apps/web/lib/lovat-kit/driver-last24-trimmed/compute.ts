/**
 * Lovat-style Driver board — last 24 matches, trimmed average.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last24-trimmed";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2055911211): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
