/**
 * Lovat-style Driver board — last 4 matches, path geometry.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last4-path";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4267120406): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
