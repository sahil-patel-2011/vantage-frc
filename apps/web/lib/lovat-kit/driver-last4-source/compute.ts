/**
 * Lovat-style Driver board — last 4 matches, event-scout mix.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last4-source";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 89453240): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
