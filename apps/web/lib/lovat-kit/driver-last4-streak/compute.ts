/**
 * Lovat-style Driver board — last 4 matches, hot-cold streak.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last4-streak";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2290152147): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
