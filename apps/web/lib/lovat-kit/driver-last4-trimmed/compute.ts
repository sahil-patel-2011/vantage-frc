/**
 * Lovat-style Driver board — last 4 matches, trimmed average.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last4-trimmed";
...
export const EWMA_ALPHA = 0.668;
export const TRIM_FRACTION = 0.239;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1800669309): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
