/**
 * Lovat-style Driver board — last 6 matches, colley ranking.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last6-colley";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2667790763): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
