/**
 * Lovat-style Driver board — last 6 matches, lead lookup notes.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last6-notes";
...
export const EWMA_ALPHA = 0.319;
export const TRIM_FRACTION = 0.123;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3335951560): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
