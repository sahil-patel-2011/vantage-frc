/**
 * Lovat-style Driver board — last 6 matches, 22-metric timeline.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last6-timeline22";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 715154044): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
