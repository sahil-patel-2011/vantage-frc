/**
 * Lovat-style Driver board — last 6 matches, match win probability.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-last6-win";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 306775115): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
