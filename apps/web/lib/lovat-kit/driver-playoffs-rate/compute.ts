/**
 * Lovat-style Driver board — playoff matches, per-match rate.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-playoffs-rate";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2746070679): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
