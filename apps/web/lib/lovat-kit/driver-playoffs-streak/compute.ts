/**
 * Lovat-style Driver board — playoff matches, hot-cold streak.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-playoffs-streak";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3212894531): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
