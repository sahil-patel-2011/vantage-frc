/**
 * Lovat-style Driver board — playoff matches, skill posterior.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-playoffs-trueskill";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4062722214): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
