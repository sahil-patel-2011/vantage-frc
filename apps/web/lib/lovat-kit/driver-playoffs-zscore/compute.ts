/**
 * Lovat-style Driver board — playoff matches, compared to this event.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-playoffs-zscore";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3308916397): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
