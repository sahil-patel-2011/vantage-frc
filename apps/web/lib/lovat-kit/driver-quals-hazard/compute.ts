/**
 * Lovat-style Driver board — qualification matches, reliability hazard.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-quals-hazard";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1786897133): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
