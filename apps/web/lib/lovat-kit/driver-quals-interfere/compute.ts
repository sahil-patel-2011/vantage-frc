/**
 * Lovat-style Driver board — qualification matches, path interference.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-quals-interfere";
...
export const EWMA_ALPHA = 0.329;
export const TRIM_FRACTION = 0.126;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4052691793): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
