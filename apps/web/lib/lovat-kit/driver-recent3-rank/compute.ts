/**
 * Lovat-style Driver board — last 3 matches, dense rank.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-recent3-rank";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3394797271): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
