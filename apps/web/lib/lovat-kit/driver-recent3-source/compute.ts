/**
 * Lovat-style Driver board — last 3 matches, event-scout mix.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-recent3-source";
...
export const EWMA_ALPHA = 0.675;
export const TRIM_FRACTION = 0.241;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4032579464): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
