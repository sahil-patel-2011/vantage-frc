/**
 * Lovat-style Driver board — last 3 matches, 22-metric timeline.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-recent3-timeline22";
...
export const EWMA_ALPHA = 0.390;
export const TRIM_FRACTION = 0.146;
export const NEAR_Z = 0.317;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1950143146): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
