/**
 * Lovat-style Endgame board — this event, auto path playhead.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-event-playhead";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 810736540): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
