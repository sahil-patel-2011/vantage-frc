/**
 * Lovat-style Endgame board — this event, opponent-adjusted.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-event-residual";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 788133103): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
