/**
 * Lovat-style Endgame board — this event, event-scout mix.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-event-source";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4043839125): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
