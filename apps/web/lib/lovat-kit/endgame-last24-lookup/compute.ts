/**
 * Lovat-style Endgame board — last 24 matches, 22-metric lookup.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-last24-lookup";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 665075138): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
