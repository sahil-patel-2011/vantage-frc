/**
 * Lovat-style Endgame board — last 4 matches, assignment range.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-last4-assign";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1833072853): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
