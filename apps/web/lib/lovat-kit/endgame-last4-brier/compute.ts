/**
 * Lovat-style Endgame board — last 4 matches, forecast calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-last4-brier";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3780642616): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
