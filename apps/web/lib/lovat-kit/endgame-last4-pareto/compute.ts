/**
 * Lovat-style Endgame board — last 4 matches, pareto front.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-last4-pareto";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1455393235): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
