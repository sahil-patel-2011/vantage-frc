/**
 * Lovat-style Endgame board — last 4 matches, match win probability.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-last4-win";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1223919930): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
