/**
 * Lovat-style Endgame board — last 4 matches, field zone occupancy.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-last4-zone";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4081699538): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
