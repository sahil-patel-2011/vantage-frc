/**
 * Lovat-style Endgame board — playoff matches, recency-weighted.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-playoffs-ewma";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 488541096): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
