/**
 * Lovat-style Endgame board — playoff matches, z-score picklist sliders.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-playoffs-picklist";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1018782515): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
