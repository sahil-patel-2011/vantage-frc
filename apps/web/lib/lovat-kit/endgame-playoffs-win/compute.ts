/**
 * Lovat-style Endgame board — playoff matches, match win probability.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-playoffs-win";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 353367164): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
