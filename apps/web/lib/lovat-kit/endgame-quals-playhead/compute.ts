/**
 * Lovat-style Endgame board — qualification matches, auto path playhead.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-quals-playhead";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 504912568): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
