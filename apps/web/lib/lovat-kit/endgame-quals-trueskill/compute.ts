/**
 * Lovat-style Endgame board — qualification matches, skill posterior.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-quals-trueskill";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1852762425): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
