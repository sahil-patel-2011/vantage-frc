/**
 * Lovat-style Endgame board — last 3 matches, hot-cold streak.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-recent3-streak";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1454145906): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
