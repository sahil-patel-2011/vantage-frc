/**
 * Lovat-style Fuel board — this event, dense rank.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-event-rank";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1453297789): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
