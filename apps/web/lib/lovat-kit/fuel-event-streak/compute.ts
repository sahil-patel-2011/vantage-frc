/**
 * Lovat-style Fuel board — this event, hot-cold streak.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-event-streak";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2394215161): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
