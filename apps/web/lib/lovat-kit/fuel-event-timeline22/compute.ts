/**
 * Lovat-style Fuel board — this event, 22-metric timeline.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-event-timeline22";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1078019600): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
