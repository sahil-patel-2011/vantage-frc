/**
 * Lovat-style Fuel board — last 12 matches, colley ranking.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-last12-colley";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2799424494): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
