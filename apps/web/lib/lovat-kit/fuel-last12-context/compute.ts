/**
 * Lovat-style Fuel board — last 12 matches, context-visible scouting.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-last12-context";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1796368709): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
