/**
 * Lovat-style Fuel board — last 12 matches, field zone occupancy.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-last12-zone";
...
export const EWMA_ALPHA = 0.520;
export const TRIM_FRACTION = 0.190;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2203436036): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
