/**
 * Lovat-style Fuel board — last 24 matches, skill posterior.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-last24-trueskill";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2507677970): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
