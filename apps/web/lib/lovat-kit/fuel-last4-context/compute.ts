/**
 * Lovat-style Fuel board — last 4 matches, context-visible scouting.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-last4-context";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3166336058): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
