/**
 * Lovat-style Fuel board — last 4 matches, reliability hazard.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-last4-hazard";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 211971451): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
