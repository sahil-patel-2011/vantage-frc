/**
 * Lovat-style Fuel board — last 4 matches, z-score picklist sliders.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-last4-picklist";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2739808082): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
