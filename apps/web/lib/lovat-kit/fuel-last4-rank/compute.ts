/**
 * Lovat-style Fuel board — last 4 matches, dense rank.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-last4-rank";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 603914681): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
