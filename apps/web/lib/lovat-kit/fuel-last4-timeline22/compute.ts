/**
 * Lovat-style Fuel board — last 4 matches, 22-metric timeline.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-last4-timeline22";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 268767572): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
