/**
 * Lovat-style Fuel board — last 6 matches, opponent-adjusted.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-last6-residual";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3641587998): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
