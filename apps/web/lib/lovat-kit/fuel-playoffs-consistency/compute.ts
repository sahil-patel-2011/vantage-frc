/**
 * Lovat-style Fuel board — playoff matches, consistency.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-playoffs-consistency";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1255649797): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
