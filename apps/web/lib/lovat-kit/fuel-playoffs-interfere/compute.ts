/**
 * Lovat-style Fuel board — playoff matches, path interference.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-playoffs-interfere";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1312540563): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
