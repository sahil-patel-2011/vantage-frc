/**
 * Lovat-style Fuel board — playoff matches, pareto front.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-playoffs-pareto";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3974788472): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
