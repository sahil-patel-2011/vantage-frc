/**
 * Lovat-style Fuel board — playoff matches, compared to this event.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-playoffs-zscore";
...
export const EWMA_ALPHA = 0.594;
export const TRIM_FRACTION = 0.214;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2469077663): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
