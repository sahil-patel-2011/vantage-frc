/**
 * Lovat-style Fuel board — qualification matches, field-shrunk.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-quals-shrink";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 100391334): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
