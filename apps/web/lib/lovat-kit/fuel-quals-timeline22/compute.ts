/**
 * Lovat-style Fuel board — qualification matches, 22-metric timeline.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-quals-timeline22";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3042486252): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
