/**
 * Lovat-style Fuel board — last 3 matches, colley ranking.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-recent3-colley";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 431817135): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
