/**
 * Lovat-style Fuel board — last 3 matches, event-scout mix.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-recent3-source";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 818045434): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
