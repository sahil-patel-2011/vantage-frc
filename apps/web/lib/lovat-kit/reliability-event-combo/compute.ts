/**
 * Lovat-style Reliability board — this event, alliance combination.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "reliability-event-combo";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3434833845): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
