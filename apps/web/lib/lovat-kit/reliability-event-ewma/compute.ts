/**
 * Lovat-style Reliability board — this event, recency-weighted.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "reliability-event-ewma";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1085551223): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
