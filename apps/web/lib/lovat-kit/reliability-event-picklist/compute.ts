/**
 * Lovat-style Reliability board — this event, z-score picklist sliders.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "reliability-event-picklist";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2276786100): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
