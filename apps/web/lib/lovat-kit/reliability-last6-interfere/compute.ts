/**
 * Lovat-style Reliability board — last 6 matches, path interference.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "reliability-last6-interfere";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1354586871): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
