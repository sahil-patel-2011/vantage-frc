/**
 * Lovat-style Teleop board — this event, forecast calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-event-brier";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2627795050): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
