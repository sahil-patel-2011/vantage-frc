...
const rows = [
  ...
  { teamKey: "frc3", qual: true, values: { teleopPoints: 38.0 } },
  { teamKey: "frc1", matchKey: "sf1", qual: false, values: { teleopPoints: 18.0 } },
...
describe("teleop-event-hazard", () => {
  it("does not invent a field average from one blank team", () => {
    ...
    expect(SLUG).toBe("teleop-event-hazard");
  });
  ...
  it("builds a report without filling missing ratings", () => {
    ...
    expect(report.slug).toBe("teleop-event-hazard");
...
describe("teleop-event-hazard-deep", () => {
