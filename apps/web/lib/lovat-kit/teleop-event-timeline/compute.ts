/**
 * Lovat-style Teleop board — this event, timed occupancy.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-event-timeline";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1863695541): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
