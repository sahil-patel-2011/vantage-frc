/**
 * Lovat-style Teleop board — last 24 matches, consistency.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-last24-consistency";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1839766154): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
