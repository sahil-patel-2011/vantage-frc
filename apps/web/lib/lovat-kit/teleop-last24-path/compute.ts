/**
 * Lovat-style Teleop board — last 24 matches, path geometry.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-last24-path";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3841664757): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
