/**
 * Lovat-style Teleop board — last 24 matches, per-match rate.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-last24-rate";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2581248156): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
