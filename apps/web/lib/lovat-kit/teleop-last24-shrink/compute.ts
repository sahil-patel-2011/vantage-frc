/**
 * Lovat-style Teleop board — last 24 matches, field-shrunk.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-last24-shrink";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 948666679): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
