/**
 * Lovat-style Teleop board — last 4 matches, auto path playhead.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-last4-playhead";
...
export const EWMA_ALPHA = 0.387;
export const TRIM_FRACTION = 0.145;
export const NEAR_Z = 0.316;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3487903082): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
