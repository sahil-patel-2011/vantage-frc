/**
 * Lovat-style Teleop board — last 4 matches, compared to this event.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-last4-zscore";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 508055902): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
