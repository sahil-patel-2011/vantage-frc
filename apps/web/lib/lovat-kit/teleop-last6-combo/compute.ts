/**
 * Lovat-style Teleop board — last 6 matches, alliance combination.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-last6-combo";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3224333390): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
