/**
 * Lovat-style Teleop board — last 6 matches, lead lookup notes.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-last6-notes";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 461426357): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
