/**
 * Lovat-style Teleop board — playoff matches, consistency.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-playoffs-consistency";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1641946056): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
