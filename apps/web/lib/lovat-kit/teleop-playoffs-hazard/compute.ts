/**
 * Lovat-style Teleop board — playoff matches, reliability hazard.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-playoffs-hazard";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3672498436): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
