/**
 * Lovat-style Teleop board — playoff matches, compared to this event.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-playoffs-zscore";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4277287984): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
