/**
 * Lovat-style Teleop board — qualification matches, red-blue alliance flip.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-quals-flip";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2419279341): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
