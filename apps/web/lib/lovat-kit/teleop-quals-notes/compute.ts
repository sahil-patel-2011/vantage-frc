/**
 * Lovat-style Teleop board — qualification matches, lead lookup notes.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-quals-notes";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1723266651): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
