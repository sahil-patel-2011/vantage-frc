/**
 * Lovat-style Teleop board — qualification matches, per-match rate.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-quals-rate";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 355737356): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
