/**
 * Lovat-style Teleop board — last 3 matches, timed occupancy.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-recent3-timeline";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1432168381): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
