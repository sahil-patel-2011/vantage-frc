...
export function scheduleCacheRequiredCopy(): { title: string; description: string } {
  return {
    ...
    description:
      "Once the schedule is posted and TBA cache has rows, matches appear here automatically — never invented slots.",
  };
}
...
export function buildScheduleView(input: {
  ...
  if (!input.context.orgId) {
    return {
      ...
      message: input.setupMessage ?? "Select a team workspace to view the match schedule.",
    };
  }
