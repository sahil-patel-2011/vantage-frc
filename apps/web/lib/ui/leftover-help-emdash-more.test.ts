 ...
 * help-emdash. Team profile / Calendar / Assembly manual / My kit drop
 * leftover Team profile — / Calendar — / Assembly manual — / My kit —
 * titles. leftover-help-emdash Ask AI / Files / Team library / Media
 * library extras stay. leftover-help extras, leftover-invites extras,
...
describe("leftover student help-emdash-more chrome", () => {
  it("does not print leftover Help Title-Case em-dash titles", () => {
    for (const rel of FILES) {
      const src = readFileSync(join(WEB, rel), "utf8");
      expect(src, rel).not.toMatch(/title: "Team profile —/);
      expect(src, rel).not.toMatch(/title: "Calendar —/);
      expect(src, rel).not.toMatch(/title: "Assembly manual —/);
      expect(src, rel).not.toMatch(/title: "My kit —/);
