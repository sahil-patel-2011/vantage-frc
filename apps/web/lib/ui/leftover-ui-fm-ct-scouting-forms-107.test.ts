...
describe("leftover Feature map Scouting forms choose team student copy", () => {
  it("uses student chrome", () => {
    ...
    expect(src).toContain("Form builder empty/setup + one primary; Match / Pit form type is a ToolStrip (not a second tab bar); custom schemas; **pit default** is drivetrain / language / driver experience / photos (CD: no claimed scoring); claimed-scoring lint on pit drafts. Load/mutate/shell is `forms-client.tsx`; chrome, option/settings editors, and preview live in sibling modules. Setup badge is **Needs setup**. Photo preview hint is **stored with this team**, not per organization. Setup badge is **Needs setup**. No-team primary is **Choose your team**.");
  });
});
