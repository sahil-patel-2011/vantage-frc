/**
 * Leftover win-plus kit — auto-event-corr.
...
/**
 * Lovat-style Auto board — this event, partner correlation.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-event-corr";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3001935680): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
