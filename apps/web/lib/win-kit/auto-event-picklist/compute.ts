/**
 * Leftover win-plus kit — auto-event-picklist.
...
/**
 * Lovat-style Auto board — this event, blend picklist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-event-picklist";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1839116283): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
