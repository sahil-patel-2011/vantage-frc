/**
 * Leftover win-plus kit — auto-event-platt.
...
/**
 * Lovat-style Auto board — this event, Platt calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-event-platt";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3543654669): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
