/**
 * Leftover win-plus kit — auto-last6-phi.
...
/**
 * Lovat-style Auto board — last 6 matches, Cody–Hart Φ.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-last6-phi";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2549577623): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
