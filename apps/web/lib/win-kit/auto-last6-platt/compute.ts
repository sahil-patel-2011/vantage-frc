/**
 * Leftover win-plus kit — auto-last6-platt.
...
/**
 * Lovat-style Auto board — last 6 matches, Platt calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-last6-platt";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 803788523): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
