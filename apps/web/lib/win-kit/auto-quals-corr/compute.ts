/**
 * Leftover win-plus kit — auto-quals-corr.
...
/**
 * Lovat-style Auto board — qualification matches, partner correlation.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-quals-corr";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3467103576): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
