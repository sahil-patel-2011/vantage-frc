/**
 * Leftover win-plus kit — auto-quals-phi.
...
/**
 * Lovat-style Auto board — qualification matches, Cody–Hart Φ.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-quals-phi";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1222067169): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
