/**
 * Leftover win-plus kit — auto-quals-platt.
...
/**
 * Lovat-style Auto board — qualification matches, Platt calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "auto-quals-platt";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2621890773): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
