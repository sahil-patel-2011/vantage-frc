/**
 * Leftover win-plus kit — defense-event-corr.
...
/**
 * Lovat-style Defense board — this event, partner correlation.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-corr";
export const PHASE_LABEL = "Defense";
export const WINDOW_LABEL = "this event";
export const KERNEL_LABEL = "partner correlation";
export const WINDOW_N: number | null = null;
export const QUALS_ONLY = false;
export const PLAYOFFS_ONLY = false;
export const EWMA_ALPHA = 0.653;
export const TRIM_FRACTION = 0.234;
export const NEAR_Z = 0.387;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1157478987): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
