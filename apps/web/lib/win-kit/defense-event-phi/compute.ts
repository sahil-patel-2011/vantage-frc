/**
 * Leftover win-plus kit — defense-event-phi.
...
/**
 * Lovat-style Defense board — this event, Cody–Hart Φ.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-phi";
...
export const EWMA_ALPHA = 0.689;
export const TRIM_FRACTION = 0.246;
export const NEAR_Z = 0.396;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2688259784): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
