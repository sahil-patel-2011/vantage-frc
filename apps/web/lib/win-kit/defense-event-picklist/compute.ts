/**
 * Leftover win-plus kit — defense-event-picklist.
...
/**
 * Lovat-style Defense board — this event, blend picklist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-picklist";
...
export const EWMA_ALPHA = 0.676;
export const TRIM_FRACTION = 0.242;
export const NEAR_Z = 0.393;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4160979500): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
