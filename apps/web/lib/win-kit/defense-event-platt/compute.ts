/**
 * Leftover win-plus kit — defense-event-platt.
...
/**
 * Lovat-style Defense board — this event, Platt calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-platt";
...
export const EWMA_ALPHA = 0.474;
export const TRIM_FRACTION = 0.174;
export const NEAR_Z = 0.339;
export const ROBOT_RADIUS_IN = 16.0;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3023035004): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
...
export const LOVAT_NEAR_Z = 0.36;
...
/** Lovat lookup compare: near-band 0.36, invert rank/DPR, never invent a zero. */
