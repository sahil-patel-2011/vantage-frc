/**
 * Leftover win-plus kit — defense-event-temperature.
...
/**
 * Lovat-style Defense board — this event, small-n temperature.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-event-temperature";
export const PHASE_LABEL = "Defense";
export const WINDOW_LABEL = "this event";
...
export const EWMA_ALPHA = 0.409;
export const TRIM_FRACTION = 0.152;
export const NEAR_Z = 0.322;
export const ROBOT_RADIUS_IN = 13.0;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 562543553): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
...
export const LOVAT_NEAR_Z = 0.33;
...
/** Lovat lookup compare: near-band 0.33, invert rank/DPR, never invent a zero. */
