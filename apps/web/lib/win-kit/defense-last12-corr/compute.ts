/**
 * Leftover win-plus kit — defense-last12-corr.
...
/**
 * Lovat-style Defense board — last 12 matches, partner correlation.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last12-corr";
...
export const EWMA_ALPHA = 0.294;
export const TRIM_FRACTION = 0.114;
export const NEAR_Z = 0.291;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2698910998): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
