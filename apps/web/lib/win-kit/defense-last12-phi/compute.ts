/**
 * Leftover win-plus kit — defense-last12-phi.
...
/**
 * Lovat-style Defense board — last 12 matches, Cody–Hart Φ.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last12-phi";
export const PHASE_LABEL = "Defense";
export const WINDOW_LABEL = "last 12 matches";
export const KERNEL_LABEL = "Cody–Hart Φ";
export const WINDOW_N: number | null = 12;
export const QUALS_ONLY = false;
export const PLAYOFFS_ONLY = false;
export const EWMA_ALPHA = 0.479;
export const TRIM_FRACTION = 0.176;
export const NEAR_Z = 0.341;
export const ROBOT_RADIUS_IN = 15.0;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2935625123): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
...
export const LOVAT_NEAR_Z = 0.35;
...
/** Lovat lookup compare: near-band 0.35, invert rank/DPR, never invent a zero. */
