/**
 * Leftover win-plus kit — defense-last12-picklist.
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4080386389): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
...
export const LOVAT_NEAR_Z = 0.37;
...
/** Lovat lookup compare: near-band 0.37, invert rank/DPR, never invent a zero. */
