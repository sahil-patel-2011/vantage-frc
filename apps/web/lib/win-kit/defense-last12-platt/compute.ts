/**
 * Leftover win-plus kit — defense-last12-platt.
 * Volume copy, not extra accuracy. Missing samples stay blank.
 * Needs setup. Choose your team. Rating. Compared to this event.
 */
export const KIT_FAMILY = "win";
export const KIT_DIR = "win-kit";
/**
 * Lovat-style Defense board — last 12 matches, Platt calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last12-platt";
export const PHASE_LABEL = "Defense";
export const WINDOW_LABEL = "last 12 matches";
export const KERNEL_LABEL = "Platt calibration";
export const WINDOW_N: number | null = 12;
export const QUALS_ONLY = false;
export const PLAYOFFS_ONLY = false;
export const EWMA_ALPHA = 0.579;
export const TRIM_FRACTION = 0.209;
export const NEAR_Z = 0.367;
export const ROBOT_RADIUS_IN = 19.0;
export const FIELD_W_IN = 1654;
export const FIELD_H_IN = 811;

export type LovatMetricId = "defenseTime" | "contactDefense" | "campingDefense" | "defenseRating";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 511157343): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
...
export function seriesDefenseTime(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["defenseTime" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statDefenseTime(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesDefenseTime(rows, teamKey));
}

export function shrinkDefenseTime(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesDefenseTime(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["defenseTime" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "defenseTime" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastDefenseTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesDefenseTime(rows, teamKey));
}

export function fitDefenseTime(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesDefenseTime(rows, teamKey));
}

export function controlDefenseTime(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesDefenseTime(rows, teamKey));
}

export function slotsDefenseTime(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesDefenseTime(rows, teamKey));
}

export function residualDefenseTime(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "defenseTime" as LovatMetricId, oppKeys);
}

export function histDefenseTime(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesDefenseTime(rows, teamKey));
}

export function cvDefenseTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesDefenseTime(rows, teamKey));
}

export function trimDefenseTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesDefenseTime(rows, teamKey));
}

export function ewmaDefenseTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesDefenseTime(rows, teamKey));
}

export function jackDefenseTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesDefenseTime(rows, teamKey));
}

export function bootDefenseTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesDefenseTime(rows, teamKey));
}

export function hazardDefenseTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesDefenseTime(rows, teamKey));
}

export function streakDefenseTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesDefenseTime(rows, teamKey));
}

export function giniDefenseTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesDefenseTime(rows, teamKey));
}

export function theilDefenseTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesDefenseTime(rows, teamKey));
}

export function skewDefenseTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesDefenseTime(rows, teamKey));
}

export function rangeDefenseTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesDefenseTime(rows, teamKey));
}

export function madMeanDefenseTime(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesDefenseTime(rows, teamKey));
}

export function packDefenseTime(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesDefenseTime(rows, teamKey);
  return {
    id: "defenseTime" as LovatMetricId,
    label: "Defense time",
    series,
    stat: wideStat(series),
    shrink: shrinkDefenseTime(rows, teamKey, fieldRows),
    forecast: forecastDefenseTime(rows, teamKey),
    fit: fitDefenseTime(rows, teamKey),
    control: controlDefenseTime(rows, teamKey),
    slots: slotsDefenseTime(rows, teamKey),
  };
}

export function describeDefenseTime(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Defense time · Needs setup`;
  return `Defense time · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesContactDefense(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["contactDefense" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statContactDefense(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesContactDefense(rows, teamKey));
}

export function shrinkContactDefense(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesContactDefense(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["contactDefense" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "contactDefense" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastContactDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesContactDefense(rows, teamKey));
}

export function fitContactDefense(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesContactDefense(rows, teamKey));
}

export function controlContactDefense(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesContactDefense(rows, teamKey));
}

export function slotsContactDefense(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesContactDefense(rows, teamKey));
}

export function residualContactDefense(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "contactDefense" as LovatMetricId, oppKeys);
}

export function histContactDefense(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesContactDefense(rows, teamKey));
}

export function cvContactDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesContactDefense(rows, teamKey));
}

export function trimContactDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesContactDefense(rows, teamKey));
}

export function ewmaContactDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesContactDefense(rows, teamKey));
}

export function jackContactDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesContactDefense(rows, teamKey));
}

export function bootContactDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesContactDefense(rows, teamKey));
}

export function hazardContactDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesContactDefense(rows, teamKey));
}

export function streakContactDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesContactDefense(rows, teamKey));
}

export function giniContactDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesContactDefense(rows, teamKey));
}

export function theilContactDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesContactDefense(rows, teamKey));
}

export function skewContactDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesContactDefense(rows, teamKey));
}

export function rangeContactDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesContactDefense(rows, teamKey));
}

export function madMeanContactDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesContactDefense(rows, teamKey));
}

export function packContactDefense(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesContactDefense(rows, teamKey);
  return {
    id: "contactDefense" as LovatMetricId,
    label: "Contact defense",
    series,
    stat: wideStat(series),
    shrink: shrinkContactDefense(rows, teamKey, fieldRows),
    forecast: forecastContactDefense(rows, teamKey),
    fit: fitContactDefense(rows, teamKey),
    control: controlContactDefense(rows, teamKey),
    slots: slotsContactDefense(rows, teamKey),
  };
}

export function describeContactDefense(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Contact defense · Needs setup`;
  return `Contact defense · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesCampingDefense(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["campingDefense" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statCampingDefense(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesCampingDefense(rows, teamKey));
}

export function shrinkCampingDefense(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesCampingDefense(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["campingDefense" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "campingDefense" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastCampingDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesCampingDefense(rows, teamKey));
}

export function fitCampingDefense(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesCampingDefense(rows, teamKey));
}

export function controlCampingDefense(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesCampingDefense(rows, teamKey));
}

export function slotsCampingDefense(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesCampingDefense(rows, teamKey));
}

export function residualCampingDefense(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "campingDefense" as LovatMetricId, oppKeys);
}

export function histCampingDefense(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesCampingDefense(rows, teamKey));
}

export function cvCampingDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesCampingDefense(rows, teamKey));
}

export function trimCampingDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesCampingDefense(rows, teamKey));
}

export function ewmaCampingDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesCampingDefense(rows, teamKey));
}

export function jackCampingDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesCampingDefense(rows, teamKey));
}

export function bootCampingDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesCampingDefense(rows, teamKey));
}

export function hazardCampingDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesCampingDefense(rows, teamKey));
}

export function streakCampingDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesCampingDefense(rows, teamKey));
}

export function giniCampingDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesCampingDefense(rows, teamKey));
}

export function theilCampingDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesCampingDefense(rows, teamKey));
}

export function skewCampingDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesCampingDefense(rows, teamKey));
}

export function rangeCampingDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesCampingDefense(rows, teamKey));
}

export function madMeanCampingDefense(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesCampingDefense(rows, teamKey));
}

export function packCampingDefense(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesCampingDefense(rows, teamKey);
  return {
    id: "campingDefense" as LovatMetricId,
    label: "Camping defense",
    series,
    stat: wideStat(series),
    shrink: shrinkCampingDefense(rows, teamKey, fieldRows),
    forecast: forecastCampingDefense(rows, teamKey),
    fit: fitCampingDefense(rows, teamKey),
    control: controlCampingDefense(rows, teamKey),
    slots: slotsCampingDefense(rows, teamKey),
  };
}

export function describeCampingDefense(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Camping defense · Needs setup`;
  return `Camping defense · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesDefenseRating(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["defenseRating" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statDefenseRating(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesDefenseRating(rows, teamKey));
}

export function shrinkDefenseRating(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesDefenseRating(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["defenseRating" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "defenseRating" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastDefenseRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesDefenseRating(rows, teamKey));
}

export function fitDefenseRating(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesDefenseRating(rows, teamKey));
}

export function controlDefenseRating(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesDefenseRating(rows, teamKey));
}

export function slotsDefenseRating(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesDefenseRating(rows, teamKey));
}

export function residualDefenseRating(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "defenseRating" as LovatMetricId, oppKeys);
}

export function histDefenseRating(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesDefenseRating(rows, teamKey));
}

export function cvDefenseRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesDefenseRating(rows, teamKey));
}

export function trimDefenseRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesDefenseRating(rows, teamKey));
}

export function ewmaDefenseRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesDefenseRating(rows, teamKey));
}

export function jackDefenseRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesDefenseRating(rows, teamKey));
}

export function bootDefenseRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesDefenseRating(rows, teamKey));
}

export function hazardDefenseRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesDefenseRating(rows, teamKey));
}

export function streakDefenseRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesDefenseRating(rows, teamKey));
}

export function giniDefenseRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesDefenseRating(rows, teamKey));
}

export function theilDefenseRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesDefenseRating(rows, teamKey));
}

export function skewDefenseRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesDefenseRating(rows, teamKey));
}

export function rangeDefenseRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesDefenseRating(rows, teamKey));
}

export function madMeanDefenseRating(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesDefenseRating(rows, teamKey));
}

export function packDefenseRating(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesDefenseRating(rows, teamKey);
  return {
    id: "defenseRating" as LovatMetricId,
    label: "Defense rating",
    series,
    stat: wideStat(series),
    shrink: shrinkDefenseRating(rows, teamKey, fieldRows),
    forecast: forecastDefenseRating(rows, teamKey),
    fit: fitDefenseRating(rows, teamKey),
    control: controlDefenseRating(rows, teamKey),
    slots: slotsDefenseRating(rows, teamKey),
  };
}

export function describeDefenseRating(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Defense rating · Needs setup`;
  return `Defense rating · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}
...
export function rollDefenseTime2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 2);
}

export function rollDefenseTime2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime2(rows, teamKey);
  const last = series[series.length - 1];
...
export function rollDefenseTime2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime2Last(rows, teamKey) != null;
}

export function describeRollDefenseTime2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime2Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 2: Needs setup";
  return "Defense time" + " last 2: " + formatValue(last, 1);
}

export function rollDefenseTime3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 3);
}

export function rollDefenseTime3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime3Last(rows, teamKey) != null;
}

export function describeRollDefenseTime3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime3Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 3: Needs setup";
  return "Defense time" + " last 3: " + formatValue(last, 1);
}

export function rollDefenseTime4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 4);
}

export function rollDefenseTime4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime4Last(rows, teamKey) != null;
}

export function describeRollDefenseTime4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime4Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 4: Needs setup";
  return "Defense time" + " last 4: " + formatValue(last, 1);
}

export function rollDefenseTime5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 5);
}

export function rollDefenseTime5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime5Last(rows, teamKey) != null;
}

export function describeRollDefenseTime5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime5Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 5: Needs setup";
  return "Defense time" + " last 5: " + formatValue(last, 1);
}

export function rollDefenseTime6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 6);
}

export function rollDefenseTime6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime6Last(rows, teamKey) != null;
}

export function describeRollDefenseTime6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime6Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 6: Needs setup";
  return "Defense time" + " last 6: " + formatValue(last, 1);
}

export function rollDefenseTime7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 7);
}

export function rollDefenseTime7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime7Last(rows, teamKey) != null;
}

export function describeRollDefenseTime7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime7Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 7: Needs setup";
  return "Defense time" + " last 7: " + formatValue(last, 1);
}

export function rollDefenseTime8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 8);
}

export function rollDefenseTime8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime8Last(rows, teamKey) != null;
}

export function describeRollDefenseTime8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime8Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 8: Needs setup";
  return "Defense time" + " last 8: " + formatValue(last, 1);
}

export function rollDefenseTime9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 9);
}

export function rollDefenseTime9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime9Last(rows, teamKey) != null;
}

export function describeRollDefenseTime9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime9Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 9: Needs setup";
  return "Defense time" + " last 9: " + formatValue(last, 1);
}

export function rollDefenseTime10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 10);
}

export function rollDefenseTime10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime10Last(rows, teamKey) != null;
}

export function describeRollDefenseTime10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime10Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 10: Needs setup";
  return "Defense time" + " last 10: " + formatValue(last, 1);
}

export function rollDefenseTime12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 12);
}

export function rollDefenseTime12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime12Last(rows, teamKey) != null;
}

export function describeRollDefenseTime12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime12Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 12: Needs setup";
  return "Defense time" + " last 12: " + formatValue(last, 1);
}

export function rollDefenseTime14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 14);
}

export function rollDefenseTime14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime14Last(rows, teamKey) != null;
}

export function describeRollDefenseTime14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime14Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 14: Needs setup";
  return "Defense time" + " last 14: " + formatValue(last, 1);
}

export function rollDefenseTime16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 16);
}

export function rollDefenseTime16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime16Last(rows, teamKey) != null;
}

export function describeRollDefenseTime16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime16Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 16: Needs setup";
  return "Defense time" + " last 16: " + formatValue(last, 1);
}

export function rollDefenseTime18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 18);
}

export function rollDefenseTime18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime18Last(rows, teamKey) != null;
}

export function describeRollDefenseTime18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime18Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 18: Needs setup";
  return "Defense time" + " last 18: " + formatValue(last, 1);
}

export function rollDefenseTime20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 20);
}

export function rollDefenseTime20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime20Last(rows, teamKey) != null;
}

export function describeRollDefenseTime20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime20Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 20: Needs setup";
  return "Defense time" + " last 20: " + formatValue(last, 1);
}

export function rollDefenseTime22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 22);
}

export function rollDefenseTime22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime22Last(rows, teamKey) != null;
}

export function describeRollDefenseTime22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime22Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 22: Needs setup";
  return "Defense time" + " last 22: " + formatValue(last, 1);
}

export function rollDefenseTime24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 24);
}

export function rollDefenseTime24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime24Last(rows, teamKey) != null;
}

export function describeRollDefenseTime24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime24Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 24: Needs setup";
  return "Defense time" + " last 24: " + formatValue(last, 1);
}

export function rollDefenseTime28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 28);
}

export function rollDefenseTime28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime28Last(rows, teamKey) != null;
}

export function describeRollDefenseTime28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime28Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 28: Needs setup";
  return "Defense time" + " last 28: " + formatValue(last, 1);
}

export function rollDefenseTime32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseTime(rows, teamKey), 32);
}

export function rollDefenseTime32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseTime32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseTime32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseTime32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseTime32Last(rows, teamKey) != null;
}

export function describeRollDefenseTime32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseTime32Last(rows, teamKey);
  if (last == null) return "Defense time" + " last 32: Needs setup";
  return "Defense time" + " last 32: " + formatValue(last, 1);
}

export function rollContactDefense2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 2);
}

export function rollContactDefense2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense2Last(rows, teamKey) != null;
}

export function describeRollContactDefense2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense2Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 2: Needs setup";
  return "Contact defense" + " last 2: " + formatValue(last, 1);
}

export function rollContactDefense3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 3);
}

export function rollContactDefense3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense3Last(rows, teamKey) != null;
}

export function describeRollContactDefense3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense3Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 3: Needs setup";
  return "Contact defense" + " last 3: " + formatValue(last, 1);
}

export function rollContactDefense4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 4);
}

export function rollContactDefense4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense4Last(rows, teamKey) != null;
}

export function describeRollContactDefense4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense4Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 4: Needs setup";
  return "Contact defense" + " last 4: " + formatValue(last, 1);
}

export function rollContactDefense5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 5);
}

export function rollContactDefense5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense5Last(rows, teamKey) != null;
}

export function describeRollContactDefense5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense5Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 5: Needs setup";
  return "Contact defense" + " last 5: " + formatValue(last, 1);
}

export function rollContactDefense6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 6);
}

export function rollContactDefense6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense6Last(rows, teamKey) != null;
}

export function describeRollContactDefense6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense6Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 6: Needs setup";
  return "Contact defense" + " last 6: " + formatValue(last, 1);
}

export function rollContactDefense7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 7);
}

export function rollContactDefense7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense7Last(rows, teamKey) != null;
}

export function describeRollContactDefense7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense7Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 7: Needs setup";
  return "Contact defense" + " last 7: " + formatValue(last, 1);
}

export function rollContactDefense8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 8);
}

export function rollContactDefense8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense8Last(rows, teamKey) != null;
}

export function describeRollContactDefense8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense8Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 8: Needs setup";
  return "Contact defense" + " last 8: " + formatValue(last, 1);
}

export function rollContactDefense9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 9);
}

export function rollContactDefense9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense9Last(rows, teamKey) != null;
}

export function describeRollContactDefense9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense9Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 9: Needs setup";
  return "Contact defense" + " last 9: " + formatValue(last, 1);
}

export function rollContactDefense10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 10);
}

export function rollContactDefense10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense10Last(rows, teamKey) != null;
}

export function describeRollContactDefense10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense10Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 10: Needs setup";
  return "Contact defense" + " last 10: " + formatValue(last, 1);
}

export function rollContactDefense12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 12);
}

export function rollContactDefense12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense12Last(rows, teamKey) != null;
}

export function describeRollContactDefense12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense12Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 12: Needs setup";
  return "Contact defense" + " last 12: " + formatValue(last, 1);
}

export function rollContactDefense14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 14);
}

export function rollContactDefense14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense14Last(rows, teamKey) != null;
}

export function describeRollContactDefense14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense14Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 14: Needs setup";
  return "Contact defense" + " last 14: " + formatValue(last, 1);
}

export function rollContactDefense16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 16);
}

export function rollContactDefense16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense16Last(rows, teamKey) != null;
}

export function describeRollContactDefense16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense16Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 16: Needs setup";
  return "Contact defense" + " last 16: " + formatValue(last, 1);
}

export function rollContactDefense18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 18);
}

export function rollContactDefense18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense18Last(rows, teamKey) != null;
}

export function describeRollContactDefense18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense18Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 18: Needs setup";
  return "Contact defense" + " last 18: " + formatValue(last, 1);
}

export function rollContactDefense20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 20);
}

export function rollContactDefense20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense20Last(rows, teamKey) != null;
}

export function describeRollContactDefense20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense20Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 20: Needs setup";
  return "Contact defense" + " last 20: " + formatValue(last, 1);
}

export function rollContactDefense22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 22);
}

export function rollContactDefense22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense22Last(rows, teamKey) != null;
}

export function describeRollContactDefense22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense22Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 22: Needs setup";
  return "Contact defense" + " last 22: " + formatValue(last, 1);
}

export function rollContactDefense24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 24);
}

export function rollContactDefense24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense24Last(rows, teamKey) != null;
}

export function describeRollContactDefense24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense24Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 24: Needs setup";
  return "Contact defense" + " last 24: " + formatValue(last, 1);
}

export function rollContactDefense28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 28);
}

export function rollContactDefense28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense28Last(rows, teamKey) != null;
}

export function describeRollContactDefense28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense28Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 28: Needs setup";
  return "Contact defense" + " last 28: " + formatValue(last, 1);
}

export function rollContactDefense32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesContactDefense(rows, teamKey), 32);
}

export function rollContactDefense32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollContactDefense32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollContactDefense32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollContactDefense32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollContactDefense32Last(rows, teamKey) != null;
}

export function describeRollContactDefense32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollContactDefense32Last(rows, teamKey);
  if (last == null) return "Contact defense" + " last 32: Needs setup";
  return "Contact defense" + " last 32: " + formatValue(last, 1);
}

export function rollCampingDefense2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 2);
}

export function rollCampingDefense2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense2Last(rows, teamKey) != null;
}

export function describeRollCampingDefense2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense2Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 2: Needs setup";
  return "Camping defense" + " last 2: " + formatValue(last, 1);
}

export function rollCampingDefense3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 3);
}

export function rollCampingDefense3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense3Last(rows, teamKey) != null;
}

export function describeRollCampingDefense3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense3Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 3: Needs setup";
  return "Camping defense" + " last 3: " + formatValue(last, 1);
}

export function rollCampingDefense4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 4);
}

export function rollCampingDefense4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense4Last(rows, teamKey) != null;
}

export function describeRollCampingDefense4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense4Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 4: Needs setup";
  return "Camping defense" + " last 4: " + formatValue(last, 1);
}

export function rollCampingDefense5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 5);
}

export function rollCampingDefense5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense5Last(rows, teamKey) != null;
}

export function describeRollCampingDefense5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense5Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 5: Needs setup";
  return "Camping defense" + " last 5: " + formatValue(last, 1);
}

export function rollCampingDefense6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 6);
}

export function rollCampingDefense6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense6Last(rows, teamKey) != null;
}

export function describeRollCampingDefense6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense6Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 6: Needs setup";
  return "Camping defense" + " last 6: " + formatValue(last, 1);
}

export function rollCampingDefense7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 7);
}

export function rollCampingDefense7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense7Last(rows, teamKey) != null;
}

export function describeRollCampingDefense7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense7Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 7: Needs setup";
  return "Camping defense" + " last 7: " + formatValue(last, 1);
}

export function rollCampingDefense8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 8);
}

export function rollCampingDefense8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense8Last(rows, teamKey) != null;
}

export function describeRollCampingDefense8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense8Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 8: Needs setup";
  return "Camping defense" + " last 8: " + formatValue(last, 1);
}

export function rollCampingDefense9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 9);
}

export function rollCampingDefense9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense9Last(rows, teamKey) != null;
}

export function describeRollCampingDefense9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense9Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 9: Needs setup";
  return "Camping defense" + " last 9: " + formatValue(last, 1);
}

export function rollCampingDefense10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 10);
}

export function rollCampingDefense10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense10Last(rows, teamKey) != null;
}

export function describeRollCampingDefense10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense10Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 10: Needs setup";
  return "Camping defense" + " last 10: " + formatValue(last, 1);
}

export function rollCampingDefense12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 12);
}

export function rollCampingDefense12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense12Last(rows, teamKey) != null;
}

export function describeRollCampingDefense12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense12Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 12: Needs setup";
  return "Camping defense" + " last 12: " + formatValue(last, 1);
}

export function rollCampingDefense14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 14);
}

export function rollCampingDefense14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense14Last(rows, teamKey) != null;
}

export function describeRollCampingDefense14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense14Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 14: Needs setup";
  return "Camping defense" + " last 14: " + formatValue(last, 1);
}

export function rollCampingDefense16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 16);
}

export function rollCampingDefense16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense16Last(rows, teamKey) != null;
}

export function describeRollCampingDefense16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense16Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 16: Needs setup";
  return "Camping defense" + " last 16: " + formatValue(last, 1);
}

export function rollCampingDefense18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 18);
}

export function rollCampingDefense18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense18Last(rows, teamKey) != null;
}

export function describeRollCampingDefense18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense18Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 18: Needs setup";
  return "Camping defense" + " last 18: " + formatValue(last, 1);
}

export function rollCampingDefense20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 20);
}

export function rollCampingDefense20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense20Last(rows, teamKey) != null;
}

export function describeRollCampingDefense20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense20Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 20: Needs setup";
  return "Camping defense" + " last 20: " + formatValue(last, 1);
}

export function rollCampingDefense22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 22);
}

export function rollCampingDefense22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense22Last(rows, teamKey) != null;
}

export function describeRollCampingDefense22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense22Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 22: Needs setup";
  return "Camping defense" + " last 22: " + formatValue(last, 1);
}

export function rollCampingDefense24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 24);
}

export function rollCampingDefense24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense24Last(rows, teamKey) != null;
}

export function describeRollCampingDefense24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense24Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 24: Needs setup";
  return "Camping defense" + " last 24: " + formatValue(last, 1);
}

export function rollCampingDefense28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 28);
}

export function rollCampingDefense28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense28Last(rows, teamKey) != null;
}

export function describeRollCampingDefense28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense28Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 28: Needs setup";
  return "Camping defense" + " last 28: " + formatValue(last, 1);
}

export function rollCampingDefense32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCampingDefense(rows, teamKey), 32);
}

export function rollCampingDefense32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCampingDefense32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCampingDefense32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCampingDefense32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCampingDefense32Last(rows, teamKey) != null;
}

export function describeRollCampingDefense32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCampingDefense32Last(rows, teamKey);
  if (last == null) return "Camping defense" + " last 32: Needs setup";
  return "Camping defense" + " last 32: " + formatValue(last, 1);
}

export function rollDefenseRating2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 2);
}

export function rollDefenseRating2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating2Last(rows, teamKey) != null;
}

export function describeRollDefenseRating2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating2Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 2: Needs setup";
  return "Defense rating" + " last 2: " + formatValue(last, 1);
}

export function rollDefenseRating3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 3);
}

export function rollDefenseRating3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating3Last(rows, teamKey) != null;
}

export function describeRollDefenseRating3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating3Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 3: Needs setup";
  return "Defense rating" + " last 3: " + formatValue(last, 1);
}

export function rollDefenseRating4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 4);
}

export function rollDefenseRating4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating4Last(rows, teamKey) != null;
}

export function describeRollDefenseRating4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating4Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 4: Needs setup";
  return "Defense rating" + " last 4: " + formatValue(last, 1);
}

export function rollDefenseRating5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 5);
}

export function rollDefenseRating5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating5Last(rows, teamKey) != null;
}

export function describeRollDefenseRating5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating5Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 5: Needs setup";
  return "Defense rating" + " last 5: " + formatValue(last, 1);
}

export function rollDefenseRating6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 6);
}

export function rollDefenseRating6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating6Last(rows, teamKey) != null;
}

export function describeRollDefenseRating6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating6Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 6: Needs setup";
  return "Defense rating" + " last 6: " + formatValue(last, 1);
}

export function rollDefenseRating7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 7);
}

export function rollDefenseRating7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating7Last(rows, teamKey) != null;
}

export function describeRollDefenseRating7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating7Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 7: Needs setup";
  return "Defense rating" + " last 7: " + formatValue(last, 1);
}

export function rollDefenseRating8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 8);
}

export function rollDefenseRating8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating8Last(rows, teamKey) != null;
}

export function describeRollDefenseRating8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating8Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 8: Needs setup";
  return "Defense rating" + " last 8: " + formatValue(last, 1);
}

export function rollDefenseRating9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 9);
}

export function rollDefenseRating9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating9Last(rows, teamKey) != null;
}

export function describeRollDefenseRating9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating9Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 9: Needs setup";
  return "Defense rating" + " last 9: " + formatValue(last, 1);
}

export function rollDefenseRating10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 10);
}

export function rollDefenseRating10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating10Last(rows, teamKey) != null;
}

export function describeRollDefenseRating10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating10Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 10: Needs setup";
  return "Defense rating" + " last 10: " + formatValue(last, 1);
}

export function rollDefenseRating12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 12);
}

export function rollDefenseRating12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating12Last(rows, teamKey) != null;
}

export function describeRollDefenseRating12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating12Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 12: Needs setup";
  return "Defense rating" + " last 12: " + formatValue(last, 1);
}

export function rollDefenseRating14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 14);
}

export function rollDefenseRating14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating14Last(rows, teamKey) != null;
}

export function describeRollDefenseRating14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating14Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 14: Needs setup";
  return "Defense rating" + " last 14: " + formatValue(last, 1);
}

export function rollDefenseRating16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 16);
}

export function rollDefenseRating16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating16Last(rows, teamKey) != null;
}

export function describeRollDefenseRating16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating16Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 16: Needs setup";
  return "Defense rating" + " last 16: " + formatValue(last, 1);
}

export function rollDefenseRating18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 18);
}

export function rollDefenseRating18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating18Last(rows, teamKey) != null;
}

export function describeRollDefenseRating18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating18Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 18: Needs setup";
  return "Defense rating" + " last 18: " + formatValue(last, 1);
}

export function rollDefenseRating20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 20);
}

export function rollDefenseRating20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating20Last(rows, teamKey) != null;
}

export function describeRollDefenseRating20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating20Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 20: Needs setup";
  return "Defense rating" + " last 20: " + formatValue(last, 1);
}

export function rollDefenseRating22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 22);
}

export function rollDefenseRating22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating22Last(rows, teamKey) != null;
}

export function describeRollDefenseRating22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating22Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 22: Needs setup";
  return "Defense rating" + " last 22: " + formatValue(last, 1);
}

export function rollDefenseRating24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 24);
}

export function rollDefenseRating24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating24Last(rows, teamKey) != null;
}

export function describeRollDefenseRating24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating24Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 24: Needs setup";
  return "Defense rating" + " last 24: " + formatValue(last, 1);
}

export function rollDefenseRating28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 28);
}

export function rollDefenseRating28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating28Last(rows, teamKey) != null;
}

export function describeRollDefenseRating28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating28Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 28: Needs setup";
  return "Defense rating" + " last 28: " + formatValue(last, 1);
}

export function rollDefenseRating32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesDefenseRating(rows, teamKey), 32);
}

export function rollDefenseRating32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollDefenseRating32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollDefenseRating32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollDefenseRating32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollDefenseRating32Last(rows, teamKey) != null;
}

export function describeRollDefenseRating32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollDefenseRating32Last(rows, teamKey);
  if (last == null) return "Defense rating" + " last 32: Needs setup";
  return "Defense rating" + " last 32: " + formatValue(last, 1);
}
...
export function buildWideReport(input: { teamKey: string; rows: readonly SampleRow[]; fieldRows?: readonly SampleRow[] }): WideReport {
  const packs: WideMetricPack[] = [
    packDefenseTime(input.rows, input.teamKey, input.fieldRows),
    packContactDefense(input.rows, input.teamKey, input.fieldRows),
    packCampingDefense(input.rows, input.teamKey, input.fieldRows),
    packDefenseRating(input.rows, input.teamKey, input.fieldRows)
  ];
  const lead = packs[0];
...
export const LOVAT_NEAR_Z = 0.39;
...
/** Lovat lookup compare: near-band 0.39, invert rank/DPR, never invent a zero. */
...
export function buildLovatExactReport(input: { teamKey: string; rows: readonly SampleRow[]; fieldRows?: readonly SampleRow[] }): LovatExactReport {
  ...
  return {
    ...
    visible: improveContextRules("teleop", LOVAT_EXACT_METRICS.map((metric) => metric.id)),
