/**
 * Leftover win-plus kit — defense-last12-temperature.
...
/**
 * Lovat-style Defense board — last 12 matches, small-n temperature.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last12-temperature";
export const PHASE_LABEL = "Defense";
export const WINDOW_LABEL = "last 12 matches";
export const KERNEL_LABEL = "small-n temperature";
export const WINDOW_N: number | null = 12;
export const QUALS_ONLY = false;
export const PLAYOFFS_ONLY = false;
export const EWMA_ALPHA = 0.612;
export const TRIM_FRACTION = 0.220;
export const NEAR_Z = 0.376;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 834548074): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
...
export const LOVAT_NEAR_Z = 0.34;
...
/** Lovat lookup compare: near-band 0.34, invert rank/DPR, never invent a zero. */
