/**
 * Leftover win-plus kit — defense-last6-corr.
...
/**
 * Lovat-style Defense board — last 6 matches, partner correlation.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-corr";
...
export const EWMA_ALPHA = 0.496;
export const TRIM_FRACTION = 0.182;
export const NEAR_Z = 0.345;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2564905489): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
