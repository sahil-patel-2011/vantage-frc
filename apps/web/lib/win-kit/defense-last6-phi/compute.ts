/**
 * Leftover win-plus kit — defense-last6-phi.
...
/**
 * Lovat-style Defense board — last 6 matches, Cody–Hart Φ.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-phi";
...
export const EWMA_ALPHA = 0.638;
export const TRIM_FRACTION = 0.229;
export const NEAR_Z = 0.383;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 131648642): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
