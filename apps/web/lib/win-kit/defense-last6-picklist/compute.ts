/**
 * Leftover win-plus kit — defense-last6-picklist.
...
/**
 * Lovat-style Defense board — last 6 matches, blend picklist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-picklist";
...
export const EWMA_ALPHA = 0.399;
export const TRIM_FRACTION = 0.149;
export const NEAR_Z = 0.319;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1196173342): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
