/**
 * Leftover win-plus kit — defense-last6-platt.
...
/**
 * Lovat-style Defense board — last 6 matches, Platt calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-platt";
...
export const EWMA_ALPHA = 0.310;
export const TRIM_FRACTION = 0.119;
export const NEAR_Z = 0.295;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2334451350): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
