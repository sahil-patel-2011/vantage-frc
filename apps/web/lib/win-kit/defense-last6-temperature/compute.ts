/**
 * Leftover win-plus kit — defense-last6-temperature.
...
/**
 * Lovat-style Defense board — last 6 matches, small-n temperature.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-last6-temperature";
...
export const EWMA_ALPHA = 0.407;
export const TRIM_FRACTION = 0.152;
export const NEAR_Z = 0.321;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 253373515): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
