/**
 * Leftover win-plus kit — defense-quals-corr.
...
/**
 * Lovat-style Defense board — qualification matches, partner correlation.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-corr";
...
export const EWMA_ALPHA = 0.384;
export const TRIM_FRACTION = 0.144;
export const NEAR_Z = 0.315;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3024853003): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
