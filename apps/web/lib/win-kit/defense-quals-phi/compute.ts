/**
 * Leftover win-plus kit — defense-quals-phi.
...
/**
 * Lovat-style Defense board — qualification matches, Cody–Hart Φ.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-phi";
...
export const EWMA_ALPHA = 0.403;
export const TRIM_FRACTION = 0.151;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2446263432): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
