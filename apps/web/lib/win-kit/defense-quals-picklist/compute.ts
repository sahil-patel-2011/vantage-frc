/**
 * Leftover win-plus kit — defense-quals-picklist.
...
/**
 * Lovat-style Defense board — qualification matches, blend picklist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-picklist";
...
export const EWMA_ALPHA = 0.401;
export const TRIM_FRACTION = 0.150;
export const NEAR_Z = 0.320;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2561073388): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
