/**
 * Leftover win-plus kit — defense-quals-platt.
...
/**
 * Lovat-style Defense board — qualification matches, Platt calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-platt";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 33558844): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
