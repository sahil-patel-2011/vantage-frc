/**
 * Leftover win-plus kit — defense-quals-temperature.
...
/**
 * Lovat-style Defense board — qualification matches, small-n temperature.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "defense-quals-temperature";
...
export const EWMA_ALPHA = 0.394;
export const TRIM_FRACTION = 0.148;
export const NEAR_Z = 0.318;
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 287183233): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
