/**
 * Leftover win-plus kit — driver-event-corr.
...
/**
 * Lovat-style Driver board — this event, partner correlation.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-event-corr";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1725365007): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
