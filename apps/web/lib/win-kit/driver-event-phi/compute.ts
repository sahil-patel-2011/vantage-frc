/**
 * Leftover win-plus kit — driver-event-phi.
...
/**
 * Lovat-style Driver board — this event, Cody–Hart Φ.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "driver-event-phi";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 529462540): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
