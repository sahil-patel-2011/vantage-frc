/**
 * Leftover win-plus kit — endgame-event-corr.
...
/**
 * Lovat-style Endgame board — this event, partner correlation.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-event-corr";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4234802806): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
