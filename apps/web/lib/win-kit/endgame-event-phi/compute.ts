/**
 * Leftover win-plus kit — endgame-event-phi.
...
/**
 * Lovat-style Endgame board — this event, Cody–Hart Φ.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-event-phi";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2606771843): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
