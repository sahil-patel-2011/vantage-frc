/**
 * Leftover win-plus kit — endgame-event-picklist.
...
/**
 * Lovat-style Endgame board — this event, blend picklist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-event-picklist";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 732296117): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
