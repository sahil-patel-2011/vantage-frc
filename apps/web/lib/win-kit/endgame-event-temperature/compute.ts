/**
 * Leftover win-plus kit — endgame-event-temperature.
...
/**
 * Lovat-style Endgame board — this event, small-n temperature.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-event-temperature";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1671095882): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
