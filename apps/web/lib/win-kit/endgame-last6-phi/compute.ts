/**
 * Leftover win-plus kit — endgame-last6-phi.
...
/**
 * Lovat-style Endgame board — last 6 matches, Cody–Hart Φ.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-last6-phi";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2415892185): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
