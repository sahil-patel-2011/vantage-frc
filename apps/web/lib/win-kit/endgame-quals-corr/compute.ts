/**
 * Leftover win-plus kit — endgame-quals-corr.
...
/**
 * Lovat-style Endgame board — qualification matches, partner correlation.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-quals-corr";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1647321298): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
