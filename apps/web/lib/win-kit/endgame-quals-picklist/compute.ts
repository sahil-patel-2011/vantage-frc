/**
 * Leftover win-plus kit — endgame-quals-picklist.
...
/**
 * Lovat-style Endgame board — qualification matches, blend picklist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-quals-picklist";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1395699849): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
