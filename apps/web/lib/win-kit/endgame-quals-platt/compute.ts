/**
 * Leftover win-plus kit — endgame-quals-platt.
...
/**
 * Lovat-style Endgame board — qualification matches, Platt calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "endgame-quals-platt";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2609582339): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
