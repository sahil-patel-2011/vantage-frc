/**
 * Leftover win-plus kit — fuel-event-corr.
...
/**
 * Lovat-style Fuel board — this event, partner correlation.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-event-corr";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2111039585): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
