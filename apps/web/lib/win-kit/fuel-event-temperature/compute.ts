/**
 * Leftover win-plus kit — fuel-event-temperature.
...
/**
 * Lovat-style Fuel board — this event, small-n temperature.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-event-temperature";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2286149755): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
