/**
 * Leftover win-plus kit — fuel-last12-temperature.
...
/**
 * Lovat-style Fuel board — last 12 matches, small-n temperature.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-last12-temperature";
export const PHASE_LABEL = "Fuel";
...
export type LovatMetricId = "throughput" | "fed" | "successRate" | "fuelScored";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2343831232): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
...
export function seriesThroughput(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["throughput" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statThroughput(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesThroughput(rows, teamKey));
}

export function shrinkThroughput(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesThroughput(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["throughput" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "throughput" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastThroughput(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesThroughput(rows, teamKey));
}

export function fitThroughput(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesThroughput(rows, teamKey));
}

export function controlThroughput(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesThroughput(rows, teamKey));
}

export function slotsThroughput(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesThroughput(rows, teamKey));
}

export function residualThroughput(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "throughput" as LovatMetricId, oppKeys);
}

export function histThroughput(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesThroughput(rows, teamKey));
}

export function cvThroughput(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesThroughput(rows, teamKey));
}

export function trimThroughput(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesThroughput(rows, teamKey));
}

export function ewmaThroughput(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesThroughput(rows, teamKey));
}

export function jackThroughput(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesThroughput(rows, teamKey));
}

export function bootThroughput(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesThroughput(rows, teamKey));
}

export function hazardThroughput(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesThroughput(rows, teamKey));
}

export function streakThroughput(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesThroughput(rows, teamKey));
}

export function giniThroughput(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesThroughput(rows, teamKey));
}

export function theilThroughput(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesThroughput(rows, teamKey));
}

export function skewThroughput(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesThroughput(rows, teamKey));
}

export function rangeThroughput(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesThroughput(rows, teamKey));
}

export function madMeanThroughput(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesThroughput(rows, teamKey));
}

export function packThroughput(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesThroughput(rows, teamKey);
  return {
    id: "throughput" as LovatMetricId,
    label: "Fuel throughput",
    series,
    stat: wideStat(series),
    shrink: shrinkThroughput(rows, teamKey, fieldRows),
    forecast: forecastThroughput(rows, teamKey),
    fit: fitThroughput(rows, teamKey),
    control: controlThroughput(rows, teamKey),
    slots: slotsThroughput(rows, teamKey),
  };
}

export function describeThroughput(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Fuel throughput · Needs setup`;
  return `Fuel throughput · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesFed(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["fed" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statFed(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesFed(rows, teamKey));
}

export function shrinkFed(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesFed(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["fed" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "fed" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastFed(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesFed(rows, teamKey));
}

export function fitFed(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesFed(rows, teamKey));
}

export function controlFed(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesFed(rows, teamKey));
}

export function slotsFed(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesFed(rows, teamKey));
}

export function residualFed(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "fed" as LovatMetricId, oppKeys);
}

export function histFed(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesFed(rows, teamKey));
}

export function cvFed(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesFed(rows, teamKey));
}

export function trimFed(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesFed(rows, teamKey));
}

export function ewmaFed(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesFed(rows, teamKey));
}

export function jackFed(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesFed(rows, teamKey));
}

export function bootFed(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesFed(rows, teamKey));
}

export function hazardFed(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesFed(rows, teamKey));
}

export function streakFed(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesFed(rows, teamKey));
}

export function giniFed(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesFed(rows, teamKey));
}

export function theilFed(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesFed(rows, teamKey));
}

export function skewFed(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesFed(rows, teamKey));
}

export function rangeFed(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesFed(rows, teamKey));
}

export function madMeanFed(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesFed(rows, teamKey));
}

export function packFed(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesFed(rows, teamKey);
  return {
    id: "fed" as LovatMetricId,
    label: "Fuel fed",
    series,
    stat: wideStat(series),
    shrink: shrinkFed(rows, teamKey, fieldRows),
    forecast: forecastFed(rows, teamKey),
    fit: fitFed(rows, teamKey),
    control: controlFed(rows, teamKey),
    slots: slotsFed(rows, teamKey),
  };
}

export function describeFed(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Fuel fed · Needs setup`;
  return `Fuel fed · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesSuccessRate(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["successRate" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statSuccessRate(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesSuccessRate(rows, teamKey));
}

export function shrinkSuccessRate(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesSuccessRate(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["successRate" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "successRate" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastSuccessRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesSuccessRate(rows, teamKey));
}

export function fitSuccessRate(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesSuccessRate(rows, teamKey));
}

export function controlSuccessRate(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesSuccessRate(rows, teamKey));
}

export function slotsSuccessRate(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesSuccessRate(rows, teamKey));
}

export function residualSuccessRate(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "successRate" as LovatMetricId, oppKeys);
}

export function histSuccessRate(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesSuccessRate(rows, teamKey));
}

export function cvSuccessRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesSuccessRate(rows, teamKey));
}

export function trimSuccessRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesSuccessRate(rows, teamKey));
}

export function ewmaSuccessRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesSuccessRate(rows, teamKey));
}

export function jackSuccessRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesSuccessRate(rows, teamKey));
}

export function bootSuccessRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesSuccessRate(rows, teamKey));
}

export function hazardSuccessRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesSuccessRate(rows, teamKey));
}

export function streakSuccessRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesSuccessRate(rows, teamKey));
}

export function giniSuccessRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesSuccessRate(rows, teamKey));
}

export function theilSuccessRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesSuccessRate(rows, teamKey));
}

export function skewSuccessRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesSuccessRate(rows, teamKey));
}

export function rangeSuccessRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesSuccessRate(rows, teamKey));
}

export function madMeanSuccessRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesSuccessRate(rows, teamKey));
}

export function packSuccessRate(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesSuccessRate(rows, teamKey);
  return {
    id: "successRate" as LovatMetricId,
    label: "Successful fuel rate",
    series,
    stat: wideStat(series),
    shrink: shrinkSuccessRate(rows, teamKey, fieldRows),
    forecast: forecastSuccessRate(rows, teamKey),
    fit: fitSuccessRate(rows, teamKey),
    control: controlSuccessRate(rows, teamKey),
    slots: slotsSuccessRate(rows, teamKey),
  };
}

export function describeSuccessRate(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Successful fuel rate · Needs setup`;
  return `Successful fuel rate · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesFuelScored(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["fuelScored" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statFuelScored(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesFuelScored(rows, teamKey));
}

export function shrinkFuelScored(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesFuelScored(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["fuelScored" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "fuelScored" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastFuelScored(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesFuelScored(rows, teamKey));
}

export function fitFuelScored(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesFuelScored(rows, teamKey));
}

export function controlFuelScored(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesFuelScored(rows, teamKey));
}

export function slotsFuelScored(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesFuelScored(rows, teamKey));
}

export function residualFuelScored(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "fuelScored" as LovatMetricId, oppKeys);
}

export function histFuelScored(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesFuelScored(rows, teamKey));
}

export function cvFuelScored(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesFuelScored(rows, teamKey));
}

export function trimFuelScored(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesFuelScored(rows, teamKey));
}

export function ewmaFuelScored(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesFuelScored(rows, teamKey));
}

export function jackFuelScored(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesFuelScored(rows, teamKey));
}

export function bootFuelScored(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesFuelScored(rows, teamKey));
}

export function hazardFuelScored(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesFuelScored(rows, teamKey));
}

export function streakFuelScored(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesFuelScored(rows, teamKey));
}

export function giniFuelScored(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesFuelScored(rows, teamKey));
}

export function theilFuelScored(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesFuelScored(rows, teamKey));
}

export function skewFuelScored(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesFuelScored(rows, teamKey));
}

export function rangeFuelScored(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesFuelScored(rows, teamKey));
}

export function madMeanFuelScored(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesFuelScored(rows, teamKey));
}

export function packFuelScored(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesFuelScored(rows, teamKey);
  return {
    id: "fuelScored" as LovatMetricId,
    label: "Fuel scored",
    series,
    stat: wideStat(series),
    shrink: shrinkFuelScored(rows, teamKey, fieldRows),
    forecast: forecastFuelScored(rows, teamKey),
    fit: fitFuelScored(rows, teamKey),
    control: controlFuelScored(rows, teamKey),
    slots: slotsFuelScored(rows, teamKey),
  };
}

export function describeFuelScored(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Fuel scored · Needs setup`;
  return `Fuel scored · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}
...
export function rollThroughput2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 2);
}

export function rollThroughput2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput2(rows, teamKey);
  const last = series[series.length - 1];
...
export function rollThroughput2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput2Last(rows, teamKey) != null;
}

export function describeRollThroughput2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput2Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 2: Needs setup";
  return "Fuel throughput" + " last 2: " + formatValue(last, 1);
}

export function rollThroughput3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 3);
}

export function rollThroughput3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput3Last(rows, teamKey) != null;
}

export function describeRollThroughput3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput3Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 3: Needs setup";
  return "Fuel throughput" + " last 3: " + formatValue(last, 1);
}

export function rollThroughput4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 4);
}

export function rollThroughput4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput4Last(rows, teamKey) != null;
}

export function describeRollThroughput4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput4Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 4: Needs setup";
  return "Fuel throughput" + " last 4: " + formatValue(last, 1);
}

export function rollThroughput5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 5);
}

export function rollThroughput5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput5Last(rows, teamKey) != null;
}

export function describeRollThroughput5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput5Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 5: Needs setup";
  return "Fuel throughput" + " last 5: " + formatValue(last, 1);
}

export function rollThroughput6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 6);
}

export function rollThroughput6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput6Last(rows, teamKey) != null;
}

export function describeRollThroughput6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput6Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 6: Needs setup";
  return "Fuel throughput" + " last 6: " + formatValue(last, 1);
}

export function rollThroughput7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 7);
}

export function rollThroughput7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput7Last(rows, teamKey) != null;
}

export function describeRollThroughput7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput7Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 7: Needs setup";
  return "Fuel throughput" + " last 7: " + formatValue(last, 1);
}

export function rollThroughput8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 8);
}

export function rollThroughput8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput8Last(rows, teamKey) != null;
}

export function describeRollThroughput8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput8Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 8: Needs setup";
  return "Fuel throughput" + " last 8: " + formatValue(last, 1);
}

export function rollThroughput9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 9);
}

export function rollThroughput9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput9Last(rows, teamKey) != null;
}

export function describeRollThroughput9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput9Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 9: Needs setup";
  return "Fuel throughput" + " last 9: " + formatValue(last, 1);
}

export function rollThroughput10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 10);
}

export function rollThroughput10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput10Last(rows, teamKey) != null;
}

export function describeRollThroughput10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput10Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 10: Needs setup";
  return "Fuel throughput" + " last 10: " + formatValue(last, 1);
}

export function rollThroughput12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 12);
}

export function rollThroughput12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput12Last(rows, teamKey) != null;
}

export function describeRollThroughput12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput12Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 12: Needs setup";
  return "Fuel throughput" + " last 12: " + formatValue(last, 1);
}

export function rollThroughput14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 14);
}

export function rollThroughput14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput14Last(rows, teamKey) != null;
}

export function describeRollThroughput14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput14Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 14: Needs setup";
  return "Fuel throughput" + " last 14: " + formatValue(last, 1);
}

export function rollThroughput16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 16);
}

export function rollThroughput16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput16Last(rows, teamKey) != null;
}

export function describeRollThroughput16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput16Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 16: Needs setup";
  return "Fuel throughput" + " last 16: " + formatValue(last, 1);
}

export function rollThroughput18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 18);
}

export function rollThroughput18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput18Last(rows, teamKey) != null;
}

export function describeRollThroughput18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput18Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 18: Needs setup";
  return "Fuel throughput" + " last 18: " + formatValue(last, 1);
}

export function rollThroughput20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 20);
}

export function rollThroughput20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput20Last(rows, teamKey) != null;
}

export function describeRollThroughput20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput20Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 20: Needs setup";
  return "Fuel throughput" + " last 20: " + formatValue(last, 1);
}

export function rollThroughput22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 22);
}

export function rollThroughput22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput22Last(rows, teamKey) != null;
}

export function describeRollThroughput22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput22Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 22: Needs setup";
  return "Fuel throughput" + " last 22: " + formatValue(last, 1);
}

export function rollThroughput24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 24);
}

export function rollThroughput24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput24Last(rows, teamKey) != null;
}

export function describeRollThroughput24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput24Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 24: Needs setup";
  return "Fuel throughput" + " last 24: " + formatValue(last, 1);
}

export function rollThroughput28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 28);
}

export function rollThroughput28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput28Last(rows, teamKey) != null;
}

export function describeRollThroughput28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput28Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 28: Needs setup";
  return "Fuel throughput" + " last 28: " + formatValue(last, 1);
}

export function rollThroughput32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesThroughput(rows, teamKey), 32);
}

export function rollThroughput32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollThroughput32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollThroughput32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollThroughput32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollThroughput32Last(rows, teamKey) != null;
}

export function describeRollThroughput32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollThroughput32Last(rows, teamKey);
  if (last == null) return "Fuel throughput" + " last 32: Needs setup";
  return "Fuel throughput" + " last 32: " + formatValue(last, 1);
}

export function rollFed2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 2);
}

export function rollFed2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed2Last(rows, teamKey) != null;
}

export function describeRollFed2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed2Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 2: Needs setup";
  return "Fuel fed" + " last 2: " + formatValue(last, 1);
}

export function rollFed3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 3);
}

export function rollFed3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed3Last(rows, teamKey) != null;
}

export function describeRollFed3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed3Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 3: Needs setup";
  return "Fuel fed" + " last 3: " + formatValue(last, 1);
}

export function rollFed4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 4);
}

export function rollFed4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed4Last(rows, teamKey) != null;
}

export function describeRollFed4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed4Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 4: Needs setup";
  return "Fuel fed" + " last 4: " + formatValue(last, 1);
}

export function rollFed5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 5);
}

export function rollFed5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed5Last(rows, teamKey) != null;
}

export function describeRollFed5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed5Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 5: Needs setup";
  return "Fuel fed" + " last 5: " + formatValue(last, 1);
}

export function rollFed6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 6);
}

export function rollFed6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed6Last(rows, teamKey) != null;
}

export function describeRollFed6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed6Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 6: Needs setup";
  return "Fuel fed" + " last 6: " + formatValue(last, 1);
}

export function rollFed7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 7);
}

export function rollFed7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed7Last(rows, teamKey) != null;
}

export function describeRollFed7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed7Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 7: Needs setup";
  return "Fuel fed" + " last 7: " + formatValue(last, 1);
}

export function rollFed8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 8);
}

export function rollFed8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed8Last(rows, teamKey) != null;
}

export function describeRollFed8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed8Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 8: Needs setup";
  return "Fuel fed" + " last 8: " + formatValue(last, 1);
}

export function rollFed9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 9);
}

export function rollFed9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed9Last(rows, teamKey) != null;
}

export function describeRollFed9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed9Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 9: Needs setup";
  return "Fuel fed" + " last 9: " + formatValue(last, 1);
}

export function rollFed10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 10);
}

export function rollFed10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed10Last(rows, teamKey) != null;
}

export function describeRollFed10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed10Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 10: Needs setup";
  return "Fuel fed" + " last 10: " + formatValue(last, 1);
}

export function rollFed12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 12);
}

export function rollFed12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed12Last(rows, teamKey) != null;
}

export function describeRollFed12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed12Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 12: Needs setup";
  return "Fuel fed" + " last 12: " + formatValue(last, 1);
}

export function rollFed14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 14);
}

export function rollFed14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed14Last(rows, teamKey) != null;
}

export function describeRollFed14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed14Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 14: Needs setup";
  return "Fuel fed" + " last 14: " + formatValue(last, 1);
}

export function rollFed16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 16);
}

export function rollFed16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed16Last(rows, teamKey) != null;
}

export function describeRollFed16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed16Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 16: Needs setup";
  return "Fuel fed" + " last 16: " + formatValue(last, 1);
}

export function rollFed18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 18);
}

export function rollFed18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed18Last(rows, teamKey) != null;
}

export function describeRollFed18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed18Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 18: Needs setup";
  return "Fuel fed" + " last 18: " + formatValue(last, 1);
}

export function rollFed20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 20);
}

export function rollFed20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed20Last(rows, teamKey) != null;
}

export function describeRollFed20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed20Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 20: Needs setup";
  return "Fuel fed" + " last 20: " + formatValue(last, 1);
}

export function rollFed22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 22);
}

export function rollFed22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed22Last(rows, teamKey) != null;
}

export function describeRollFed22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed22Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 22: Needs setup";
  return "Fuel fed" + " last 22: " + formatValue(last, 1);
}

export function rollFed24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 24);
}

export function rollFed24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed24Last(rows, teamKey) != null;
}

export function describeRollFed24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed24Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 24: Needs setup";
  return "Fuel fed" + " last 24: " + formatValue(last, 1);
}

export function rollFed28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 28);
}

export function rollFed28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed28Last(rows, teamKey) != null;
}

export function describeRollFed28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed28Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 28: Needs setup";
  return "Fuel fed" + " last 28: " + formatValue(last, 1);
}

export function rollFed32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFed(rows, teamKey), 32);
}

export function rollFed32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFed32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFed32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFed32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFed32Last(rows, teamKey) != null;
}

export function describeRollFed32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFed32Last(rows, teamKey);
  if (last == null) return "Fuel fed" + " last 32: Needs setup";
  return "Fuel fed" + " last 32: " + formatValue(last, 1);
}

export function rollSuccessRate2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 2);
}

export function rollSuccessRate2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate2Last(rows, teamKey) != null;
}

export function describeRollSuccessRate2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate2Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 2: Needs setup";
  return "Successful fuel rate" + " last 2: " + formatValue(last, 1);
}

export function rollSuccessRate3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 3);
}

export function rollSuccessRate3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate3Last(rows, teamKey) != null;
}

export function describeRollSuccessRate3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate3Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 3: Needs setup";
  return "Successful fuel rate" + " last 3: " + formatValue(last, 1);
}

export function rollSuccessRate4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 4);
}

export function rollSuccessRate4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate4Last(rows, teamKey) != null;
}

export function describeRollSuccessRate4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate4Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 4: Needs setup";
  return "Successful fuel rate" + " last 4: " + formatValue(last, 1);
}

export function rollSuccessRate5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 5);
}

export function rollSuccessRate5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate5Last(rows, teamKey) != null;
}

export function describeRollSuccessRate5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate5Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 5: Needs setup";
  return "Successful fuel rate" + " last 5: " + formatValue(last, 1);
}

export function rollSuccessRate6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 6);
}

export function rollSuccessRate6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate6Last(rows, teamKey) != null;
}

export function describeRollSuccessRate6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate6Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 6: Needs setup";
  return "Successful fuel rate" + " last 6: " + formatValue(last, 1);
}

export function rollSuccessRate7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 7);
}

export function rollSuccessRate7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate7Last(rows, teamKey) != null;
}

export function describeRollSuccessRate7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate7Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 7: Needs setup";
  return "Successful fuel rate" + " last 7: " + formatValue(last, 1);
}

export function rollSuccessRate8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 8);
}

export function rollSuccessRate8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate8Last(rows, teamKey) != null;
}

export function describeRollSuccessRate8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate8Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 8: Needs setup";
  return "Successful fuel rate" + " last 8: " + formatValue(last, 1);
}

export function rollSuccessRate9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 9);
}

export function rollSuccessRate9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate9Last(rows, teamKey) != null;
}

export function describeRollSuccessRate9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate9Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 9: Needs setup";
  return "Successful fuel rate" + " last 9: " + formatValue(last, 1);
}

export function rollSuccessRate10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 10);
}

export function rollSuccessRate10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate10Last(rows, teamKey) != null;
}

export function describeRollSuccessRate10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate10Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 10: Needs setup";
  return "Successful fuel rate" + " last 10: " + formatValue(last, 1);
}

export function rollSuccessRate12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 12);
}

export function rollSuccessRate12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate12Last(rows, teamKey) != null;
}

export function describeRollSuccessRate12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate12Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 12: Needs setup";
  return "Successful fuel rate" + " last 12: " + formatValue(last, 1);
}

export function rollSuccessRate14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 14);
}

export function rollSuccessRate14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate14Last(rows, teamKey) != null;
}

export function describeRollSuccessRate14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate14Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 14: Needs setup";
  return "Successful fuel rate" + " last 14: " + formatValue(last, 1);
}

export function rollSuccessRate16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 16);
}

export function rollSuccessRate16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate16Last(rows, teamKey) != null;
}

export function describeRollSuccessRate16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate16Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 16: Needs setup";
  return "Successful fuel rate" + " last 16: " + formatValue(last, 1);
}

export function rollSuccessRate18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 18);
}

export function rollSuccessRate18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate18Last(rows, teamKey) != null;
}

export function describeRollSuccessRate18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate18Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 18: Needs setup";
  return "Successful fuel rate" + " last 18: " + formatValue(last, 1);
}

export function rollSuccessRate20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 20);
}

export function rollSuccessRate20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate20Last(rows, teamKey) != null;
}

export function describeRollSuccessRate20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate20Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 20: Needs setup";
  return "Successful fuel rate" + " last 20: " + formatValue(last, 1);
}

export function rollSuccessRate22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 22);
}

export function rollSuccessRate22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate22Last(rows, teamKey) != null;
}

export function describeRollSuccessRate22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate22Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 22: Needs setup";
  return "Successful fuel rate" + " last 22: " + formatValue(last, 1);
}

export function rollSuccessRate24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 24);
}

export function rollSuccessRate24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate24Last(rows, teamKey) != null;
}

export function describeRollSuccessRate24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate24Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 24: Needs setup";
  return "Successful fuel rate" + " last 24: " + formatValue(last, 1);
}

export function rollSuccessRate28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 28);
}

export function rollSuccessRate28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate28Last(rows, teamKey) != null;
}

export function describeRollSuccessRate28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate28Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 28: Needs setup";
  return "Successful fuel rate" + " last 28: " + formatValue(last, 1);
}

export function rollSuccessRate32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesSuccessRate(rows, teamKey), 32);
}

export function rollSuccessRate32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollSuccessRate32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollSuccessRate32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollSuccessRate32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollSuccessRate32Last(rows, teamKey) != null;
}

export function describeRollSuccessRate32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollSuccessRate32Last(rows, teamKey);
  if (last == null) return "Successful fuel rate" + " last 32: Needs setup";
  return "Successful fuel rate" + " last 32: " + formatValue(last, 1);
}

export function rollFuelScored2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 2);
}

export function rollFuelScored2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored2Last(rows, teamKey) != null;
}

export function describeRollFuelScored2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored2Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 2: Needs setup";
  return "Fuel scored" + " last 2: " + formatValue(last, 1);
}

export function rollFuelScored3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 3);
}

export function rollFuelScored3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored3Last(rows, teamKey) != null;
}

export function describeRollFuelScored3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored3Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 3: Needs setup";
  return "Fuel scored" + " last 3: " + formatValue(last, 1);
}

export function rollFuelScored4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 4);
}

export function rollFuelScored4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored4Last(rows, teamKey) != null;
}

export function describeRollFuelScored4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored4Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 4: Needs setup";
  return "Fuel scored" + " last 4: " + formatValue(last, 1);
}

export function rollFuelScored5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 5);
}

export function rollFuelScored5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored5Last(rows, teamKey) != null;
}

export function describeRollFuelScored5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored5Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 5: Needs setup";
  return "Fuel scored" + " last 5: " + formatValue(last, 1);
}

export function rollFuelScored6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 6);
}

export function rollFuelScored6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored6Last(rows, teamKey) != null;
}

export function describeRollFuelScored6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored6Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 6: Needs setup";
  return "Fuel scored" + " last 6: " + formatValue(last, 1);
}

export function rollFuelScored7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 7);
}

export function rollFuelScored7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored7Last(rows, teamKey) != null;
}

export function describeRollFuelScored7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored7Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 7: Needs setup";
  return "Fuel scored" + " last 7: " + formatValue(last, 1);
}

export function rollFuelScored8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 8);
}

export function rollFuelScored8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored8Last(rows, teamKey) != null;
}

export function describeRollFuelScored8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored8Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 8: Needs setup";
  return "Fuel scored" + " last 8: " + formatValue(last, 1);
}

export function rollFuelScored9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 9);
}

export function rollFuelScored9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored9Last(rows, teamKey) != null;
}

export function describeRollFuelScored9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored9Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 9: Needs setup";
  return "Fuel scored" + " last 9: " + formatValue(last, 1);
}

export function rollFuelScored10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 10);
}

export function rollFuelScored10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored10Last(rows, teamKey) != null;
}

export function describeRollFuelScored10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored10Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 10: Needs setup";
  return "Fuel scored" + " last 10: " + formatValue(last, 1);
}

export function rollFuelScored12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 12);
}

export function rollFuelScored12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored12Last(rows, teamKey) != null;
}

export function describeRollFuelScored12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored12Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 12: Needs setup";
  return "Fuel scored" + " last 12: " + formatValue(last, 1);
}

export function rollFuelScored14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 14);
}

export function rollFuelScored14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored14Last(rows, teamKey) != null;
}

export function describeRollFuelScored14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored14Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 14: Needs setup";
  return "Fuel scored" + " last 14: " + formatValue(last, 1);
}

export function rollFuelScored16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 16);
}

export function rollFuelScored16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored16Last(rows, teamKey) != null;
}

export function describeRollFuelScored16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored16Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 16: Needs setup";
  return "Fuel scored" + " last 16: " + formatValue(last, 1);
}

export function rollFuelScored18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 18);
}

export function rollFuelScored18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored18Last(rows, teamKey) != null;
}

export function describeRollFuelScored18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored18Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 18: Needs setup";
  return "Fuel scored" + " last 18: " + formatValue(last, 1);
}

export function rollFuelScored20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 20);
}

export function rollFuelScored20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored20Last(rows, teamKey) != null;
}

export function describeRollFuelScored20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored20Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 20: Needs setup";
  return "Fuel scored" + " last 20: " + formatValue(last, 1);
}

export function rollFuelScored22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 22);
}

export function rollFuelScored22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored22Last(rows, teamKey) != null;
}

export function describeRollFuelScored22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored22Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 22: Needs setup";
  return "Fuel scored" + " last 22: " + formatValue(last, 1);
}

export function rollFuelScored24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 24);
}

export function rollFuelScored24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored24Last(rows, teamKey) != null;
}

export function describeRollFuelScored24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored24Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 24: Needs setup";
  return "Fuel scored" + " last 24: " + formatValue(last, 1);
}

export function rollFuelScored28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 28);
}

export function rollFuelScored28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored28Last(rows, teamKey) != null;
}

export function describeRollFuelScored28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored28Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 28: Needs setup";
  return "Fuel scored" + " last 28: " + formatValue(last, 1);
}

export function rollFuelScored32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesFuelScored(rows, teamKey), 32);
}

export function rollFuelScored32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollFuelScored32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollFuelScored32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollFuelScored32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollFuelScored32Last(rows, teamKey) != null;
}

export function describeRollFuelScored32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollFuelScored32Last(rows, teamKey);
  if (last == null) return "Fuel scored" + " last 32: Needs setup";
  return "Fuel scored" + " last 32: " + formatValue(last, 1);
}
...
export function buildWideReport(input: { teamKey: string; rows: readonly SampleRow[]; fieldRows?: readonly SampleRow[] }): WideReport {
  const packs: WideMetricPack[] = [
    packThroughput(input.rows, input.teamKey, input.fieldRows),
    packFed(input.rows, input.teamKey, input.fieldRows),
    packSuccessRate(input.rows, input.teamKey, input.fieldRows),
    packFuelScored(input.rows, input.teamKey, input.fieldRows)
  ];
  const lead = packs[0];
