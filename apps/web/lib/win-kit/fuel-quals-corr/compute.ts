/**
 * Leftover win-plus kit — fuel-quals-corr.
...
/**
 * Lovat-style Fuel board — qualification matches, partner correlation.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-quals-corr";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 72752645): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
