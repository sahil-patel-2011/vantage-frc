/**
 * Leftover win-plus kit — fuel-quals-phi.
...
/**
 * Lovat-style Fuel board — qualification matches, Cody–Hart Φ.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "fuel-quals-phi";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1569177686): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
