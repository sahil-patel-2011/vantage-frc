/**
 * Leftover win-plus kit — teleop-event-corr.
...
/**
 * Lovat-style Teleop board — this event, partner correlation.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-event-corr";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1876418228): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
