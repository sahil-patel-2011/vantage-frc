/**
 * Leftover win-plus kit — teleop-event-phi.
...
/**
 * Lovat-style Teleop board — this event, Cody–Hart Φ.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-event-phi";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 1435055277): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
