/**
 * Leftover win-plus kit — teleop-last12-temperature.
...
/**
 * Lovat-style Teleop board — last 12 matches, small-n temperature.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-last12-temperature";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 3542168177): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
...
export function seriesTeleopPoints(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["teleopPoints" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statTeleopPoints(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesTeleopPoints(rows, teamKey));
}

export function shrinkTeleopPoints(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesTeleopPoints(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["teleopPoints" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "teleopPoints" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastTeleopPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesTeleopPoints(rows, teamKey));
}

export function fitTeleopPoints(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesTeleopPoints(rows, teamKey));
}

export function controlTeleopPoints(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesTeleopPoints(rows, teamKey));
}

export function slotsTeleopPoints(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesTeleopPoints(rows, teamKey));
}

export function residualTeleopPoints(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "teleopPoints" as LovatMetricId, oppKeys);
}

export function histTeleopPoints(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesTeleopPoints(rows, teamKey));
}

export function cvTeleopPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesTeleopPoints(rows, teamKey));
}

export function trimTeleopPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesTeleopPoints(rows, teamKey));
}

export function ewmaTeleopPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesTeleopPoints(rows, teamKey));
}

export function jackTeleopPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesTeleopPoints(rows, teamKey));
}

export function bootTeleopPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesTeleopPoints(rows, teamKey));
}

export function hazardTeleopPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesTeleopPoints(rows, teamKey));
}

export function streakTeleopPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesTeleopPoints(rows, teamKey));
}

export function giniTeleopPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesTeleopPoints(rows, teamKey));
}

export function theilTeleopPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesTeleopPoints(rows, teamKey));
}

export function skewTeleopPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesTeleopPoints(rows, teamKey));
}

export function rangeTeleopPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesTeleopPoints(rows, teamKey));
}

export function madMeanTeleopPoints(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesTeleopPoints(rows, teamKey));
}

export function packTeleopPoints(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesTeleopPoints(rows, teamKey);
  return {
    id: "teleopPoints" as LovatMetricId,
    label: "Teleop points",
    series,
    stat: wideStat(series),
    shrink: shrinkTeleopPoints(rows, teamKey, fieldRows),
    forecast: forecastTeleopPoints(rows, teamKey),
    fit: fitTeleopPoints(rows, teamKey),
    control: controlTeleopPoints(rows, teamKey),
    slots: slotsTeleopPoints(rows, teamKey),
  };
}

export function describeTeleopPoints(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Teleop points · Needs setup`;
  return `Teleop points · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesTeleopFuel(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["teleopFuel" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statTeleopFuel(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesTeleopFuel(rows, teamKey));
}

export function shrinkTeleopFuel(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesTeleopFuel(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["teleopFuel" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "teleopFuel" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastTeleopFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesTeleopFuel(rows, teamKey));
}

export function fitTeleopFuel(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesTeleopFuel(rows, teamKey));
}

export function controlTeleopFuel(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesTeleopFuel(rows, teamKey));
}

export function slotsTeleopFuel(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesTeleopFuel(rows, teamKey));
}

export function residualTeleopFuel(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "teleopFuel" as LovatMetricId, oppKeys);
}

export function histTeleopFuel(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesTeleopFuel(rows, teamKey));
}

export function cvTeleopFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesTeleopFuel(rows, teamKey));
}

export function trimTeleopFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesTeleopFuel(rows, teamKey));
}

export function ewmaTeleopFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesTeleopFuel(rows, teamKey));
}

export function jackTeleopFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesTeleopFuel(rows, teamKey));
}

export function bootTeleopFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesTeleopFuel(rows, teamKey));
}

export function hazardTeleopFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesTeleopFuel(rows, teamKey));
}

export function streakTeleopFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesTeleopFuel(rows, teamKey));
}

export function giniTeleopFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesTeleopFuel(rows, teamKey));
}

export function theilTeleopFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesTeleopFuel(rows, teamKey));
}

export function skewTeleopFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesTeleopFuel(rows, teamKey));
}

export function rangeTeleopFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesTeleopFuel(rows, teamKey));
}

export function madMeanTeleopFuel(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesTeleopFuel(rows, teamKey));
}

export function packTeleopFuel(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesTeleopFuel(rows, teamKey);
  return {
    id: "teleopFuel" as LovatMetricId,
    label: "Teleop fuel",
    series,
    stat: wideStat(series),
    shrink: shrinkTeleopFuel(rows, teamKey, fieldRows),
    forecast: forecastTeleopFuel(rows, teamKey),
    fit: fitTeleopFuel(rows, teamKey),
    control: controlTeleopFuel(rows, teamKey),
    slots: slotsTeleopFuel(rows, teamKey),
  };
}

export function describeTeleopFuel(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Teleop fuel · Needs setup`;
  return `Teleop fuel · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesCycles(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["cycles" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statCycles(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesCycles(rows, teamKey));
}

export function shrinkCycles(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesCycles(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["cycles" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "cycles" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastCycles(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesCycles(rows, teamKey));
}

export function fitCycles(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesCycles(rows, teamKey));
}

export function controlCycles(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesCycles(rows, teamKey));
}

export function slotsCycles(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesCycles(rows, teamKey));
}

export function residualCycles(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "cycles" as LovatMetricId, oppKeys);
}

export function histCycles(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesCycles(rows, teamKey));
}

export function cvCycles(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesCycles(rows, teamKey));
}

export function trimCycles(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesCycles(rows, teamKey));
}

export function ewmaCycles(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesCycles(rows, teamKey));
}

export function jackCycles(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesCycles(rows, teamKey));
}

export function bootCycles(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesCycles(rows, teamKey));
}

export function hazardCycles(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesCycles(rows, teamKey));
}

export function streakCycles(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesCycles(rows, teamKey));
}

export function giniCycles(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesCycles(rows, teamKey));
}

export function theilCycles(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesCycles(rows, teamKey));
}

export function skewCycles(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesCycles(rows, teamKey));
}

export function rangeCycles(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesCycles(rows, teamKey));
}

export function madMeanCycles(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesCycles(rows, teamKey));
}

export function packCycles(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesCycles(rows, teamKey);
  return {
    id: "cycles" as LovatMetricId,
    label: "Cycles",
    series,
    stat: wideStat(series),
    shrink: shrinkCycles(rows, teamKey, fieldRows),
    forecast: forecastCycles(rows, teamKey),
    fit: fitCycles(rows, teamKey),
    control: controlCycles(rows, teamKey),
    slots: slotsCycles(rows, teamKey),
  };
}

export function describeCycles(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Cycles · Needs setup`;
  return `Cycles · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}

export function seriesScoringRate(rows: readonly SampleRow[], teamKey: string): number[] {
  return windowRows(rows)
    .filter((row) => uniqueTeamKey(row.teamKey) === uniqueTeamKey(teamKey))
    .map((row) => asNumber(row.values["scoringRate" as LovatMetricId]))
    .filter((value): value is number => value != null);
}

export function statScoringRate(rows: readonly SampleRow[], teamKey: string): WideStat {
  return wideStat(seriesScoringRate(rows, teamKey));
}

export function shrinkScoringRate(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): number | null {
  const series = seriesScoringRate(rows, teamKey);
  const field = fieldStats(fieldRows ?? rows)["scoringRate" as LovatMetricId];
  return shrinkTowardField(teamValue(rows, teamKey, "scoringRate" as LovatMetricId), field?.mean ?? null, series.length);
}

export function forecastScoringRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return forecastNext(seriesScoringRate(rows, teamKey));
}

export function fitScoringRate(rows: readonly SampleRow[], teamKey: string): LinearFit | null {
  return linearFit(seriesScoringRate(rows, teamKey));
}

export function controlScoringRate(rows: readonly SampleRow[], teamKey: string): ControlLimits | null {
  return controlLimits(seriesScoringRate(rows, teamKey));
}

export function slotsScoringRate(rows: readonly SampleRow[], teamKey: string): SlotSlice[] {
  return slotSlices(seriesScoringRate(rows, teamKey));
}

export function residualScoringRate(rows: readonly SampleRow[], teamKey: string, oppKeys: readonly string[] = []): ResidualRow {
  return residualFor(rows, teamKey, "scoringRate" as LovatMetricId, oppKeys);
}

export function histScoringRate(rows: readonly SampleRow[], teamKey: string): HistogramBin[] {
  return histogram(seriesScoringRate(rows, teamKey));
}

export function cvScoringRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return coefficientOfVariation(seriesScoringRate(rows, teamKey));
}

export function trimScoringRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return trimmedMean(seriesScoringRate(rows, teamKey));
}

export function ewmaScoringRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return ewmaSeries(seriesScoringRate(rows, teamKey));
}

export function jackScoringRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return jackknifeMean(seriesScoringRate(rows, teamKey));
}

export function bootScoringRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return bootstrapMean(seriesScoringRate(rows, teamKey));
}

export function hazardScoringRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return reliabilityHazard(seriesScoringRate(rows, teamKey));
}

export function streakScoringRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return streakValue(seriesScoringRate(rows, teamKey));
}

export function giniScoringRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return gini(seriesScoringRate(rows, teamKey));
}

export function theilScoringRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return theil(seriesScoringRate(rows, teamKey));
}

export function skewScoringRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return quartileSkew(seriesScoringRate(rows, teamKey));
}

export function rangeScoringRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return rangeWidth(seriesScoringRate(rows, teamKey));
}

export function madMeanScoringRate(rows: readonly SampleRow[], teamKey: string): number | null {
  return meanAbsDev(seriesScoringRate(rows, teamKey));
}

export function packScoringRate(rows: readonly SampleRow[], teamKey: string, fieldRows?: readonly SampleRow[]): WideMetricPack {
  const series = seriesScoringRate(rows, teamKey);
  return {
    id: "scoringRate" as LovatMetricId,
    label: "Scoring rate",
    series,
    stat: wideStat(series),
    shrink: shrinkScoringRate(rows, teamKey, fieldRows),
    forecast: forecastScoringRate(rows, teamKey),
    fit: fitScoringRate(rows, teamKey),
    control: controlScoringRate(rows, teamKey),
    slots: slotsScoringRate(rows, teamKey),
  };
}

export function describeScoringRate(pack: WideMetricPack): string {
  if (pack.stat.n === 0) return `Scoring rate · Needs setup`;
  return `Scoring rate · ${formatWideStat(pack.stat)} · forecast ${formatValue(pack.forecast, 1)}`;
}
...
export function rollTeleopPoints2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 2);
}

export function rollTeleopPoints2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints2(rows, teamKey);
  const last = series[series.length - 1];
...
export function rollTeleopPoints2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints2Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints2Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 2: Needs setup";
  return "Teleop points" + " last 2: " + formatValue(last, 1);
}

export function rollTeleopPoints3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 3);
}

export function rollTeleopPoints3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints3Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints3Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 3: Needs setup";
  return "Teleop points" + " last 3: " + formatValue(last, 1);
}

export function rollTeleopPoints4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 4);
}

export function rollTeleopPoints4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints4Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints4Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 4: Needs setup";
  return "Teleop points" + " last 4: " + formatValue(last, 1);
}

export function rollTeleopPoints5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 5);
}

export function rollTeleopPoints5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints5Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints5Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 5: Needs setup";
  return "Teleop points" + " last 5: " + formatValue(last, 1);
}

export function rollTeleopPoints6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 6);
}

export function rollTeleopPoints6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints6Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints6Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 6: Needs setup";
  return "Teleop points" + " last 6: " + formatValue(last, 1);
}

export function rollTeleopPoints7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 7);
}

export function rollTeleopPoints7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints7Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints7Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 7: Needs setup";
  return "Teleop points" + " last 7: " + formatValue(last, 1);
}

export function rollTeleopPoints8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 8);
}

export function rollTeleopPoints8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints8Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints8Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 8: Needs setup";
  return "Teleop points" + " last 8: " + formatValue(last, 1);
}

export function rollTeleopPoints9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 9);
}

export function rollTeleopPoints9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints9Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints9Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 9: Needs setup";
  return "Teleop points" + " last 9: " + formatValue(last, 1);
}

export function rollTeleopPoints10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 10);
}

export function rollTeleopPoints10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints10Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints10Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 10: Needs setup";
  return "Teleop points" + " last 10: " + formatValue(last, 1);
}

export function rollTeleopPoints12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 12);
}

export function rollTeleopPoints12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints12Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints12Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 12: Needs setup";
  return "Teleop points" + " last 12: " + formatValue(last, 1);
}

export function rollTeleopPoints14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 14);
}

export function rollTeleopPoints14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints14Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints14Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 14: Needs setup";
  return "Teleop points" + " last 14: " + formatValue(last, 1);
}

export function rollTeleopPoints16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 16);
}

export function rollTeleopPoints16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints16Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints16Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 16: Needs setup";
  return "Teleop points" + " last 16: " + formatValue(last, 1);
}

export function rollTeleopPoints18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 18);
}

export function rollTeleopPoints18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints18Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints18Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 18: Needs setup";
  return "Teleop points" + " last 18: " + formatValue(last, 1);
}

export function rollTeleopPoints20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 20);
}

export function rollTeleopPoints20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints20Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints20Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 20: Needs setup";
  return "Teleop points" + " last 20: " + formatValue(last, 1);
}

export function rollTeleopPoints22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 22);
}

export function rollTeleopPoints22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints22Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints22Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 22: Needs setup";
  return "Teleop points" + " last 22: " + formatValue(last, 1);
}

export function rollTeleopPoints24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 24);
}

export function rollTeleopPoints24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints24Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints24Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 24: Needs setup";
  return "Teleop points" + " last 24: " + formatValue(last, 1);
}

export function rollTeleopPoints28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 28);
}

export function rollTeleopPoints28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints28Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints28Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 28: Needs setup";
  return "Teleop points" + " last 28: " + formatValue(last, 1);
}

export function rollTeleopPoints32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopPoints(rows, teamKey), 32);
}

export function rollTeleopPoints32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopPoints32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopPoints32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopPoints32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopPoints32Last(rows, teamKey) != null;
}

export function describeRollTeleopPoints32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopPoints32Last(rows, teamKey);
  if (last == null) return "Teleop points" + " last 32: Needs setup";
  return "Teleop points" + " last 32: " + formatValue(last, 1);
}

export function rollTeleopFuel2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 2);
}

export function rollTeleopFuel2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel2Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel2Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 2: Needs setup";
  return "Teleop fuel" + " last 2: " + formatValue(last, 1);
}

export function rollTeleopFuel3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 3);
}

export function rollTeleopFuel3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel3Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel3Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 3: Needs setup";
  return "Teleop fuel" + " last 3: " + formatValue(last, 1);
}

export function rollTeleopFuel4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 4);
}

export function rollTeleopFuel4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel4Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel4Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 4: Needs setup";
  return "Teleop fuel" + " last 4: " + formatValue(last, 1);
}

export function rollTeleopFuel5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 5);
}

export function rollTeleopFuel5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel5Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel5Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 5: Needs setup";
  return "Teleop fuel" + " last 5: " + formatValue(last, 1);
}

export function rollTeleopFuel6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 6);
}

export function rollTeleopFuel6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel6Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel6Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 6: Needs setup";
  return "Teleop fuel" + " last 6: " + formatValue(last, 1);
}

export function rollTeleopFuel7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 7);
}

export function rollTeleopFuel7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel7Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel7Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 7: Needs setup";
  return "Teleop fuel" + " last 7: " + formatValue(last, 1);
}

export function rollTeleopFuel8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 8);
}

export function rollTeleopFuel8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel8Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel8Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 8: Needs setup";
  return "Teleop fuel" + " last 8: " + formatValue(last, 1);
}

export function rollTeleopFuel9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 9);
}

export function rollTeleopFuel9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel9Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel9Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 9: Needs setup";
  return "Teleop fuel" + " last 9: " + formatValue(last, 1);
}

export function rollTeleopFuel10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 10);
}

export function rollTeleopFuel10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel10Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel10Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 10: Needs setup";
  return "Teleop fuel" + " last 10: " + formatValue(last, 1);
}

export function rollTeleopFuel12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 12);
}

export function rollTeleopFuel12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel12Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel12Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 12: Needs setup";
  return "Teleop fuel" + " last 12: " + formatValue(last, 1);
}

export function rollTeleopFuel14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 14);
}

export function rollTeleopFuel14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel14Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel14Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 14: Needs setup";
  return "Teleop fuel" + " last 14: " + formatValue(last, 1);
}

export function rollTeleopFuel16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 16);
}

export function rollTeleopFuel16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel16Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel16Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 16: Needs setup";
  return "Teleop fuel" + " last 16: " + formatValue(last, 1);
}

export function rollTeleopFuel18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 18);
}

export function rollTeleopFuel18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel18Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel18Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 18: Needs setup";
  return "Teleop fuel" + " last 18: " + formatValue(last, 1);
}

export function rollTeleopFuel20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 20);
}

export function rollTeleopFuel20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel20Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel20Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 20: Needs setup";
  return "Teleop fuel" + " last 20: " + formatValue(last, 1);
}

export function rollTeleopFuel22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 22);
}

export function rollTeleopFuel22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel22Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel22Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 22: Needs setup";
  return "Teleop fuel" + " last 22: " + formatValue(last, 1);
}

export function rollTeleopFuel24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 24);
}

export function rollTeleopFuel24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel24Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel24Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 24: Needs setup";
  return "Teleop fuel" + " last 24: " + formatValue(last, 1);
}

export function rollTeleopFuel28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 28);
}

export function rollTeleopFuel28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel28Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel28Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 28: Needs setup";
  return "Teleop fuel" + " last 28: " + formatValue(last, 1);
}

export function rollTeleopFuel32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesTeleopFuel(rows, teamKey), 32);
}

export function rollTeleopFuel32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollTeleopFuel32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollTeleopFuel32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollTeleopFuel32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollTeleopFuel32Last(rows, teamKey) != null;
}

export function describeRollTeleopFuel32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollTeleopFuel32Last(rows, teamKey);
  if (last == null) return "Teleop fuel" + " last 32: Needs setup";
  return "Teleop fuel" + " last 32: " + formatValue(last, 1);
}

export function rollCycles2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 2);
}

export function rollCycles2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles2Last(rows, teamKey) != null;
}

export function describeRollCycles2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles2Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 2: Needs setup";
  return "Cycles" + " last 2: " + formatValue(last, 1);
}

export function rollCycles3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 3);
}

export function rollCycles3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles3Last(rows, teamKey) != null;
}

export function describeRollCycles3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles3Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 3: Needs setup";
  return "Cycles" + " last 3: " + formatValue(last, 1);
}

export function rollCycles4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 4);
}

export function rollCycles4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles4Last(rows, teamKey) != null;
}

export function describeRollCycles4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles4Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 4: Needs setup";
  return "Cycles" + " last 4: " + formatValue(last, 1);
}

export function rollCycles5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 5);
}

export function rollCycles5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles5Last(rows, teamKey) != null;
}

export function describeRollCycles5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles5Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 5: Needs setup";
  return "Cycles" + " last 5: " + formatValue(last, 1);
}

export function rollCycles6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 6);
}

export function rollCycles6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles6Last(rows, teamKey) != null;
}

export function describeRollCycles6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles6Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 6: Needs setup";
  return "Cycles" + " last 6: " + formatValue(last, 1);
}

export function rollCycles7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 7);
}

export function rollCycles7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles7Last(rows, teamKey) != null;
}

export function describeRollCycles7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles7Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 7: Needs setup";
  return "Cycles" + " last 7: " + formatValue(last, 1);
}

export function rollCycles8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 8);
}

export function rollCycles8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles8Last(rows, teamKey) != null;
}

export function describeRollCycles8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles8Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 8: Needs setup";
  return "Cycles" + " last 8: " + formatValue(last, 1);
}

export function rollCycles9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 9);
}

export function rollCycles9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles9Last(rows, teamKey) != null;
}

export function describeRollCycles9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles9Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 9: Needs setup";
  return "Cycles" + " last 9: " + formatValue(last, 1);
}

export function rollCycles10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 10);
}

export function rollCycles10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles10Last(rows, teamKey) != null;
}

export function describeRollCycles10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles10Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 10: Needs setup";
  return "Cycles" + " last 10: " + formatValue(last, 1);
}

export function rollCycles12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 12);
}

export function rollCycles12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles12Last(rows, teamKey) != null;
}

export function describeRollCycles12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles12Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 12: Needs setup";
  return "Cycles" + " last 12: " + formatValue(last, 1);
}

export function rollCycles14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 14);
}

export function rollCycles14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles14Last(rows, teamKey) != null;
}

export function describeRollCycles14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles14Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 14: Needs setup";
  return "Cycles" + " last 14: " + formatValue(last, 1);
}

export function rollCycles16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 16);
}

export function rollCycles16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles16Last(rows, teamKey) != null;
}

export function describeRollCycles16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles16Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 16: Needs setup";
  return "Cycles" + " last 16: " + formatValue(last, 1);
}

export function rollCycles18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 18);
}

export function rollCycles18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles18Last(rows, teamKey) != null;
}

export function describeRollCycles18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles18Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 18: Needs setup";
  return "Cycles" + " last 18: " + formatValue(last, 1);
}

export function rollCycles20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 20);
}

export function rollCycles20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles20Last(rows, teamKey) != null;
}

export function describeRollCycles20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles20Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 20: Needs setup";
  return "Cycles" + " last 20: " + formatValue(last, 1);
}

export function rollCycles22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 22);
}

export function rollCycles22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles22Last(rows, teamKey) != null;
}

export function describeRollCycles22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles22Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 22: Needs setup";
  return "Cycles" + " last 22: " + formatValue(last, 1);
}

export function rollCycles24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 24);
}

export function rollCycles24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles24Last(rows, teamKey) != null;
}

export function describeRollCycles24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles24Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 24: Needs setup";
  return "Cycles" + " last 24: " + formatValue(last, 1);
}

export function rollCycles28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 28);
}

export function rollCycles28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles28Last(rows, teamKey) != null;
}

export function describeRollCycles28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles28Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 28: Needs setup";
  return "Cycles" + " last 28: " + formatValue(last, 1);
}

export function rollCycles32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesCycles(rows, teamKey), 32);
}

export function rollCycles32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollCycles32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollCycles32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollCycles32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollCycles32Last(rows, teamKey) != null;
}

export function describeRollCycles32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollCycles32Last(rows, teamKey);
  if (last == null) return "Cycles" + " last 32: Needs setup";
  return "Cycles" + " last 32: " + formatValue(last, 1);
}

export function rollScoringRate2(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 2);
}

export function rollScoringRate2Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate2(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate2Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate2(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate2Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate2Last(rows, teamKey) != null;
}

export function describeRollScoringRate2(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate2Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 2: Needs setup";
  return "Scoring rate" + " last 2: " + formatValue(last, 1);
}

export function rollScoringRate3(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 3);
}

export function rollScoringRate3Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate3(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate3Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate3(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate3Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate3Last(rows, teamKey) != null;
}

export function describeRollScoringRate3(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate3Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 3: Needs setup";
  return "Scoring rate" + " last 3: " + formatValue(last, 1);
}

export function rollScoringRate4(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 4);
}

export function rollScoringRate4Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate4(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate4Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate4(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate4Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate4Last(rows, teamKey) != null;
}

export function describeRollScoringRate4(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate4Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 4: Needs setup";
  return "Scoring rate" + " last 4: " + formatValue(last, 1);
}

export function rollScoringRate5(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 5);
}

export function rollScoringRate5Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate5(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate5Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate5(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate5Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate5Last(rows, teamKey) != null;
}

export function describeRollScoringRate5(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate5Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 5: Needs setup";
  return "Scoring rate" + " last 5: " + formatValue(last, 1);
}

export function rollScoringRate6(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 6);
}

export function rollScoringRate6Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate6(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate6Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate6(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate6Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate6Last(rows, teamKey) != null;
}

export function describeRollScoringRate6(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate6Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 6: Needs setup";
  return "Scoring rate" + " last 6: " + formatValue(last, 1);
}

export function rollScoringRate7(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 7);
}

export function rollScoringRate7Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate7(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate7Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate7(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate7Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate7Last(rows, teamKey) != null;
}

export function describeRollScoringRate7(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate7Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 7: Needs setup";
  return "Scoring rate" + " last 7: " + formatValue(last, 1);
}

export function rollScoringRate8(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 8);
}

export function rollScoringRate8Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate8(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate8Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate8(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate8Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate8Last(rows, teamKey) != null;
}

export function describeRollScoringRate8(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate8Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 8: Needs setup";
  return "Scoring rate" + " last 8: " + formatValue(last, 1);
}

export function rollScoringRate9(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 9);
}

export function rollScoringRate9Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate9(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate9Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate9(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate9Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate9Last(rows, teamKey) != null;
}

export function describeRollScoringRate9(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate9Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 9: Needs setup";
  return "Scoring rate" + " last 9: " + formatValue(last, 1);
}

export function rollScoringRate10(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 10);
}

export function rollScoringRate10Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate10(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate10Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate10(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate10Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate10Last(rows, teamKey) != null;
}

export function describeRollScoringRate10(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate10Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 10: Needs setup";
  return "Scoring rate" + " last 10: " + formatValue(last, 1);
}

export function rollScoringRate12(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 12);
}

export function rollScoringRate12Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate12(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate12Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate12(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate12Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate12Last(rows, teamKey) != null;
}

export function describeRollScoringRate12(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate12Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 12: Needs setup";
  return "Scoring rate" + " last 12: " + formatValue(last, 1);
}

export function rollScoringRate14(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 14);
}

export function rollScoringRate14Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate14(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate14Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate14(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate14Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate14Last(rows, teamKey) != null;
}

export function describeRollScoringRate14(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate14Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 14: Needs setup";
  return "Scoring rate" + " last 14: " + formatValue(last, 1);
}

export function rollScoringRate16(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 16);
}

export function rollScoringRate16Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate16(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate16Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate16(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate16Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate16Last(rows, teamKey) != null;
}

export function describeRollScoringRate16(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate16Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 16: Needs setup";
  return "Scoring rate" + " last 16: " + formatValue(last, 1);
}

export function rollScoringRate18(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 18);
}

export function rollScoringRate18Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate18(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate18Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate18(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate18Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate18Last(rows, teamKey) != null;
}

export function describeRollScoringRate18(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate18Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 18: Needs setup";
  return "Scoring rate" + " last 18: " + formatValue(last, 1);
}

export function rollScoringRate20(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 20);
}

export function rollScoringRate20Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate20(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate20Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate20(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate20Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate20Last(rows, teamKey) != null;
}

export function describeRollScoringRate20(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate20Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 20: Needs setup";
  return "Scoring rate" + " last 20: " + formatValue(last, 1);
}

export function rollScoringRate22(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 22);
}

export function rollScoringRate22Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate22(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate22Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate22(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate22Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate22Last(rows, teamKey) != null;
}

export function describeRollScoringRate22(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate22Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 22: Needs setup";
  return "Scoring rate" + " last 22: " + formatValue(last, 1);
}

export function rollScoringRate24(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 24);
}

export function rollScoringRate24Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate24(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate24Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate24(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate24Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate24Last(rows, teamKey) != null;
}

export function describeRollScoringRate24(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate24Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 24: Needs setup";
  return "Scoring rate" + " last 24: " + formatValue(last, 1);
}

export function rollScoringRate28(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 28);
}

export function rollScoringRate28Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate28(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate28Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate28(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate28Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate28Last(rows, teamKey) != null;
}

export function describeRollScoringRate28(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate28Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 28: Needs setup";
  return "Scoring rate" + " last 28: " + formatValue(last, 1);
}

export function rollScoringRate32(rows: readonly SampleRow[], teamKey: string): Array<number | null> {
  return rollingMean(seriesScoringRate(rows, teamKey), 32);
}

export function rollScoringRate32Last(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate32(rows, teamKey);
  const last = series[series.length - 1];
  return last == null ? null : last;
}

export function rollScoringRate32Delta(rows: readonly SampleRow[], teamKey: string): number | null {
  const series = rollScoringRate32(rows, teamKey);
  const last = series[series.length - 1];
  const prev = series[series.length - 2];
  if (last == null || prev == null) return null;
...
export function rollScoringRate32Ready(rows: readonly SampleRow[], teamKey: string): boolean {
  return rollScoringRate32Last(rows, teamKey) != null;
}

export function describeRollScoringRate32(rows: readonly SampleRow[], teamKey: string): string {
  const last = rollScoringRate32Last(rows, teamKey);
  if (last == null) return "Scoring rate" + " last 32: Needs setup";
  return "Scoring rate" + " last 32: " + formatValue(last, 1);
}
...
export function buildWideReport(input: { teamKey: string; rows: readonly SampleRow[]; fieldRows?: readonly SampleRow[] }): WideReport {
  const packs: WideMetricPack[] = [
    packTeleopPoints(input.rows, input.teamKey, input.fieldRows),
    packTeleopFuel(input.rows, input.teamKey, input.fieldRows),
    packCycles(input.rows, input.teamKey, input.fieldRows),
    packScoringRate(input.rows, input.teamKey, input.fieldRows)
  ];
  const lead = packs[0];
