/**
 * Leftover win-plus kit — teleop-last6-platt.
...
/**
 * Lovat-style Teleop board — last 6 matches, Platt calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-last6-platt";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2929316279): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
