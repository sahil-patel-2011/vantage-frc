/**
 * Leftover win-plus kit — teleop-quals-corr.
...
/**
 * Lovat-style Teleop board — qualification matches, partner correlation.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-quals-corr";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2327418196): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
