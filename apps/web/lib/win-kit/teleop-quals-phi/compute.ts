/**
 * Leftover win-plus kit — teleop-quals-phi.
...
/**
 * Lovat-style Teleop board — qualification matches, Cody–Hart Φ.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-quals-phi";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4239372493): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
