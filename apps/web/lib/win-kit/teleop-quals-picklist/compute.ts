/**
 * Leftover win-plus kit — teleop-quals-picklist.
...
/**
 * Lovat-style Teleop board — qualification matches, blend picklist.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-quals-picklist";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 2448302647): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
