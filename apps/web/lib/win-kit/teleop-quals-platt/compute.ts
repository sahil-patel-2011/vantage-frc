/**
 * Leftover win-plus kit — teleop-quals-platt.
...
/**
 * Lovat-style Teleop board — qualification matches, Platt calibration.
 * Missing samples stay blank. Nothing here invents a rating.
 */

export const SLUG = "teleop-quals-platt";
...
export function bootstrapMean(values: readonly number[], draws = 24, seed = 4125363417): number | null {
  const clean = values.filter((value) => Number.isFinite(value));
  if (clean.length === 0) return null;
  const means: number[] = [];
  let state = seed >>> 0;
