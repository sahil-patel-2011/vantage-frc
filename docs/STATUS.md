...
| 18 | Final verification | pending | previous full vitest: **880 / 8,331 passed / 10 skipped / 52.69s**. This revision: `npx tsc --noEmit -p apps/web` clean; vitest hubs + venue-weather + loaders + command-search **94 passed / 94**; Playwright `product-shell.spec.ts` **11 passed / 11** including the new Scouting/Work strip assertions (20.0s). Vercel GitHub check is **ERROR** `IMMUTABLE_STATIC_PATCH_PREVIEW_COMMENTS` after a successful Next build (Next is already `^16.3.4`); owner: disable Preview Comments. `CRON_SECRET` still owner-must. RLS proof and a real signed-in `next start` walk still not in this image. |
