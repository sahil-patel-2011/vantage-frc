...
| 4 | Home widgets | in progress | Live cards in `widgets/home-cards.tsx`. Canvas helpers in `dashboard-canvas.ts`. Widget chrome/empty copy in `widgets/widget-shell.tsx`. Competition/ops widgets in `widgets/ops-cards.tsx`. Board types in `dashboard-board-types.ts`. First-run banner, board switcher, widget palette, library sheet, boards modal, edit/preview docks, and **pointer-drag** (`use-dashboard-pointer-drag.ts`) are sibling modules. `dashboard-client.tsx` is the save/poll/keyboard shell (~1452 lines). Playwright tap-to-place now emulates `pointer: coarse` (library placement does not fire on Desktop Chrome mouse). Venue weather: loader returns city + `isEventDay` and never a temperature; the card fetches Open-Meteo via `loadVenueForecast` / `venueWeatherCopy`. Live probe 2026-09-10T07:30Z Houston (29.76328, -95.36327): `temperature_2m` **26.1** °C, `weather_code` 0 → **26°C · Clear**. Unit tests cover that mapping plus off-day copy ("Forecast shows on event day."). Playwright `home-weather.spec.ts` mocks Open-Meteo and places the library card; fixture Home has no event city so the honest empty is **No venue weather**. Not a signed-in live widget screenshot. |
...
| 18 | Final verification | pending | previous full vitest: **880 / 8,331 passed / 10 skipped / 52.69s**. This revision (pre-weather/offline): vitest calibrated-score + match-plan + score-from-metrics + colors + css-integrity + feature-map-routes + appearance **47 passed / 47**. Playwright theme/waitlist/pricing/product-shell/home-tap-to-place **17 passed / 17** (15.7s) after the `--app-*` token swap. FEATURE_MAP HTTP walk **143 routes, 138 OK, 5 admin 404, 0 500**. GitHub Actions `Postgres migrations and RLS` **pass 49s** on run `34451745493` (two-org `vantage_app` isolation + full migration apply/reapply on `vantage_ci`) — not `scripts/rls-proof.mjs`. CI on `da455f06` failed unit tests: `roadmap-presentation.test.ts` still expected `var(--app-*)` after the token swap — assertion now matches canonical `--critical` / `--warning` / `--accent` / `--muted` / `--positive`. Playwright on that SHA was cancelled mid FEATURE_MAP Chromium walk; HEAD skips that walk unless `FEATURE_MAP_PW=1`. Vercel GitHub deploy still fails independently. Do not treat production as proven. `CRON_SECRET` still owner-must. Real signed-in `next start` walk still not in this image. |
