...
| 18 | Final verification | pending | GHA Playwright on `ae41cf16`: **112 passed, 1 failed, 12 skipped** — hard fail `tests/browser/product-shell.spec.ts:8` First-run setup region (banner waited on hung `/api/me`). This revision paints the banner on the loading shell and aborts session fetch at 8s. GHA Playwright on `7921757c`: **111 passed, 1 failed** (`inventory-shell` stuck because a fixture 401 paints **Your session ended**, not Could not load Inventory), **1 flaky** (`waitlist` Neon upsert). Specs now accept `loadFailureHeading`; CI waitlist uses the in-memory store (`9be1b48f`). Vercel GitHub deploy still fails independently (`IMMUTABLE_STATIC_PATCH_PREVIEW_COMMENTS`). Do not treat production as proven. `CRON_SECRET` still owner-must. |
