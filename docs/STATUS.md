...
| 18 | Final verification | pending | previous full vitest: **880 / 8,331 passed / 10 skipped / 52.69s**. This revision: Playwright files that failed on `580634e9` are **16 passed / 16** locally (theme, waitlist, pricing, product-shell, home-tap-to-place). Vercel GitHub check is **ERROR** `IMMUTABLE_STATIC_PATCH_PREVIEW_COMMENTS` after a successful Next build (Next is already `^16.3.4`); owner: disable Preview Comments. `CRON_SECRET` still owner-must. RLS proof and a real signed-in `next start` walk still not in this image. |
