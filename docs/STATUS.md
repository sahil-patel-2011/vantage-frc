...
| 18 | Final verification | pending | latest full vitest: **887 files passed / 2 skipped (889), 8,365 passed, 10 skipped, 53.50s** (before calendar/scouting/CAD/inspection/files extracts). `npx tsc --noEmit -p apps/web` clean after CAD, Inspection Copilot, and Files splits. Fixture `:3310` Playwright `cad-shell.spec.ts` **1 passed / 1** (2.3s), `inspection-copilot-shell.spec.ts` **1 passed / 1** (2.2s), `files-shell.spec.ts` **1 passed / 1** (3.1s). GitHub Actions Playwright on `b82e1440` **passed** (one worker). GHA Playwright on `b4871bbb` **failed** `scouting-shell.spec.ts`: that job does not start Postgres, `signInAs` ECONNREFUSED, and the fixture cookie is not a Better Auth session, so `HubOrgGate` paints **Choose a team** and never mounts `ScoutingClient`. Specs for hub-gated shells (Scouting, CAD) now accept that heading as the honest no-org gate. Do not invent an org to make the ToolStrip appear in CI. Vercel GitHub deploy still fails independently (`IMMUTABLE_STATIC_PATCH_PREVIEW_COMMENTS`). Do not treat production as proven. `CRON_SECRET` still owner-must. |
