...
| FEATURE_MAP fixture walk | parser yields **143** visit-able routes from `docs/FEATURE_MAP.md`; Playwright spec walks them under `E2E_AUTH_FIXTURE` | this commit |
...
| 2 | One design system | in progress | tokens in `system.css`; Button emits `.app-button`. Product CSS/TS consumption of `var(--soft-*)` and `var(--app-*)` is **zero** (css-integrity, including `lib/`). Canonical color/type/space: `--bg` `--surface` `--ink` `--accent` `--accent-hover` `--positive-fill` `--critical-fill` `--title-*` `--text-*` `--space-*` `--control-h` `--island`. `--soft-*` / `--app-*` remain only as alias declarations in `system.css`. Team accent runtime writes `--accent` / `--accent-hover` as well as the leftover aliases. **`--m-*` consumption still exists** in marketing CSS (`marketing.css` 581, `marketing-v3.css` 181, `marketing-showcase.css` 71, `legal.css` 27, plus 2 TSX). |
| 3 | Full UI pass | in progress | Student copy says team, not workspace, on Home widgets, related strips, Event Day setup, intel crumbs, and `/workspace`. Empty/setup cards keep one primary. Video re-scout review tools are a ToolStrip. Remaining TabBars are hub-level (Business, Media, Account, Help, Notifications). Playwright landing/pricing headings match current copy. FEATURE_MAP parser: **143** visit-able product routes (`apps/web/lib/nav/feature-map-routes.ts`). Playwright `tests/browser/feature-map-walk.spec.ts` walks them under the fixture (not a signed-in `next start` walk; APIs still 401). Local walk evidence recorded after the Playwright run on this revision. |
...
| 18 | Final verification | pending | previous full vitest: **880 / 8,331 passed / 10 skipped / 52.69s**. This revision (pre-Playwright): vitest calibrated-score + match-plan + score-from-metrics + colors + css-integrity + feature-map-routes + appearance **47 passed / 47**. Playwright job timeout raised to **40m** for the 143-route FEATURE_MAP walk. GitHub Actions `Postgres migrations and RLS` **pass 49s** on run `34451745493` (two-org `vantage_app` isolation + full migration apply/reapply on `vantage_ci`) — not `scripts/rls-proof.mjs`. Vercel GitHub deploy still fails independently (`IMMUTABLE_STATIC_PATCH_PREVIEW_COMMENTS` on earlier SHAs; Preview Comments check later reported pass). Do not treat production as proven. `CRON_SECRET` still owner-must. Real signed-in `next start` walk still not in this image. |
