...
| 18 | Final verification | pending | previous pass: `tsc -p apps/web` clean; `npx eslint .` clean (30.7s); full `vitest run` **880 / 8,331 passed / 10 skipped / 52.69s**. Fixture Chromium walk (`E2E_AUTH_FIXTURE=1`, `next dev :3001`): `/` `/dashboard` `/bugbot` `/connectors` `/offline` all 200. Playwright CI on `580634e9` was **73 passed / 5 failed / 8 skipped** — copy + coarse pointer + Account loading hang; this commit retargets those specs. Vercel GitHub check is **ERROR** `IMMUTABLE_STATIC_PATCH_PREVIEW_COMMENTS` after a successful Next build (Next is already `^16.3.4`); owner: disable Preview Comments or wait for the Vercel adapter. `CRON_SECRET` still owner-must. RLS proof and a real signed-in `next start` walk still not in this image. |
