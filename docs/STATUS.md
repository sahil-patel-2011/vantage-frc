...
| 18 | Final verification | pending | latest full vitest: **883 files passed / 2 skipped (885), 8,354 passed, 10 skipped, 53.88s**. Targeted: venue-weather + weather-venue-live + home-widget-loaders + shell-routes **24 passed / 24**. Playwright theme/waitlist/pricing/product-shell/home-tap-to-place + packing/batteries/pit offline **24 passed**; home-weather library empty-state **1 passed / 1** (4.9s). FEATURE_MAP HTTP walk **143 routes, 138 OK, 5 admin 404, 0 500**. Production `next build` **695 pages**, compile 23.9s, TypeScript 47s, zero warnings. GitHub Actions `Postgres migrations and RLS` **pass** on run `34458163082` (SHA `28d33811`). **`scripts/rls-proof.mjs` 40/40** this image against local `vantage_ci` as `vantage_ci_app` (session login, effective `vantage_app`, not superuser / not BYPASSRLS). Covers Drive personal files, outreach participants, dossier, funding_model, relay_nodes, video_analysis_jobs, assembly_manual_runs. Vercel GitHub deploy still fails independently. Do not treat production as proven. `CRON_SECRET` still owner-must. Real signed-in `next start` walk still not in this image. |
