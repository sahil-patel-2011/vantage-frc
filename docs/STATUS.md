...
| 18 | Final verification | pending | latest full vitest: **887 files passed / 2 skipped (889), 8,365 passed, 10 skipped, 53.50s** (before calendar/scouting/CAD/inspection/files extracts). `npx tsc --noEmit -p apps/web` clean after the Logistics split. `npx eslint apps/web/app/logistics apps/web/lib/logistics` clean. `logistics-related.test.ts` + `logistics-model.test.ts` **14 passed / 14**. Fixture `:3310` Playwright `ai-keys-shell.spec.ts` + `logistics-shell.spec.ts` + `feature-integration` logistics related-once **3 passed / 3** (5.0s, one worker). `npx tsc --noEmit -p apps/web` clean after the Kickoff HubOrgGate + onboarding split. `team-admin-related.test.ts` + `onboarding-model.test.ts` + `onboarding-flow.test.ts` **25 passed / 25**. Fixture `:3310` Playwright Kickoff / Team admin / onboarding / feature-integration (incl. team-admin related-once) **18 passed / 18** after the onboarding Home-redirect matcher (first onboarding heading matcher failed because a complete profile leaves `/onboarding`). `npx tsc --noEmit -p apps/web` clean after the Inventory split. `npx eslint apps/web/app/inventory apps/web/lib/inventory` clean. `inventory-related.test.ts` + `inventory-model.test.ts` + `setup-actions-contract.test.ts` + `team-admin-related.test.ts` **34 passed / 34**. Fixture `:3310` Playwright `inventory-shell.spec.ts` + `feature-integration` (incl. inventory related-once) **17 passed / 17** (15.4s, one worker). Owner Team 6925 empty Inventory paints **Add a part before tracking stock** with one **Add a part** primary; header related strip is the only Vendors / Orders / Spare Forecast control; Stock / Locations / BOM are ToolStrip chips, not tabs. Kickoff with owner Team 6925 paints **Game release intelligence** and honest empty scoring/priority cards. No-org Team admin keeps one **Choose your team** primary; header related strip is the only Account / Discord / Account Connections control. Complete onboarding profiles redirect to Home (`Good afternoon, coach` on the fixture). GHA without Postgres should now hit Kickoff **Choose a team** via HubOrgGate instead of hanging on `/api/kickoff`. AI keys empty copy is **Select a team to add API keys** / **No team**. Logistics empty is **Travel plan not published** + one **Add a trip** primary; header related strip is the only Event Day / My Day / Calendar / Packing / Visit invites control. Hub-gated specs accept **Choose a team** when GHA has no Postgres. Vercel GitHub deploy still fails independently (`IMMUTABLE_STATIC_PATCH_PREVIEW_COMMENTS`). Do not treat production as proven. `CRON_SECRET` still owner-must. |
