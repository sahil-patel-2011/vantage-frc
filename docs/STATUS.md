...
| 18 | Final verification | pending | latest full vitest: **883 files passed / 2 skipped (885), 8,354 passed, 10 skipped, 53.88s**. Targeted this revision: dashboard-home-model + catalog + dashboard-related + module-boundaries **30 passed / 30**. `npx tsc --noEmit -p apps/web` clean. Prior: account-types + business-helpers + funding-profile + hub-access-filter **17 passed / 17**. Fixture :3310 Playwright (10.8s): home-tap-to-place + product-shell + home-weather **13 passed / 13** after the Home split. Prior: account-tool-strip, business-funding-model (3), empty-states Business tab bar **6 passed / 6**. Prior: Media header/docs/hub-tabs/library **4 passed / 4**; signed-in `next start` docs-tool-strip / signed-in-start / weather / offline; FEATURE_MAP HTTP walk **143 / 143 OK**; `rls-proof.mjs` **40/40** as `vantage_ci_app`. GitHub Actions Postgres **pass** on run `34465756241`. Playwright on that same SHA (`18376399`, Media split) was **9 failed / 76 passed / 12 skipped** in 6.1m: `net::ERR_ABORTED` / detached frames while running **2 workers** against Next 16 Turbopack. That is compiler starvation, not assertion bugs. This revision serializes GHA Playwright (`workers: 1`, `retries: 1`, `timeout: 45s`, job `timeout-minutes: 30`). Better Auth `ECONNREFUSED` in that job is expected — the Playwright GHA job has no Postgres; `with real sessions` tests skip. Vercel GitHub deploy still fails independently (`IMMUTABLE_STATIC_PATCH_PREVIEW_COMMENTS`). Do not treat production as proven. `CRON_SECRET` still owner-must. |
