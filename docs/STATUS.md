...
| `scripts/rls-proof.mjs` | **40/40 passed** this image | local Postgres 16 `vantage_ci`, `RLS_PROOF_SUPERUSER_URL=postgres@127.0.0.1/vantage_ci`, `RLS_PROOF_APP_URL=vantage_ci_app` (session login, effective `vantage_app`, `rolsuper=f`, `rolbypassrls=f`). Drive / outreach / dossier / funding / relay / video / assembly-manual. **CI** job `Postgres migrations and RLS` also **passed (49s)** on run `34451745493` (`packages/db/src/postgres-integration.test.ts` — two-org isolation + full migration apply/reapply). |
...
| 4 | Home widgets | in progress | Live cards in `widgets/home-cards.tsx`. Canvas helpers in `dashboard-canvas.ts`. Widget chrome/empty copy in `widgets/widget-shell.tsx`. Competition/ops widgets in `widgets/ops-cards.tsx`. Board types in `dashboard-board-types.ts`. First-run banner, board switcher, widget palette, library sheet, boards modal, edit/preview docks, and **pointer-drag** (`use-dashboard-pointer-drag.ts`) are sibling modules. `dashboard-client.tsx` is the save/poll/keyboard shell (~1452 lines). Playwright tap-to-place now emulates `pointer: coarse` (library placement does not fire on Desktop Chrome mouse). Venue weather: loader returns city + `isEventDay` and never a temperature; the card fetches Open-Meteo via `loadVenueForecast` / `venueWeatherCopy`. Live probe 2026-09-10T07:30Z Houston (29.76328, -95.36327): `temperature_2m` **26.1** °C, `weather_code` 0 → **26°C · Clear**. Unit tests cover that mapping plus off-day copy ("Forecast shows on event day."). Playwright `home-weather.spec.ts` mocks Open-Meteo and adds the library card on desktop (fine pointer adds immediately); fixture Home has no event city so the honest empty is **No venue weather** / "Weather appears when an event with a location is active." Not a signed-in live widget screenshot. |
| 5 | Offline shell | in progress | Season calendar, packing, batteries, pit, season-tasks already queued. **My Day**, **Schedule**, and **Event Day** (`/command`) restore the last IndexedDB snapshot when the fetch fails. `/my-day` and `/command` are on the service-worker shell list. Playwright spec covers `/competition`, `/calendar`, `/my-day`, `/command`, `/packing`, `/batteries`, and `/pit` offline reloads. No signed-in `next start` walk. |
...
| 17 | Performance / cost | in progress | Home, Event day, Pit, Display, assembly-manual, exports, storage, hours kiosk, and the outbox badge skip while the tab is hidden. Production `next build` after the CSS token swap: compiled **23.9s**, TypeScript **47s**, **695 pages**, no warnings in the log. Route first-load uncompressed JS (`.next/diagnostics/route-bundle-stats.json`): Home `/dashboard` 834 KB, Scouting 838 KB, Strategy 762 KB, Pit 729 KB, `/workspace` 702 KB. Unique on-disk chunks (not shared with every of those five routes): Home 143 KB, Scouting 148 KB, Strategy 73 KB, Pit 41 KB, `/workspace` 15 KB. Next 16 Turbopack does not print a First Load JS column. |
| 18 | Final verification | pending | previous full vitest: **880 / 8,331 passed / 10 skipped / 52.69s**. This revision: vitest venue-weather + weather-venue-live + home-widget-loaders + shell-routes **24 passed / 24**. Playwright theme/waitlist/pricing/product-shell/home-tap-to-place + packing/batteries/pit offline **24 passed**; home-weather library empty-state **1 passed / 1** (4.9s) after switching off tap-to-place (desktop add). FEATURE_MAP HTTP walk **143 routes, 138 OK, 5 admin 404, 0 500**. Production `next build` **695 pages**, compile 23.9s, TypeScript 47s, zero warnings. GitHub Actions `Postgres migrations and RLS` **pass** on run `34458163082` (HEAD `28d33811`) and earlier `34451745493`. **`scripts/rls-proof.mjs` 40/40** this image against local `vantage_ci` as `vantage_ci_app` (session login, effective `vantage_app`, not superuser / not BYPASSRLS). Covers Drive personal files, outreach participants, dossier, funding_model, relay_nodes, video_analysis_jobs, assembly_manual_runs. Vercel GitHub deploy still fails independently. Do not treat production as proven. `CRON_SECRET` still owner-must. Real signed-in `next start` walk still not in this image. |
