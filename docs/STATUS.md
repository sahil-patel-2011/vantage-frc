...
| Home venue weather | Open-Meteo URL builders extracted; live Houston 26.1°C / code 0 at 2026-09-10T07:30Z | this commit |
...
| 2 | One design system | in progress | tokens in `system.css`; Button emits `.app-button`. Product CSS consumption of `var(--soft-*)` is **zero** (css-integrity). Canonical type/space/control: `--title-*`, `--text-*`, `--space-*`, `--control-h`, `--island`, `--font-sans`. `--soft-*` remains only as alias declarations. Team accent runtime writes `--accent` / `--accent-soft` / `--accent-ink` as well as the `--soft-*` / `--app-*` aliases. `--app-*` and `--m-*` consumption still exists. |
| 3 | Full UI pass | in progress | Student copy says team, not workspace, on Home widgets, related strips, Event Day setup, intel crumbs, and `/workspace`. Empty/setup cards keep one primary. Video re-scout review tools are a ToolStrip. Remaining TabBars are hub-level (Business, Media, Account, Help, Notifications). Playwright landing/pricing headings match current copy. FEATURE_MAP walk unfinished. No signed-in student/mentor browser walk. |
...
| 18 | Final verification | pending | previous full vitest: **880 / 8,331 passed / 10 skipped / 52.69s**. This revision: `npx tsc --noEmit -p apps/web` clean; vitest hubs + venue-weather + loaders + command-search **94 passed / 94**; branding + css-integrity **34 passed / 34**. Playwright after type-token CSS: theme, waitlist, pricing, product-shell, home-tap-to-place **17 passed / 17** (16.2s), including Scouting/Work strip assertions. Vercel GitHub check was **ERROR** `IMMUTABLE_STATIC_PATCH_PREVIEW_COMMENTS` on earlier SHAs; Preview Comments check now reports pass on a later deploy — do not treat production as proven. `CRON_SECRET` still owner-must. RLS proof and a real signed-in `next start` walk still not in this image. |
