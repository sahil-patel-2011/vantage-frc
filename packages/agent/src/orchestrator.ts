...
export type OrchestratorRequest={
  ...
  billingOwner?:{type:"user"|"org";id:string};usesOrgData?:boolean;autoTools?:boolean;
};
...
export class AIOrchestrator{
  ...
  async run(request:OrchestratorRequest):Promise<OrchestratorResult>{
    ...
    try{
      ...
      const text=await meteredAI({client:this.client,orgId:request.orgId,userId:request.userId,feature:usageFeature,requestId:request.requestId,estimatedCostUsd:.01,estimatedPromptTokens:Math.ceil(request.message.length/4)+context.estimatedTokens+toolContext.reduce((sum,item)=>sum+Math.ceil(item.content.length/4),0),estimatedCompletionTokens:700,provider:request.adapter.provider,model:request.adapter.model,billingOwner,metadata:{runId,threadId:request.threadId,activeEventKey:active,contextSources:context.provenance,tools:annotatedTools.map((item)=>({name:item.name,status:item.status,classification:item.classification})),usageTag:annotatedTools.some((t)=>t.name.startsWith("scouting."))?"chat.scouting":annotatedTools.some((t)=>t.name.startsWith("strategy."))?"chat.strategy":annotatedTools.length?"chat.tools":"chat"},invoke:async()=>{const result=await request.adapter.complete({message:request.message,context:[...context.items,...toolContext]});return{value:result.text,...result,provider:request.adapter.provider,model:request.adapter.model};}});
