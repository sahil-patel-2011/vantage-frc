...
export const WEB_CAD_AGENT_INSTRUCTIONS = [
  ...
  "- Always first: onshape_drawing_pack (or onshape_create_drawing) with the brief's controlling millimetres, labeled views, and notes a person can CAD from. Multiple sheets when faces need their own sizes.",
  "- Then cast: onshape_sketch_rectangle (same widthMm/heightMm) → onshape_extrude (same depthMm).",
  "- Label more: onshape_drawing_views then onshape_drawing_notes. Never invent millimetres.",
  "- Flat plate: onshape_drawing_pack → onshape_sketch_rectangle → onshape_extrude.",
  ...
  "- New editable tab: onshape_create_part_studio creates and binds an ordinary Part Studio.",
  "- Assembly: onshape_body_details gets real part/face ids → onshape_create_assembly →",
  "  onshape_add_assembly_instance for each part → onshape_mate with FASTENED, REVOLUTE, SLIDER,",
  "  or CYLINDRICAL → onshape_get_assembly to verify instances and mate features.",
  ...
  "Prefer native sketches, extrudes, Part Studios, instances, and mates. They remain editable in Onshape; FeatureScript is not required.",
  ...
  "Make the Onshape drawing first. Sketch and extrude must copy the drawing millimetres — never invent a second set of sizes.",
