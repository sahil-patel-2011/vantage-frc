...
class BriefAdapter implements ChatAdapter{
  ...
  async complete(input:{message:string;context:ContextItem[]}){
    ...
    return{text,promptTokens:Math.ceil(text.length/4),completionTokens:Math.ceil(text.length/4),costUsd:0};
  }
}
function briefFrom(input:{request:string;sources:ContextSource[];knowledgeNotes?:string[]}):EngineeringBrief{
  const repeatRisks=input.sources.filter((source)=>String(source.id??"").startsWith("fmea:repeat:")).map((source)=>String(source.content??"").trim()).filter(Boolean).slice(0,6);
  const knowledgeConstraints=(input.knowledgeNotes??[]).slice(0,6).map((note)=>`Team knowledge: ${note}`);
  return{
    summary:input.request.trim(),
    requirements:["Translate the confirmed scoring objective into a serviceable mechanism","Preserve access for inspection and repair"],
    constraints:["Use only dimensions and weight values confirmed by the user","Stay within configured manufacturing and API complexity limits",...knowledgeConstraints],
    scoringTasks:["User-selected game task from the strategy context"],
    assumptions:[{name:"Envelope dimensions",value:"Not yet confirmed",needsConfirmation:true},{name:"Weight budget",value:"Not yet confirmed",needsConfirmation:true},{name:"Manufacturing process",value:"Not yet confirmed",needsConfirmation:true}],
    risks:["Clearance and collision require geometry verification","Loads, fasteners, wire routing, and unsupported cantilevers require human engineering review",...repeatRisks],
    acceptanceCriteria:["User confirms requirements and assumptions before mutation","Every mutation produces topology and render verification","Design review checklist is reviewed by a human"],
    sourceRefs:input.sources.map(source=>({type:source.type,id:source.id,classification:source.classification})),
    disclaimer:"AI-generated design review suggestions are not engineering or safety certification.",
  };
}
  ...
  for(const call of knowledgeCalls){
    try{
      ...
      if(Array.isArray(output)){
        ...
        if(page){
          knowledgeNotes.push(`wiki: ${String(page.title??"")}`.slice(0,280));
          knowledgeSources.push({
            type:"team_memory",
            id:`wiki:${String(page.id??page.slug??"page")}`,
            content:JSON.stringify(output),
            importance:50,
            classification:"team_memory",
            label:String(page.title??"Wiki page"),
          });
        }
      }
    }catch{
      // Missing wiki tables or empty corpus — continue without inventing knowledge.
    }
  }
