-- Purpose-built FRC sponsor pipeline CRM (Chief Delphi fundraising CRM ask).
-- Extends existing org-scoped sponsors tables — never merges across orgs.
...
COMMENT ON COLUMN sponsors.ask_amount_usd IS
  'Season ask target for this sponsor relationship (org-local; not shared)';
COMMENT ON COLUMN sponsors.pledged_amount_usd IS
  'Verbal or written pledge before cash/in-kind is recorded';
COMMENT ON COLUMN sponsors.thank_you_due_on IS
  'When a thank-you should be sent after a contribution';
COMMENT ON COLUMN sponsors.renewal_due_on IS
  'When renewal outreach should begin for an active partner';
