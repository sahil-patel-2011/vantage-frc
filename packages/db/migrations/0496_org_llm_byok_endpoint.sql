-- BYOK endpoint overrides on org_llm_keys (Groq / custom OpenAI-compatible base URLs).
-- Personal BYOK lives in member_llm_keys (one row per member per provider).

ALTER TABLE org_llm_keys
...
CREATE INDEX IF NOT EXISTS member_llm_keys_org_user_idx
  ON member_llm_keys (org_id, user_id);
