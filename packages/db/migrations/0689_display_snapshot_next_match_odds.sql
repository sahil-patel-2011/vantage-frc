-- Pit TV: win odds only for the match that is actually next.
--
-- get_display_snapshot (0153) picked the prediction for our next match first, but fell back to
-- the newest prediction for any match at the event. Once our last match was played (or before
-- the next one was scored) the TV showed stale odds for a match already over. The pit TV page
-- hides them today; this makes the snapshot itself honest. Everything else is unchanged from
-- 0153. Grants repeat 0153 (display role) and 0685 (app role).
