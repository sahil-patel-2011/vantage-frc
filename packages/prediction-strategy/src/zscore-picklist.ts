...
/** Lovat Dashboard slider presets — event on, scout off, or the published worked example. */
export type PicklistPresetId = "event" | "scouting" | "teleopDriver";
...
export function picklistPresetWeights(id: PicklistPresetId): MetricWeight[] {
  switch (id) {
    case "event":
      return defaultPicklistWeights();
    case "scouting":
      return PICKLIST_METRICS.map((metric) => ({
        id: metric.id,
        weight: metric.source === "scout" ? 1 : 0,
      }));
    case "teleopDriver":
      return PICKLIST_METRICS.map((metric) => {
        if (metric.id === "teleopPoints") return { id: metric.id, weight: 0.4 };
        if (metric.id === "driverAbility") return { id: metric.id, weight: 0.3 };
        if (metric.id === "totalFuelFed") return { id: metric.id, weight: 0.1 };
        return { id: metric.id, weight: 0 };
      });
    default: {
