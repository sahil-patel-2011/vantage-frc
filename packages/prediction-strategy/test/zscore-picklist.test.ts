...
describe("picklistPresetWeights", () => {
  it("uses Lovat's published teleop + driver + fuel weights", () => {
    const weights = picklistPresetWeights("teleopDriver");
    expect(weights.find((item) => item.id === "teleopPoints")?.weight).toBe(0.4);
    expect(weights.find((item) => item.id === "driverAbility")?.weight).toBe(0.3);
    expect(weights.find((item) => item.id === "totalFuelFed")?.weight).toBe(0.1);
    expect(weights.find((item) => item.id === "totalPoints")?.weight).toBe(0);
  });

  it("turns scout sliders on only for the scouting preset", () => {
    expect(picklistPresetWeights("scouting").find((item) => item.id === "driverAbility")?.weight).toBe(1);
    expect(picklistPresetWeights("scouting").find((item) => item.id === "totalPoints")?.weight).toBe(0);
  });
});
