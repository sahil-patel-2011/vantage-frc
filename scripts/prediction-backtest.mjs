 ...
 * table from `scorePredictionMetrics(fixtureSeasonRows())` — that is not a ±3 claim.
 *
 *   node --experimental-strip-types scripts/prediction-backtest.mjs
...
} from "../packages/prediction-strategy/src/calibrated-score.ts";
...
const fixture = scorePredictionMetrics(fixtureSeasonRows());
const fixtureBand = errorBandFromMae(fixture.mae);
const fixtureRow = [
  "fixture (2 matches)",
  String(fixture.n),
  `**${fixture.mae}**`,
  `**${fixture.rmse}**`,
  `**${fixture.within3.toFixed(3)}**`,
  `**${fixture.within5.toFixed(3)}**`,
  "`packages/prediction-strategy/test/calibrated-score.test.ts` — toy scores vs summed EPA; not a season claim",
];
...
const markdown = [
  ...
  "",
  `UI error band on the next-match widget is \`FIXTURE_ERROR_BAND\` = **${FIXTURE_ERROR_BAND}**, from \`errorBandFromMae\` on this fixture MAE (${fixture.mae} → ${fixtureBand}). Copy reads **${typicalScoreErrorCopy(FIXTURE_ERROR_BAND)}**. That is not a ±3 promise and not a live-season interval — it is the measured error on this 2-match toy set, shown instead of a placeholder 8. A season backtest can pass a different \`errorBand\` into \`predictAllianceScores\`.`,
  "",
  "## Method",
  "",
  ...
  "",
  `Generated ${new Date().toISOString().slice(0, 10)}. Target year this run: ${yearFlag}. Fixture metrics printed this session from \`scorePredictionMetrics(fixtureSeasonRows())\`.`,
  "",
].join("\n");
