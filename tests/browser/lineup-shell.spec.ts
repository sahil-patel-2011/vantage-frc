...
test("Lineup coverage still loads after the Saturday shell pass", async ({ page }) => {
  await page.goto("/scouting/lineup");
  await expect(page.locator("body")).not.toContainText("Application error");

  const board = page.getByRole("heading", { name: /Lineup & coverage|Live coverage gaps/i });
  const setup = page.getByRole("heading", { name: /Choose your team|Choose your team/i });
  const empty = page.getByRole("heading", { name: /Waiting on match slots/i });
  const unavailable = loadFailureHeading(page);
  if (!(await expectHubReadyOrGate(page, board, empty.or(setup).or(unavailable)))) {
    await page.screenshot({ path: "/opt/cursor/artifacts/lineup-after-shell.png", fullPage: true });
    return;
  }
