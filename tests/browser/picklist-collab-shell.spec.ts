...
test("Collaborative pick list still loads after the Saturday shell pass", async ({ page }) => {
  await page.goto("/picklist-collab");
  ...
  if (!(await expectHubReadyOrGate(page, board, empty.or(setup).or(unavailable)))) {
    await page.screenshot({ path: "/opt/cursor/artifacts/picklist-collab-after-shell.png", fullPage: true });
    return;
  }
